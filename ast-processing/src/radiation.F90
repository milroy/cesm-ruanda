    ...
    if (dosw .or. dolw) then
	  ...
	  ! add fillvalue for night columns
       ...
       if (dolw) then
          ...
          !
          ! write (*,*) 'TESTING LZF DJM'
          ! CONFIRMED: SUBROUTINE CALLED
          ! Introducing bug here.
          do i=1,ncol
             ! lwupcgs(i) = cam_in%lwup(i)*1000._r8
             lwupcgs(i) = cam_in%lwup(i)*10000._r8
             if(single_column.and.scm_crm_mode.and.have_tg) &
                  ! lwupcgs(i) = 1000*stebol*tground(1)**4
                  lwupcgs(i) = 10000*stebol*tground(1)**4
          end do
...
! CONVERSION BUG, FACTOR OF TEN
   do k=1,pver
      do i=1,ncol
         ! pmidrd(i,k) = pmid(i,k)*10.0_r8
         pmidrd(i,k) = pmid(i,k)*100.0_r8
         ! pintrd(i,k) = pint(i,k)*10.0_r8
         pintrd(i,k) = pint(i,k)*100.0_r8
      end do
   end do
   do i=1,ncol
      ! pintrd(i,pverp) = pint(i,pverp)*10.0_r8
      pintrd(i,pverp) = pint(i,pverp)*100.0_r8
   end do
