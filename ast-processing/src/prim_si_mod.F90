...
!#endif
       do j=1,np   !   Loop inversion (AAM)
          ...
          do i=1,np
             ...
             hkl = 2*hkk
! ORIG            phii(i,j,nlev)  = Rgas*T_v(i,j,nlev)*hkl
! ORIG            phi(i,j,nlev) = phis(i,j) + Rgas*T_v(i,j,nlev)*hkk 
          ...
          do k=nlev-1,2,-1
             do i=1,np
                ...
                hkl = 2*hkk
! ORIG                phii(i,j,k) = phii(i,j,k+1) + Rgas*T_v(i,j,k)*hkl
! ORIG                phi(i,j,k) = phis(i,j) + phii(i,j,k+1) + Rgas*T_v(i,j,k)*hkk
                tt_real = T_v(i,j,k)*hkl
                phii(i,j,k) = phii(i,j,k+1) + Rgas*tt_real
                tt_real = T_v(i,j,k)*hkk
                phi(i,j,k) = Rgas*tt_real + phis(i,j) + phii(i,j,k+1)
             end do
          end do
...
! ORIG             phi(i,j,1) = phis(i,j) + phii(i,j,2) + Rgas*T_v(i,j,1)*hkk
             tt_real = T_v(i,j,1)*hkk
             phi(i,j,1) = Rgas*tt_real + phis(i,j) + phii(i,j,2)
          end do
