...
contains
  ...
  subroutine cg_create(cg,len,ninst,nblks,hybrid,debug_level,wts)
    ...
    if(err/=0) then
       print *, "<stdin>",99,len,ninst,nblks,err
       call abortmp(' ')
    end if
    allocate(cg%gammam1(ninst),stat=err)
    if(err/=0) then
       print *, "<stdin>",104,len,ninst,nblks,err
       call abortmp(' ')
    end if
    allocate(cg%sigmam1(ninst),stat=err)
    if(err/=0) then
       print *, "<stdin>",109,len,ninst,nblks,err
       call abortmp(' ')
    end if
    allocate(cg%rhs_norm(ninst),stat=err)
    if(err/=0) then
       print *, "<stdin>",114,len,ninst,nblks,err
       call abortmp(' ')
    end if
    allocate(cg%wts(len,nblks),stat=err)
    if(err/=0) then
       print *, "<stdin>",119,len,ninst,nblks,err
       call abortmp(' ')
    end if
    ...
    if(err/=0) then
       print *, "<stdin>",126,len,ninst,nblks,err
       call abortmp(' ')
    end if
    do i=1,nblks
