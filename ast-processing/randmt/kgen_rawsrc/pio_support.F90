...
!=============================================
    ...
    do n = 0,npes-1
       ...
       if (myrank == masterproc .and. n == masterproc) then
          ...
          if (myrank == n .and. sdof > 0) then
             ...
             if (ierr /= MPI_SUCCESS) call piodie("pio_support.F90",228,' pio_writedof mpi_recv')
             call MPI_SEND(dof,int(sdof),PIO_OFFSET_KIND,masterproc,n,comm,ierr)
             if (ierr /= MPI_SUCCESS) call piodie("pio_support.F90",231,' pio_writedof mpi_send')
          endif
          if (myrank == masterproc .and. sdof1d(n) > 0) then
             call MPI_IRECV(wdof,int(sdof1d(n)),PIO_OFFSET_KIND,n,n,comm,rcv_request,ierr)
             if (ierr /= MPI_SUCCESS) call piodie("pio_support.F90",236,' pio_writedof mpi_irecv')
             call MPI_SEND(hs,1,MPI_INTEGER,n,n,comm,ierr)
             if (ierr /= MPI_SUCCESS) call piodie("pio_support.F90",238,' pio_writedof mpi_send')
             call MPI_WAIT(rcv_request,status,ierr)
             if (ierr /= MPI_SUCCESS) call piodie("pio_support.F90",240,' pio_writedof mpi_wait')
          endif
       endif
       if (myrank == masterproc) then
  ...
  subroutine pio_readdof (file, DOF, comm, punit)
    ...
    if (myrank == masterproc) then
       ...
       if (rnpes /= npes) then
          call piodie("pio_support.F90",322,'pio_readdof npes incorrect')
       endif

       do n = 0,npes-1
          read(unit,*) rn,sdof
          if (rn /= n) then
             call piodie("pio_support.F90",328,'pio_readdof rn out of sync')
          endif
          allocate(wdof(sdof))
          do m = 1,sdof
             read(unit,*) wdof(m)
          ...
          if (n == masterproc) then
             ...
             if (ierr /= MPI_SUCCESS) call piodie("pio_support.F90",340,' pio_readdof mpi_send1')
             if (sdof > 0) then
                call MPI_SEND(wdof,int(sdof),PIO_OFFSET_KIND,n,npes+n,comm,ierr)
                if (ierr /= MPI_SUCCESS) call piodie("pio_support.F90",343,' pio_readdof mpi_send2')
             endif
          endif
          deallocate(wdof)
       enddo
       ...
       if (ierr /= MPI_SUCCESS) call piodie("pio_support.F90",351,' pio_readdof mpi_recv1')
       if (sdof > 0) then
          deallocate(dof)
          allocate(dof(sdof))
          call MPI_RECV(dof,int(sdof),PIO_OFFSET_KIND,masterproc,npes+myrank,comm,status,ierr)
          if (ierr /= MPI_SUCCESS) call piodie("pio_support.F90",356,' pio_readdof mpi_recv2')
       endif
    endif
