...
!       case(iotype_netcdf,PIO_iotype_netcdf4c,PIO_iotype_netcdf4p)
       case(iotype_netcdf,PIO_iotype_netcdf4c)
          if (Ios%io_rank==0) then
             if(debug) print *,"/glade/u/home/milroy/cesm/cesm1_3_beta11/models/utils/pio/pionfatt_mod.F90.in",158,name,value
       ...
       case default
          call bad_iotype(iotype,"pionfatt_mod.F90",166)

       end select
    endif
    call check_netcdf(File, ierr,"pionfatt_mod.F90",170)
...
!<
  integer function put_att_1d_real (File, varid, name, value) result(ierr)
    ...
    if(Debug.or.DebugAsync) print *,"pionfatt_mod.F90",229,clen,value
    ...
    if(Ios%IOproc) then
       ...
       case default
          call bad_iotype(iotype,"pionfatt_mod.F90",258)

       end select
    endif
    call check_netcdf(File, ierr,"pionfatt_mod.F90",262)
...
!<
  integer function get_att_text (File,varid,name,value) result(ierr)
    ...
    if(Ios%IOproc) then
       ...
       case default
          call bad_iotype(iotype,"pionfatt_mod.F90",431)

       end select
    endif
    call check_netcdf(File, ierr,"pionfatt_mod.F90",435)
...
!<
  integer function get_att_1d_real (File,varid,name,value) result(ierr)
    ...
    if(Ios%IOproc) then
       ...
       case default
          call bad_iotype(iotype,"pionfatt_mod.F90",510)

       end select
    endif
    call check_netcdf(File, ierr,"pionfatt_mod.F90",514)
    if(ios%async_interface .or. ios%num_tasks>ios%num_iotasks) then
       call MPI_BCAST(value, clen, MPI_REAL4,Ios%iomaster,Ios%my_comm, mpierr)
       call CheckMPIReturn(subName, mpierr)
    end if
