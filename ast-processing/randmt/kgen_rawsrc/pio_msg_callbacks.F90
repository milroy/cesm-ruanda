subroutine pio_callback_handler(iosystem, msg)
  ...
  case default
     ...
     call piodie("pio_msg_callbacks.F90",73)  
  end select
...
subroutine create_file_handler(iosystem)
  ...
  if(Debugasync) print *,"pio_msg_callbacks.F90",131,file%fh
...
subroutine open_file_handler(iosystem)
  ...
  call add_to_file_list(file)
  if(Debugasync) print *,"pio_msg_callbacks.F90",167,file%fh
...
subroutine writedarray_handler(iosystem)
  ...
  file=> lookupfile(fh)
  if(debugasync) print *,"<stdin>",240,v%varid,iod_id
...
subroutine readdarray_handler(iosystem)
  ...
  if(debugasync) print *,"<stdin>",304,iod_id, type
...
subroutine string_handler_for_att(file, varid, name, strlen, msg)
  ...
  if(msg==PIO_MSG_GETATT) then
     if(Debugasync) print *,"pio_msg_callbacks.F90",351, varid, name
     ierr = pio_get_att(file, varid, name, str )  
     if(Debugasync) print *,"pio_msg_callbacks.F90",353, str
  else
...
subroutine att_handler(ios, msg)
  ...
  if(Debugasync) print *,"pio_msg_callbacks.F90",382
  ...
  if(Debugasync) print *,"pio_msg_callbacks.F90",389, itype,nlen
  ...
  case (100)
     call mpi_bcast(strlen, 1, mpi_integer, ios%compmaster, ios%intercomm, ierr)
       if(Debugasync) print *,"pio_msg_callbacks.F90",396, strlen,nlen
