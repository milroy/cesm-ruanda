...
!    use pio_types, only : 
    ...
    do while(msg /= -1)
       if(iorank==0) then
          if(Debugasync) print *,"pio_msg_mod.F90",143, ' waiting'
          call mpi_waitany(numcomps, req, index, status, ierr)
          if(Debugasync) print *,"pio_msg_mod.F90",145, ' recieved on ', index
       end if

       call mpi_bcast(index, 1, mpi_integer, 0, io_comm, ierr)
       ios => iosystem(index)

       if(Debugasync) print *,"pio_msg_mod.F90",151, index, ios%intercomm
       call mpi_bcast(msg, 1, mpi_integer, 0, io_comm, ierr)

       if(debugasync) print *,"pio_msg_mod.F90",154, msg ,' recieved on ', index
       select case(msg) 
...
!          call mpi_barrier(ios%io_comm,ierr)
    ...
    if(Debugasync) print *,"pio_msg_mod.F90",247
  ...
  subroutine add_to_file_list(file)
    ...
    if(associated(list_item%file)) then
       ...
       if(Debugasync) print *,"pio_msg_mod.F90",262,list_item%file%fh
    ...
    if(Debugasync) print *,"pio_msg_mod.F90",271,file%fh
    list_item%file => file
...
!          id = max(id+1, list_item%iodesc%async_id+1)
       ...
       if(debugasync) print *,"<stdin>",302,index
    end if
    ...
    if(debugasync) print *,"<stdin>",309,index,list_item%iodesc%async_id
       ...
       if(associated(list_item%next)) then
          ...
          if(debugasync) then
             ...
             do while(associated(list_item))
                print *,"<stdin>",355,id,list_item%index,list_item%iodesc%async_id
                list_item=>list_item%next
             enddo
          endif

          call piodie("pio_msg_mod.F90",360,'delete_from_iodesc_list',id)
       end if
    end do
  ...
  subroutine delete_from_file_list(fh)
    ...
    do while(associated(list_item%file) )
       ...
       if(associated(list_item%next)) then
          ...
          call piodie("pio_msg_mod.F90",392,'delete_from_file_list')
       end if
    end do
  ...
  function lookupiodesc(async_id) result(iodesc)
    ...
    do while(associated(list_item%iodesc) )
       ...
       if(debugasync) print *,"<stdin>",432,list_item%index,async_id,list_item%iodesc%async_id
       if(abs(list_item%iodesc%async_id) == async_id) then
          iodesc => list_item%iodesc
          if(debugasync) print *,"<stdin>",435,async_id,list_item%index,iodesc%write%n_elemtype
          exit
       end if
       list_item=>list_item%next
    end do
    if(.not.associated(iodesc)) then
       call piodie("pio_msg_mod.F90",441)
    end if
