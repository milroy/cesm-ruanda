...
! Subprogram not used subroutine dyn_grid_get_elem_coords( ie, rlon, rlat, cdex )
! Subprogram not used   use dof_mod, only : UniqueCoords
! Subprogram not used   use dimensions_mod, only : np
! Subprogram not used 
! Subprogram not used   integer, intent(in) :: ie ! block element index
! Subprogram not used 
! Subprogram not used   real(r8),optional, intent(out) :: rlon(:) ! longitudes of the columns in the element
! Subprogram not used   real(r8),optional, intent(out) :: rlat(:) ! latitudes of the columns in the element
! Subprogram not used   integer, optional, intent(out) :: cdex(:) ! global column index
! Subprogram not used 
! Subprogram not used   integer :: sb,eb, ii, i,j, icol, igcol
! Subprogram not used   real(r8), allocatable :: clat(:), clon(:)
! Subprogram not used 
! Subprogram not used   sb = elem(ie)%idxp%UniquePtOffset
! Subprogram not used   eb = sb + elem(ie)%idxp%NumUniquePts-1
! Subprogram not used 
! Subprogram not used   allocate( clat(sb:eb), clon(sb:eb) )
! Subprogram not used   call UniqueCoords( elem(ie)%idxP, elem(ie)%spherep, clat(sb:eb), clon(sb:eb) )
! Subprogram not used 
! Subprogram not used   if (present(cdex)) cdex(:) = -1
! Subprogram not used   if (present(rlat)) rlat(:) = -999._r8
! Subprogram not used   if (present(rlon)) rlon(:) = -999._r8
! Subprogram not used 
! Subprogram not used   do ii=1,elem(ie)%idxp%NumUniquePts
! Subprogram not used      i=elem(ie)%idxp%ia(ii)
! Subprogram not used      j=elem(ie)%idxp%ja(ii)
! Subprogram not used      icol = i+(j-1)*np
! Subprogram not used      igcol = elem(ie)%idxp%UniquePtoffset+ii-1
! Subprogram not used      if (present(cdex)) cdex(icol) = igcol
! Subprogram not used      if (present(rlat)) rlat(icol) = clat( igcol )
! Subprogram not used      if (present(rlon)) rlon(icol) = clon( igcol )
! Subprogram not used   end do
! Subprogram not used 
! Subprogram not used   deallocate( clat, clon )
! Subprogram not used 
! Subprogram not used endsubroutine dyn_grid_get_elem_coords
