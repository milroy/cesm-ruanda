...
! This flag preserves answers for vanilla 1 by making a few changes (e.g.
...
! Subprogram not used subroutine momentum_energy_conservation(ncol, tend_level, dt, taucd, &
! Subprogram not used      pint, pdel, u, v, dudt, dvdt, dsdt, utgw, vtgw, ttgw)
! Subprogram not used 
! Subprogram not used   ! C.-C. Chen, momentum & energy conservation
! Subprogram not used 
! Subprogram not used   integer, intent(in) :: ncol
! Subprogram not used   integer, intent(in) :: tend_level(ncol)
! Subprogram not used   real(r8), intent(in) :: dt
! Subprogram not used   real(r8), intent(in) :: taucd(ncol,0:pver,4)
! Subprogram not used   real(r8), intent(in) :: pint(ncol,pver)
! Subprogram not used   real(r8), intent(in) :: pdel(ncol,pver)
! Subprogram not used   real(r8), intent(in) :: u(ncol,pver)
! Subprogram not used   real(r8), intent(in) :: v(ncol,pver)
! Subprogram not used   ! The following are assumed-shape because, for now, 1 is passing them
! Subprogram not used   ! with size pcols.
! Subprogram not used   real(r8), intent(inout) :: dudt(:,:)
! Subprogram not used   real(r8), intent(inout) :: dvdt(:,:)
! Subprogram not used   real(r8), intent(inout) :: dsdt(:,:)
! Subprogram not used   real(r8), intent(inout) :: utgw(ncol,pver)
! Subprogram not used   real(r8), intent(inout) :: vtgw(ncol,pver)
! Subprogram not used   real(r8), intent(inout) :: ttgw(ncol,pver)
! Subprogram not used   ! Local variables.
! Subprogram not used   integer :: i, k
! Subprogram not used   real(r8) :: dz(ncol), dE(ncol), ut_dz(ncol), vt_dz(ncol)
! Subprogram not used 
! Subprogram not used   ! Total mass from ground to source level: rho*dz = dp/gravit
! Subprogram not used   dz = 0.0_r8
! Subprogram not used   do k = pver, minval(tend_level)+1, -1
! Subprogram not used      where (k > tend_level)
! Subprogram not used         dz = dz + pdel(:,k)/gravit
! Subprogram not used      end where
! Subprogram not used   end do
! Subprogram not used 
! Subprogram not used   ! Tendency for U & V below source level.
! Subprogram not used   do i = 1, ncol
! Subprogram not used      ut_dz(i) = -(taucd(i,tend_level(i), east) + &
! Subprogram not used           taucd(i,tend_level(i), west))/dz(i)
! Subprogram not used      vt_dz(i) = -(taucd(i,tend_level(i),north) + &
! Subprogram not used           taucd(i,tend_level(i),south))/dz(i)
! Subprogram not used   end do
! Subprogram not used 
! Subprogram not used   do k = minval(tend_level)+1, pver
! Subprogram not used      where (k > tend_level)
! Subprogram not used         dudt(:ncol,k) = dudt(:ncol,k)+ut_dz
! Subprogram not used         dvdt(:ncol,k) = dvdt(:ncol,k)+vt_dz
! Subprogram not used         utgw(:,k) = utgw(:,k)+ut_dz
! Subprogram not used         vtgw(:,k) = vtgw(:,k)+vt_dz
! Subprogram not used      end where
! Subprogram not used   end do
! Subprogram not used 
! Subprogram not used   ! Net gain/loss of total energy in the column.
! Subprogram not used   dE = 0.0_r8
! Subprogram not used   do k = 1, pver
! Subprogram not used      dE = dE + pdel(:,k) * (dsdt(:ncol,k) + &
! Subprogram not used           dudt(:ncol,k)*(u(:,k)+dudt(:ncol,k)*0.5_r8*dt) + &
! Subprogram not used           dvdt(:ncol,k)*(v(:,k)+dvdt(:ncol,k)*0.5_r8*dt) )
! Subprogram not used   end do
! Subprogram not used 
! Subprogram not used   do i = 1, ncol
! Subprogram not used      dE(i) = dE(i)/(pint(i,pver)-pint(i,tend_level(i)))
! Subprogram not used   end do
! Subprogram not used 
! Subprogram not used   ! Subtract net gain/loss of total energy below source level.
! Subprogram not used   do k = minval(tend_level)+1, pver
! Subprogram not used      where (k > tend_level)
! Subprogram not used         dsdt(:ncol,k) = dsdt(:ncol,k)-dE
! Subprogram not used         ttgw(:,k) = ttgw(:,k)-dE
! Subprogram not used      end where
! Subprogram not used   end do
! Subprogram not used 
! Subprogram not used end subroutine momentum_energy_conservation
