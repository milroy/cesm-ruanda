...
!<
  subroutine shr_pio_init1(ncomps, nlfilename, Global_Comm)
    ...
    if(pio_async_interface) then
       ...
       print *,"<stdin>",104,subname, ' complete'
    end if
    total_comps = ncomps
...
!===============================================================================
! Subprogram not used   function shr_pio_getioroot_fromid(compid) result(io_root)
! Subprogram not used     ! 'component' must be equal to some element of io_compname(:)
! Subprogram not used     ! (but it is case-insensitive)
! Subprogram not used     integer, intent(in) :: compid
! Subprogram not used     integer :: io_root
! Subprogram not used 
! Subprogram not used     io_root = pio_comp_settings(shr_pio_getindex(compid))%pio_root
! Subprogram not used 
! Subprogram not used   end function shr_pio_getioroot_fromid

! Subprogram not used   function shr_pio_getioroot_fromname(component) result(io_root)
! Subprogram not used     ! 'component' must be equal to some element of io_compname(:)
! Subprogram not used     ! (but it is case-insensitive)
! Subprogram not used     character(len=*), intent(in) :: component
! Subprogram not used     integer :: io_root
! Subprogram not used 
! Subprogram not used     io_root = pio_comp_settings(shr_pio_getindex(component))%pio_root
! Subprogram not used 
! Subprogram not used 
! Subprogram not used   end function shr_pio_getioroot_fromname
