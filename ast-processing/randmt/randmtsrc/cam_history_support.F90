...
! Subprogram not used   subroutine field_copy(f_out, f_in)
! Subprogram not used     type(field_info), intent(in) :: f_in
! Subprogram not used     type(field_info), intent(out) :: f_out
! Subprogram not used 
! Subprogram not used     f_out%flag_xyfill = f_in%flag_xyfill
! Subprogram not used     f_out%is_subcol = f_in%is_subcol
! Subprogram not used     f_out%fillvalue = f_in%fillvalue
! Subprogram not used     f_out%numlev =  f_in%numlev                      ! vertical dimension (.nc file and internal arr)
! Subprogram not used     f_out%begdim1 = f_in%begdim1                     ! on-node dim1 start index
! Subprogram not used     f_out%enddim1 = f_in%enddim1                     ! on-node dim1 end index
! Subprogram not used     f_out%begdim2 = f_in%begdim2                     ! on-node dim2 start index
! Subprogram not used     f_out%enddim2 = f_in%enddim2                     ! on-node dim2 end index
! Subprogram not used     f_out%begdim3 = f_in%begdim3                     ! on-node chunk or lat start index
! Subprogram not used     f_out%enddim3 = f_in%enddim3                     ! on-node chunk or lat end index
! Subprogram not used     f_out%decomp_type = f_in%decomp_type             ! type of decomposition (physics or dynamics)
! Subprogram not used 
! Subprogram not used     f_out%vector_compliment = f_in%vector_compliment ! id for vector compliment for interpolation
! Subprogram not used 
! Subprogram not used     f_out%name = f_in%name                           ! field name
! Subprogram not used     f_out%long_name = f_in%long_name                 ! long name
! Subprogram not used     f_out%units = f_in%units                         ! units
! Subprogram not used     f_out%sampling_seq =  f_in%sampling_seq          ! sampling sequence - if not every timestep, how often field is sampled
! Subprogram not used 
! Subprogram not used     if(associated(f_in%mdims)) then
! Subprogram not used       f_out%mdims=>f_in%mdims
! Subprogram not used     else
! Subprogram not used       nullify(f_out%mdims)
! Subprogram not used     end if
! Subprogram not used     if(associated(f_in%colperdim3)) then
! Subprogram not used       f_out%colperdim3=>f_in%colperdim3
! Subprogram not used     else
! Subprogram not used       nullify(f_out%colperdim3)
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used   end subroutine field_copy
...
! Subprogram not used   logical function check_hist_coord_char(defined, input)
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     ! Input variables
! Subprogram not used     character(len=*), intent(in)            :: defined
! Subprogram not used     character(len=*), intent(in), optional  :: input
! Subprogram not used 
! Subprogram not used     if (len_trim(defined) == 0) then
! Subprogram not used       ! In this case, we assume the current value is undefined so any input OK
! Subprogram not used       check_hist_coord_char = .true.
! Subprogram not used     else if (present(input)) then
! Subprogram not used       ! We have to match definitions
! Subprogram not used       check_hist_coord_char = (trim(input) == trim(defined))
! Subprogram not used     else
! Subprogram not used       ! Not sure here. We have a value and are redefining without one?
! Subprogram not used       check_hist_coord_char = .false.
! Subprogram not used     end if
! Subprogram not used   end function check_hist_coord_char

! Subprogram not used   logical function check_hist_coord_int(defined, input)
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     ! Input variables
! Subprogram not used     integer, intent(in)            :: defined
! Subprogram not used     integer, intent(in), optional  :: input
! Subprogram not used 
! Subprogram not used     if (defined == 0) then
! Subprogram not used       ! In this case, we assume the current value is undefined so any input OK
! Subprogram not used       check_hist_coord_int = .true.
! Subprogram not used     else if (present(input)) then
! Subprogram not used       ! We have to match definitions
! Subprogram not used       check_hist_coord_int = (input == defined)
! Subprogram not used     else
! Subprogram not used       ! Not sure here. We have a value and are redefining without one?
! Subprogram not used       check_hist_coord_int = .false.
! Subprogram not used     end if
! Subprogram not used   end function check_hist_coord_int

! Subprogram not used   logical function check_hist_coord_int1(defined, input)
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     ! Input variables
! Subprogram not used     integer,             pointer            :: defined(:)
! Subprogram not used     integer, intent(in),          optional  :: input(:)
! Subprogram not used 
! Subprogram not used     ! Local variables
! Subprogram not used     integer                                 :: i
! Subprogram not used 
! Subprogram not used     if (.not. associated(defined)) then
! Subprogram not used       ! In this case, we assume the current value is undefined so any input OK
! Subprogram not used       check_hist_coord_int1 = .true.
! Subprogram not used     else if (present(input)) then
! Subprogram not used       ! We have to match definitions
! Subprogram not used       check_hist_coord_int1 = (size(input) == size(defined))
! Subprogram not used     else
! Subprogram not used       ! Not sure here. We have a value and are redefining without one?
! Subprogram not used       check_hist_coord_int1 = .false.
! Subprogram not used     end if
! Subprogram not used     if (check_hist_coord_int1 .and. associated(defined)) then
! Subprogram not used       ! Need to check the values
! Subprogram not used       do i = 1, size(defined)
! Subprogram not used         if (defined(i) /= input(i)) then
! Subprogram not used           check_hist_coord_int1 = .false.
! Subprogram not used           exit
! Subprogram not used         end if
! Subprogram not used       end do
! Subprogram not used     end if
! Subprogram not used   end function check_hist_coord_int1

! Subprogram not used   logical function check_hist_coord_r8(defined, input)
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     ! Input variables
! Subprogram not used     real(r8), intent(in)            :: defined
! Subprogram not used     real(r8), intent(in), optional  :: input
! Subprogram not used 
! Subprogram not used     if (defined == fillvalue) then
! Subprogram not used       ! In this case, we assume the current value is undefined so any input OK
! Subprogram not used       check_hist_coord_r8 = .true.
! Subprogram not used     else if (present(input)) then
! Subprogram not used       ! We have to match definitions
! Subprogram not used       check_hist_coord_r8 = (input == defined)
! Subprogram not used     else
! Subprogram not used       ! Not sure here. We have a value and are redefining without one?
! Subprogram not used       check_hist_coord_r8 = .false.
! Subprogram not used     end if
! Subprogram not used   end function check_hist_coord_r8

! Subprogram not used   logical function check_hist_coord_r81(defined, input)
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     ! Input variables
! Subprogram not used     real(r8),             pointer            :: defined(:)
! Subprogram not used     real(r8), intent(in),          optional  :: input(:)
! Subprogram not used 
! Subprogram not used     ! Local variables
! Subprogram not used     integer                                  :: i
...
! Subprogram not used     if (.not. associated(defined)) then
! Subprogram not used       ! In this case, we assume the current value is undefined so any input OK
! Subprogram not used       check_hist_coord_r81 = .true.
! Subprogram not used     else if (present(input)) then
! Subprogram not used       ! We have to match definitions
! Subprogram not used       check_hist_coord_r81 = (size(input) == size(defined))
! Subprogram not used     else
! Subprogram not used       ! Not sure here. We have a value and are redefining without one?
! Subprogram not used       check_hist_coord_r81 = .false.
! Subprogram not used     end if
! Subprogram not used     if (check_hist_coord_r81 .and. associated(defined)) then
! Subprogram not used       ! Need to check the values
! Subprogram not used       do i = 1, size(defined)
! Subprogram not used         if (defined(i) /= input(i)) then
! Subprogram not used           check_hist_coord_r81 = .false.
...
! Subprogram not used       end do
! Subprogram not used     end if
! Subprogram not used   end function check_hist_coord_r81

! Subprogram not used   logical function check_hist_coord_r82(defined, input)
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     ! Input variables
! Subprogram not used     real(r8),             pointer            :: defined(:,:)
...
! Subprogram not used     if (.not. associated(defined)) then
! Subprogram not used       ! In this case, we assume the current value is undefined so any input OK
! Subprogram not used       check_hist_coord_r82 = .true.
! Subprogram not used     else if (present(input)) then
! Subprogram not used       ! We have to match definitions
! Subprogram not used       check_hist_coord_r82 = ((size(input, 1) == size(defined, 1)) .and.    &
! Subprogram not used                               (size(input, 2) == size(defined, 2)))
! Subprogram not used     else
! Subprogram not used       ! Not sure here. We have a value and are redefining without one?
! Subprogram not used       check_hist_coord_r82 = .false.
! Subprogram not used     end if
! Subprogram not used     if (check_hist_coord_r82 .and. associated(defined)) then
! Subprogram not used       ! Need to check the values
! Subprogram not used       do j = 1, size(defined, 2)
! Subprogram not used         do i = 1, size(defined, 1)
! Subprogram not used           if (defined(i, j) /= input(i, j)) then
! Subprogram not used             check_hist_coord_r82 = .false.
! Subprogram not used             exit
! Subprogram not used           end if
! Subprogram not used         end do
! Subprogram not used       end do
! Subprogram not used     end if
! Subprogram not used   end function check_hist_coord_r82

! Subprogram not used   logical function check_hist_coord_ft(defined, input)
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     ! Input variables
! Subprogram not used     type(formula_terms_t), intent(in)           :: defined
! Subprogram not used     type(formula_terms_t), intent(in), optional :: input
! Subprogram not used 
! Subprogram not used     ! We will assume that if formula_terms has been defined, a_name has a value
! Subprogram not used     if (len_trim(defined%a_name) == 0) then
! Subprogram not used       ! In this case, we assume the current value is undefined so any input OK
! Subprogram not used       check_hist_coord_ft = .true.
! Subprogram not used     else if (present(input)) then
! Subprogram not used       ! We have to match definitions
! Subprogram not used       ! Need to check the values
! Subprogram not used       check_hist_coord_ft =                                                   &
! Subprogram not used            check_hist_coord(defined%a_name,       input%a_name)         .and. &
! Subprogram not used            check_hist_coord(defined%a_long_name,  input%a_long_name)    .and. &
! Subprogram not used            check_hist_coord(defined%a_values,     input%a_values)       .and. &
! Subprogram not used            check_hist_coord(defined%b_name,       input%b_name)         .and. &
! Subprogram not used            check_hist_coord(defined%b_long_name,  input%b_long_name)    .and. &
! Subprogram not used            check_hist_coord(defined%b_values,     input%b_values)       .and. &
! Subprogram not used            check_hist_coord(defined%p0_name,      input%p0_name)        .and. &
! Subprogram not used            check_hist_coord(defined%p0_long_name, input%p0_long_name)   .and. &
! Subprogram not used            check_hist_coord(defined%p0_units,     input%p0_units)       .and. &
! Subprogram not used            check_hist_coord(defined%p0_value,     input%p0_value)       .and. &
! Subprogram not used            check_hist_coord(defined%ps_name,      input%ps_name)
! Subprogram not used     else
! Subprogram not used       ! Not sure here. We have a value and are redefining without one?
! Subprogram not used       check_hist_coord_ft = .false.
! Subprogram not used     end if
! Subprogram not used   end function check_hist_coord_ft
...
! Subprogram not used   subroutine add_hist_coord_int(name, vlen, long_name, units, values,         &
! Subprogram not used        positive, standard_name)
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     ! Input variables
! Subprogram not used     character(len=*), intent(in)                    :: name
! Subprogram not used     integer,          intent(in)                    :: vlen
! Subprogram not used     character(len=*), intent(in)                    :: long_name
! Subprogram not used     character(len=*), intent(in),          optional :: units
! Subprogram not used     integer,          intent(in),  target, optional :: values(:)
! Subprogram not used     character(len=*), intent(in),          optional :: positive
! Subprogram not used     character(len=*), intent(in),          optional :: standard_name
! Subprogram not used 
! Subprogram not used     ! Local variables
! Subprogram not used     integer                                         :: i
! Subprogram not used 
! Subprogram not used     ! First, check to see if it is OK to add this coord
! Subprogram not used     i = check_hist_coord(name, vlen=vlen, long_name=long_name, units=units,   &
! Subprogram not used          i_values=values, positive=positive, standard_name=standard_name)
! Subprogram not used     ! Register the name if necessary
! Subprogram not used     if (i == 0) then
! Subprogram not used       call add_hist_coord(trim(name), i)
! Subprogram not used       !  if(masterproc) write(iulog,*) 'Registering hist coord',name,'(',i,') with length: ',vlen
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used     ! Set the coord's values
! Subprogram not used     hist_coords(i)%dimsize = vlen
! Subprogram not used     if (len_trim(long_name) > max_chars) then
! Subprogram not used       if(masterproc) then
! Subprogram not used         write(iulog,*) 'WARNING: long_name for ',trim(name),' too long'
! Subprogram not used       end if
! Subprogram not used     end if
! Subprogram not used     hist_coords(i)%long_name = trim(long_name)
! Subprogram not used     if (present(units)) then
! Subprogram not used       hist_coords(i)%units = trim(units)
! Subprogram not used     else
! Subprogram not used       hist_coords(i)%units = ''
! Subprogram not used     end if
! Subprogram not used     hist_coords(i)%integer_dim = .true.
! Subprogram not used     if (present(values)) then
! Subprogram not used       hist_coords(i)%integer_values => values
! Subprogram not used     endif
! Subprogram not used     if (present(positive)) then
! Subprogram not used       hist_coords(i)%positive = trim(positive)
! Subprogram not used     end if
! Subprogram not used     if (present(standard_name)) then
! Subprogram not used       hist_coords(i)%standard_name = trim(standard_name)
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used   end subroutine add_hist_coord_int
  ...
  subroutine lookup_hist_coord_indices(mdimnames, mdimindicies)
    ...
    do j=1,cnt
      if(mdimindicies(j)<0) then
        do i=1,registeredmdims		
          print *,"<stdin>",1163,i,hist_coords(i)%name
        end do
