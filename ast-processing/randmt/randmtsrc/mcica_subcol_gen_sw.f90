...
! Variables related to random number and seed 
      ...
      integer, dimension(ncol) :: iseed                        ! seed to create random number (Mersenne Twister)
      real(kind=r8), dimension(ncol) :: rand_num_mt                     ! random number (Mersenne Twister)
...
!-------------------------------------------------------------------------------------------------- 
! Subprogram not used       subroutine kissvec(seed1,seed2,seed3,seed4,ran_arr)
! Subprogram not used !-------------------------------------------------------------------------------------------------- 
! Subprogram not used 
! Subprogram not used ! public domain code
! Subprogram not used ! made available from http://www.fortran.com/
! Subprogram not used ! downloaded by pjr on 03/16/04 for NCAR CAM
! Subprogram not used ! converted to vector form, functions inlined by pjr,mvr on 05/10/2004
! Subprogram not used 
! Subprogram not used ! safeguard against integer overflow, statement function changed to
! Subprogram not used ! internal function by santos, Nov. 2012
! Subprogram not used 
! Subprogram not used ! The  KISS (Keep It Simple Stupid) random number generator. Combines:
! Subprogram not used ! (1) The congruential generator x(n)=69069*x(n-1)+1327217885, period 2^32.
! Subprogram not used ! (2) A 3-shift shift-register generator, period 2^32-1,
! Subprogram not used ! (3) Two 16-bit multiply-with-carry generators, period 597273182964842497>2^59
! Subprogram not used !  Overall period>2^123; 
! Subprogram not used !
! Subprogram not used       use shr_kind_mod, only: i8 => shr_kind_i8
! Subprogram not used 
! Subprogram not used       real(kind=r8), dimension(:), intent(inout)  :: ran_arr
! Subprogram not used       integer, dimension(:), intent(inout) :: seed1,seed2,seed3,seed4
! Subprogram not used       integer(i8) :: kiss
! Subprogram not used       integer :: i
! Subprogram not used 
! Subprogram not used       logical :: big_endian
! Subprogram not used 
! Subprogram not used       big_endian = (transfer(1_i8, 1) == 0)
! Subprogram not used 
! Subprogram not used       do i = 1, size(ran_arr)
! Subprogram not used          kiss = 69069_i8 * seed1(i) + 1327217885
! Subprogram not used          seed1(i) = low_byte(kiss)
! Subprogram not used          seed2(i) = m (m (m (seed2(i), 13), - 17), 5)
! Subprogram not used          seed3(i) = 18000 * iand (seed3(i), 65535) + ishft (seed3(i), - 16)
! Subprogram not used          seed4(i) = 30903 * iand (seed4(i), 65535) + ishft (seed4(i), - 16)
! Subprogram not used          kiss = int(seed1(i), i8) + seed2(i) + ishft (seed3(i), 16) + seed4(i)
! Subprogram not used          ran_arr(i) = low_byte(kiss)*2.328306e-10_r8 + 0.5_r8
! Subprogram not used       end do
! Subprogram not used     
! Subprogram not used       contains
! Subprogram not used 
! Subprogram not used         pure integer function m(k, n)
! Subprogram not used           integer, intent(in) :: k
! Subprogram not used           integer, intent(in) :: n
! Subprogram not used 
! Subprogram not used           m = ieor (k, ishft (k, n) )
! Subprogram not used 
! Subprogram not used         end function m
! Subprogram not used 
! Subprogram not used         pure integer function low_byte(i)
! Subprogram not used           integer(i8), intent(in) :: i
! Subprogram not used 
! Subprogram not used           if (big_endian) then
! Subprogram not used              low_byte = transfer(ishft(i,bit_size(1)),1)
! Subprogram not used           else
! Subprogram not used              low_byte = transfer(i,1)
! Subprogram not used           end if
! Subprogram not used 
! Subprogram not used         end function low_byte
! Subprogram not used 
! Subprogram not used       end subroutine kissvec
