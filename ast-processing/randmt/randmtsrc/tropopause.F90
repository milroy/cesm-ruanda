...
! Subprogram not used   subroutine tropopause_analytic(pstate, tropLev, tropP, tropT, tropZ)
! Subprogram not used 
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     type(physics_state), intent(in)     :: pstate
! Subprogram not used     integer,            intent(inout)   :: tropLev(pcols)             ! tropopause level index   
! Subprogram not used     real(r8), optional, intent(inout)   :: tropP(pcols)               ! tropopause pressure (Pa)  
! Subprogram not used     real(r8), optional, intent(inout)   :: tropT(pcols)               ! tropopause temperature (K)
! Subprogram not used     real(r8), optional, intent(inout)   :: tropZ(pcols)               ! tropopause height (m)
! Subprogram not used     
! Subprogram not used     ! Local Variables
! Subprogram not used     integer       :: i
! Subprogram not used     integer       :: k
! Subprogram not used     integer       :: ncol                     ! number of columns in the chunk
! Subprogram not used     integer       :: lchnk                    ! chunk identifier
! Subprogram not used     real(r8)      :: tP                       ! tropopause pressure (Pa)
! Subprogram not used  
! Subprogram not used     ! Information about the chunk.  
! Subprogram not used     lchnk = pstate%lchnk
! Subprogram not used     ncol  = pstate%ncol
! Subprogram not used 
! Subprogram not used     ! Iterate over all of the columns.
! Subprogram not used     do i = 1, ncol
! Subprogram not used      
! Subprogram not used       ! Skip column in which the tropopause has already been found.
! Subprogram not used       if (tropLev(i) == NOTFOUND) then
! Subprogram not used       
! Subprogram not used         ! Calculate the pressure of the tropopause.
! Subprogram not used         tP = (25000.0_r8 - 15000.0_r8 * (cos(pstate%lat(i)))**2)
! Subprogram not used       
! Subprogram not used         ! Find the level that contains the tropopause.
! Subprogram not used         do k = pver, 2, -1
! Subprogram not used           if (tP >= pstate%pint(i, k)) then
! Subprogram not used             tropLev(i) = k
! Subprogram not used             exit
! Subprogram not used           end if
! Subprogram not used         end do
! Subprogram not used         
! Subprogram not used         ! Return the optional outputs
! Subprogram not used         if (present(tropP)) tropP(i) = tP
! Subprogram not used         
! Subprogram not used         if (present(tropT)) then
! Subprogram not used           tropT(i) = tropopause_interpolateT(pstate, i, tropLev(i), tP)
! Subprogram not used         end if
! Subprogram not used 
! Subprogram not used         if (present(tropZ)) then
! Subprogram not used           tropZ(i) = tropopause_interpolateZ(pstate, i, tropLev(i), tP)
! Subprogram not used         end if
! Subprogram not used       end if
! Subprogram not used     end do
! Subprogram not used   end subroutine tropopause_analytic
  ...
  !-----------------------------------------------------------------------
  !-----------------------------------------------------------------------
! Subprogram not used   subroutine tropopause_hybridstobie(pstate, tropLev, tropP, tropT, tropZ)
! Subprogram not used     use cam_history,  only : outfld
! Subprogram not used   
! Subprogram not used     !-----------------------------------------------------------------------
! Subprogram not used     ! Originally written by Philip Cameron-Smith, LLNL
! Subprogram not used     !
! Subprogram not used     !   Stobie-Linoz hybrid: the highest altitude of 
! Subprogram not used     !          a) Stobie algorithm, or 
! Subprogram not used     !          b) minimum Linoz pressure.
! Subprogram not used     !
! Subprogram not used     ! NOTE: the ltrop(i) gridbox itself is assumed to be a STRATOSPHERIC gridbox.
! Subprogram not used     !-----------------------------------------------------------------------
! Subprogram not used     !-----------------------------------------------------------------------
! Subprogram not used     !        ... Local variables
! Subprogram not used     !-----------------------------------------------------------------------
...
! Subprogram not used     real(r8), optional, intent(inout)   :: tropZ(pcols)               ! tropopause height (m)
! Subprogram not used     
! Subprogram not used     real(r8),parameter  ::  min_Stobie_Pressure= 40.E2_r8 !For case 2 & 4.  [Pa]
! Subprogram not used     real(r8),parameter  ::  max_Linoz_Pressure =208.E2_r8 !For case     4.  [Pa]
! Subprogram not used 
! Subprogram not used     integer      :: i, k, ncol
! Subprogram not used     real(r8)     :: stobie_min, shybrid_temp      !temporary variable for case 2 & 3.
! Subprogram not used     integer      :: ltrop_linoz(pcols)            !Lowest possible Linoz vertical level
! Subprogram not used     integer      :: ltrop_trop(pcols)             !Tropopause level for hybrid case.
! Subprogram not used     logical      :: ltrop_linoz_set               !Flag that lowest linoz level already found.
! Subprogram not used     real(r8)     :: trop_output(pcols,pver)        !For output purposes only.
! Subprogram not used     real(r8)     :: trop_linoz_output(pcols,pver)  !For output purposes only.
! Subprogram not used     real(r8)     :: trop_trop_output(pcols,pver)   !For output purposes only.
! Subprogram not used 
! Subprogram not used     !    write(iulog,*) 'In set_chem_trop, o3_ndx =',o3_ndx
! Subprogram not used     ltrop_linoz(:) = 1  ! Initialize to default value.
! Subprogram not used     ltrop_trop(:) = 1   ! Initialize to default value.
! Subprogram not used     ncol = pstate%ncol
! Subprogram not used 
! Subprogram not used     LOOP_COL4: do i=1,ncol
! Subprogram not used 
! Subprogram not used        ! Skip column in which the tropopause has already been found.
! Subprogram not used        not_found: if (tropLev(i) == NOTFOUND) then
! Subprogram not used 
! Subprogram not used           stobie_min = 1.e10_r8    ! An impossibly large number
! Subprogram not used           ltrop_linoz_set = .FALSE.
! Subprogram not used           LOOP_LEV: do k=pver,1,-1
! Subprogram not used              IF (pstate%pmid(i,k) < min_stobie_pressure) cycle
! Subprogram not used              shybrid_temp = ALPHA * pstate%t(i,k) - Log10(pstate%pmid(i,k))
! Subprogram not used              !PJC_NOTE: the units of pmid won't matter, because it is just an additive offset.
! Subprogram not used              IF (shybrid_temp<stobie_min) then 
! Subprogram not used                 ltrop_trop(i)=k     
! Subprogram not used                 stobie_min = shybrid_temp
! Subprogram not used              ENDIF
! Subprogram not used              IF (pstate%pmid(i,k) < max_Linoz_pressure .AND. .NOT. ltrop_linoz_set) THEN
! Subprogram not used                 ltrop_linoz(i) = k
! Subprogram not used                 ltrop_linoz_set = .TRUE.
! Subprogram not used              ENDIF
! Subprogram not used           enddo LOOP_LEV
! Subprogram not used 
! Subprogram not used           tropLev(i) = MIN(ltrop_trop(i),ltrop_linoz(i))
! Subprogram not used 
! Subprogram not used           if (present(tropP)) then
! Subprogram not used              tropP(i) = pstate%pmid(i,tropLev(i))
! Subprogram not used           endif
! Subprogram not used           if (present(tropT)) then
! Subprogram not used              tropT(i) = pstate%   t(i,tropLev(i))
! Subprogram not used           endif
! Subprogram not used           if (present(tropZ)) then
! Subprogram not used              tropZ(i) = pstate%  zm(i,tropLev(i))
! Subprogram not used           endif
! Subprogram not used 
! Subprogram not used        endif not_found
! Subprogram not used 
! Subprogram not used     enddo LOOP_COL4
! Subprogram not used 
! Subprogram not used     trop_output(:,:)=0._r8
! Subprogram not used     trop_linoz_output(:,:)=0._r8
! Subprogram not used     trop_trop_output(:,:)=0._r8
! Subprogram not used     do i=1,ncol
! Subprogram not used        trop_output(i,tropLev(i))=1._r8
! Subprogram not used        trop_linoz_output(i,ltrop_linoz(i))=1._r8
! Subprogram not used        trop_trop_output(i,ltrop_trop(i))=1._r8
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used     call outfld( 'hstobie_trop',   trop_output(:ncol,:),       ncol, pstate%lchnk )
! Subprogram not used     call outfld( 'hstobie_linoz',  trop_linoz_output(:ncol,:), ncol, pstate%lchnk )
! Subprogram not used     call outfld( 'hstobie_tropop', trop_trop_output(:ncol,:),  ncol, pstate%lchnk )
! Subprogram not used 
! Subprogram not used   endsubroutine tropopause_hybridstobie
  ...
  !
! Subprogram not used   subroutine tropopause_stobie(pstate, tropLev, tropP, tropT, tropZ)
! Subprogram not used 
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     type(physics_state), intent(in)     :: pstate 
...
! Subprogram not used     integer       :: ncol                     ! number of columns in the chunk
! Subprogram not used     integer       :: lchnk                    ! chunk identifier
! Subprogram not used     integer       :: tLev                     ! tropopause level
! Subprogram not used     real(r8)      :: tP                       ! tropopause pressure (Pa)
! Subprogram not used     real(r8)      :: stobie(pver)             ! stobie weighted temperature
! Subprogram not used     real(r8)      :: sTrop                    ! stobie value at the tropopause
! Subprogram not used  
! Subprogram not used     ! Information about the chunk.  
! Subprogram not used     lchnk = pstate%lchnk
! Subprogram not used     ncol  = pstate%ncol
...
! Subprogram not used       if (tropLev(i) == NOTFOUND) then
! Subprogram not used       
! Subprogram not used         ! Caclulate a pressure weighted temperature.
! Subprogram not used         stobie(:) = ALPHA * pstate%t(i,:) - log10(pstate%pmid(i, :))
! Subprogram not used 
! Subprogram not used         ! Search from the bottom up, looking for the first minimum.
! Subprogram not used         tLev  = -1
! Subprogram not used   
! Subprogram not used         do k = pver-1, 1, -1
! Subprogram not used     
! Subprogram not used           if (pstate%pmid(i, k) <= 4000._r8) then
! Subprogram not used             exit
! Subprogram not used           end if
! Subprogram not used     
! Subprogram not used           if (pstate%pmid(i, k) >= 55000._r8) then
! Subprogram not used             cycle
! Subprogram not used           end if
! Subprogram not used           
! Subprogram not used           if ((tLev == -1) .or. (stobie(k) < sTrop)) then
! Subprogram not used             tLev  = k
! Subprogram not used             tP    = pstate%pmid(i, k)
! Subprogram not used             sTrop = stobie(k)
! Subprogram not used           end if
! Subprogram not used         end do
! Subprogram not used         
! Subprogram not used         if (tLev /= -1) then
! Subprogram not used           tropLev(i) = tLev
! Subprogram not used         
! Subprogram not used           ! Return the optional outputs
! Subprogram not used           if (present(tropP)) tropP(i) = tP
! Subprogram not used           
...
! Subprogram not used   end subroutine tropopause_stobie
  ...
  ! made to have the routine work within the 1 framework (i.e. using
...
! Subprogram not used   subroutine tropopause_wmo(pstate, tropLev, tropP, tropT, tropZ)
! Subprogram not used 
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     type(physics_state), intent(in)    :: pstate 
...
! Subprogram not used 
! Subprogram not used     ! Local Variables 
! Subprogram not used     real(r8), parameter    :: ztrop_low   = 5000._r8        ! lowest tropopause level allowed (m)
! Subprogram not used     real(r8), parameter    :: ztrop_high  = 20000._r8       ! highest tropopause level allowed (m)
! Subprogram not used     real(r8), parameter    :: max_dtdz    = 0.002_r8        ! max dt/dz for tropopause level (K/m)
! Subprogram not used     real(r8), parameter    :: min_trop_dz = 2000._r8        ! min tropopause thickness (m)
! Subprogram not used 
! Subprogram not used     integer                 :: i
! Subprogram not used     integer                 :: k
! Subprogram not used     integer                 :: k2
! Subprogram not used     integer                 :: ncol                         ! number of columns in the chunk
! Subprogram not used     integer                 :: lchnk                        ! chunk identifier
! Subprogram not used     real(r8)                :: tP                           ! tropopause pressure (Pa)
! Subprogram not used     real(r8)                :: dt
! Subprogram not used 
! Subprogram not used     ! Information about the chunk.  
! Subprogram not used     lchnk = pstate%lchnk
! Subprogram not used     ncol  = pstate%ncol
...
! Subprogram not used       if (tropLev(i) == NOTFOUND) then
! Subprogram not used 
! Subprogram not used         kloop: do k = pver-1, 2, -1
! Subprogram not used          
! Subprogram not used           ! Skip levels below the minimum and stop if nothing is found
! Subprogram not used           ! before the maximum.
! Subprogram not used           if (pstate%zm(i, k) < ztrop_low) then
! Subprogram not used             cycle kloop
! Subprogram not used           else if (pstate%zm(i, k) > ztrop_high) then
! Subprogram not used             exit kloop
! Subprogram not used           end if
! Subprogram not used           
! Subprogram not used           ! Compare the actual lapse rate to the threshold
! Subprogram not used           dt = pstate%t(i, k) - pstate%t(i, k-1)
! Subprogram not used             
! Subprogram not used           if (dt <= (max_dtdz * (pstate%zm(i, k-1) - pstate%zm(i, k)))) then
! Subprogram not used             
! Subprogram not used             ! Make sure that the lapse rate stays below the threshold for the
! Subprogram not used             ! specified range.
! Subprogram not used             k2loop: do k2 = k-1, 2, -1
! Subprogram not used               if ((pstate%zm(i, k2) - pstate%zm(i, k)) >= min_trop_dz) then
! Subprogram not used                 tP = pstate%pmid(i, k)
! Subprogram not used                 tropLev(i) = k
! Subprogram not used                 exit k2loop
! Subprogram not used               end if
! Subprogram not used               
! Subprogram not used               dt = pstate%t(i, k) - pstate%t(i, k2)
! Subprogram not used               if (dt > (max_dtdz * (pstate%zm(i, k2) - pstate%zm(i, k)))) then
! Subprogram not used                 exit k2loop
! Subprogram not used               end if
! Subprogram not used            end do k2loop
! Subprogram not used 
! Subprogram not used            if (tropLev(i) == NOTFOUND) then
! Subprogram not used               cycle kloop
...
! Subprogram not used             end if
! Subprogram not used           end if
! Subprogram not used         end do kloop
! Subprogram not used       end if
! Subprogram not used     end do
! Subprogram not used     
! Subprogram not used     return
! Subprogram not used   end subroutine tropopause_wmo
