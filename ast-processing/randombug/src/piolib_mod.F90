...
! Subprogram not used        end if
! Subprogram not used     
! Subprogram not used        call mpi_info_create(iosystem(i)%info,ierr)
! Subprogram not used        ! turn on mpi-io aggregation 
! Subprogram not used        !DBG    print *,'PIO_init: before call to setnumagg'
! Subprogram not used !       itmp = num_aggregator
! Subprogram not used !       call mpi_bcast(itmp, 1, mpi_integer, 0, iosystem%union_comm, ierr)
! Subprogram not used !       if(itmp .gt. 0) then 
! Subprogram not used !          write(cb_nodes,('(i5)')) itmp
! Subprogram not used !#ifdef BGx
! Subprogram not used !          call PIO_set_hint(iosystem(i),"bgl_nodes_pset",trim(adjustl(cb_nodes)))
! Subprogram not used !#else
! Subprogram not used !          call PIO_set_hint(iosystem(i),"cb_nodes",trim(adjustl(cb_nodes)))
! Subprogram not used !#endif       
! Subprogram not used !       endif
! Subprogram not used 
! Subprogram not used        call PIO_set_hint(iosystem(i),"ibm_largeblock_io","true")
! Subprogram not used     end do
! Subprogram not used 
...
!<
! Subprogram not used   integer function numtoread(iodesc) result(num)
! Subprogram not used 
! Subprogram not used     type (io_desc_t) :: iodesc
! Subprogram not used 
! Subprogram not used     num = iodesc%read%n_words
! Subprogram not used 
! Subprogram not used   end function numtoread
...
!! @param iodesc : @copydoc io_desc_t
!<
! Subprogram not used   integer function numtowrite(iodesc) result(num)
! Subprogram not used 
! Subprogram not used     type (io_desc_t) :: iodesc
! Subprogram not used 
! Subprogram not used     num = iodesc%write%n_words
! Subprogram not used 
! Subprogram not used   end function numtowrite
...
!<
! Subprogram not used   subroutine PIO_setnum_OST(iosystem,numOST)
! Subprogram not used      type (iosystem_desc_t), intent(inout), target :: iosystem
! Subprogram not used      integer(i4) :: numOST
! Subprogram not used      iosystem%numOST = numOST
! Subprogram not used   end subroutine PIO_setnum_OST
!>
!! @public
...
!<
! Subprogram not used   integer function PIO_getnum_OST(iosystem) result(numOST)
! Subprogram not used      type (iosystem_desc_t), intent(inout), target :: iosystem
! Subprogram not used      numOST = iosystem%numOST
! Subprogram not used   end function PIO_getnum_OST
!> 
!! @public
...
!! @param file @copydoc file_desc_t
!<
! Subprogram not used   subroutine syncfile(file)
! Subprogram not used     use piodarray, only : darray_write_complete
! Subprogram not used     implicit none
! Subprogram not used     type (file_desc_t), target :: file
! Subprogram not used     integer :: ierr, msg
...
! Subprogram not used        msg = PIO_MSG_SYNC_FILE
! Subprogram not used        if(ios%comp_rank==0) then
! Subprogram not used           call mpi_send(msg, 1, mpi_integer, ios%ioroot, 1, ios%union_comm, ierr)
! Subprogram not used        end if
! Subprogram not used       
...
! Subprogram not used     case( pio_iotype_pnetcdf, pio_iotype_netcdf)
! Subprogram not used        call darray_write_complete(file)
! Subprogram not used        ierr = sync_nf(file)
! Subprogram not used     case(pio_iotype_pbinary, pio_iotype_direct_pbinary)
! Subprogram not used     case(pio_iotype_binary) 
! Subprogram not used     end select
! Subprogram not used   end subroutine syncfile
!> 
!! @public 
...
! Subprogram not used   subroutine read_ascii(rank,iobuf,size)
! Subprogram not used 
! Subprogram not used     integer, intent(in) :: rank
! Subprogram not used     real (r8), dimension(:) :: iobuf
! Subprogram not used     integer, intent(in) :: size
! Subprogram not used 
! Subprogram not used     character(len=80) filename
! Subprogram not used     integer lun
! Subprogram not used     integer ios
! Subprogram not used     integer i
! Subprogram not used 
! Subprogram not used     lun=10+rank
! Subprogram not used     write(filename,"('fort.',i2)" ) lun
! Subprogram not used     write(6,*) 'filename is:', filename
! Subprogram not used 
! Subprogram not used     open(lun,file=filename,status='old',iostat=ios)
! Subprogram not used     if (ios /= 0) then
! Subprogram not used        write(6,*) rank,': could not open ascii file: ',filename
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used     do i=1,size
! Subprogram not used        read(unit=lun,fmt=*,iostat=ios) iobuf(i)
! Subprogram not used        if (ios /= 0) then
! Subprogram not used           write (6,*) rank,': error reading item ',i,' of ',size
! Subprogram not used           call abort
! Subprogram not used        endif
! Subprogram not used 
! Subprogram not used     end do
! Subprogram not used 
! Subprogram not used     close(lun)
! Subprogram not used 
! Subprogram not used   end subroutine read_ascii
