...
! !INTERFACE:
!
! Subprogram not used       subroutine get_matrix_elements_know_Tsfc &
! Subprogram not used                                      (nx_block, ny_block,         &
! Subprogram not used                                       isolve,   icells,           &
! Subprogram not used                                       indxii,   indxjj,   indxij, &
! Subprogram not used                                       l_snow,   Tbot,             &
! Subprogram not used                                       Tin_init, Tsn_init,         &
! Subprogram not used                                       kh,       Sswabs,           &
! Subprogram not used                                       Iswabs,                     &
! Subprogram not used                                       etai,     etas,             &
! Subprogram not used                                       sbdiag,   diag,             &
! Subprogram not used                                       spdiag,   rhs,              &
! Subprogram not used                                       fcondtopn)
! Subprogram not used !
...
! Subprogram not used          isolve            , & ! number of cells with temps not converged
...
! Subprogram not used          indxii, indxjj  ! compressed indices for cells not converged
! Subprogram not used 
! Subprogram not used       integer (kind=int_kind), dimension (icells), &
! Subprogram not used          intent(in) :: &
! Subprogram not used          indxij          ! compressed 1D index for cells not converged
! Subprogram not used 
! Subprogram not used       logical (kind=log_kind), dimension (icells), &
! Subprogram not used          intent(in) :: &
! Subprogram not used          l_snow          ! true if snow temperatures are computed
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), dimension (nx_block,ny_block), intent(in) :: &
! Subprogram not used          Tbot            ! ice bottom surface temperature (deg C)
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), dimension (isolve,nilyr), &
! Subprogram not used          intent(in) :: &
! Subprogram not used          etai            ! dt / (rho*cp*h) for ice layers
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), dimension (icells,nilyr), &
! Subprogram not used          intent(in) :: &
! Subprogram not used          Tin_init        ! ice temp at beginning of time step
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), dimension (nx_block,ny_block,nslyr), &
! Subprogram not used          intent(in) :: &
! Subprogram not used          Sswabs          ! SW radiation absorbed in snow layers (W m-2)
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), dimension (nx_block,ny_block,nilyr), &
! Subprogram not used          intent(in) :: &
! Subprogram not used          Iswabs          ! absorbed SW flux in ice layers
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), dimension (icells,nslyr), &
! Subprogram not used          intent(in) :: &
! Subprogram not used          etas        , & ! dt / (rho*cp*h) for snow layers
! Subprogram not used          Tsn_init        ! snow temp at beginning of time step
! Subprogram not used                          ! Note: no absorbed SW in snow layers
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), dimension (icells,nslyr+nilyr+1), &
! Subprogram not used          intent(in) :: &
! Subprogram not used          kh              ! effective conductivity at layer interfaces
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), dimension (isolve,nslyr+nilyr+1), &
! Subprogram not used          intent(inout) :: &
! Subprogram not used          sbdiag      , & ! sub-diagonal matrix elements
! Subprogram not used          diag        , & ! diagonal matrix elements
! Subprogram not used          spdiag      , & ! super-diagonal matrix elements
! Subprogram not used          rhs             ! rhs of tri-diagonal matrix eqn.
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), dimension (nx_block,ny_block), intent(in),  &
! Subprogram not used          optional :: &
! Subprogram not used          fcondtopn       ! conductive flux at top sfc, positive down (W/m^2)
! Subprogram not used !
! Subprogram not used !EOP
! Subprogram not used !
! Subprogram not used       integer (kind=int_kind) :: &
! Subprogram not used          i, j        , & ! horizontal indices
! Subprogram not used          ij, m       , & ! horizontal indices, combine i and j loops
! Subprogram not used          k, ks, ki, kr   ! vertical indices and row counters
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! Initialize matrix elements.
! Subprogram not used       ! Note: When we do not need to solve for the surface or snow
! Subprogram not used       !       temperature, we solve dummy equations with solution T = 0.
! Subprogram not used       !       Ice layers are fully initialized below.
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used       do k = 1, nslyr+1
! Subprogram not used          do ij = 1, isolve
! Subprogram not used             sbdiag(ij,k) = c0
! Subprogram not used             diag  (ij,k) = c1
! Subprogram not used             spdiag(ij,k) = c0
! Subprogram not used             rhs   (ij,k) = c0
! Subprogram not used          enddo
! Subprogram not used       enddo
! Subprogram not used             
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! Compute matrix elements
! Subprogram not used       !
! Subprogram not used       ! Four possible cases to solve:
! Subprogram not used       !   (1) Cold surface (Tsf < 0), snow present
! Subprogram not used       !   (2) Melting surface (Tsf = 0), snow present
! Subprogram not used       !   (3) Cold surface (Tsf < 0), no snow
! Subprogram not used       !   (4) Melting surface (Tsf = 0), no snow
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! top snow layer
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used !        k = 1
! Subprogram not used !        kr = 2
! Subprogram not used 
! Subprogram not used          do ij = 1, isolve
...
! Subprogram not used 
! Subprogram not used             if (l_snow(m)) then
! Subprogram not used                sbdiag(ij,2) = c0
! Subprogram not used                spdiag(ij,2) = -etas(m,1) * kh(m,2)
! Subprogram not used                diag  (ij,2) = c1                                 &
! Subprogram not used                              + etas(m,1) * kh(m,2)
! Subprogram not used                rhs   (ij,2) = Tsn_init(m,1)                      &
! Subprogram not used                              + etas(m,1) * Sswabs(i,j,1)         &
! Subprogram not used                              + etas(m,1) * fcondtopn(i,j)
! Subprogram not used             endif   ! l_snow
! Subprogram not used          enddo   ! ij
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! remaining snow layers
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used       if (nslyr > 1) then
! Subprogram not used 
! Subprogram not used          do k = 2, nslyr
! Subprogram not used             kr = k + 1
! Subprogram not used 
! Subprogram not used             do ij = 1, isolve
! Subprogram not used                i = indxii(ij)
! Subprogram not used                j = indxjj(ij)
...
! Subprogram not used 
! Subprogram not used                if (l_snow(m)) then
! Subprogram not used                   sbdiag(ij,kr) = -etas(m,k) * kh(m,k)
! Subprogram not used                   spdiag(ij,kr) = -etas(m,k) * kh(m,k+1)
! Subprogram not used                   diag  (ij,kr) = c1 &
! Subprogram not used                                + etas(m,k) * (kh(m,k) + kh(m,k+1))
! Subprogram not used                   rhs   (ij,kr) = Tsn_init(m,k) &
! Subprogram not used                                + etas(m,k) * Sswabs(i,j,k)
! Subprogram not used                endif
! Subprogram not used             enddo               ! ij
! Subprogram not used          enddo                  ! nslyr
! Subprogram not used 
! Subprogram not used       endif                     ! nslyr > 1
! Subprogram not used 
! Subprogram not used 
! Subprogram not used       if (nilyr > 1) then
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! top ice layer
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used          ki = 1
! Subprogram not used          k  = ki + nslyr
! Subprogram not used          kr = k + 1
! Subprogram not used 
! Subprogram not used             do ij = 1, isolve
! Subprogram not used                i = indxii(ij)
...
! Subprogram not used                if (l_snow(m)) then
! Subprogram not used 
! Subprogram not used                   sbdiag(ij,kr) = -etai(ij,ki) * kh(m,k)
! Subprogram not used                   spdiag(ij,kr) = -etai(ij,ki) * kh(m,k+1)
! Subprogram not used                   diag  (ij,kr) = c1                                &
! Subprogram not used                                  + etai(ij,ki) * (kh(m,k) + kh(m,k+1))
! Subprogram not used                   rhs   (ij,kr) = Tin_init(m,ki)                    &
! Subprogram not used                                  + etai(ij,ki) * Iswabs(i,j,ki)
! Subprogram not used                else                  
! Subprogram not used                   sbdiag(ij,kr) = c0
! Subprogram not used                   spdiag(ij,kr) = -etai(ij,ki) * kh(m,k+1)
! Subprogram not used                   diag  (ij,kr) = c1                                &
! Subprogram not used                                  + etai(ij,ki) * kh(m,k+1)
! Subprogram not used                   rhs   (ij,kr) = Tin_init(m,ki)                    &
! Subprogram not used                                  + etai(ij,ki) * Iswabs(i,j,ki)       &
! Subprogram not used                                  + etai(ij,ki) * fcondtopn(i,j)
! Subprogram not used                endif  ! l_snow
! Subprogram not used             enddo   ! ij
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! bottom ice layer
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used          ki = nilyr
! Subprogram not used          k  = ki + nslyr
! Subprogram not used          kr = k + 1
! Subprogram not used       
! Subprogram not used          do ij = 1, isolve
...
! Subprogram not used             m = indxij(ij)
! Subprogram not used             sbdiag(ij,kr) = -etai(ij,ki) * kh(m,k)
! Subprogram not used             spdiag(ij,kr) = c0
! Subprogram not used             diag  (ij,kr) = c1  &
! Subprogram not used                            + etai(ij,ki) * (kh(m,k) + kh(m,k+1))
! Subprogram not used             rhs   (ij,kr) = Tin_init(m,ki) &
! Subprogram not used                            + etai(ij,ki)*Iswabs(i,j,ki) &
! Subprogram not used                            + etai(ij,ki)*kh(m,k+1)*Tbot(i,j)
! Subprogram not used 
! Subprogram not used          enddo                   ! ij
! Subprogram not used       
! Subprogram not used       else         ! nilyr = 1
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! single ice layer
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used          ki = 1
! Subprogram not used          k  = ki + nslyr
...
! Subprogram not used             enddo                     ! ij
! Subprogram not used 
! Subprogram not used       endif        ! nilyr > 1
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! interior ice layers
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used       do ki = 2, nilyr-1
! Subprogram not used            
! Subprogram not used          k  = ki + nslyr
! Subprogram not used          kr = k + 1
! Subprogram not used          do ij = 1, isolve
...
! Subprogram not used 
! Subprogram not used       end subroutine get_matrix_elements_know_Tsfc
...
! !INTERFACE:
!
! Subprogram not used       subroutine zerolayer_temperature(nx_block, ny_block, &
! Subprogram not used                                        my_task,  istep1,   &
! Subprogram not used                                        dt,       icells,   & 
! Subprogram not used                                        indxi,    indxj,    &
! Subprogram not used                                        rhoa,     flw,      &
! Subprogram not used                                        potT,     Qa,       &
! Subprogram not used                                        shcoef,   lhcoef,   &
! Subprogram not used                                        fswsfc,   fswthrun, &
! Subprogram not used                                        hilyr,    hslyr,    &
! Subprogram not used                                        Tsf,      Tbot,     &
! Subprogram not used                                        fsensn,   flatn,    &
! Subprogram not used                                        fswabsn,  flwoutn,  &
! Subprogram not used                                        fsurfn,             &
! Subprogram not used                                        fcondtopn,fcondbot, &
...
! Subprogram not used       integer (kind=int_kind), intent(in) :: &
! Subprogram not used          nx_block, ny_block, & ! block dimensions
! Subprogram not used          my_task     , & ! task number (diagnostic only)
! Subprogram not used          istep1      , & ! time step index (diagnostic only)
! Subprogram not used          icells          ! number of cells with aicen > puny
! Subprogram not used 
...
! Subprogram not used          intent(in) :: &
! Subprogram not used          indxi, indxj    ! compressed indices for cells with aicen > puny
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), dimension (nx_block,ny_block), intent(in) :: &
! Subprogram not used          rhoa        , & ! air density (kg/m^3)
! Subprogram not used          flw         , & ! incoming longwave radiation (W/m^2)
! Subprogram not used          potT        , & ! air potential temperature  (K)
! Subprogram not used          Qa          , & ! specific humidity (kg/kg)
! Subprogram not used          shcoef      , & ! transfer coefficient for sensible heat
! Subprogram not used          lhcoef      , & ! transfer coefficient for latent heat
! Subprogram not used          Tbot        , & ! ice bottom surface temperature (deg C)
! Subprogram not used          fswsfc      , & ! SW absorbed at ice/snow surface (W m-2)
! Subprogram not used          fswthrun        ! SW through ice to ocean         (W m-2)
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), dimension (icells), intent(in) :: &
! Subprogram not used          hilyr       , & ! ice layer thickness (m)
! Subprogram not used          hslyr           ! snow layer thickness (m)
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), dimension (nx_block,ny_block), intent(inout):: &
! Subprogram not used          fsensn      , & ! surface downward sensible heat (W m-2)
! Subprogram not used          fswabsn     , & ! shortwave flux absorbed by ice (W/m-2) 
! Subprogram not used          flatn       , & ! surface downward latent heat (W m-2)
! Subprogram not used          flwoutn     , & ! upward LW at surface (W m-2)
! Subprogram not used          fsurfn      , & ! net flux to top surface, excluding fcondtopn
! Subprogram not used          fcondtopn       ! downward cond flux at top surface (W m-2)
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), dimension (icells), intent(out):: &
! Subprogram not used          fcondbot        ! downward cond flux at bottom surface (W m-2)
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), dimension (icells), &
! Subprogram not used          intent(inout):: &
! Subprogram not used          Tsf             ! ice/snow surface temperature, Tsfcn
! Subprogram not used 
! Subprogram not used       logical (kind=log_kind), intent(inout) :: &
! Subprogram not used          l_stop          ! if true, print diagnostics and abort model
! Subprogram not used 
! Subprogram not used       integer (kind=int_kind), intent(inout) :: &
! Subprogram not used          istop, jstop    ! i and j indices of cell where model fails
! Subprogram not used !
! Subprogram not used !EOP
! Subprogram not used !
! Subprogram not used       logical (kind=log_kind), parameter :: &
! Subprogram not used          l_zerolayerchecks = .true.
! Subprogram not used 
! Subprogram not used       integer (kind=int_kind), parameter :: &
! Subprogram not used          nitermax = 50   ! max number of iterations in temperature solver
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), parameter :: &
! Subprogram not used          Tsf_errmax = 5.e-4_dbl_kind ! max allowed error in Tsf
! Subprogram not used                                      ! recommend Tsf_errmax < 0.01 K
! Subprogram not used 
! Subprogram not used       integer (kind=int_kind) :: &
! Subprogram not used          i, j        , & ! horizontal indices
! Subprogram not used          ij, m       , & ! horizontal indices, combine i and j loops
! Subprogram not used          niter           ! iteration counter in temperature solver
! Subprogram not used 
! Subprogram not used       integer (kind=int_kind) :: &
! Subprogram not used          isolve          ! number of cells with temps not converged
! Subprogram not used 
...
! Subprogram not used          Tsf_start   , & ! Tsf at start of iteration
! Subprogram not used          dTsf        , & ! Tsf - Tsf_start
! Subprogram not used          dfsurf_dT       ! derivative of fsurfn wrt Tsf
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), dimension (icells) :: &
! Subprogram not used          dTsf_prev   , & ! dTsf from previous iteration
! Subprogram not used          dfsens_dT   , & ! deriv of fsens wrt Tsf (W m-2 deg-1)
! Subprogram not used          dflat_dT    , & ! deriv of flat wrt Tsf (W m-2 deg-1)
! Subprogram not used          dflwout_dT      ! deriv of flwout wrt Tsf (W m-2 deg-1)
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), dimension (:), allocatable :: &
! Subprogram not used          heff        , & ! effective ice thickness (m)
! Subprogram not used                          ! ( hice + hsno*kseaice/ksnow)
! Subprogram not used          kh          , & ! effective conductivity
! Subprogram not used          diag        , & ! diagonal matrix elements
! Subprogram not used          rhs             ! rhs of tri-diagonal matrix equation
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind) :: &
! Subprogram not used          kratio      , & ! ratio of ice and snow conductivies
! Subprogram not used          avg_Tsf         ! = 1. if Tsf averaged w/Tsf_start, else = 0.
! Subprogram not used 
! Subprogram not used       logical (kind=log_kind), dimension (icells) :: &
! Subprogram not used          converged      ! = true when local solution has converged
! Subprogram not used 
! Subprogram not used       logical (kind=log_kind) :: &
! Subprogram not used          all_converged  ! = true when all cells have converged
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! Initialize
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used       all_converged   = .false.
! Subprogram not used 
! Subprogram not used       do ij = 1, icells
! Subprogram not used          fcondbot(ij) = c0
! Subprogram not used 
! Subprogram not used          converged (ij) = .false.
! Subprogram not used 
! Subprogram not used          dTsf_prev (ij) = c0
! Subprogram not used 
! Subprogram not used       enddo                     ! ij
! Subprogram not used       
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! Solve for new temperatures.
! Subprogram not used       ! Iterate until temperatures converge with minimal temperature
! Subprogram not used       ! change.
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used       do niter = 1, nitermax
! Subprogram not used 
! Subprogram not used          if (all_converged) then  ! thermo calculation is done
! Subprogram not used             exit
! Subprogram not used          else                     ! identify cells not yet converged
! Subprogram not used             isolve = 0
! Subprogram not used             do ij = 1, icells
! Subprogram not used                i = indxi(ij)
! Subprogram not used                j = indxj(ij)
! Subprogram not used                if (.not.converged(ij)) then
! Subprogram not used                   isolve = isolve + 1
! Subprogram not used                   indxii(isolve) = i
! Subprogram not used                   indxjj(isolve) = j
! Subprogram not used                   indxij(isolve) = ij
! Subprogram not used                endif
! Subprogram not used             enddo               ! ij
! Subprogram not used          endif
! Subprogram not used 
! Subprogram not used          allocate(     diag(isolve))
! Subprogram not used          allocate(      rhs(isolve))
! Subprogram not used          allocate(       kh(isolve))
! Subprogram not used          allocate(     heff(isolve))
! Subprogram not used          allocate(Tsf_start(isolve))
! Subprogram not used          allocate(     dTsf(isolve))
! Subprogram not used          allocate(dfsurf_dT(isolve))
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! Update radiative and turbulent fluxes and their derivatives
! Subprogram not used       ! with respect to Tsf.
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used          call surface_fluxes (nx_block,    ny_block,          &
! Subprogram not used                               isolve,      icells,            &
! Subprogram not used                               indxii,      indxjj,    indxij, &
! Subprogram not used                               Tsf,         fswsfc,            &
! Subprogram not used                               rhoa,        flw,               &
! Subprogram not used                               potT,        Qa,                &
! Subprogram not used                               shcoef,      lhcoef,            &
! Subprogram not used                               flwoutn,     fsensn,            &
! Subprogram not used                               flatn,       fsurfn,            &
! Subprogram not used                               dflwout_dT,  dfsens_dT,         &
! Subprogram not used                               dflat_dT,    dfsurf_dT)
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! Compute effective ice thickness (includes snow) and thermal 
! Subprogram not used       ! conductivity 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used          kratio = kseaice/ksno
! Subprogram not used  
! Subprogram not used          do ij = 1, isolve
! Subprogram not used              m = indxij(ij)
! Subprogram not used    
! Subprogram not used              heff(ij) = hilyr(m) + kratio * hslyr(m)
! Subprogram not used              kh(ij) = kseaice / heff(ij)        
! Subprogram not used          enddo                     ! ij
! Subprogram not used 
! Subprogram not used 
...
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! Compute conductive flux at top surface, fcondtopn.
! Subprogram not used       ! If fsurfn < fcondtopn and Tsf = 0, then reset Tsf to slightly less
! Subprogram not used       !  than zero (but not less than -puny).
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used             fcondtopn(i,j) = kh(ij) * (Tsf(m) - Tbot(i,j))
! Subprogram not used 
! Subprogram not used             if (fsurfn(i,j) < fcondtopn(i,j)) &
! Subprogram not used                  Tsf(m) = min (Tsf(m), -puny)
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! Save surface temperature at start of iteration
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used             Tsf_start(ij) = Tsf(m)
! Subprogram not used 
! Subprogram not used          enddo                  ! ij
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! Solve surface balance equation to obtain the new temperatures.
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used          do ij = 1, isolve
! Subprogram not used             i = indxii(ij)
...
! Subprogram not used             m = indxij(ij)
! Subprogram not used 
! Subprogram not used             diag(ij)  = dfsurf_dT(ij) - kh(ij)
! Subprogram not used             rhs(ij)   = dfsurf_dT(ij)*Tsf(m) - fsurfn(i,j)   &
! Subprogram not used                         - kh(ij)*Tbot(i,j)
! Subprogram not used             Tsf(m)  = rhs(ij) / diag(ij)
! Subprogram not used 
! Subprogram not used          enddo
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! Determine whether the computation has converged to an acceptable
! Subprogram not used       ! solution.  Four conditions must be satisfied:
! Subprogram not used       !
! Subprogram not used       !    (1) Tsf <= 0 C.
! Subprogram not used       !    (2) Tsf is not oscillating; i.e., if both dTsf(niter) and
! Subprogram not used       !        dTsf(niter-1) have magnitudes greater than puny, then
! Subprogram not used       !        dTsf(niter)/dTsf(niter-1) cannot be a negative number
! Subprogram not used       !        with magnitude greater than 0.5.  
! Subprogram not used       !    (3) abs(dTsf) < Tsf_errmax
! Subprogram not used       !    (4) If Tsf = 0 C, then the downward turbulent/radiative 
! Subprogram not used       !        flux, fsurfn, must be greater than or equal to the downward
! Subprogram not used       !        conductive flux, fcondtopn.
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used          ! initialize global convergence flag
! Subprogram not used          all_converged = .true.
! Subprogram not used 
! Subprogram not used !DIR$ CONCURRENT !Cray
! Subprogram not used !cdir nodep      !NEC
! Subprogram not used !ocl novrec      !Fujitsu
...
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! Initialize convergence flag (true until proven false), dTsf,
! Subprogram not used       !  and temperature-averaging coefficients.
! Subprogram not used       ! Average only if test 1 or 2 fails.
! Subprogram not used       ! Initialize energy.
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used             converged(m) = .true.
! Subprogram not used             dTsf(ij) = Tsf(m) - Tsf_start(ij)
! Subprogram not used             avg_Tsf      = c0
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! Condition 1: check for Tsf > 0
! Subprogram not used       ! If Tsf > 0, set Tsf = 0 and leave converged=.true.
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used             if (Tsf(m) > puny) then
! Subprogram not used                Tsf(m) = c0
! Subprogram not used                dTsf(ij) = -Tsf_start(ij)
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! Condition 2: check for oscillating Tsf
! Subprogram not used       ! If oscillating, average all temps to increase rate of convergence.
! Subprogram not used       ! It is possible that this may never occur.
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used             elseif (niter > 1 &                ! condition (2)
! Subprogram not used               .and. Tsf_start(ij) <= -puny &
! Subprogram not used               .and. abs(dTsf(ij)) > puny &
! Subprogram not used               .and. abs(dTsf_prev(m)) > puny &
! Subprogram not used               .and. -dTsf(ij)/(dTsf_prev(m)+puny*puny) > p5) then
! Subprogram not used 
! Subprogram not used                avg_Tsf  = c1  ! average with starting temp  
! Subprogram not used                dTsf(ij) = p5 * dTsf(ij)
! Subprogram not used                converged(m) = .false.
! Subprogram not used                all_converged = .false.
! Subprogram not used             endif
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! If condition 2 failed, average new surface temperature with
! Subprogram not used       !  starting value.
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used             Tsf(m)  = Tsf(m) &
! Subprogram not used                       + avg_Tsf * p5 * (Tsf_start(ij) - Tsf(m))
! Subprogram not used 
! Subprogram not used          enddo  ! ij
! Subprogram not used 
! Subprogram not used !DIR$ CONCURRENT !Cray
...
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! Condition 3: check for large change in Tsf
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used             if (abs(dTsf(ij)) > Tsf_errmax) then
! Subprogram not used                converged(m) = .false.
! Subprogram not used                all_converged = .false.
! Subprogram not used             endif
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! Condition 4: check for fsurfn < fcondtopn with Tsf > 0
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used             fsurfn(i,j) = fsurfn(i,j) + dTsf(ij)*dfsurf_dT(ij)
! Subprogram not used             fcondtopn(i,j) = kh(ij) * (Tsf(m)-Tbot(i,j))
! Subprogram not used 
! Subprogram not used             if (Tsf(m) > -puny .and. fsurfn(i,j) < fcondtopn(i,j)) then
! Subprogram not used                converged(m) = .false.
! Subprogram not used                all_converged = .false.
! Subprogram not used             endif
! Subprogram not used 
! Subprogram not used             fcondbot(m) = fcondtopn(i,j)
! Subprogram not used 
! Subprogram not used             dTsf_prev(m) = dTsf(ij)
! Subprogram not used 
! Subprogram not used          enddo                  ! ij
! Subprogram not used 
! Subprogram not used          deallocate(diag)
! Subprogram not used          deallocate(rhs)
! Subprogram not used          deallocate(kh)
! Subprogram not used          deallocate(heff)
! Subprogram not used          deallocate(Tsf_start)
! Subprogram not used          deallocate(dTsf)
! Subprogram not used          deallocate(dfsurf_dT)
! Subprogram not used 
! Subprogram not used       enddo                     ! temperature iteration niter
! Subprogram not used 
! Subprogram not used       if (.not.all_converged) then
! Subprogram not used 
! Subprogram not used          do ij = 1, icells
! Subprogram not used             i = indxi(ij)
! Subprogram not used             j = indxj(ij)
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! Check for convergence failures.
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used             if (.not.converged(ij)) then
! Subprogram not used                write(nu_diag,*) 'Thermo iteration does not converge,', &
! Subprogram not used                                 'istep1, my_task, i, j:', &
! Subprogram not used                                  istep1, my_task, i, j
! Subprogram not used                write(nu_diag,*) 'Ice thickness:',  hilyr(ij)*nilyr
! Subprogram not used                write(nu_diag,*) 'Snow thickness:', hslyr(ij)*nslyr
! Subprogram not used                write(nu_diag,*) 'dTsf, Tsf_errmax:',dTsf_prev(ij), &
! Subprogram not used                                  Tsf_errmax
! Subprogram not used                write(nu_diag,*) 'Tsf:', Tsf(ij)
! Subprogram not used                write(nu_diag,*) 'fsurfn:', fsurfn(i,j)
! Subprogram not used                write(nu_diag,*) 'fcondtopn, fcondbot', &
! Subprogram not used                                  fcondtopn(i,j), fcondbot(ij)
...
! Subprogram not used             endif
! Subprogram not used          enddo                  ! ij
! Subprogram not used       endif                     ! all_converged
! Subprogram not used 
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used       ! Check that if Tsfc < 0, then fcondtopn = fsurfn
! Subprogram not used       !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used       if (l_zerolayerchecks) then
! Subprogram not used          do ij = 1, icells
! Subprogram not used             i = indxi(ij)
! Subprogram not used             j = indxj(ij)
! Subprogram not used             
! Subprogram not used             if (Tsf(ij) < c0 .and. & 
! Subprogram not used                   abs(fcondtopn(i,j)-fsurfn(i,j)) > puny) then
! Subprogram not used 
! Subprogram not used                write(nu_diag,*) 'fcondtopn does not equal fsurfn,', &
! Subprogram not used                                 'istep1, my_task, i, j:', &
! Subprogram not used                                  istep1, my_task, i, j
! Subprogram not used                write(nu_diag,*) 'Tsf=',Tsf(ij)
! Subprogram not used                write(nu_diag,*) 'fcondtopn=',fcondtopn(i,j)
! Subprogram not used                write(nu_diag,*) 'fsurfn=',fsurfn(i,j)
! Subprogram not used                l_stop = .true.
! Subprogram not used                istop = i
! Subprogram not used                jstop = j
...
! Subprogram not used             endif
! Subprogram not used          enddo                  ! ij
! Subprogram not used       endif                     ! l_zerolayerchecks
! Subprogram not used 
! Subprogram not used 
! Subprogram not used !DIR$ CONCURRENT !Cray
! Subprogram not used !cdir nodep      !NEC
...
! Subprogram not used          j = indxj(ij)
! Subprogram not used 
! Subprogram not used          ! update fluxes that depend on Tsf
! Subprogram not used          flwoutn(i,j) = flwoutn(i,j) + dTsf_prev(ij) * dflwout_dT(ij)
! Subprogram not used          fsensn(i,j)  = fsensn(i,j)  + dTsf_prev(ij) * dfsens_dT(ij)
! Subprogram not used          flatn(i,j)   = flatn(i,j)   + dTsf_prev(ij) * dflat_dT(ij)
! Subprogram not used 
! Subprogram not used          ! absorbed shortwave flux for coupler
! Subprogram not used          fswabsn(i,j) = fswsfc(i,j) + fswthrun(i,j)
! Subprogram not used 
! Subprogram not used       enddo                     ! ij
! Subprogram not used 
! Subprogram not used       end subroutine zerolayer_temperature
