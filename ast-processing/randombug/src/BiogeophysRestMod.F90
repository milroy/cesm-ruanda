...
!
! !INTERFACE:
! Subprogram not used   logical function weights_tooDifferent( begp, endp, pptr, wtgcell, adiff, maxdiff )
! Subprogram not used !
! Subprogram not used ! !DESCRIPTION:
! Subprogram not used ! Determine if the weights read in are too different and should flag an error
! Subprogram not used !
...
! Subprogram not used     integer, intent(IN)     :: begp, endp         ! per-proc beginning and ending pft indices
! Subprogram not used     type(pft_type), pointer :: pptr               ! pointer to pft derived subtype
! Subprogram not used     real(r8), intent(IN)    :: wtgcell(begp:endp) ! grid cell weights for each PFT
! Subprogram not used     real(r8), intent(IN)    :: adiff              ! tolerance of acceptible difference
! Subprogram not used     real(r8), intent(OUT)   :: maxdiff            ! maximum difference found
! Subprogram not used !
...
! Subprogram not used      real(r8) :: diff     ! difference in weights
! Subprogram not used 
! Subprogram not used      ! Assume weights are NOT different and only change if find weights too different
! Subprogram not used      weights_tooDifferent = .false.
! Subprogram not used      maxdiff = 0.0_r8
! Subprogram not used      do p = begp, endp
! Subprogram not used 
! Subprogram not used         diff = abs(pptr%wtgcell(p) - wtgcell(p))
! Subprogram not used         if ( diff > maxdiff ) maxdiff = diff
! Subprogram not used         if ( diff > adiff   ) weights_tooDifferent = .true.
! Subprogram not used      end do
! Subprogram not used 
! Subprogram not used   end function weights_tooDifferent
