...
! Subprogram not used    subroutine opnfil (locfn, iun, form, status)
! Subprogram not used  
! Subprogram not used !-----------------------------------------------------------------------
...
! Subprogram not used    character(len=1), intent(in):: form   !file format: u = unformatted. f = formatted
! Subprogram not used    character(len=*), optional, intent(in):: status !file status
! Subprogram not used ! --------------------------------------------------------------------
! Subprogram not used  
...
! Subprogram not used    character(len=11) st    !file status: old or unknown
! Subprogram not used ! --------------------------------------------------------------------
! Subprogram not used  
! Subprogram not used    if (len_trim(locfn) == 0) then
! Subprogram not used       call endrun ('(OPNFIL): local filename has zero length')
...
! Subprogram not used       st = status
! Subprogram not used    else
! Subprogram not used       st = "unknown"
! Subprogram not used    end if
! Subprogram not used    open (unit=iun,file=locfn,status=st, form=ft,iostat=ioe)
! Subprogram not used    if (ioe /= 0) then
! Subprogram not used       if(masterproc) write(iulog,*)'(OPNFIL): failed to open file ',trim(locfn), ' on unit ',iun,' ierr=',ioe
! Subprogram not used       call endrun ('opnfil') 
! Subprogram not used    else
! Subprogram not used       if(masterproc) write(iulog,*)'(OPNFIL): Successfully opened file ',trim(locfn), ' on unit= ',iun
! Subprogram not used    end if
! Subprogram not used  
