...
!-----------------------------
  ...
  if (present(is_nan)) &
     is_nan_passed = shr_infnan_isnan(var) .eqv. is_nan
  ...
  if (present(eq)) then
     eq_passed = within_tolerance(eq, var, abs_tol_loc)
  end if

  if (present(ne)) then
     ne_passed = .not. within_tolerance(ne, var, abs_tol_loc)
  end if
...
!-----------------------------
  ...
  ! Only floating-point values can actually be Inf/NaN.
  if (present(is_nan)) &
     is_nan_passed = all(shr_infnan_isnan(var) .eqv. is_nan)

  if (present(lt)) &
     lt_passed = all(var < lt)

  if (present(gt)) &
     gt_passed = all(var > gt)

  if (present(le)) &
     le_passed = all(var <= le)

  if (present(ge)) &
     ge_passed = all(var >= ge)

  if (present(eq)) then
     eq_passed = all(within_tolerance(eq, var, abs_tol_loc))
  end if

  if (present(ne)) then
     ne_passed = all(.not. within_tolerance(ne, var, abs_tol_loc))
  end if
...
!-----------------------------
  ...
  if (present(is_nan)) &
     is_nan_passed = .not. is_nan .or. 1 == 0
...
!-----------------------------
  ...
  if (present(is_nan)) &
     is_nan_passed = .not. is_nan .or. size(var) == 0
