...
9241   format( i4, 2( 2x, i12, 2x, a ),                                &
     ...
     !==============================================================
! Subprogram not used      subroutine modal_aero_initialize_q( name, q )
! Subprogram not used        !
! Subprogram not used        ! this routine is for initial testing of the modal aerosol cam3
! Subprogram not used        !
! Subprogram not used        ! it initializes several gas and aerosol species to 
! Subprogram not used        !    "low background" values, so that very short (e.g., 1 day)
! Subprogram not used        !    test runs are working with non-zero values
! Subprogram not used        !
! Subprogram not used        use constituents, only: pcnst, cnst_name
! Subprogram not used        use pmgrid,      only: plat, plon, plev
! Subprogram not used 
...
! Subprogram not used        real(r8), intent(inout) :: q(plon,plev,plat)           !  mass mixing ratio
! Subprogram not used 
! Subprogram not used        !--------------------------------------------------------------
! Subprogram not used        ! ... local variables
! Subprogram not used        !--------------------------------------------------------------
! Subprogram not used        integer k, l
! Subprogram not used        real(r8) duma, dumb, dumz
! Subprogram not used 
! Subprogram not used 
! Subprogram not used        !
! Subprogram not used        ! to deactivate this routine, just return here
! Subprogram not used        !
! Subprogram not used        !     return
! Subprogram not used 
! Subprogram not used 
! Subprogram not used        if ( masterproc ) then
! Subprogram not used           write( *, '(2a)' )   &
! Subprogram not used                '*** modal_aero_initialize_q - name = ', name
! Subprogram not used           if (name == 'H2O2'   ) write( *, '(2a)' ) '    doing ', name
! Subprogram not used           if (name == 'SO2'    ) write( *, '(2a)' ) '    doing ', name
! Subprogram not used           if (name == 'H2SO4'  ) write( *, '(2a)' ) '    doing ', name
! Subprogram not used           if (name == 'DMS'    ) write( *, '(2a)' ) '    doing ', name
! Subprogram not used           if (name == 'NH3'    ) write( *, '(2a)' ) '    doing ', name
! Subprogram not used           if (name == 'so4_a1' ) write( *, '(2a)' ) '    doing ', name
! Subprogram not used           if (name == 'so4_a2' ) write( *, '(2a)' ) '    doing ', name
! Subprogram not used           if (name == 'pom_a3' ) write( *, '(2a)' ) '    doing ', name
! Subprogram not used           if (name == 'ncl_a4' ) write( *, '(2a)' ) '    doing ', name
! Subprogram not used           if (name == 'dst_a5' ) write( *, '(2a)' ) '    doing ', name
! Subprogram not used           if (name == 'ncl_a6' ) write( *, '(2a)' ) '    doing ', name
! Subprogram not used           if (name == 'dst_a7' ) write( *, '(2a)' ) '    doing ', name
! Subprogram not used        end if
! Subprogram not used 
! Subprogram not used        do k = 1, plev
! Subprogram not used 
! Subprogram not used           ! init gases
! Subprogram not used           dumz = (k+1.0e-5_r8)/(plev+1.0e-5_r8)
! Subprogram not used           dumb = dumz*1.0e-9_r8/28.966_r8
! Subprogram not used           if (name == 'H2O2'   ) q(:,k,:) = dumb*34.0_r8*1.0_r8
! Subprogram not used           if (name == 'SO2'    ) q(:,k,:) = dumb*64.0_r8*0.1_r8
! Subprogram not used           if (name == 'H2SO4'  ) q(:,k,:) = dumb*98.0_r8*0.001_r8
! Subprogram not used           if (name == 'DMS'    ) q(:,k,:) = dumb*62.0_r8*0.01_r8
! Subprogram not used           if (name == 'NH3'    ) q(:,k,:) = dumb*17.0_r8*0.1_r8
! Subprogram not used 
! Subprogram not used           ! init first mass species of each aerosol mode
! Subprogram not used           duma = dumz*1.0e-10_r8
! Subprogram not used           if (name == 'so4_a1' ) q(:,k,:) = duma*1.0_r8
! Subprogram not used           if (name == 'so4_a2' ) q(:,k,:) = duma*0.002_r8
! Subprogram not used           if (name == 'pom_a3' ) q(:,k,:) = duma*0.3_r8
! Subprogram not used           if (name == 'ncl_a4' ) q(:,k,:) = duma*0.4_r8
! Subprogram not used           if (name == 'dst_a5' ) q(:,k,:) = duma*0.5_r8
! Subprogram not used           if (name == 'ncl_a6' ) q(:,k,:) = duma*0.6_r8
! Subprogram not used           if (name == 'dst_a7' ) q(:,k,:) = duma*0.7_r8
! Subprogram not used 
! Subprogram not used           ! init aerosol number
! Subprogram not used           !
! Subprogram not used           ! at k=plev, duma = 1e-10 kgaero/kgair = 0.1 ugaero/kgair
! Subprogram not used           !            dumb = duma/(2000 kgaero/m3aero)
! Subprogram not used           duma = dumz*1.0e-10_r8
! Subprogram not used           dumb = duma/2.0e3_r8
! Subprogram not used           ! following produces number 1000X too small, and Dp 10X too big
! Subprogram not used           !        dumb = dumb*1.0e-3
! Subprogram not used           ! following produces number 1000X too big, and Dp 10X too small
! Subprogram not used           !        dumb = dumb*1.0e3
! Subprogram not used           if (name == 'num_a1' ) q(:,k,:) = dumb*1.0_r8  *3.0e20_r8
! Subprogram not used           if (name == 'num_a2' ) q(:,k,:) = dumb*0.002_r8*4.0e22_r8
! Subprogram not used           if (name == 'num_a3' ) q(:,k,:) = dumb*0.3_r8  *5.7e21_r8
! Subprogram not used           if (name == 'num_a4' ) q(:,k,:) = dumb*0.4_r8  *2.7e19_r8
! Subprogram not used           if (name == 'num_a5' ) q(:,k,:) = dumb*0.5_r8  *4.0e20_r8
! Subprogram not used           if (name == 'num_a6' ) q(:,k,:) = dumb*0.6_r8  *2.7e16_r8
! Subprogram not used           if (name == 'num_a7' ) q(:,k,:) = dumb*0.7_r8  *4.0e17_r8
! Subprogram not used 
! Subprogram not used           !*** modal_aero_calcsize_sub - ntot_amode    7
! Subprogram not used           !mode, dgn, dp*, v2n, v2nhi, v2nlo    1  1.100E-07  1.847E-07  3.031E+20  4.736E+18  2.635E+21
! Subprogram not used           !mode, dgn, dp*, v2n, v2nhi, v2nlo    2  2.600E-08  3.621E-08  4.021E+22  5.027E+21  1.073E+24
! Subprogram not used           !mode, dgn, dp*, v2n, v2nhi, v2nlo    3  5.000E-08  6.964E-08  5.654E+21  7.068E+20  7.068E+23
! Subprogram not used           !mode, dgn, dp*, v2n, v2nhi, v2nlo    4  2.000E-07  4.112E-07  2.748E+19  2.198E+17  1.758E+21
! Subprogram not used           !mode, dgn, dp*, v2n, v2nhi, v2nlo    5  1.000E-07  1.679E-07  4.035E+20  3.228E+18  3.228E+21
! Subprogram not used           !mode, dgn, dp*, v2n, v2nhi, v2nlo    6  2.000E-06  4.112E-06  2.748E+16  3.434E+15  2.198E+17
! Subprogram not used           !mode, dgn, dp*, v2n, v2nhi, v2nlo    7  1.000E-06  1.679E-06  4.035E+17  5.043E+16  3.228E+18
! Subprogram not used 
! Subprogram not used        end do   ! k
! Subprogram not used 
! Subprogram not used        if ( masterproc ) then
! Subprogram not used           write( *, '(7x,a,1p,10e10.2)' )   &
! Subprogram not used                name, (q(1,k,1), k=plev,1,-5) 
! Subprogram not used        end if
! Subprogram not used 
! Subprogram not used        if (plev > 0) return
! Subprogram not used 
! Subprogram not used 
! Subprogram not used        if ( masterproc ) then
! Subprogram not used           write( *, '(/a,i5)' )   &
! Subprogram not used                '*** modal_aero_initialize_q - ntot_amode', ntot_amode
! Subprogram not used           do k = 1, ntot_amode
! Subprogram not used              write( *, '(/a)' ) 'mode, dgn, v2n',   &
! Subprogram not used                   k, dgnum_amode(k), voltonumb_amode(k)
! Subprogram not used           end do
! Subprogram not used        end if
! Subprogram not used 
! Subprogram not used        return
! Subprogram not used      end subroutine modal_aero_initialize_q
