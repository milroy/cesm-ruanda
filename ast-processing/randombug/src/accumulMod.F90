...
1004 format((1x,i2),(1x,a8),(1x,a8),(1x,a8),'  N.A.',(1x,f4.0),(1x,a40))
  ...
  !------------------------------------------------------------------------
! Subprogram not used   subroutine extract_accum_field_ml (name, field, nstep)
! Subprogram not used     !
! Subprogram not used     ! !DESCRIPTION:
! Subprogram not used     ! Extract mutli-level accumulated field.
! Subprogram not used     ! This routine extracts the field values from the multi-level
! Subprogram not used     ! accumulation field. It extracts the current value except if
! Subprogram not used     ! the field type is a time average. In this case, an absurd value
! Subprogram not used     ! is assigned to  indicate the time average is not yet valid.
! Subprogram not used     !
! Subprogram not used     ! !USES:
! Subprogram not used     use clm_varcon, only : spval
! Subprogram not used     !
! Subprogram not used     ! !ARGUMENTS:
! Subprogram not used     implicit none
! Subprogram not used     character(len=*), intent(in) :: name       !field name
! Subprogram not used     real(r8), pointer, dimension(:,:) :: field !field values for current time step
! Subprogram not used     integer, intent(in) :: nstep               !timestep index
! Subprogram not used     !
! Subprogram not used     ! !LOCAL VARIABLES:
! Subprogram not used     integer :: i,j,k,nf        !indices
! Subprogram not used     integer :: beg,end         !subgrid beginning,ending indices
! Subprogram not used     integer :: numlev          !number of vertical levels
! Subprogram not used     !------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used     ! find field index. return if "name" is not on list
! Subprogram not used 
! Subprogram not used     nf = 0
! Subprogram not used     do i = 1, naccflds
! Subprogram not used        if (name == accum(i)%name) nf = i
! Subprogram not used     end do
! Subprogram not used     if (nf == 0) then
! Subprogram not used        write(iulog,*) 'EXTRACT_ACCUM_FIELD_ML error: field name ',name,' not found'
! Subprogram not used        call shr_sys_abort
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used     ! error check
! Subprogram not used 
! Subprogram not used     numlev = accum(nf)%numlev
! Subprogram not used     beg = accum(nf)%beg1d
! Subprogram not used     end = accum(nf)%end1d
! Subprogram not used     if (size(field,dim=1) /= end-beg+1) then
! Subprogram not used        write(iulog,*)'ERROR in extract_accum_field for field ',accum(nf)%name
! Subprogram not used        write(iulog,*)'size of first dimension of field is ',&
! Subprogram not used             size(field,dim=1),' and should be ',end-beg+1
! Subprogram not used        call shr_sys_abort
! Subprogram not used     else if (size(field,dim=2) /= numlev) then
! Subprogram not used        write(iulog,*)'ERROR in extract_accum_field for field ',accum(nf)%name
! Subprogram not used        write(iulog,*)'size of second dimension of field iis ',&
! Subprogram not used             size(field,dim=2),' and should be ',numlev
! Subprogram not used        call shr_sys_abort
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used     !extract field
! Subprogram not used 
! Subprogram not used     if (accum(nf)%acctype == 'timeavg' .and. &
! Subprogram not used          mod(nstep,accum(nf)%period) /= 0) then
! Subprogram not used        do j = 1,numlev
! Subprogram not used           do k = beg,end
! Subprogram not used              field(k,j) = spval  !assign absurd value when avg not ready
! Subprogram not used           end do
! Subprogram not used        end do
! Subprogram not used     else
! Subprogram not used        do j = 1,numlev
! Subprogram not used           do k = beg,end
! Subprogram not used              field(k,j) = accum(nf)%val(k,j)
! Subprogram not used           end do
! Subprogram not used        end do
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used   end subroutine extract_accum_field_ml
  ...
  !------------------------------------------------------------------------
! Subprogram not used   subroutine update_accum_field_ml (name, field, nstep)
! Subprogram not used     !
! Subprogram not used     ! !DESCRIPTION:
! Subprogram not used     ! Accumulate multi level field over specified time interval.
! Subprogram not used     !
! Subprogram not used     ! !ARGUMENTS:
! Subprogram not used     implicit none
! Subprogram not used     character(len=*), intent(in) :: name       !field name
! Subprogram not used     real(r8), pointer, dimension(:,:) :: field !field values for current time step
! Subprogram not used     integer , intent(in) :: nstep              !time step index
! Subprogram not used     !
! Subprogram not used     ! !LOCAL VARIABLES:
! Subprogram not used     integer :: i,j,k,nf            !indices
! Subprogram not used     integer :: accper              !temporary accumulation period
! Subprogram not used     integer :: beg,end             !subgrid beginning,ending indices
! Subprogram not used     integer :: numlev              !number of vertical levels
! Subprogram not used     !------------------------------------------------------------------------
! Subprogram not used 
...
! Subprogram not used     end do
! Subprogram not used     if (nf == 0) then
! Subprogram not used        write(iulog,*) 'UPDATE_ACCUM_FIELD_ML error: field name ',name,' not found'
! Subprogram not used        call shr_sys_abort
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used     ! error check
...
! Subprogram not used     end = accum(nf)%end1d
! Subprogram not used     if (size(field,dim=1) /= end-beg+1) then
! Subprogram not used        write(iulog,*)'ERROR in UPDATE_ACCUM_FIELD_ML for field ',accum(nf)%name
! Subprogram not used        write(iulog,*)'size of first dimension of field is ',size(field,dim=1),&
! Subprogram not used             ' and should be ',end-beg+1
! Subprogram not used        call shr_sys_abort
! Subprogram not used     else if (size(field,dim=2) /= numlev) then
! Subprogram not used        write(iulog,*)'ERROR in UPDATE_ACCUM_FIELD_ML for field ',accum(nf)%name
! Subprogram not used        write(iulog,*)'size of second dimension of field is ',size(field,dim=2),&
! Subprogram not used             ' and should be ',numlev
! Subprogram not used        call shr_sys_abort
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used     ! accumulate field
! Subprogram not used 
! Subprogram not used     if (accum(nf)%acctype /= 'timeavg' .AND. &
! Subprogram not used         accum(nf)%acctype /= 'runmean' .AND. &
! Subprogram not used         accum(nf)%acctype /= 'runaccum') then
! Subprogram not used        write(iulog,*) 'UPDATE_ACCUM_FIELD_ML error: incorrect accumulation type'
! Subprogram not used        write(iulog,*) ' was specified for field ',name
! Subprogram not used        write(iulog,*)' accumulation type specified is ',accum(nf)%acctype
! Subprogram not used        write(iulog,*)' only [timeavg, runmean, runaccum] are currently acceptable'
! Subprogram not used        call shr_sys_abort()
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used     ! accumulate field
! Subprogram not used 
! Subprogram not used     ! reset accumulated field value if necessary and  update
! Subprogram not used     ! accumulation field
! Subprogram not used     ! running mean never reset
! Subprogram not used 
! Subprogram not used     if (accum(nf)%acctype == 'timeavg') then
! Subprogram not used 
! Subprogram not used        !time average field reset every accumulation period
! Subprogram not used        !normalize at end of accumulation period
! Subprogram not used 
! Subprogram not used        if ((mod(nstep,accum(nf)%period) == 1 .or. accum(nf)%period == 1) .and. (nstep /= 0))then
! Subprogram not used           accum(nf)%val(beg:end,1:numlev) = 0._r8
! Subprogram not used        endif
! Subprogram not used        accum(nf)%val(beg:end,1:numlev) =  accum(nf)%val(beg:end,1:numlev) + field(beg:end,1:numlev)
! Subprogram not used        if (mod(nstep,accum(nf)%period) == 0) then
! Subprogram not used           accum(nf)%val(beg:end,1:numlev) = accum(nf)%val(beg:end,1:numlev) / accum(nf)%period
! Subprogram not used        endif
! Subprogram not used 
! Subprogram not used     else if (accum(nf)%acctype == 'runmean') then
! Subprogram not used 
! Subprogram not used        !running mean - reset accumulation period until greater than nstep
! Subprogram not used 
! Subprogram not used        accper = min (nstep,accum(nf)%period)
! Subprogram not used        accum(nf)%val(beg:end,1:numlev) = &
! Subprogram not used             ((accper-1)*accum(nf)%val(beg:end,1:numlev) + field(beg:end,1:numlev)) / accper
! Subprogram not used 
! Subprogram not used     else if (accum(nf)%acctype == 'runaccum') then
! Subprogram not used 
! Subprogram not used        !running accumulation field reset at trigger -99999
! Subprogram not used 
! Subprogram not used        do j = 1,numlev
! Subprogram not used           do k = beg,end
! Subprogram not used              if (nint(field(k,j)) == -99999) then
! Subprogram not used                 accum(nf)%val(k,j) = 0._r8
! Subprogram not used              end if
! Subprogram not used           end do
! Subprogram not used        end do
! Subprogram not used        accum(nf)%val(beg:end,1:numlev) = &
! Subprogram not used             min(max(accum(nf)%val(beg:end,1:numlev) + field(beg:end,1:numlev), 0._r8), 99999._r8)
! Subprogram not used 
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used   end subroutine update_accum_field_ml
