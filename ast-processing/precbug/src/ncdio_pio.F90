...
contains
  ...
  subroutine check_dim(ncid, dimname, value)
    ...
    if (dimlen /= value) then
       ...
       call shr_sys_abort(errMsg("ncdio_pio.F90.in", 327))
    end if
  ...
  subroutine ncd_inqfdims(ncid, isgrid2d, ni, nj, ns)
    ...
    if (ni == 0 .or. nj == 0) then
       ...
       call shr_sys_abort(errMsg("ncdio_pio.F90.in", 476))
    end if
  ...
  subroutine ncd_defvar_bynf(ncid, varname, xtype, ndims, dimid, varid, &
    ...
    if (present(dimid)) then
       ...
       if (ndims /= 0) then
          ...
          call shr_sys_abort(errMsg("ncdio_pio.F90.in", 837))
       endif
    endif
    ...
    if (present(flag_values)) then
       ...
       if ( .not. present(flag_meanings)) then
          write(iulog,*) 'Error in defining variable = ', trim(varname)
          call shr_sys_abort(" ERROR:: flag_values set -- but not flag_meanings"//errMsg("ncdio_pio.F90.in", 873))
       end if
    end if
    if (present(flag_meanings)) then
       if ( .not. present(flag_values)) then
          write(iulog,*) 'Error in defining variable = ', trim(varname)
          call shr_sys_abort(" ERROR:: flag_meanings set -- but not flag_values"//errMsg("ncdio_pio.F90.in", 879) )
       end if
       if ( size(flag_values) /= size(flag_meanings) ) then
          write(iulog,*) 'Error in defining variable = ', trim(varname)
          call shr_sys_abort(" ERROR:: flag_meanings and flag_values dimension different"//errMsg("ncdio_pio.F90.in", 883))
       end if
       str = flag_meanings(1)
       do n = 1, size(flag_meanings)
          if ( index(flag_meanings(n), ' ') /= 0 )then
             write(iulog,*) 'Error in defining variable = ', trim(varname)
             call shr_sys_abort(" ERROR:: flag_meanings has an invalid space in it"//errMsg("ncdio_pio.F90.in", 889) )
          end if
    ...
    if (flag == 'read') then
       ...
       if (varpresent) then
          ...
          if ( idata == 0 )then
             ...
             call shr_sys_abort(' ERROR: bad integer value for logical data'//errMsg("ncdio_pio.F90.in", 1084))
          end if
       endif
    ...
    if (flag == 'read') then
       ...
       if (varpresent) then
          ...
          if ( any(idata1d /= 0 .and. idata1d /= 1) )then
             call shr_sys_abort(' ERROR: read in bad integer value(s) for logical data'//errMsg("ncdio_pio.F90.in", 1090))
          end if
          deallocate(idata1d)
       endif
       if (present(readvar)) readvar = varpresent
  ...
  subroutine ncd_io_1d_int(varname, data, dim1name, flag, ncid, nt, readvar, cnvrtnan2fill)
    ...
    elseif (flag == 'write') then
       ...
       if (masterproc) then
          ...
          call shr_sys_abort(errMsg("ncdio_pio.F90.in", 1480))
       endif
       ...
       if (varpresent) then
          if (single_column) then
             ...
             if ( any(idata /= 0 .and. idata /= 1) )then
                call shr_sys_abort(' ERROR: read in bad integer value(s) for logical data'//errMsg("ncdio_pio.F90.in", 1393))
             end if
             deallocate( idata )
          else
             status = pio_inq_varndims(ncid, vardesc, ndims)
             ...
             data = (idata == 1)
             if ( any(idata /= 0 .and. idata /= 1) )then
                call shr_sys_abort(' ERROR: read in bad integer value(s) for logical data'//errMsg("ncdio_pio.F90.in", 1427))
             end if
             deallocate( idata )
          end if
       end if
  ...
  subroutine ncd_io_2d_int(varname, data, dim1name, lowerb2, upperb2, &
    ...
    if (flag == 'read') then
       ...
       if (varpresent) then
          if (single_column) then
             ...
             if (ndims == 0) then
                ...
                call shr_sys_abort(errMsg("ncdio_pio.F90.in", 1598))
             end if
             if ('time' == trim(dimname)) then
                ndims_iod = ndims - 1
             else
       ...
       if (ndims == 0) then
          write(iulog,*) trim(subname),' ERROR: ndims must be greater than 0'
          call shr_sys_abort(errMsg("ncdio_pio.F90.in", 1651))
       end if
       status = pio_inq_dimname(ncid,dids(ndims),dimname)
       if ('time' == trim(dimname)) then
          ndims_iod = ndims - 1
       ...
       if (masterproc) then
          write(iulog,*) subname,' error: unsupported flag ',trim(flag)
          call shr_sys_abort(errMsg("ncdio_pio.F90.in", 1707))
       endif
    ...
    if (flag == 'read') then
       ...
       if (varpresent) then
          if (single_column) then
             ...
             if (ndims == 0) then
                write(iulog,*) trim(subname),' ERROR: ndims must be greater than 0'
                call shr_sys_abort(errMsg("ncdio_pio.F90.in", 1791))
             end if
             if ('time' == trim(dimname)) then
                ndims_iod = ndims - 1
             else
       ...
       if (ndims == 0) then
          write(iulog,*) trim(subname),' ERROR: ndims must be greater than 0'
          call shr_sys_abort(errMsg("ncdio_pio.F90.in", 1819))
       end if
       status = pio_inq_dimname(ncid,dids(ndims),dimname)
       if ('time' == trim(dimname)) then
          ndims_iod = ndims - 1
       ...
       if (masterproc) then
          write(iulog,*) subname,' error: unsupported flag ',trim(flag)
          call shr_sys_abort(errMsg("ncdio_pio.F90.in", 1842))
       endif
    ...
    do i = 1,ndims
       ...
       if ( trim(dimname)=='nj'.or. trim(dimname)=='lat'.or. trim(dimname)=='lsmlat') then
          ...
          if (ndata == 0) then
             ...
             call shr_sys_abort('ERROR:: no columns for this position'//errMsg("ncdio_pio.F90.in", 1956))
          else
             data_offset=cols(1)
          end if
          ...
          if (ndata == 0) then
             ...
             call shr_sys_abort('ERROR:: no PFTs for this position'//errMsg("ncdio_pio.F90.in", 1992))
          else
             data_offset=pfts(1)
          end if
          ...
          if (ndata == 0) then
             ...
             call shr_sys_abort('ERROR:: no landunits for this position'//errMsg("ncdio_pio.F90.in", 2028))
          else
  ...
  subroutine ncd_getiodesc(ncid, clmlevel, ndims, dims, dimids, &
       ...
       if (ndims == iodesc_list(n)%ndims .and. xtype == iodesc_list(n)%type) then
          ...
          if (found) then
             ...
             if (iodnum > num_iodesc) then
                ...
                call shr_sys_abort(errMsg("ncdio_pio.F90.in", 2122))
             endif
             RETURN
          endif
       endif
    ...
    if (ndims > 0) then 
       ...
       if (num_iodesc > max_iodesc) then
          ...
          call shr_sys_abort(errMsg("ncdio_pio.F90.in", 2136))
       endif
       iodnum = num_iodesc
       if (masterproc .and. debug > 1) then
    ...
    if (mod(fullsize,gsize) /= 0) then
       ...
       call shr_sys_abort(errMsg("ncdio_pio.F90.in", 2168))
    endif
    ...
    if (present(switchdim)) then
       if (switchdim) then
          ...
          call shr_sys_abort(errMsg("ncdio_pio.F90.in", 2184))
       end if
       ...
       if (vsize /= 1 .and. vsize /= dims(ndims)) then
          ...
          if (vsize1*vsize2 /= vsize) then
             ...
             call shr_sys_abort('error in vsize1 and vsize2 computation'//errMsg("ncdio_pio.F90.in", 2192))
          end if
          cnt = 0
          do k = 1,vsize2
             do n = 1,vsize1
