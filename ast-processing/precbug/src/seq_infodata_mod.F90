...
! SVN $URL: https://svn-ccsm-models.cgd.ucar.edu/drv/seq_mct/trunk_tags/drvseq5_0_14/shr/seq_infodata_mod.F90 $
!===============================================================================
!BOP ===========================================================================
!
...
! Subprogram not used SUBROUTINE seq_infodata_print( infodata )
! Subprogram not used     
! Subprogram not used    implicit none
! Subprogram not used 
! Subprogram not used ! !INPUT/OUTPUT PARAMETERS:
! Subprogram not used 
! Subprogram not used    type(seq_infodata_type), intent(IN) :: infodata  ! Output CCSM structure
! Subprogram not used 
! Subprogram not used !EOP
! Subprogram not used 
! Subprogram not used     !----- local -----
! Subprogram not used     character(len=*), parameter :: subname = '(seq_infodata_print) '
! Subprogram not used     character(len=*), parameter ::  F0A = "(2A,A)"
! Subprogram not used     character(len=*), parameter ::  F0L = "(2A,L3)"
! Subprogram not used     character(len=*), parameter ::  F0I = "(2A,I10)"
! Subprogram not used     character(len=*), parameter ::  F0S = "(2A,I4)"
! Subprogram not used     character(len=*), parameter ::  F0R = "(2A,g22.14)"
! Subprogram not used 
! Subprogram not used !-------------------------------------------------------------------------------
! Subprogram not used ! Notes:
! Subprogram not used !-------------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used !    if (loglevel > 0) then
! Subprogram not used        write(logunit,F0A) subname,'Start type               = ', trim(infodata%start_type)
! Subprogram not used        write(logunit,F0A) subname,'Case name                = ', trim(infodata%case_name)
! Subprogram not used        write(logunit,F0A) subname,'Case description         = ', trim(infodata%case_desc)
! Subprogram not used        write(logunit,F0A) subname,'Model version            = ', trim(infodata%model_version)
! Subprogram not used        write(logunit,F0A) subname,'Username                 = ', trim(infodata%username)
! Subprogram not used        write(logunit,F0A) subname,'Hostname                 = ', trim(infodata%hostname)
! Subprogram not used        write(logunit,F0A) subname,'Timing Dir               = ', trim(infodata%timing_dir)
! Subprogram not used        write(logunit,F0A) subname,'Timing Checkpoint Dir    = ', trim(infodata%tchkpt_dir)
! Subprogram not used        write(logunit,F0A) subname,'Restart case name        = ', trim(infodata%rest_case_name)
! Subprogram not used 
! Subprogram not used        write(logunit,F0L) subname,'atm_ideal_phys           = ', infodata%atm_ideal_phys
! Subprogram not used        write(logunit,F0L) subname,'atm adiabatic mode       = ', infodata%atm_adiabatic
! Subprogram not used        write(logunit,F0L) subname,'aqua_planet mode         = ', infodata%aqua_planet
! Subprogram not used        write(logunit,F0I) subname,'aqua_planet analytic sst = ', infodata%aqua_planet_sst
! Subprogram not used        write(logunit,F0L) subname,'brnch_retain_casename    = ', infodata%brnch_retain_casename
! Subprogram not used 
! Subprogram not used        write(logunit,F0L) subname,'read_restart flag        = ', infodata%read_restart
! Subprogram not used        write(logunit,F0A) subname,'Restart pointer file     = ', trim(infodata%restart_pfile)
! Subprogram not used        write(logunit,F0A) subname,'Restart file (full path) = ', trim(infodata%restart_file)
! Subprogram not used 
! Subprogram not used        write(logunit,F0L) subname,'single_column            = ', infodata%single_column
! Subprogram not used        write(logunit,F0R) subname,'scmlat                   = ', infodata%scmlat
! Subprogram not used        write(logunit,F0R) subname,'scmlon                   = ', infodata%scmlon
! Subprogram not used 
! Subprogram not used        write(logunit,F0A) subname,'Log output end name      = ', trim(infodata%logFilePostFix)
! Subprogram not used        write(logunit,F0A) subname,'Output path dir          = ', trim(infodata%outPathRoot)
! Subprogram not used 
! Subprogram not used        write(logunit,F0L) subname,'perpetual                = ', infodata%perpetual
! Subprogram not used        write(logunit,F0I) subname,'perpetual_ymd            = ', infodata%perpetual_ymd
! Subprogram not used 
! Subprogram not used        write(logunit,F0A) subname,'orb_mode                 = ', trim(infodata%orb_mode)
! Subprogram not used      if (trim(infodata%orb_mode) == trim(seq_infodata_orb_fixed_parameters)) then
! Subprogram not used        write(logunit,F0R) subname,'orb_eccen                = ', infodata%orb_eccen
! Subprogram not used        write(logunit,F0R) subname,'orb_obliq                = ', infodata%orb_obliq
! Subprogram not used        write(logunit,F0R) subname,'orb_mvelp                = ', infodata%orb_mvelp
! Subprogram not used        write(logunit,F0R) subname,'orb_obliqr               = ', infodata%orb_obliqr
! Subprogram not used        write(logunit,F0R) subname,'orb_mvelpp               = ', infodata%orb_mvelpp
! Subprogram not used        write(logunit,F0R) subname,'orb_lambm0               = ', infodata%orb_lambm0
! Subprogram not used      elseif (trim(infodata%orb_mode) == trim(seq_infodata_orb_fixed_year)) then
! Subprogram not used        write(logunit,F0I) subname,'orb_iyear                = ', infodata%orb_iyear
! Subprogram not used        write(logunit,F0R) subname,'orb_eccen                = ', infodata%orb_eccen
! Subprogram not used        write(logunit,F0R) subname,'orb_obliq                = ', infodata%orb_obliq
! Subprogram not used        write(logunit,F0R) subname,'orb_mvelp                = ', infodata%orb_mvelp
! Subprogram not used        write(logunit,F0R) subname,'orb_obliqr               = ', infodata%orb_obliqr
! Subprogram not used        write(logunit,F0R) subname,'orb_mvelpp               = ', infodata%orb_mvelpp
! Subprogram not used        write(logunit,F0R) subname,'orb_lambm0               = ', infodata%orb_lambm0
! Subprogram not used      elseif (trim(infodata%orb_mode) == trim(seq_infodata_orb_variable_year)) then
! Subprogram not used        write(logunit,F0I) subname,'orb_iyear                = ', infodata%orb_iyear
! Subprogram not used        write(logunit,F0I) subname,'orb_iyear_align          = ', infodata%orb_iyear_align
! Subprogram not used      endif
! Subprogram not used 
! Subprogram not used        write(logunit,F0A) subname,'flux_epbal               = ', trim(infodata%flux_epbal)
! Subprogram not used        write(logunit,F0L) subname,'flux_albav               = ', infodata%flux_albav
! Subprogram not used        write(logunit,F0R) subname,'wall_time_limit          = ', infodata%wall_time_limit
! Subprogram not used        write(logunit,F0A) subname,'force_stop_at            = ', trim(infodata%force_stop_at)
! Subprogram not used        write(logunit,F0A) subname,'atm_gridname             = ', trim(infodata%atm_gnam)
! Subprogram not used        write(logunit,F0A) subname,'lnd_gridname             = ', trim(infodata%lnd_gnam)
! Subprogram not used        write(logunit,F0A) subname,'ocn_gridname             = ', trim(infodata%ocn_gnam)
! Subprogram not used        write(logunit,F0A) subname,'ice_gridname             = ', trim(infodata%ice_gnam)
! Subprogram not used        write(logunit,F0A) subname,'rof_gridname             = ', trim(infodata%rof_gnam)
! Subprogram not used        write(logunit,F0A) subname,'glc_gridname             = ', trim(infodata%glc_gnam)
! Subprogram not used        write(logunit,F0A) subname,'wav_gridname             = ', trim(infodata%wav_gnam)
! Subprogram not used        write(logunit,F0L) subname,'shr_map_dopole           = ', infodata%shr_map_dopole
! Subprogram not used        write(logunit,F0A) subname,'vect_map                 = ', trim(infodata%vect_map)
! Subprogram not used        write(logunit,F0A) subname,'aoflux_grid              = ', trim(infodata%aoflux_grid)
! Subprogram not used        write(logunit,F0A) subname,'cpl_seq_option           = ', trim(infodata%cpl_seq_option)
! Subprogram not used        write(logunit,F0S) subname,'cpl_decomp               = ', infodata%cpl_decomp
! Subprogram not used        write(logunit,F0L) subname,'cpl_cdf64                = ', infodata%cpl_cdf64
! Subprogram not used        write(logunit,F0L) subname,'do_budgets               = ', infodata%do_budgets
! Subprogram not used        write(logunit,F0L) subname,'do_histinit              = ', infodata%do_histinit
! Subprogram not used        write(logunit,F0S) subname,'budget_inst              = ', infodata%budget_inst
! Subprogram not used        write(logunit,F0S) subname,'budget_daily             = ', infodata%budget_daily 
! Subprogram not used        write(logunit,F0S) subname,'budget_month             = ', infodata%budget_month
! Subprogram not used        write(logunit,F0S) subname,'budget_ann               = ', infodata%budget_ann
! Subprogram not used        write(logunit,F0S) subname,'budget_ltann             = ', infodata%budget_ltann
! Subprogram not used        write(logunit,F0S) subname,'budget_ltend             = ', infodata%budget_ltend
! Subprogram not used        write(logunit,F0L) subname,'histaux_a2x              = ', infodata%histaux_a2x   
! Subprogram not used        write(logunit,F0L) subname,'histaux_a2x3hr           = ', infodata%histaux_a2x3hr
! Subprogram not used        write(logunit,F0L) subname,'histaux_a2x3hrp          = ', infodata%histaux_a2x3hrp
! Subprogram not used        write(logunit,F0L) subname,'histaux_a2x24hr          = ', infodata%histaux_a2x24hr
! Subprogram not used        write(logunit,F0L) subname,'histaux_l2x1yr           = ', infodata%histaux_l2x1yr
! Subprogram not used        write(logunit,F0L) subname,'histaux_l2x              = ', infodata%histaux_l2x   
! Subprogram not used        write(logunit,F0L) subname,'histaux_r2x              = ', infodata%histaux_r2x   
! Subprogram not used        write(logunit,F0L) subname,'drv_threading            = ', infodata%drv_threading
! Subprogram not used 
! Subprogram not used        write(logunit,F0R) subname,'eps_frac                 = ', infodata%eps_frac
! Subprogram not used        write(logunit,F0R) subname,'eps_amask                = ', infodata%eps_amask
! Subprogram not used        write(logunit,F0R) subname,'eps_agrid                = ', infodata%eps_agrid
! Subprogram not used        write(logunit,F0R) subname,'eps_aarea                = ', infodata%eps_aarea
! Subprogram not used        write(logunit,F0R) subname,'eps_omask                = ', infodata%eps_omask
! Subprogram not used        write(logunit,F0R) subname,'eps_ogrid                = ', infodata%eps_ogrid
! Subprogram not used        write(logunit,F0R) subname,'eps_oarea                = ', infodata%eps_oarea
! Subprogram not used 
! Subprogram not used        write(logunit,F0L) subname,'reprosum_use_ddpdd       = ', infodata%reprosum_use_ddpdd
! Subprogram not used        write(logunit,F0R) subname,'reprosum_diffmax         = ', infodata%reprosum_diffmax
! Subprogram not used        write(logunit,F0L) subname,'reprosum_recompute       = ', infodata%reprosum_recompute
! Subprogram not used 
! Subprogram not used        write(logunit,F0L) subname,'mct_usealltoall          = ', infodata%mct_usealltoall
! Subprogram not used        write(logunit,F0L) subname,'mct_usevector            = ', infodata%mct_usevector
! Subprogram not used 
! Subprogram not used        write(logunit,F0S) subname,'info_debug               = ', infodata%info_debug
! Subprogram not used        write(logunit,F0L) subname,'bfbflag                  = ', infodata%bfbflag
! Subprogram not used        write(logunit,F0L) subname,'esmf_map_flag            = ', infodata%esmf_map_flag
! Subprogram not used        write(logunit,F0L) subname,'dead_comps               = ', infodata%dead_comps
! Subprogram not used        write(logunit,F0L) subname,'run_barriers             = ', infodata%run_barriers
! Subprogram not used 
! Subprogram not used        write(logunit,F0L) subname,'atm_present              = ', infodata%atm_present
! Subprogram not used        write(logunit,F0L) subname,'atm_prognostic           = ', infodata%atm_prognostic
! Subprogram not used        write(logunit,F0L) subname,'lnd_present              = ', infodata%lnd_present
! Subprogram not used        write(logunit,F0L) subname,'lnd_prognostic           = ', infodata%lnd_prognostic
! Subprogram not used        write(logunit,F0L) subname,'rof_present              = ', infodata%rof_present
! Subprogram not used        write(logunit,F0L) subname,'rofice_present           = ', infodata%rofice_present
! Subprogram not used        write(logunit,F0L) subname,'rof_prognostic           = ', infodata%rof_prognostic
! Subprogram not used        write(logunit,F0L) subname,'flood_present            = ', infodata%flood_present
! Subprogram not used        write(logunit,F0L) subname,'ocn_present              = ', infodata%ocn_present
! Subprogram not used        write(logunit,F0L) subname,'ocn_prognostic           = ', infodata%ocn_prognostic
! Subprogram not used        write(logunit,F0L) subname,'ocnrof_prognostic        = ', infodata%ocnrof_prognostic
! Subprogram not used        write(logunit,F0L) subname,'ice_present              = ', infodata%ice_present
! Subprogram not used        write(logunit,F0L) subname,'ice_prognostic           = ', infodata%ice_prognostic
! Subprogram not used        write(logunit,F0L) subname,'iceberg_prognostic       = ', infodata%iceberg_prognostic
! Subprogram not used        write(logunit,F0L) subname,'glc_present              = ', infodata%glc_present
! Subprogram not used        write(logunit,F0L) subname,'glclnd_present           = ', infodata%glclnd_present
! Subprogram not used        write(logunit,F0L) subname,'glcocn_present           = ', infodata%glcocn_present
! Subprogram not used        write(logunit,F0L) subname,'glcice_present           = ', infodata%glcice_present
! Subprogram not used        write(logunit,F0L) subname,'glc_prognostic           = ', infodata%glc_prognostic
! Subprogram not used        write(logunit,F0L) subname,'wav_present              = ', infodata%wav_present
! Subprogram not used        write(logunit,F0L) subname,'wav_prognostic           = ', infodata%wav_prognostic
! Subprogram not used 
! Subprogram not used        write(logunit,F0I) subname,'atm_nx                   = ', infodata%atm_nx
! Subprogram not used        write(logunit,F0I) subname,'atm_ny                   = ', infodata%atm_ny
! Subprogram not used        write(logunit,F0I) subname,'lnd_nx                   = ', infodata%lnd_nx
! Subprogram not used        write(logunit,F0I) subname,'lnd_ny                   = ', infodata%lnd_ny
! Subprogram not used        write(logunit,F0I) subname,'rof_nx                   = ', infodata%rof_nx
! Subprogram not used        write(logunit,F0I) subname,'rof_ny                   = ', infodata%rof_ny
! Subprogram not used        write(logunit,F0I) subname,'ice_nx                   = ', infodata%ice_nx
! Subprogram not used        write(logunit,F0I) subname,'ice_ny                   = ', infodata%ice_ny
! Subprogram not used        write(logunit,F0I) subname,'ocn_nx                   = ', infodata%ocn_nx
! Subprogram not used        write(logunit,F0I) subname,'ocn_ny                   = ', infodata%ocn_ny
! Subprogram not used        write(logunit,F0I) subname,'glc_nx                   = ', infodata%glc_nx
! Subprogram not used        write(logunit,F0I) subname,'glc_ny                   = ', infodata%glc_ny
! Subprogram not used        write(logunit,F0I) subname,'wav_nx                   = ', infodata%wav_nx
! Subprogram not used        write(logunit,F0I) subname,'wav_ny                   = ', infodata%wav_ny
! Subprogram not used 
! Subprogram not used        write(logunit,F0R) subname,'nextsw_cday              = ', infodata%nextsw_cday
! Subprogram not used        write(logunit,F0R) subname,'precip_fact              = ', infodata%precip_fact
! Subprogram not used        write(logunit,F0L) subname,'atm_aero                 = ', infodata%atm_aero
! Subprogram not used 
! Subprogram not used        write(logunit,F0S) subname,'atm_phase                = ', infodata%atm_phase
! Subprogram not used        write(logunit,F0S) subname,'lnd_phase                = ', infodata%lnd_phase
! Subprogram not used        write(logunit,F0S) subname,'ocn_phase                = ', infodata%ocn_phase
! Subprogram not used        write(logunit,F0S) subname,'ice_phase                = ', infodata%ice_phase
! Subprogram not used        write(logunit,F0S) subname,'glc_phase                = ', infodata%glc_phase
! Subprogram not used        write(logunit,F0S) subname,'rof_phase                = ', infodata%rof_phase
! Subprogram not used        write(logunit,F0S) subname,'wav_phase                = ', infodata%wav_phase
! Subprogram not used 
! Subprogram not used        write(logunit,F0L) subname,'glcrun_alarm             = ', infodata%glcrun_alarm
! Subprogram not used        write(logunit,F0L) subname,'glc_g2lupdate            = ', infodata%glc_g2lupdate
! Subprogram not used !     endif
! Subprogram not used 
! Subprogram not used END SUBROUTINE seq_infodata_print
