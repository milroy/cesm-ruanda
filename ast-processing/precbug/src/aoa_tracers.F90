...
! Subprogram not used   function aoa_tracers_implements_cnst(name)
! Subprogram not used     !----------------------------------------------------------------------- 
! Subprogram not used     ! 
...
! Subprogram not used     logical :: aoa_tracers_implements_cnst        ! return value
! Subprogram not used 
! Subprogram not used     !---------------------------Local workspace-----------------------------
! Subprogram not used     integer :: m
! Subprogram not used     !-----------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used     aoa_tracers_implements_cnst = .false.
! Subprogram not used 
! Subprogram not used     if (.not. aoa_tracers_flag) return
! Subprogram not used 
! Subprogram not used     do m = 1, ncnst
! Subprogram not used        if (name == c_names(m)) then
! Subprogram not used           aoa_tracers_implements_cnst = .true.
! Subprogram not used           return
! Subprogram not used        end if
! Subprogram not used     end do
! Subprogram not used 
! Subprogram not used   end function aoa_tracers_implements_cnst

!===============================================================================

! Subprogram not used   subroutine aoa_tracers_init_cnst(name, q, gcid)
! Subprogram not used 
! Subprogram not used     !----------------------------------------------------------------------- 
! Subprogram not used     !
! Subprogram not used     ! Purpose: initialize test tracers mixing ratio fields 
! Subprogram not used     !  This subroutine is called at the beginning of an initial run ONLY
! Subprogram not used     !
! Subprogram not used     !-----------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used     character(len=*), intent(in)  :: name
! Subprogram not used     real(r8),         intent(out) :: q(:,:)   ! kg tracer/kg dry air (gcol, plev)
...
! Subprogram not used     do m = 1, ncnst
! Subprogram not used        if (name ==  c_names(m))  then
! Subprogram not used           ! pass global constituent index
! Subprogram not used           call init_cnst_3d(ifirst+m-1, q, gcid)
! Subprogram not used        endif
! Subprogram not used     end do
! Subprogram not used 
! Subprogram not used   end subroutine aoa_tracers_init_cnst
...
! Subprogram not used   subroutine init_cnst_3d(m, q, gcid)
! Subprogram not used 
! Subprogram not used     use dyn_grid, only : get_horiz_grid_d, get_horiz_grid_dim_d
! Subprogram not used     use dycore,   only : dycore_is
! Subprogram not used 
! Subprogram not used     integer,  intent(in)  :: m       ! global constituent index
! Subprogram not used     real(r8), intent(out) :: q(:,:)  ! kg tracer/kg dry air (gcol,plev)
! Subprogram not used     integer,  intent(in)  :: gcid(:) ! global column id
! Subprogram not used 
! Subprogram not used     real(r8), allocatable :: lat(:)
! Subprogram not used     integer :: plon, plat, ngcols
! Subprogram not used     integer :: j, k, gsize
! Subprogram not used     !-----------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used     if (masterproc) write(iulog,*) 'AGE-OF-AIR CONSTITUENTS: INITIALIZING ',cnst_name(m),m
! Subprogram not used 
! Subprogram not used     if (m == ixaoa1) then
! Subprogram not used 
! Subprogram not used        q(:,:) = 0.0_r8
! Subprogram not used 
! Subprogram not used     else if (m == ixaoa2) then
! Subprogram not used 
! Subprogram not used        q(:,:) = 0.0_r8
! Subprogram not used 
! Subprogram not used     else if (m == ixht) then
! Subprogram not used 
! Subprogram not used        call get_horiz_grid_dim_d( plon, plat )
! Subprogram not used        ngcols = plon*plat
! Subprogram not used        gsize = size(gcid)
! Subprogram not used        allocate(lat(ngcols))
! Subprogram not used        call get_horiz_grid_d(ngcols,clat_d_out=lat)
! Subprogram not used        do j = 1, gsize
! Subprogram not used           q(j,:) = 2._r8 + sin(lat(gcid(j)))
! Subprogram not used        end do
! Subprogram not used        deallocate(lat)
! Subprogram not used 
! Subprogram not used     else if (m == ixvt) then
! Subprogram not used 
! Subprogram not used        do k = 1, pver
! Subprogram not used           do j = 1, size(q,1)
! Subprogram not used              q(j,k) = qrel_vert(k)
! Subprogram not used           end do
! Subprogram not used        end do
! Subprogram not used 
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used   end subroutine init_cnst_3d
