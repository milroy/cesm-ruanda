...
!                  ierr = nf90_set_fill(File%fh, NF90_NOFILL, nmode)
       ...
       case(PIO_iotype_netcdf)
          ...
          if (File%iosystem%io_rank == 0) then
             ! Stores the ncid in File%fh
             ierr = nf90_create(fname, nmode , File%fh)
             if(Debug .or. Debugasync) print *,"ionf_mod.F90",113,file%fh, ierr, nmode
! Set default to NOFILL for performance.  
             if(ierr==NF90_NOERR) &
                  ierr = nf90_set_fill(File%fh, NF90_NOFILL, nmode)
          endif
...
! we need to go through some contortions to make sure a file we are opening is okay for parallel access
       ...
       if(iotype==PIO_iotype_netcdf .or. iotype==PIO_IOTYPE_NETCDF4C) then
          if (File%iosystem%io_rank == 0) then
             ! Stores the ncid in File%fh
             ierr = nf90_open(fname,amode,File%fh)
             if(Debug .or. Debugasync) print *,"ionf_mod.F90",218,file%fh, ierr
             ! Set default to NOFILL for performance.  
    ...
    if(File%iosystem%IOproc) then
       ...
       case(PIO_iotype_netcdf, pio_iotype_netcdf4c, pio_iotype_netcdf4p)
          if (File%fh>0) then
             ...
             if(Debug) print *,"<stdin>",262,ierr
             ierr= nf90_close(File%fh)
             if(Debug) print *,"<stdin>",264,ierr
          endif
       case default
...
! Subprogram not used   integer function sync_nf(File) result(ierr)
! Subprogram not used 
! Subprogram not used     type (File_desc_t), intent(inout) :: File
! Subprogram not used 
! Subprogram not used     ierr=PIO_noerr
! Subprogram not used 
! Subprogram not used     if(File%iosystem%IOproc) then
! Subprogram not used        if(Debug) print *,"ionf_mod.F90",288,'CFILE syncing : ',file%fh
! Subprogram not used        select case (File%iotype) 
! Subprogram not used        case(PIO_iotype_pnetcdf)
! Subprogram not used           ierr=nfmpi_sync(file%fh)
! Subprogram not used        case(PIO_iotype_netcdf, pio_iotype_netcdf4c,PIO_IOTYPE_NETCDF4P)
! Subprogram not used           if (File%fh>0) then
! Subprogram not used              ierr= nf90_sync(File%fh)
! Subprogram not used           endif
! Subprogram not used        case default
! Subprogram not used           call bad_iotype(File%iotype,"ionf_mod.F90",301)
! Subprogram not used        end select
! Subprogram not used     end if
! Subprogram not used     call check_netcdf(File, ierr,"ionf_mod.F90",304)
! Subprogram not used   end function sync_nf

! Subprogram not used   subroutine check_file_type(File, filename) 
! Subprogram not used     include 'mpif.h'      ! _EXTERNAL
! Subprogram not used 
! Subprogram not used     type (File_desc_t), intent(inout) :: File
! Subprogram not used     character(len=*), intent(in) :: filename
! Subprogram not used     character(len=4) :: magic
! Subprogram not used     integer :: fh, mpierr, reclength=4, i, eof 
! Subprogram not used     logical :: UNITOK, UNITOP
! Subprogram not used 
! Subprogram not used !   Check format of existing files opened to read.
! Subprogram not used 
! Subprogram not used     inquire(file=filename, exist=UNITOK) 
! Subprogram not used     if(.not. UNITOK) return
! Subprogram not used 
! Subprogram not used     magic='fail'
! Subprogram not used    
! Subprogram not used     if(File%iosystem%ioproc) then      
! Subprogram not used        if(File%iosystem%io_rank==0) then
! Subprogram not used !  Find a unique unit number to open the file
! Subprogram not used           do fh=12,99
! Subprogram not used              inquire (unit=fh,exist=UNITOK,opened=UNITOP)
! Subprogram not used              if (UNITOK .and. .not. UNITOP) then
! Subprogram not used                 open (unit = fh,File=filename,access='direct',recl=reclength,&
! Subprogram not used                      FORM='UNFORMATTED',STATUS='OLD',err=100)
! Subprogram not used ! Read the first 4 bytes and look for the CDF or HDF stamp
! Subprogram not used                 read (fh,rec=1,err=101) magic
! Subprogram not used                 close(fh)
! Subprogram not used                 exit
! Subprogram not used              endif
! Subprogram not used           end do
! Subprogram not used           if(magic(1:3) .eq. 'CDF') then
! Subprogram not used              ! No need to do anything here
! Subprogram not used           else if(magic(2:4).eq.'HDF') then
! Subprogram not used              if(File%iotype /= PIO_IOTYPE_NETCDF4C .and. &
! Subprogram not used                   File%iotype /= PIO_IOTYPE_NETCDF4P) then
! Subprogram not used                 print *,'Changing file type to netcdf4p'
! Subprogram not used                 File%iotype=pio_iotype_netcdf4c
! Subprogram not used              end if
! Subprogram not used           else 
! Subprogram not used              ! The HDF identifier could be offset further into the file.
! Subprogram not used              
! Subprogram not used              open (unit = fh,file=filename,access='direct',recl=reclength,&
! Subprogram not used                   form='UNFORMATTED',STATUS='OLD',err=100)
! Subprogram not used 
! Subprogram not used              i=128
! Subprogram not used              eof=0
! Subprogram not used              do while(eof>=0)
! Subprogram not used                 read (fh,rec=i, iostat=eof, err=101) magic
! Subprogram not used 
! Subprogram not used                 if(magic(2:4).eq.'HDF') then
! Subprogram not used                    if(debug) print *,'Changing file type to netcdf4p'
! Subprogram not used                    File%iotype=pio_iotype_netcdf4p
! Subprogram not used                    exit
! Subprogram not used                 end if
! Subprogram not used                 i=i*2
! Subprogram not used              end do
! Subprogram not used              close(fh)
! Subprogram not used              if(eof<0) call piodie("ionf_mod.F90",373,'Unrecognized file format ',0,filename)             
! Subprogram not used           end if
! Subprogram not used 
! Subprogram not used        end if
! Subprogram not used        
! Subprogram not used        call mpi_bcast(file%iotype,1,mpi_integer, 0, file%iosystem%io_comm, mpierr)
! Subprogram not used        call CheckMPIReturn('nf_mod',mpierr)
! Subprogram not used     end if
! Subprogram not used     return
! Subprogram not used 100 call piodie("ionf_mod.F90",382,'File open error ',0,filename)
! Subprogram not used 101 call piodie("ionf_mod.F90",383,'File read error ',0,filename)
! Subprogram not used 
! Subprogram not used 
! Subprogram not used 
! Subprogram not used   end subroutine check_file_type
