module physpkg
  ...
  ! Purpose:
  !
  ! Provides the interface to 1 physics package
  !
  ! Revision history:
...
subroutine phys_register
    ...
    ! If using an ideal/adiabatic physics option, the 1 physics parameterizations 
    ! aren't called.
    if (moist_physics) then
    ...
    !                  to constituent number 25 -- regardless of what 25 is set to.
...
! Subprogram not used subroutine phys_run1_adiabatic_or_ideal(ztodt, phys_state, phys_tend,  pbuf2d)
! Subprogram not used     !----------------------------------------------------------------------- 
! Subprogram not used     ! 
! Subprogram not used     ! Purpose: 
! Subprogram not used     ! Physics for adiabatic or idealized physics case.
! Subprogram not used     ! 
! Subprogram not used     !-----------------------------------------------------------------------
! Subprogram not used     use physics_buffer, only : physics_buffer_desc, pbuf_set_field, pbuf_get_chunk, pbuf_old_tim_idx
! Subprogram not used     use time_manager,     only: get_nstep
! Subprogram not used     use cam_diagnostics,  only: diag_phys_writeout
! Subprogram not used     use check_energy,     only: check_energy_fix, check_energy_chng
! Subprogram not used     use dycore,           only: dycore_is
! Subprogram not used 
! Subprogram not used     !
! Subprogram not used     ! Input arguments
! Subprogram not used     !
! Subprogram not used     real(r8), intent(in) :: ztodt            ! physics time step unless nstep=0
! Subprogram not used     !
! Subprogram not used     ! Input/Output arguments
! Subprogram not used     !
! Subprogram not used     type(physics_state), intent(inout), dimension(begchunk:endchunk) :: phys_state
! Subprogram not used     type(physics_tend ), intent(inout), dimension(begchunk:endchunk) :: phys_tend
! Subprogram not used 
! Subprogram not used     type(physics_buffer_desc), pointer :: pbuf2d(:,:)
! Subprogram not used     !-----------------------------------------------------------------------
! Subprogram not used     !---------------------------Local workspace-----------------------------
! Subprogram not used     !
! Subprogram not used     integer             :: c               ! indices
! Subprogram not used     integer             :: nstep           ! current timestep number
! Subprogram not used     type(physics_ptend) :: ptend(begchunk:endchunk) ! indivdual parameterization tendencies
! Subprogram not used     real(r8)            :: flx_heat(pcols) ! effective sensible heat flux
! Subprogram not used     real(r8)            :: zero(pcols)     ! array of zeros
! Subprogram not used 
! Subprogram not used     ! physics buffer field for total energy
! Subprogram not used     real(r8), pointer, dimension(:) :: teout
! Subprogram not used     logical, SAVE :: first_exec_of_phys_run1_adiabatic_or_ideal  = .TRUE.
! Subprogram not used     !-----------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used     nstep = get_nstep()
! Subprogram not used     zero  = 0._r8
! Subprogram not used 
! Subprogram not used     ! Associate pointers with physics buffer fields
! Subprogram not used     if (first_exec_of_phys_run1_adiabatic_or_ideal) then
! Subprogram not used        first_exec_of_phys_run1_adiabatic_or_ideal  = .FALSE.
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used !$OMP PARALLEL DO PRIVATE (C, FLX_HEAT)
! Subprogram not used     do c=begchunk, endchunk
! Subprogram not used 
! Subprogram not used        ! Initialize the physics tendencies to zero.
! Subprogram not used        call physics_tend_init(phys_tend(c))
! Subprogram not used 
! Subprogram not used        ! Dump dynamics variables to history buffers
! Subprogram not used        call diag_phys_writeout(phys_state(c))
! Subprogram not used 
! Subprogram not used        if (dycore_is('LR') .or. dycore_is('SE') ) then
! Subprogram not used           call check_energy_fix(phys_state(c), ptend(c), nstep, flx_heat)
! Subprogram not used           call physics_update(phys_state(c), ptend(c), ztodt, phys_tend(c))
! Subprogram not used           call check_energy_chng(phys_state(c), phys_tend(c), "chkengyfix", nstep, ztodt, &
! Subprogram not used                zero, zero, zero, flx_heat)
! Subprogram not used           call physics_ptend_dealloc(ptend(c))
! Subprogram not used        end if
! Subprogram not used 
! Subprogram not used        if ( ideal_phys )then
! Subprogram not used           call t_startf('tphysidl')
! Subprogram not used           call tphysidl(ztodt, phys_state(c), phys_tend(c))
! Subprogram not used           call t_stopf('tphysidl')
! Subprogram not used        end if
! Subprogram not used 
! Subprogram not used        ! Save total enery after physics for energy conservation checks
! Subprogram not used        call pbuf_set_field(pbuf_get_chunk(pbuf2d, c), teout_idx, phys_state(c)%te_cur)
! Subprogram not used 
! Subprogram not used     end do
! Subprogram not used 
! Subprogram not used end subroutine phys_run1_adiabatic_or_ideal
...
subroutine tphysac (ztodt,   cam_in,  &
   ...
   ! that cam_out%xxxdryxxx fields have already been set for 1 aerosols and cam_out
...
subroutine tphysbc (ztodt,               &
    ...
    ! for 1-RT.  But it's done here so that the microphysics code which is called
    ...
    if ( .not. deep_scheme_does_scav_trans() ) then
       ...
       if (carma_do_wetdep) then
          ...
          ! fields have already been set for 1 aerosols and cam_out can be added
