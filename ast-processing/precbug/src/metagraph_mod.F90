...
! Subprogram not used   recursive subroutine copy_metaedge(edge2,edge1)
! Subprogram not used 
! Subprogram not used     type (MetaEdge_t), intent(out) :: edge2
! Subprogram not used     type (MetaEdge_t), intent(in)  :: edge1
! Subprogram not used 
! Subprogram not used     integer i
! Subprogram not used 
! Subprogram not used     edge2%number   = edge1%number
! Subprogram not used     edge2%type     = edge1%type
! Subprogram not used     edge2%wgtP      = edge1%wgtP
! Subprogram not used     edge2%wgtP_ghost      = edge1%wgtP_ghost
! Subprogram not used     edge2%nmembers = edge1%nmembers
! Subprogram not used 
! Subprogram not used     if (associated(edge1%members)) then
! Subprogram not used        allocate(edge2%members(edge2%nmembers))
! Subprogram not used        do i=1,edge2%nmembers
! Subprogram not used           edge2%members(i)=edge1%members(i)
! Subprogram not used        end do
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used     if (associated(edge1%edgeptrP)) then
! Subprogram not used        allocate(edge2%edgeptrP(edge2%nmembers))
! Subprogram not used        allocate(edge2%edgeptrP_ghost(edge2%nmembers))
! Subprogram not used        do i=1,edge2%nmembers
! Subprogram not used           edge2%edgeptrP(i)=edge1%edgeptrP(i)
! Subprogram not used           edge2%edgeptrP_ghost(i)=edge1%edgeptrP_ghost(i)
! Subprogram not used        end do
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used     edge2%HeadVertex = edge1%HeadVertex
! Subprogram not used     edge2%TailVertex = edge1%TailVertex
! Subprogram not used 
! Subprogram not used   end subroutine copy_metaedge
...
! Subprogram not used   function edge_uses_vertex(Vertex,Edge) result(log)
! Subprogram not used 
! Subprogram not used     implicit none
! Subprogram not used     type(MetaVertex_t), intent(in) :: Vertex
! Subprogram not used     type(MetaEdge_t),   intent(in) :: Edge
! Subprogram not used     logical :: log
! Subprogram not used     integer  :: number
! Subprogram not used 
! Subprogram not used     number = Vertex%number
! Subprogram not used     if(number == Edge%HeadVertex .or. number == Edge%TailVertex) then 
! Subprogram not used        log = .TRUE.
! Subprogram not used     else
! Subprogram not used        log = .FALSE. 
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used   end function edge_uses_vertex

! Subprogram not used   subroutine PrintMetaEdge(Edge)
! Subprogram not used     use gridgraph_mod, only : PrintGridEdge
! Subprogram not used     implicit none
! Subprogram not used     type (MetaEdge_t), intent(in) :: Edge(:)
! Subprogram not used     integer          :: i,nedge
! Subprogram not used 
! Subprogram not used     nedge = SIZE(Edge)
! Subprogram not used     do i=1,nedge
! Subprogram not used        print *
! Subprogram not used        write(iulog,90)  Edge(i)%number,Edge(i)%type,Edge(i)%wgtP,Edge(i)%nmembers, &
! Subprogram not used             Edge(i)%TailVertex, Edge(i)%HeadVertex
! Subprogram not used        if(associated(Edge(i)%members)) then
! Subprogram not used           call PrintGridEdge(Edge(i)%members)
! Subprogram not used        endif
! Subprogram not used     enddo
! Subprogram not used 90  format('METAEDGE #',I4,2x,'TYPE ',I1,2x,'WGT ',I4,2x,'NUM ',I6,2x,'Processors ',I4,' ---> ',I4)
! Subprogram not used 
! Subprogram not used   end subroutine PrintMetaEdge

! Subprogram not used   subroutine PrintMetaVertex(Vertex)
! Subprogram not used     use gridgraph_mod, only : PrintGridVertex
! Subprogram not used     implicit none 
! Subprogram not used     type (MetaVertex_t), intent(in),target  :: Vertex
! Subprogram not used 
! Subprogram not used     integer               :: i,j,npart
! Subprogram not used 
! Subprogram not used 
! Subprogram not used     write(iulog,*)
! Subprogram not used     write(iulog,90) i
! Subprogram not used     write(iulog,95) Vertex%nmembers
! Subprogram not used     call PrintGridVertex(Vertex%members)
! Subprogram not used     write(iulog,96) Vertex%nedges
! Subprogram not used     if(associated(Vertex%edges)) then 
! Subprogram not used        do j=1,Vertex%nedges
! Subprogram not used           write(iulog,97) Vertex%edges(j)%number,     Vertex%edges(j)%type,              &
! Subprogram not used                Vertex%edges(j)%wgtP,        Vertex%edges(j)%HeadVertex,        &
! Subprogram not used                Vertex%edges(j)%TailVertex	 
! Subprogram not used        enddo
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used 90  format('METAVERTEX #',I2,2x)
! Subprogram not used 95  format(5x,I2,' Member Grid Vertices')
! Subprogram not used 96  format(5x,I2,' Incident Meta Edges ')
! Subprogram not used 97  format(10x,'METAEDGE #',I2,2x,'TYPE ',I1,2x,'WGT ',I4,2x,'Processors ',I2,' ---> ',I2)
! Subprogram not used 98  format(5x,'GRIDVERTEX #',5x,'West',5x,'East',5x,'South',5x,'North')
! Subprogram not used 99  format(10x,I2,7x,4(2x,I3,1x,'(',I1,')'))
! Subprogram not used 
! Subprogram not used   end subroutine PrintMetaVertex
