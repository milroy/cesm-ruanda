  ...
  !--------------------------------------------------- !
  ! Purpose     : 1 Interface for Cloud Macrophysics !
  ! Author      : Sungsu Park                          !
...
! Subprogram not used    subroutine astG_RHU_single( U, p, qv, landfrac, snowh, a, Ga, orhmin )
! Subprogram not used 
! Subprogram not used    ! --------------------------------------------------------- !
! Subprogram not used    ! Compute 'stratus fraction(a)' and Gs=(dU/da) from the     !
! Subprogram not used    ! CAM35 cloud fraction formula.                             !
! Subprogram not used    ! Below is valid only for CAMUW at 1.9x2.5 fv dynamics core !  
! Subprogram not used    ! For the other cases, I should re-define 'rhminl,rhminh' & !
! Subprogram not used    ! 'premib,premit'.                                          !
! Subprogram not used    ! Note that if U > 1, Ga = 1.e10 instead of Ga = 0, that is !
! Subprogram not used    ! G is discontinuous across U = 1.                          !
! Subprogram not used    ! --------------------------------------------------------- !
! Subprogram not used 
! Subprogram not used    implicit none
! Subprogram not used 
! Subprogram not used    real(r8), intent(in)  :: U               ! Relative humidity
! Subprogram not used    real(r8), intent(in)  :: p               ! Pressure [Pa]
! Subprogram not used    real(r8), intent(in)  :: qv              ! Grid-mean water vapor specific humidity [kg/kg]
! Subprogram not used    real(r8), intent(in)  :: landfrac        ! Land fraction
! Subprogram not used    real(r8), intent(in)  :: snowh           ! Snow depth (liquid water equivalent)
! Subprogram not used 
! Subprogram not used    real(r8), intent(out) :: a               ! Stratus fraction
! Subprogram not used    real(r8), intent(out) :: Ga              ! dU/da
! Subprogram not used    real(r8), optional, intent(out) :: orhmin ! Critical RH
! Subprogram not used 
! Subprogram not used    ! Local variables
! Subprogram not used    real(r8) rhmin                                 ! Critical RH
! Subprogram not used    real(r8) rhdif                                 ! Factor for stratus fraction
! Subprogram not used    real(r8) rhwght
! Subprogram not used 
! Subprogram not used    ! Statement functions
! Subprogram not used    logical land
! Subprogram not used    land = nint(landfrac) == 1
! Subprogram not used 
! Subprogram not used    ! ---------------- !
! Subprogram not used    ! Main computation !
! Subprogram not used    ! ---------------- !
! Subprogram not used 
! Subprogram not used    if( p .ge. premib ) then
! Subprogram not used 
! Subprogram not used        if( land .and. (snowh.le.0.000001_r8) ) then
! Subprogram not used            rhmin = rhminl - rhminl_adj_land
! Subprogram not used        else
! Subprogram not used            rhmin = rhminl
! Subprogram not used        endif
! Subprogram not used        rhdif = (U-rhmin)/(1.0_r8-rhmin)
! Subprogram not used        a  = min(1._r8,(max(rhdif,0.0_r8))**2) 
! Subprogram not used        if( (U.ge.1._r8) .or. (U.le.rhmin) ) then
! Subprogram not used             Ga = 1.e20_r8
! Subprogram not used        else          
! Subprogram not used             Ga = 0.5_r8*(1._r8-rhmin)*((1._r8-rhmin)/(U-rhmin))
! Subprogram not used        endif
! Subprogram not used        if( freeze_dry ) then
! Subprogram not used            a  = a*max(0.15_r8,min(1.0_r8,qv/0.0030_r8))
! Subprogram not used            Ga = Ga/max(0.15_r8,min(1.0_r8,qv/0.0030_r8)) 
! Subprogram not used        endif
! Subprogram not used 
! Subprogram not used    elseif( p .lt. premit ) then
! Subprogram not used 
! Subprogram not used        rhmin = rhminh
! Subprogram not used        rhdif = (U-rhmin)/(1.0_r8-rhmin)
! Subprogram not used        a  = min(1._r8,(max(rhdif,0._r8))**2)
! Subprogram not used        if( (U.ge.1._r8) .or. (U.le.rhmin) ) then
! Subprogram not used             Ga = 1.e20_r8
! Subprogram not used        else          
! Subprogram not used             Ga = 0.5_r8*(1._r8-rhmin)*((1._r8-rhmin)/(U-rhmin))
...
! Subprogram not used    else
! Subprogram not used 
! Subprogram not used        rhwght = (premib-(max(p,premit)))/(premib-premit)
! Subprogram not used 
! Subprogram not used      ! if( land .and. (snowh.le.0.000001_r8) ) then
! Subprogram not used      !     rhmin = rhminh*rhwght + (rhminl - rhminl_adj_land)*(1.0_r8-rhwght)
! Subprogram not used      ! else
! Subprogram not used            rhmin = rhminh*rhwght + rhminl*(1.0_r8-rhwght)
! Subprogram not used      ! endif
! Subprogram not used 
! Subprogram not used        rhdif = (U-rhmin)/(1.0_r8-rhmin)
! Subprogram not used        a  = min(1._r8,(max(rhdif,0._r8))**2)
! Subprogram not used        if( (U.ge.1._r8) .or. (U.le.rhmin) ) then
! Subprogram not used             Ga = 1.e10_r8
! Subprogram not used        else          
! Subprogram not used             Ga = 0.5_r8*(1._r8-rhmin)*((1._r8-rhmin)/(U-rhmin))
! Subprogram not used        endif
! Subprogram not used 
! Subprogram not used    endif
! Subprogram not used 
! Subprogram not used    if (present(orhmin)) orhmin = rhmin
! Subprogram not used 
! Subprogram not used    return
! Subprogram not used    end subroutine astG_RHU_single

   ! ----------------- !
   ! End of subroutine !
   ! ----------------- !

! Subprogram not used    subroutine astG_RHU( U_in, p_in, qv_in, landfrac_in, snowh_in, a_out, Ga_out, ncol )
! Subprogram not used 
! Subprogram not used    ! --------------------------------------------------------- !
! Subprogram not used    ! Compute 'stratus fraction(a)' and Gs=(dU/da) from the     !
! Subprogram not used    ! CAM35 cloud fraction formula.                             !
...
! Subprogram not used    real(r8), intent(in)  :: U_in(pcols)           ! Relative humidity
! Subprogram not used    real(r8), intent(in)  :: p_in(pcols)           ! Pressure [Pa]
! Subprogram not used    real(r8), intent(in)  :: qv_in(pcols)          ! Grid-mean water vapor specific humidity [kg/kg]
! Subprogram not used    real(r8), intent(in)  :: landfrac_in(pcols)    ! Land fraction
! Subprogram not used    real(r8), intent(in)  :: snowh_in(pcols)       ! Snow depth (liquid water equivalent)
! Subprogram not used 
! Subprogram not used    real(r8), intent(out) :: a_out(pcols)          ! Stratus fraction
! Subprogram not used    real(r8), intent(out) :: Ga_out(pcols)         ! dU/da
! Subprogram not used 
! Subprogram not used    real(r8)              :: U                     ! Relative humidity
! Subprogram not used    real(r8)              :: p                     ! Pressure [Pa]
! Subprogram not used    real(r8)              :: qv                    ! Grid-mean water vapor specific humidity [kg/kg]
! Subprogram not used    real(r8)              :: landfrac              ! Land fraction
! Subprogram not used    real(r8)              :: snowh                 ! Snow depth (liquid water equivalent)
! Subprogram not used 
! Subprogram not used    real(r8)              :: a                     ! Stratus fraction
! Subprogram not used    real(r8)              :: Ga                    ! dU/da
! Subprogram not used 
! Subprogram not used    ! Local variables
...
! Subprogram not used    ! Statement functions
! Subprogram not used    logical land
! Subprogram not used    land(i) = nint(landfrac_in(i)) == 1
! Subprogram not used 
! Subprogram not used    ! ---------------- !
! Subprogram not used    ! Main computation !
! Subprogram not used    ! ---------------- !
! Subprogram not used 
! Subprogram not used    a_out(:) = 0._r8
! Subprogram not used    Ga_out(:) = 0._r8
! Subprogram not used 
! Subprogram not used    do i = 1, ncol
! Subprogram not used 
! Subprogram not used    U        = U_in(i)      
! Subprogram not used    p        = p_in(i)        
! Subprogram not used    qv       = qv_in(i)       
! Subprogram not used    landfrac = landfrac_in(i) 
! Subprogram not used    snowh    = snowh_in(i)    
! Subprogram not used 
! Subprogram not used    if( p .ge. premib ) then
! Subprogram not used 
! Subprogram not used        if( land(i) .and. (snowh.le.0.000001_r8) ) then
! Subprogram not used            rhmin = rhminl - rhminl_adj_land
! Subprogram not used        else
! Subprogram not used            rhmin = rhminl
! Subprogram not used        endif
...
! Subprogram not used    endif
! Subprogram not used 
! Subprogram not used    a_out(i)  = a
! Subprogram not used    Ga_out(i) = Ga 
! Subprogram not used 
! Subprogram not used    enddo
! Subprogram not used 
! Subprogram not used    return
! Subprogram not used    end subroutine astG_RHU
