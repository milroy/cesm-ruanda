...
! Subprogram not used subroutine wv_sat_reset_default()
! Subprogram not used 
! Subprogram not used   default_idx = initial_default_idx
! Subprogram not used 
! Subprogram not used end subroutine wv_sat_reset_default
...
! Subprogram not used elemental subroutine wv_sat_qsat_ice(t, p, es, qs, idx)
! Subprogram not used   !------------------------------------------------------------------!
! Subprogram not used   ! Purpose:                                                         !
! Subprogram not used   !   Calculate SVP over ice at a given temperature, and then        !
! Subprogram not used   !   calculate and return saturation specific humidity.             !
! Subprogram not used   !------------------------------------------------------------------!
! Subprogram not used 
! Subprogram not used   ! Inputs
! Subprogram not used   real(r8), intent(in) :: t    ! Temperature
! Subprogram not used   real(r8), intent(in) :: p    ! Pressure
! Subprogram not used   ! Outputs
! Subprogram not used   real(r8), intent(out) :: es  ! Saturation vapor pressure
! Subprogram not used   real(r8), intent(out) :: qs  ! Saturation specific humidity
! Subprogram not used 
! Subprogram not used   integer,  intent(in), optional :: idx ! Scheme index
! Subprogram not used 
! Subprogram not used   es = wv_sat_svp_ice(t, idx)
! Subprogram not used 
! Subprogram not used   qs = wv_sat_svp_to_qsat(es, p)
! Subprogram not used 
! Subprogram not used   ! Ensures returned es is consistent with limiters on qs.
! Subprogram not used   es = min(es, p)
! Subprogram not used 
! Subprogram not used end subroutine wv_sat_qsat_ice

! Subprogram not used elemental subroutine wv_sat_qsat_trans(t, p, es, qs, idx)
! Subprogram not used   !------------------------------------------------------------------!
! Subprogram not used   ! Purpose:                                                         !
! Subprogram not used   !   Calculate SVP over ice at a given temperature, and then        !
! Subprogram not used   !   calculate and return saturation specific humidity.             !
...
! Subprogram not used   integer,  intent(in), optional :: idx ! Scheme index
! Subprogram not used 
! Subprogram not used   es = wv_sat_svp_trans(t, idx)
! Subprogram not used 
! Subprogram not used   qs = wv_sat_svp_to_qsat(es, p)
! Subprogram not used 
! Subprogram not used   ! Ensures returned es is consistent with limiters on qs.
! Subprogram not used   es = min(es, p)
! Subprogram not used 
! Subprogram not used end subroutine wv_sat_qsat_trans
...
! Subprogram not used elemental function GoffGratch_svp_water(t) result(es)
! Subprogram not used   real(r8), intent(in) :: t  ! Temperature in Kelvin
! Subprogram not used   real(r8) :: es             ! SVP in Pa
! Subprogram not used 
! Subprogram not used   ! uncertain below -70 C
! Subprogram not used   es = 10._r8**(-7.90298_r8*(tboil/t-1._r8)+ &
! Subprogram not used        5.02808_r8*log10(tboil/t)- &
! Subprogram not used        1.3816e-7_r8*(10._r8**(11.344_r8*(1._r8-t/tboil))-1._r8)+ &
! Subprogram not used        8.1328e-3_r8*(10._r8**(-3.49149_r8*(tboil/t-1._r8))-1._r8)+ &
! Subprogram not used        log10(1013.246_r8))*100._r8
! Subprogram not used 
! Subprogram not used end function GoffGratch_svp_water

! NEW MODIFIED ELEMENTAL FUNCTION
...
!       8.1328e-3_r4*(10._r4**(-3.49149_r4*(tboil4/t4-1._r4))-1._r4)+ &
       8.1828e-3_r4*(10._r4**(-3.49149_r4*(tboil4/t4-1._r4))-1._r4)+ &
...
! Subprogram not used elemental function MurphyKoop_svp_water(t) result(es)
! Subprogram not used   real(r8), intent(in) :: t  ! Temperature in Kelvin
! Subprogram not used   real(r8) :: es             ! SVP in Pa
! Subprogram not used 
! Subprogram not used   ! (good for 123 < T < 332 K)
! Subprogram not used   es = exp(54.842763_r8 - (6763.22_r8 / t) - (4.210_r8 * log(t)) + &
! Subprogram not used        (0.000367_r8 * t) + (tanh(0.0415_r8 * (t - 218.8_r8)) * &
! Subprogram not used        (53.878_r8 - (1331.22_r8 / t) - (9.44523_r8 * log(t)) + &
! Subprogram not used        0.014025_r8 * t)))
! Subprogram not used 
! Subprogram not used end function MurphyKoop_svp_water

! Subprogram not used elemental function MurphyKoop_svp_ice(t) result(es)
! Subprogram not used   real(r8), intent(in) :: t  ! Temperature in Kelvin
! Subprogram not used   real(r8) :: es             ! SVP in Pa
! Subprogram not used 
! Subprogram not used   ! (good down to 110 K)
! Subprogram not used   es = exp(9.550426_r8 - (5723.265_r8 / t) + (3.53068_r8 * log(t)) &
! Subprogram not used        - (0.00728332_r8 * t))
! Subprogram not used 
! Subprogram not used end function MurphyKoop_svp_ice
...
! Subprogram not used elemental function OldGoffGratch_svp_water(t) result(es)
! Subprogram not used   real(r8), intent(in) :: t
! Subprogram not used   real(r8) :: es
! Subprogram not used   real(r8) :: ps, e1, e2, f1, f2, f3, f4, f5, f
! Subprogram not used 
! Subprogram not used   ps = 1013.246_r8
! Subprogram not used   e1 = 11.344_r8*(1.0_r8 - t/tboil)
! Subprogram not used   e2 = -3.49149_r8*(tboil/t - 1.0_r8)
! Subprogram not used   f1 = -7.90298_r8*(tboil/t - 1.0_r8)
! Subprogram not used   f2 = 5.02808_r8*log10(tboil/t)
! Subprogram not used   f3 = -1.3816_r8*(10.0_r8**e1 - 1.0_r8)/10000000.0_r8
! Subprogram not used   f4 = 8.1328_r8*(10.0_r8**e2 - 1.0_r8)/1000.0_r8
! Subprogram not used   f5 = log10(ps)
! Subprogram not used   f  = f1 + f2 + f3 + f4 + f5
! Subprogram not used 
! Subprogram not used   es = (10.0_r8**f)*100.0_r8
! Subprogram not used   
! Subprogram not used end function OldGoffGratch_svp_water

! Subprogram not used elemental function OldGoffGratch_svp_ice(t) result(es)
! Subprogram not used   real(r8), intent(in) :: t
! Subprogram not used   real(r8) :: es
! Subprogram not used   real(r8) :: term1, term2, term3
! Subprogram not used 
! Subprogram not used   term1 = 2.01889049_r8/(tmelt/t)
! Subprogram not used   term2 = 3.56654_r8*log(tmelt/t)
! Subprogram not used   term3 = 20.947031_r8*(tmelt/t)
! Subprogram not used 
! Subprogram not used   es = 575.185606e10_r8*exp(-(term1 + term2 + term3))
! Subprogram not used   
! Subprogram not used end function OldGoffGratch_svp_ice
...
! Subprogram not used elemental function Bolton_svp_water(t) result(es)
! Subprogram not used   real(r8),parameter :: c1 = 611.2_r8
! Subprogram not used   real(r8),parameter :: c2 = 17.67_r8
! Subprogram not used   real(r8),parameter :: c3 = 243.5_r8
! Subprogram not used 
! Subprogram not used   real(r8), intent(in) :: t  ! Temperature in Kelvin
! Subprogram not used   real(r8) :: es             ! SVP in Pa
! Subprogram not used 
! Subprogram not used   es = c1*exp( (c2*(t - tmelt))/((t - tmelt)+c3) )
! Subprogram not used 
! Subprogram not used end function Bolton_svp_water
