...
! Subprogram not used   subroutine htape_create (t, histrest)
! Subprogram not used 
! Subprogram not used     ! !DESCRIPTION:
! Subprogram not used     ! Define contents of history file t. Issue the required netcdf
! Subprogram not used     ! wrapper calls to define the history file contents.
! Subprogram not used     !
! Subprogram not used 
! Subprogram not used     ! !ARGUMENTS:
! Subprogram not used     implicit none
! Subprogram not used     integer, intent(in) :: t                   ! tape index
! Subprogram not used     logical, intent(in), optional :: histrest  ! if creating the history restart file
! Subprogram not used 
! Subprogram not used     ! !LOCAL VARIABLES:
! Subprogram not used     integer :: f                   ! field index
! Subprogram not used     integer :: p,c,l,n             ! indices
! Subprogram not used     integer :: ier                 ! error code
! Subprogram not used     integer :: dimid               ! dimension id temporary
! Subprogram not used     integer :: dim1id(1)           ! netCDF dimension id
! Subprogram not used     integer :: dim2id(2)           ! netCDF dimension id
! Subprogram not used     integer :: ndims               ! dimension counter
! Subprogram not used     integer :: omode               ! returned mode from netCDF call
! Subprogram not used     integer :: ncprec              ! output netCDF write precision
! Subprogram not used     integer :: ret                 ! netCDF error status
! Subprogram not used     integer :: num_lndrof          ! total number of land runoff across all procs
! Subprogram not used     integer :: num_ocnrof          ! total number of ocean runoff across all procs
! Subprogram not used     integer :: numrtm              ! total number of rtm cells on all procs
! Subprogram not used     logical :: lhistrest           ! local history restart flag
! Subprogram not used     type(file_desc_t) :: lnfid     ! local file id
! Subprogram not used     character(len=  8) :: curdate  ! current date
! Subprogram not used     character(len=  8) :: curtime  ! current time
! Subprogram not used     character(len=256) :: name     ! name of attribute
! Subprogram not used     character(len=256) :: units    ! units of attribute
! Subprogram not used     character(len=256) :: str      ! global attribute string
! Subprogram not used     character(len=  1) :: avgflag  ! time averaging flag
! Subprogram not used     character(len=*),parameter :: subname = 'htape_create'
! Subprogram not used     !-----------------------------------------------------
! Subprogram not used 
! Subprogram not used     if ( present(histrest) )then
! Subprogram not used        lhistrest = histrest
! Subprogram not used     else
! Subprogram not used        lhistrest = .false.
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used     ! Define output write precsion for tape
! Subprogram not used     ncprec = tape(t)%ncprec
! Subprogram not used 
! Subprogram not used     ! Create new netCDF file. It will be in define mode
! Subprogram not used     if ( .not. lhistrest )then
! Subprogram not used        if (masterproc) then
! Subprogram not used           write(iulog,*) trim(subname),' : Opening netcdf htape ', &
! Subprogram not used                                       trim(locfnh(t))
! Subprogram not used           call shr_sys_flush(iulog)
! Subprogram not used        end if
! Subprogram not used        call ncd_pio_createfile(lnfid, trim(locfnh(t)))
! Subprogram not used        call ncd_putatt(lnfid, ncd_global, 'title', 'RTM History file information' )
! Subprogram not used        call ncd_putatt(lnfid, ncd_global, 'comment', &
! Subprogram not used           "NOTE: None of the variables are weighted by land fraction!" )
! Subprogram not used     else
! Subprogram not used        if (masterproc) then
! Subprogram not used           write(iulog,*) trim(subname),' : Opening netcdf rhtape ', &
! Subprogram not used                                       trim(locfnhr(t))
! Subprogram not used           call shr_sys_flush(iulog)
! Subprogram not used        end if
! Subprogram not used        call ncd_pio_createfile(lnfid, trim(locfnhr(t)))
! Subprogram not used        call ncd_putatt(lnfid, ncd_global, 'title', &
! Subprogram not used             'RTM Restart History information, required to continue a simulation' )
! Subprogram not used        call ncd_putatt(lnfid, ncd_global, 'comment', &
! Subprogram not used             "This entire file NOT needed for startup or branch simulations")
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used     ! Create global attributes. Attributes are used to store information
! Subprogram not used     ! about the data set. Global attributes are information about the
! Subprogram not used     ! data set as a whole, as opposed to a single variable
! Subprogram not used 
! Subprogram not used     call ncd_putatt(lnfid, ncd_global, 'Conventions', trim(conventions))
! Subprogram not used     call getdatetime(curdate, curtime)
! Subprogram not used     str = 'created on ' // curdate // ' ' // curtime
! Subprogram not used     call ncd_putatt(lnfid, ncd_global, 'history' , trim(str))
! Subprogram not used     call ncd_putatt(lnfid, ncd_global, 'source'  , trim(source))
! Subprogram not used     call ncd_putatt(lnfid, ncd_global, 'hostname', trim(hostname))
! Subprogram not used     call ncd_putatt(lnfid, ncd_global, 'username', trim(username))
! Subprogram not used     call ncd_putatt(lnfid, ncd_global, 'version' , trim(version))
! Subprogram not used 
! Subprogram not used     str = &
! Subprogram not used     '$Id: histFileMod.F90 36692 2012-04-27 18:39:55Z tcraig $'
! Subprogram not used     call ncd_putatt(lnfid, ncd_global, 'revision_id', trim(str))
! Subprogram not used     call ncd_putatt(lnfid, ncd_global, 'case_title', trim(ctitle))
! Subprogram not used     call ncd_putatt(lnfid, ncd_global, 'case_id', trim(caseid))
! Subprogram not used 
! Subprogram not used     str = get_filename(frivinp_rtm)
! Subprogram not used     call ncd_putatt(lnfid, ncd_global, 'RTM_input_dataset', trim(str))
! Subprogram not used 
! Subprogram not used     ! Define dimensions.
! Subprogram not used     ! Time is an unlimited dimension. Character string is treated as an array of characters.
! Subprogram not used 
! Subprogram not used     ! Global uncompressed dimensions (including non-land points)
! Subprogram not used     numrtm     = runoff%numr
! Subprogram not used     num_lndrof = runoff%numrl
! Subprogram not used     num_ocnrof = runoff%numro
! Subprogram not used     call ncd_defdim( lnfid, 'lon', rtmlon    , dimid)
! Subprogram not used     call ncd_defdim( lnfid, 'lat', rtmlat    , dimid)
! Subprogram not used     call ncd_defdim( lnfid, 'ocnrof', num_ocnrof, dimid)
! Subprogram not used     call ncd_defdim( lnfid, 'lndrof', num_lndrof, dimid)
! Subprogram not used     call ncd_defdim( lnfid, 'allrof', numrtm    , dimid)
! Subprogram not used 
! Subprogram not used     call ncd_defdim(lnfid, 'string_length', 8, strlen_dimid)
! Subprogram not used 
! Subprogram not used     if ( .not. lhistrest )then
! Subprogram not used        call ncd_defdim(lnfid, 'hist_interval', 2, hist_interval_dimid)
! Subprogram not used        call ncd_defdim(lnfid, 'time', ncd_unlimited, time_dimid)
! Subprogram not used        nfid(t) = lnfid
! Subprogram not used        if (masterproc)then
! Subprogram not used           write(iulog,*) trim(subname), &
! Subprogram not used                           ' : Successfully defined netcdf history file ',t
! Subprogram not used           call shr_sys_flush(iulog)
! Subprogram not used        end if
! Subprogram not used     else
! Subprogram not used        ncid_hist(t) = lnfid
! Subprogram not used        if (masterproc)then
! Subprogram not used           write(iulog,*) trim(subname), &
! Subprogram not used                           ' : Successfully defined netcdf restart history file ',t
! Subprogram not used           call shr_sys_flush(iulog)
! Subprogram not used        end if
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used   end subroutine htape_create

!-----------------------------------------------------------------------

! Subprogram not used   subroutine htape_timeconst(t, mode)
! Subprogram not used 
! Subprogram not used     ! !DESCRIPTION:
! Subprogram not used     ! Write time constant values to primary history tape.
! Subprogram not used 
! Subprogram not used     ! !ARGUMENTS:
! Subprogram not used     implicit none
! Subprogram not used     integer, intent(in) :: t              ! tape index
...
! Subprogram not used 
! Subprogram not used     ! !LOCAL VARIABLES:
! Subprogram not used     integer :: vid,n,i,j,m                ! indices
! Subprogram not used     integer :: nstep                      ! current step
! Subprogram not used     integer :: mcsec                      ! seconds of current date
! Subprogram not used     integer :: mdcur                      ! current day
! Subprogram not used     integer :: mscur                      ! seconds of current day
! Subprogram not used     integer :: mcdate                     ! current date
! Subprogram not used     integer :: yr,mon,day,nbsec           ! year,month,day,seconds components of a date
! Subprogram not used     integer :: hours,minutes,secs         ! hours,minutes,seconds of hh:mm:ss
! Subprogram not used     character(len= 10) :: basedate        ! base date (yyyymmdd)
! Subprogram not used     character(len=  8) :: basesec         ! base seconds
! Subprogram not used     character(len=  8) :: cdate           ! system date
! Subprogram not used     character(len=  8) :: ctime           ! system time
! Subprogram not used     real(r8):: time                       ! current time
! Subprogram not used     real(r8):: timedata(2)                ! time interval boundaries
! Subprogram not used     integer :: dim1id(1)                  ! netCDF dimension id
! Subprogram not used     integer :: dim2id(2)                  ! netCDF dimension id
! Subprogram not used     integer :: varid                      ! netCDF variable id
! Subprogram not used     type(Var_desc_t) :: vardesc           ! netCDF variable description
! Subprogram not used     character(len=max_chars) :: long_name ! variable long name
! Subprogram not used     character(len=max_namlen):: varname   ! variable name
! Subprogram not used     character(len=max_namlen):: units     ! variable units
! Subprogram not used     character(len=256):: str              ! global attribute string
! Subprogram not used     integer :: status
! Subprogram not used     logical, save :: writeTC = .true. ! true => write out time-constant data
! Subprogram not used     character(len=*),parameter :: subname = 'htape_timeconst'
! Subprogram not used     !--------------------------------------------------------
! Subprogram not used 
! Subprogram not used     ! For define mode -- only do this for first time-sample
! Subprogram not used     if (mode == 'define' .and. tape(t)%ntimes == 1) then
! Subprogram not used 
! Subprogram not used        call get_ref_date(yr, mon, day, nbsec)
! Subprogram not used        nstep = get_nstep()
! Subprogram not used        hours   = nbsec / 3600
! Subprogram not used        minutes = (nbsec - hours*3600) / 60
! Subprogram not used        secs    = (nbsec - hours*3600 - minutes*60)
! Subprogram not used        write(basedate,80) yr,mon,day
! Subprogram not used 80     format(i4.4,'-',i2.2,'-',i2.2)
! Subprogram not used        write(basesec ,90) hours, minutes, secs
! Subprogram not used 90     format(i2.2,':',i2.2,':',i2.2)
! Subprogram not used 
! Subprogram not used        dim1id(1) = time_dimid
! Subprogram not used        str = 'days since ' // basedate // " " // basesec
! Subprogram not used        call ncd_defvar(nfid(t), 'time', tape(t)%ncprec, 1, dim1id, varid, &
! Subprogram not used             long_name='time',units=str) 
! Subprogram not used        call ncd_putatt(nfid(t), varid, 'calendar', 'noleap')
! Subprogram not used        call ncd_putatt(nfid(t), varid, 'bounds', 'time_bounds')
! Subprogram not used 
! Subprogram not used        dim1id(1) = time_dimid
! Subprogram not used        call ncd_defvar(nfid(t) , 'mcdate', ncd_int, 1, dim1id , varid, &
! Subprogram not used           long_name = 'current date (YYYYMMDD)')
! Subprogram not used        call ncd_defvar(nfid(t) , 'mcsec' , ncd_int, 1, dim1id , varid, &
! Subprogram not used           long_name = 'current seconds of current date', units='s')
! Subprogram not used        call ncd_defvar(nfid(t) , 'mdcur' , ncd_int, 1, dim1id , varid, &
! Subprogram not used           long_name = 'current day (from base day)')
! Subprogram not used        call ncd_defvar(nfid(t) , 'mscur' , ncd_int, 1, dim1id , varid, &
! Subprogram not used           long_name = 'current seconds of current day')
! Subprogram not used        call ncd_defvar(nfid(t) , 'nstep' , ncd_int, 1, dim1id , varid, &
! Subprogram not used           long_name = 'time step')
! Subprogram not used 
! Subprogram not used        dim2id(1) = hist_interval_dimid;  dim2id(2) = time_dimid
! Subprogram not used        call ncd_defvar(nfid(t), 'time_bounds', ncd_double, 2, dim2id, varid, &
! Subprogram not used           long_name = 'history time interval endpoints')
! Subprogram not used 
! Subprogram not used        dim2id(1) = strlen_dimid;  dim2id(2) = time_dimid
! Subprogram not used        call ncd_defvar(nfid(t), 'date_written', ncd_char, 2, dim2id, varid)
! Subprogram not used        call ncd_defvar(nfid(t), 'time_written', ncd_char, 2, dim2id, varid)
! Subprogram not used 
! Subprogram not used        call ncd_defvar(varname='lon', xtype=tape(t)%ncprec, dim1name='lon', &
! Subprogram not used             long_name='runoff coordinate longitude', units='degrees_east', ncid=nfid(t))
! Subprogram not used        call ncd_defvar(varname='lat', xtype=tape(t)%ncprec, dim1name='lat', &
! Subprogram not used             long_name='runoff coordinate latitude', units='degrees_north', ncid=nfid(t))
! Subprogram not used 
! Subprogram not used        if(writeTC .and. t == 1) then 
! Subprogram not used           call ncd_defvar(varname='fthresh', xtype=tape(t)%ncprec, dim1name='lon', &
! Subprogram not used                dim2name='lat', long_name='flooding threshold', &
! Subprogram not used                missing_value=spval, fill_value=spval, units='m3', ncid=nfid(t))
! Subprogram not used        endif
! Subprogram not used     else if (mode == 'write') then
! Subprogram not used 
! Subprogram not used        call get_curr_time (mdcur, mscur)
! Subprogram not used        call get_curr_date (yr, mon, day, mcsec)
! Subprogram not used        mcdate = yr*10000 + mon*100 + day
! Subprogram not used        nstep = get_nstep()
! Subprogram not used 
! Subprogram not used        call ncd_io('mcdate', mcdate, 'write', nfid(t), nt=tape(t)%ntimes)
! Subprogram not used        call ncd_io('mcsec' , mcsec , 'write', nfid(t), nt=tape(t)%ntimes)
! Subprogram not used        call ncd_io('mdcur' , mdcur , 'write', nfid(t), nt=tape(t)%ntimes)
! Subprogram not used        call ncd_io('mscur' , mscur , 'write', nfid(t), nt=tape(t)%ntimes)
! Subprogram not used        call ncd_io('nstep' , nstep , 'write', nfid(t), nt=tape(t)%ntimes)
! Subprogram not used 
! Subprogram not used        time = mdcur + mscur/secspday
! Subprogram not used        call ncd_io('time'  , time  , 'write', nfid(t), nt=tape(t)%ntimes)
! Subprogram not used 
! Subprogram not used        timedata(1) = tape(t)%begtime
! Subprogram not used        timedata(2) = time
! Subprogram not used        call ncd_io('time_bounds', timedata, 'write', nfid(t), nt=tape(t)%ntimes)
! Subprogram not used 
! Subprogram not used        call getdatetime (cdate, ctime)
! Subprogram not used        call ncd_io('date_written', cdate, 'write', nfid(t), nt=tape(t)%ntimes)
! Subprogram not used 
! Subprogram not used        call ncd_io('time_written', ctime, 'write', nfid(t), nt=tape(t)%ntimes)
! Subprogram not used 
! Subprogram not used        call ncd_io(varname='lon', data=runoff%rlon, ncid=nfid(t), flag='write')
! Subprogram not used        call ncd_io(varname='lat', data=runoff%rlat, ncid=nfid(t), flag='write')
! Subprogram not used 
! Subprogram not used        if(writeTC .and. t == 1 .and. tape(t)%ntimes == 1 ) then 
! Subprogram not used           call ncd_io(varname='fthresh', data=runoff%fthresh, ncid=nfid(t), flag='write' &
! Subprogram not used                ,dim1name='allrof')
! Subprogram not used           writeTC = .false.
! Subprogram not used        endif
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used   end subroutine htape_timeconst
...
! Subprogram not used   character(len=256) function set_hist_filename (hist_freq, rtmhist_mfilt, hist_file)
! Subprogram not used 
! Subprogram not used     ! Determine history dataset filenames.
! Subprogram not used     
! Subprogram not used     ! !ARGUMENTS:
! Subprogram not used     implicit none
! Subprogram not used     integer, intent(in)  :: hist_freq   !history file frequency
! Subprogram not used     integer, intent(in)  :: rtmhist_mfilt  !history file number of time-samples
! Subprogram not used     integer, intent(in)  :: hist_file   !history file index
! Subprogram not used     
! Subprogram not used     ! !LOCAL VARIABLES:
! Subprogram not used     character(len=256) :: cdate       !date char string
! Subprogram not used     character(len=  1) :: hist_index  !p,1 or 2 (currently)
! Subprogram not used     integer :: day                    !day (1 -> 31)
! Subprogram not used     integer :: mon                    !month (1 -> 12)
! Subprogram not used     integer :: yr                     !year (0 -> ...)
! Subprogram not used     integer :: sec                    !seconds into current day
! Subprogram not used     character(len=*),parameter :: subname = 'set_hist_filename'
! Subprogram not used     
! Subprogram not used     if (hist_freq == 0 .and. rtmhist_mfilt == 1) then   !monthly
! Subprogram not used        call get_prev_date (yr, mon, day, sec)
! Subprogram not used        write(cdate,'(i4.4,"-",i2.2)') yr,mon
! Subprogram not used     else                        !other
! Subprogram not used        call get_curr_date (yr, mon, day, sec)
! Subprogram not used        write(cdate,'(i4.4,"-",i2.2,"-",i2.2,"-",i5.5)') yr,mon,day,sec
! Subprogram not used     endif
! Subprogram not used     write(hist_index,'(i1.1)') hist_file - 1
! Subprogram not used     set_hist_filename = "./"//trim(caseid)//".rtm"//trim(inst_suffix)//&
! Subprogram not used                         ".h"//hist_index//"."//trim(cdate)//".nc"
! Subprogram not used 
! Subprogram not used   end function set_hist_filename
