...
class Function(object):
    ...
    def __init__(self, name, location, mgraph):
        ...
        if self.name in mgraph.functions:
            isFunc = True
            isIface, isIntrinsic, defArgs, out, loc = getFuncAttrs(
                self.name, location["subprogs"], mgraph
            )

        else:
            for idx, _ in enumerate(location["subprogs"]):
                keyLoc = tuple([self.name]) + tuple(
                    location["subprogs"][::-1][idx:][::-1]
                )
                if keyLoc in mgraph.useMaps:
                    target = mgraph.useMaps[keyLoc]
                    isIface, isIntrinsic, defArgs, out, loc = getFuncAttrs(
                        target, location["subprogs"], mgraph
                    )
                    isFunc = True
                    useMap = True
                    fTarget = target
...
class ParenTree(object):
    ...
    def __init__(self, string, location, functionStruct):
        ...
        for lvl in parenthetic_contents(self.string):
            node = Node(
                depth=lvl[0], string=lvl[1], order=order, location=self.location
            )
                    ...
                    for defArg, inputArg in zip(node.parentFunction.defArgs, newArgs):
                        for subname in inputArg:
                            if subname in node.newNames:
                                ...
                                dargattrs = {
                                    "location": node.parentFunction.location,
                                    "cname": defArg,
                                    "lines": lines,
                                }
                                snname = subname + "__" + node.location["subprogs"][-1]
                                snattrs = {
                                    "location": node.location["subprogs"],
                                    "cname": subname,
                                    "lines": node.location["lines"],
                                }
                        ...
                        for defArg, inputArg in zip(defArgSet, newArgs):
                            for subname in inputArg:
                                if subname in node.newNames:
                                    ...
                                    if (
                                        node.parentFunction.name
                                        in self.mgraph.functions
                                    ):
                                        for child in self.mgraph.functions[
                                            node.parentFunction.name
                                        ]["functions"]:
                                            if (
                                                defArg
                                                in self.mgraph.functions[
                                                    node.parentFunction.name
                                                ]["functions"][child]["vars"]
                                            ):
                                                dargname = defArg + "__" + child
                                                lines = self.mgraph.node[dargname][
                                                    "attrs"
                                                ]["lines"]
                                                dargattrs = {
                                                    "location": self.mgraph.functions[
                                                        node.parentFunction.name
                                                    ]["functions"][child]["location"],
                                                    "cname": defArg,
                                                    "lines": lines,
                                                }
                                                break
                                        ...
                                        node.parentFunction.name
                                        in self.mgraph.subroutines
                                    ):
                                        b = False
                                        for child in self.mgraph.subroutines[
                                            node.parentFunction.name
                                        ]["subroutines"]:
                                            for arg in self.mgraph.subroutines[
                                                node.parentFunction.name
                                            ]["subroutines"][child]["args"]:
                                                if defArg == arg[0]:
                                                    dargname = defArg + "__" + child
                                                    lines = self.mgraph.node[dargname][
                                                        "attrs"
                                                    ...
                                                    dargattrs = {
                                                        "location": self.mgraph.subroutines[
                                                            node.parentFunction.name
                                                        ][
                                                            "subroutines"
                                                        ][
                                        ...
                                        dargname = (
                                            defArg + "__" + node.parentFunction.name
                                        )
                                        dargattrs = {
                                            "location": node.parentFunction.location,
                                            "cname": defArg,
                                    ...
                                    snname = (
                                        subname + "__" + node.location["subprogs"][-1]
                                    )
                                    snattrs = {
                                        "location": node.location["subprogs"],
                                        "cname": subname,
                ...
                for arg in newArgs:
                    for sn in arg:
                        if sn in node.newNames:
                            # print darg, sn
                            funname = (
                                node.parentFunction.output
                                + "__"
                                + str(node.location["lines"]["line"])
                                + "__"
                                + node.location["subprogs"][-1]
                            )
                            funattrs = {
                                "location": node.location["subprogs"],
                                "cname": node.parentFunction.output,
                                "lines": node.location["lines"],
                            }
        ...
        for lvl in reversed(xrange(1, self.argTree.depth)):
            for node in self.argTree.levels[lvl]:
                if node.parentFunction == "":
                    ...
                    node.parent.inFromBelow = node.parent.inFromBelow.replace(
                        "(" + node.strNoSpace + ")", node.output, 1
                    )
                    ...
                    # now rewrite string
                    node.parent.inFromBelow = node.parent.inFromBelow.replace(
                        node.parentFunction.name + "(" + node.strNoSpace + ")",
                        node.parentFunction.output,
                        1,
                    )
                    # and add edge
...
def stmt_to_fnnames(instmt):
    ...
    if isinstance(instmt, str):
        ...
        if not instmt:
            ...
            if derivedTypes:
                ...
                for dt, dtn in zip(derivedTypes, dtnew):
                    ...
                    string = string.replace(
                        dtnospace, "REMOVETHISSTRING" + "_" + str(num) + "_", 1
                    )
                    dtmap[num] = dtn
                    num += 1
            ...
            traverse(
                stmt, get_names, zerofunc, fparser.Fortran2003.Structure_Constructor
            )
...
def connectOutput(
    functionTree, location, mgraph, targetname=None, targetattrs=None, isOut=True
):
    ...
    if functionTree.root.isFunc:
        ...
        if isOut:
            mgraph.addEdge(
                ...
                attrs1=outattrs,
                attrs2=targetattrs,
                edgeLine=targetattrs["lines"]["line"],
            )
...
def processSubArgs(mgraph, carg, defarg, location, subLoc):
    ...
    if "=" in carg[0]:
        ...
        if defarg[1] == "out":
            ...
            if functions:
                deepStmts(
                    mgraph, carg[0], location, defargName, defargAttrs, isOut=False
                )
