...
WRITE(*,'(4A)') 'NAME: ', trim(fields(i)%fldnam)//'_D', ' FILE: prescribed_aero.F90',  ' LINE: call outfld( trim(fields(i)%fldnam)//_D, outdata(:ncol,:), ncol, state(c)%lchnk )' 
...
WRITE(*,'(4A)') 'NAME: ', trim(spec_a)//'_D', ' FILE: prescribed_aero.F90',  ' LINE: call outfld( trim(spec_a)//_D, outdata(:ncol,:), ncol, state(c)%lchnk )' 
             cld_borne_aero = .TRUE.
          else
             call pbuf_get_field(pbuf_chnk, fields(i)%pbuf_ndx, outdata)
             call outfld( trim(fields(i)%fldnam)//'_D', outdata(:ncol,:), ncol, state(c)%lchnk )
