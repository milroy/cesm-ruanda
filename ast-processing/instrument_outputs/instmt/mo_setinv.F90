...
WRITE(*,'(4A)') 'NAME: ', trim(inv_lst(i))//'_dens', ' FILE: mo_setinv.F90',  ' LINE: call outfld( trim(inv_lst(i))//_dens, tmp_out(:ncol,:), ncol, lchnk )' 
      tmp_out(:ncol,:) =  invariants(:ncol,:,i) / invariants(:ncol,:,m_ndx)
      call outfld( trim(inv_lst(i))//'_vmr',  tmp_out(:ncol,:), ncol, lchnk )
WRITE(*,'(4A)') 'NAME: ', trim(inv_lst(i))//'_vmr', ' FILE: mo_setinv.F90',  ' LINE: call outfld( trim(inv_lst(i))//_vmr,  tmp_out(:ncol,:), ncol, lchnk )' 
    enddo
