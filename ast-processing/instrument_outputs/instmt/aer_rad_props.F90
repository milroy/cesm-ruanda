...
WRITE(*,'(4A)') 'NAME: ', odv_names(iaer), ' FILE: aer_rad_props.F90',  ' LINE: call outfld(odv_names(iaer), tmp, pcols, lchnk)' 
   else
      call outfld('AEROD_v', tmp, pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'AEROD_v', ' FILE: aer_rad_props.F90',  ' LINE: call outfld(AEROD_v, tmp, pcols, lchnk)' 
   end if
