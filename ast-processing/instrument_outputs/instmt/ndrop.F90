...
WRITE(*,'(4A)') 'NAME: ', 'NDROPCOL', ' FILE: ndrop.F90',  ' LINE: call outfld(NDROPCOL, ndropcol, pcols, lchnk)' 
   call outfld('NDROPSRC', nsource,  pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'NDROPSRC', ' FILE: ndrop.F90',  ' LINE: call outfld(NDROPSRC, nsource,  pcols, lchnk)' 
   call outfld('NDROPMIX', ndropmix, pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'NDROPMIX', ' FILE: ndrop.F90',  ' LINE: call outfld(NDROPMIX, ndropmix, pcols, lchnk)' 
   call outfld('WTKE    ', wtke,     pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'WTKE    ', ' FILE: ndrop.F90',  ' LINE: call outfld(WTKE    , wtke,     pcols, lchnk)' 

   call ccncalc(state, pbuf, cs, ccn)
   do l = 1, psat
      call outfld(ccn_name(l), ccn(1,1,l), pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', ccn_name(l), ' FILE: ndrop.F90',  ' LINE: call outfld(ccn_name(l), ccn(1,1,l), pcols, lchnk)' 
   enddo
...
WRITE(*,'(4A)') 'NAME: ', fieldname(mm), ' FILE: ndrop.F90',  ' LINE: call outfld(fieldname(mm),    coltend(:,mm),    pcols, lchnk)' 
            call outfld(fieldname_cw(mm), coltend_cw(:,mm), pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', fieldname_cw(mm), ' FILE: ndrop.F90',  ' LINE: call outfld(fieldname_cw(mm), coltend_cw(:,mm), pcols, lchnk)' 
         end do
      end do
   end if
