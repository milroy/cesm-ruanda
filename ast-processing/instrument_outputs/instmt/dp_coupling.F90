...
WRITE(*,'(4A)') 'NAME: ', 'T&IC', ' FILE: dp_coupling.F90',  ' LINE: call outfld(T&IC,phys_state(lchnk)%t,pcols,lchnk)' 
         call outfld('U&IC',phys_state(lchnk)%u,pcols,lchnk)
WRITE(*,'(4A)') 'NAME: ', 'U&IC', ' FILE: dp_coupling.F90',  ' LINE: call outfld(U&IC,phys_state(lchnk)%u,pcols,lchnk)' 
         call outfld('V&IC',phys_state(lchnk)%v,pcols,lchnk)
WRITE(*,'(4A)') 'NAME: ', 'V&IC', ' FILE: dp_coupling.F90',  ' LINE: call outfld(V&IC,phys_state(lchnk)%v,pcols,lchnk)' 
         call outfld('PS&IC',phys_state(lchnk)%ps,pcols,lchnk)
WRITE(*,'(4A)') 'NAME: ', 'PS&IC', ' FILE: dp_coupling.F90',  ' LINE: call outfld(PS&IC,phys_state(lchnk)%ps,pcols,lchnk)' 
         do m=1,pcnst
            call outfld(trim(cnst_name(m))//'&IC',phys_state(lchnk)%q(1,1,m), pcols,lchnk)
WRITE(*,'(4A)') 'NAME: ', trim(cnst_name(m))//'&IC', ' FILE: dp_coupling.F90',  ' LINE: call outfld(trim(cnst_name(m))//&IC,phys_state(lchnk)%q(1,1,m), pcols,lchnk)' 
         end do
      end do
   endif
