...
WRITE(*,'(4A)') 'NAME: ', 'CAPE', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(CAPE, cape, pcols, lchnk)        ! RBN - CAPE output' 
!
...
WRITE(*,'(4A)') 'NAME: ', 'FREQZM  ', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(FREQZM  ,freqzm          ,pcols   ,lchnk   )' 
!
...
WRITE(*,'(4A)') 'NAME: ', 'ZMMU', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMMU, mu_out(1,1), pcols, lchnk)' 
   call outfld('ZMMD', md_out(1,1), pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'ZMMD', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMMD, md_out(1,1), pcols, lchnk)' 

   ftem(:ncol,:pver) = ptend_loc%s(:ncol,:pver)/cpair
   call outfld('ZMDT    ',ftem           ,pcols   ,lchnk   )
WRITE(*,'(4A)') 'NAME: ', 'ZMDT    ', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMDT    ,ftem           ,pcols   ,lchnk   )' 
   call outfld('ZMDQ    ',ptend_loc%q(1,1,1) ,pcols   ,lchnk   )
WRITE(*,'(4A)') 'NAME: ', 'ZMDQ    ', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMDQ    ,ptend_loc%q(1,1,1) ,pcols   ,lchnk   )' 
...
WRITE(*,'(4A)') 'NAME: ', 'PCONVT  ', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(PCONVT  ,pcont          ,pcols   ,lchnk   )' 
    call outfld('PCONVB  ',pconb          ,pcols   ,lchnk   )
WRITE(*,'(4A)') 'NAME: ', 'PCONVB  ', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(PCONVB  ,pconb          ,pcols   ,lchnk   )' 
...
WRITE(*,'(4A)') 'NAME: ', 'EVAPTZM ', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(EVAPTZM ,ftem           ,pcols   ,lchnk   )' 
   ftem(:ncol,:pver) = tend_s_snwprd  (:ncol,:pver)/cpair
   call outfld('FZSNTZM ',ftem           ,pcols   ,lchnk   )
WRITE(*,'(4A)') 'NAME: ', 'FZSNTZM ', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(FZSNTZM ,ftem           ,pcols   ,lchnk   )' 
   ftem(:ncol,:pver) = tend_s_snwevmlt(:ncol,:pver)/cpair
   call outfld('EVSNTZM ',ftem           ,pcols   ,lchnk   )
WRITE(*,'(4A)') 'NAME: ', 'EVSNTZM ', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(EVSNTZM ,ftem           ,pcols   ,lchnk   )' 
   call outfld('EVAPQZM ',ptend_loc%q(1,1,1) ,pcols   ,lchnk   )
WRITE(*,'(4A)') 'NAME: ', 'EVAPQZM ', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(EVAPQZM ,ptend_loc%q(1,1,1) ,pcols   ,lchnk   )' 
   call outfld('ZMFLXPRC', flxprec, pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'ZMFLXPRC', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMFLXPRC, flxprec, pcols, lchnk)' 
   call outfld('ZMFLXSNW', flxsnow, pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'ZMFLXSNW', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMFLXSNW, flxsnow, pcols, lchnk)' 
   call outfld('ZMNTPRPD', ntprprd, pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'ZMNTPRPD', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMNTPRPD, ntprprd, pcols, lchnk)' 
   call outfld('ZMNTSNPD', ntsnprd, pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'ZMNTSNPD', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMNTSNPD, ntsnprd, pcols, lchnk)' 
   call outfld('ZMEIHEAT', ptend_loc%s, pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'ZMEIHEAT', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMEIHEAT, ptend_loc%s, pcols, lchnk)' 
   call outfld('CMFMCDZM   ',mcon ,  pcols   ,lchnk   )
WRITE(*,'(4A)') 'NAME: ', 'CMFMCDZM   ', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(CMFMCDZM   ,mcon ,  pcols   ,lchnk   )' 
   call outfld('PRECCDZM   ',prec,  pcols   ,lchnk   )
WRITE(*,'(4A)') 'NAME: ', 'PRECCDZM   ', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(PRECCDZM   ,prec,  pcols   ,lchnk   )' 


   call t_stopf ('zm_conv_evap')

   call outfld('PRECZ   ', prec   , pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'PRECZ   ', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(PRECZ   , prec   , pcols, lchnk)' 
...
WRITE(*,'(4A)') 'NAME: ', 'ZMMTT', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMMTT, ftem             , pcols, lchnk)' 
     call outfld('ZMMTU', wind_tends(1,1,1), pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'ZMMTU', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMMTU, wind_tends(1,1,1), pcols, lchnk)' 
     call outfld('ZMMTV', wind_tends(1,1,2), pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'ZMMTV', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMMTV, wind_tends(1,1,2), pcols, lchnk)' 
   
     ! Output apparent force from  pressure gradient
     call outfld('ZMUPGU', pguall(1,1,1), pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'ZMUPGU', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMUPGU, pguall(1,1,1), pcols, lchnk)' 
     call outfld('ZMUPGD', pgdall(1,1,1), pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'ZMUPGD', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMUPGD, pgdall(1,1,1), pcols, lchnk)' 
     call outfld('ZMVPGU', pguall(1,1,2), pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'ZMVPGU', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMVPGU, pguall(1,1,2), pcols, lchnk)' 
     call outfld('ZMVPGD', pgdall(1,1,2), pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'ZMVPGD', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMVPGD, pgdall(1,1,2), pcols, lchnk)' 

     ! Output in-cloud winds
     call outfld('ZMICUU', icwu(1,1,1), pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'ZMICUU', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMICUU, icwu(1,1,1), pcols, lchnk)' 
     call outfld('ZMICUD', icwd(1,1,1), pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'ZMICUD', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMICUD, icwd(1,1,1), pcols, lchnk)' 
     call outfld('ZMICVU', icwu(1,1,2), pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'ZMICVU', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMICVU, icwu(1,1,2), pcols, lchnk)' 
     call outfld('ZMICVD', icwd(1,1,2), pcols, lchnk)
WRITE(*,'(4A)') 'NAME: ', 'ZMICVD', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMICVD, icwd(1,1,2), pcols, lchnk)' 
...
WRITE(*,'(4A)') 'NAME: ', 'ZMDICE ', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMDICE ,ptend_loc%q(1,1,ixcldice) ,pcols   ,lchnk   )' 
   call outfld('ZMDLIQ ',ptend_loc%q(1,1,ixcldliq) ,pcols   ,lchnk   )
WRITE(*,'(4A)') 'NAME: ', 'ZMDLIQ ', ' FILE: zm_conv_intr.F90',  ' LINE: call outfld(ZMDLIQ ,ptend_loc%q(1,1,ixcldliq) ,pcols   ,lchnk   )' 
