...
WRITE(*,'(4A)') 'NAME: ', trim(solsym(l))//'_CHMP', ' FILE: mo_exp_sol.F90',  ' LINE: call outfld( trim(solsym(l))//_CHMP, wrk(:,:), ncol, lchnk )' 
       wrk(:,:) = (loss(:,:,m))*xhnm
       call outfld( trim(solsym(l))//'_CHML', wrk(:,:), ncol, lchnk )
WRITE(*,'(4A)') 'NAME: ', trim(solsym(l))//'_CHML', ' FILE: mo_exp_sol.F90',  ' LINE: call outfld( trim(solsym(l))//_CHML, wrk(:,:), ncol, lchnk )' 
