...
!         if (lphase == 1) then
  ...
  !-------------------------------------------------------------------------
! Subprogram not used   subroutine aero_model_surfarea( &
! Subprogram not used                   mmr, radmean, relhum, pmid, temp, strato_sad, &
! Subprogram not used                   sulfate, rho, ltrop, het1_ndx, pbuf, ncol, sfc, dm_aer, sad_total )
! Subprogram not used 
! Subprogram not used     use modal_aero_data, only : sigmag_amode
! Subprogram not used     use mo_constants,    only : pi
! Subprogram not used     use modal_aero_data, only : nspec_amode
! Subprogram not used     use modal_aero_data, only : alnsg_amode
! Subprogram not used 
! Subprogram not used     ! dummy args
! Subprogram not used     real(r8), intent(in)    :: pmid(:,:)
! Subprogram not used     real(r8), intent(in)    :: temp(:,:)
! Subprogram not used     real(r8), intent(in)    :: mmr(:,:,:)
! Subprogram not used     real(r8), intent(in)    :: radmean      ! mean radii in cm
! Subprogram not used     real(r8), intent(in)    :: strato_sad(:,:)
! Subprogram not used     integer,  intent(in)    :: ncol
! Subprogram not used     integer,  intent(in)    :: ltrop(:)
! Subprogram not used     integer,  intent(in)    :: het1_ndx
! Subprogram not used     real(r8), intent(in)    :: relhum(:,:)
! Subprogram not used     real(r8), intent(in)    :: rho(:,:) ! total atm density (/cm^3)
! Subprogram not used     real(r8), intent(in)    :: sulfate(:,:)
! Subprogram not used     type(physics_buffer_desc), pointer :: pbuf(:)
! Subprogram not used 
! Subprogram not used     real(r8), intent(inout) :: sfc(:,:,:)
! Subprogram not used     real(r8), intent(inout) :: dm_aer(:,:,:)
! Subprogram not used     real(r8), intent(inout) :: sad_total(:,:)
! Subprogram not used 
! Subprogram not used     ! local vars
! Subprogram not used 
! Subprogram not used     real(r8), target :: sad_mode(pcols,pver,ntot_amode)
! Subprogram not used     real(r8) :: rho_air
! Subprogram not used     real(r8), pointer, dimension(:,:,:) :: dgnumwet
! Subprogram not used     integer :: l
! Subprogram not used     integer :: i,k,m 
! Subprogram not used 
! Subprogram not used     real(r8) :: chm_mass,tot_mass
! Subprogram not used 
! Subprogram not used     call pbuf_get_field(pbuf, dgnumwet_idx, dgnumwet )
! Subprogram not used 
! Subprogram not used     !
! Subprogram not used     ! compute surface aero for each mode; however, at this point we only use Aitken mode (mode 2 in MAM3; how
! Subprogram not used     ! can we move from hard-wiring this?) as the surface area for chemical reactions.
! Subprogram not used     !
! Subprogram not used     sad_mode = 0._r8
! Subprogram not used     sad_total = 0._r8
! Subprogram not used     dm_aer = 0._r8
! Subprogram not used     do i = 1,ncol
! Subprogram not used        do k = ltrop(i),pver
! Subprogram not used           rho_air = pmid(i,k)/(temp(i,k)*287.04_r8)
! Subprogram not used           do l=1,ntot_amode
! Subprogram not used              !
! Subprogram not used              ! compute a mass weighting of the number
! Subprogram not used              !
! Subprogram not used              tot_mass = 0._r8
! Subprogram not used              chm_mass = 0._r8
! Subprogram not used              do m=1,nspec_amode(l)
! Subprogram not used                if ( index_tot_mass(l,m) > 0 ) &
! Subprogram not used                     tot_mass = tot_mass + mmr(i,k,index_tot_mass(l,m))
! Subprogram not used                if ( index_chm_mass(l,m) > 0 ) &
! Subprogram not used                     chm_mass = chm_mass + mmr(i,k,index_chm_mass(l,m))
! Subprogram not used              end do
! Subprogram not used              if ( tot_mass > 0._r8 ) then
! Subprogram not used                sad_mode(i,k,l) = chm_mass/tot_mass * &
! Subprogram not used                     mmr(i,k,num_idx(l))*rho_air*pi*dgnumwet(i,k,l)**2*&
! Subprogram not used                     exp(2*alnsg_amode(l)**2)  ! m^2/m^3
! Subprogram not used                sad_mode(i,k,l) = 1.e-2_r8 * sad_mode(i,k,l) ! cm^2/cm^3
! Subprogram not used              else
! Subprogram not used                sad_mode(i,k,l) = 0._r8
! Subprogram not used              end if
! Subprogram not used           end do
! Subprogram not used           sad_total(i,k) = sum(sad_mode(i,k,:))
! Subprogram not used           dm_aer(i,k,:) = dgnumwet(i,k,:) * 1.e2_r8 ! convert m to cm
! Subprogram not used 
! Subprogram not used        enddo
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used     sfc(:,:,:) = sad_mode(:,:,:) 
! Subprogram not used 
! Subprogram not used   end subroutine aero_model_surfarea
