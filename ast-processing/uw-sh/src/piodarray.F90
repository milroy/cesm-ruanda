...
contains
  ...
  subroutine pio_set_buffer_size_limit(limit)
    ...
    if(limit<0) then
       call piodie('piodarray',118,' bad value to buffer_size_limit',limit)
    end if
...
!<  
  subroutine write_darray_1d_real (File,varDesc,ioDesc, array, iostat, fillval)
    ...
    if(ios%async_interface .and. .not. ios%ioproc) then
       ...
       if(debugasync) print *,'piodarray',172, iodesc%async_id
       ...
       if(debugasync) print *,'piodarray',181, MPI_REAL4
       ...
       if(debugasync) print *,'piodarray',191
    endif

    if(debugasync .and. ios%ioproc) print *,"/glade/u/home/milroy/cesm/cesm1_3_beta11/models/utils/pio/piodarray.F90.in",194,iodesc%async_id
...
!<  
  subroutine write_darray_1d_int (File,varDesc,ioDesc, array, iostat, fillval)
    ...
    if(ios%async_interface .and. .not. ios%ioproc) then
       ...
       itype = MPI_INTEGER  
       call mpi_bcast(itype, 1, mpi_integer, ios%compmaster, ios%intercomm, ierr)
       if(debugasync) print *,'piodarray',181, MPI_INTEGER
...
!<  
  subroutine write_darray_1d_double (File,varDesc,ioDesc, array, iostat, fillval)
    ...
    if(ios%async_interface .and. .not. ios%ioproc) then
       ...
       itype = MPI_REAL8  
       call mpi_bcast(itype, 1, mpi_integer, ios%compmaster, ios%intercomm, ierr)
       if(debugasync) print *,'piodarray',181, MPI_REAL8
...
!<
  subroutine read_darray_1d_real (File,varDesc, ioDesc, array, iostat)
    ...
    if(ios%async_interface .and. .not. ios%ioproc) then
       ...
       if(DebugAsync) print *,'piodarray',330
       if(ios%comp_rank==0) call mpi_send(msg, 1, mpi_integer, ios%ioroot, 1, ios%union_comm, ierr)
       call mpi_bcast(file%fh, 1, mpi_integer, ios%compmaster, ios%intercomm, ierr)
       call mpi_bcast(vardesc%varid, 1, mpi_integer, ios%compmaster, ios%intercomm, ierr)
       call mpi_bcast(vardesc%rec, 1, mpi_integer, ios%compmaster, ios%intercomm, ierr)
       ...
       itype = MPI_REAL4
       call mpi_bcast(itype, 1, mpi_integer, ios%compmaster, ios%intercomm, ierr)
       if(DebugAsync) print *,'piodarray',338, MPI_REAL4       
    endif
...
!<
  subroutine read_darray_1d_int (File,varDesc, ioDesc, array, iostat)
    ...
    if(ios%async_interface .and. .not. ios%ioproc) then
       ...
       itype = MPI_INTEGER
       call mpi_bcast(itype, 1, mpi_integer, ios%compmaster, ios%intercomm, ierr)
       if(DebugAsync) print *,'piodarray',338, MPI_INTEGER       
    endif
...
!<
  subroutine read_darray_1d_double (File,varDesc, ioDesc, array, iostat)
    ...
    if(ios%async_interface .and. .not. ios%ioproc) then
       ...
       itype = MPI_REAL8
       call mpi_bcast(itype, 1, mpi_integer, ios%compmaster, ios%intercomm, ierr)
       if(DebugAsync) print *,'piodarray',338, MPI_REAL8       
    endif
...
!<
  subroutine write_darray_nf_real (File,varDesc,ioDesc,array, iostat, fillval)
    ...
    if(Debug) print *,'piodarray',480,' NAME : IAM: ', &
    ...
    if (IOproc) then
       ...
       if(vardesc%rec>=0 .and. iodesc%start(1)>=0) then
          ...
          count(ndims:ndims)=1
          if(Debug) print *, 'piodarray',575,'start:',start, &
          ...
          if(Debug) print *, 'piodarray',583,'start:',start,' count:',count
          ...
          start=iodesc%start
          count=iodesc%count
          if(Debug) print *, 'piodarray',592,'start:',start,' count:',count,' ndims:',ndims
       end if
...
!<  
  subroutine write_darray_bin_real(File,varDesc,ioDesc,array, iostat, fillval)
    ...
    ! -------------------------------------------------
    if(Debug) print *, 'piodarray',714,iodesc%Write%elemTYPE, iodesc%Write%fileTYPE
    len        = iodesc%IOmap%length
...
!    if(debug) 
  ...
  subroutine darray_write_complete(File)
    ...
    if(associated(file%data_list_top)) then
       ...
       if(DEBUG) print *,"/glade/u/home/milroy/cesm/cesm1_3_beta11/models/utils/pio/piodarray.F90.in",1253,status, ierr, total_buffsize
