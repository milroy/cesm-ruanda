...
! Subprogram not used subroutine get_hygro_rad_props(ncol, krh, wrh, mass, ext, ssa, asm, &
! Subprogram not used                                tau, tau_w, tau_w_g, tau_w_f)
! Subprogram not used 
! Subprogram not used    ! Arguments
! Subprogram not used    integer,  intent(in) :: ncol
! Subprogram not used    integer,  intent(in) :: krh(pcols,pver)  ! index for linear interpolation of optics on rh
! Subprogram not used    real(r8), intent(in) :: wrh(pcols,pver)  ! weight for linear interpolation of optics on rh
! Subprogram not used    real(r8), intent(in) :: mass(pcols,pver)
...
! Subprogram not used 
! Subprogram not used    real(r8), intent(out) :: tau    (pcols,pver,nswbands)
! Subprogram not used    real(r8), intent(out) :: tau_w  (pcols,pver,nswbands)
! Subprogram not used    real(r8), intent(out) :: tau_w_g(pcols,pver,nswbands)
! Subprogram not used    real(r8), intent(out) :: tau_w_f(pcols,pver,nswbands)
! Subprogram not used 
! Subprogram not used    ! Local variables
! Subprogram not used    real(r8) :: ext1, ssa1, asm1
! Subprogram not used    integer :: icol, ilev, iswband
! Subprogram not used    !-----------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used    do iswband = 1, nswbands
! Subprogram not used       do icol = 1, ncol
! Subprogram not used          do ilev = 1, pver
! Subprogram not used             ext1 = (1 + wrh(icol,ilev)) * ext(krh(icol,ilev)+1,iswband) &
! Subprogram not used                       - wrh(icol,ilev)  * ext(krh(icol,ilev),  iswband)
! Subprogram not used             ssa1 = (1 + wrh(icol,ilev)) * ssa(krh(icol,ilev)+1,iswband) &
! Subprogram not used                       - wrh(icol,ilev)  * ssa(krh(icol,ilev),  iswband)
! Subprogram not used             asm1 = (1 + wrh(icol,ilev)) * asm(krh(icol,ilev)+1,iswband) &
! Subprogram not used                       - wrh(icol,ilev)  * asm(krh(icol,ilev),  iswband)
! Subprogram not used   
! Subprogram not used             tau    (icol, ilev, iswband) = mass(icol, ilev) * ext1
! Subprogram not used             tau_w  (icol, ilev, iswband) = mass(icol, ilev) * ext1 * ssa1
! Subprogram not used             tau_w_g(icol, ilev, iswband) = mass(icol, ilev) * ext1 * ssa1 * asm1
! Subprogram not used             tau_w_f(icol, ilev, iswband) = mass(icol, ilev) * ext1 * ssa1 * asm1 * asm1
! Subprogram not used          enddo
! Subprogram not used       enddo
! Subprogram not used    enddo
! Subprogram not used 
! Subprogram not used end subroutine get_hygro_rad_props 

!==============================================================================
    
! Subprogram not used subroutine get_nonhygro_rad_props(ncol, mass, ext, ssa, asm, &
! Subprogram not used                                   tau, tau_w, tau_w_g, tau_w_f)
! Subprogram not used 
! Subprogram not used    ! Arguments
! Subprogram not used    integer,  intent(in) :: ncol
...
! Subprogram not used 
! Subprogram not used    ! Local variables
! Subprogram not used    integer  :: iswband
! Subprogram not used    real(r8) :: ext1, ssa1, asm1
! Subprogram not used    !-----------------------------------------------------------------------------
! Subprogram not used    
! Subprogram not used    do iswband = 1, nswbands
! Subprogram not used       ext1 = ext(iswband)
! Subprogram not used       ssa1 = ssa(iswband)
! Subprogram not used       asm1 = asm(iswband)
! Subprogram not used       tau    (1:ncol,1:pver,iswband) = mass(1:ncol,1:pver) * ext1
! Subprogram not used       tau_w  (1:ncol,1:pver,iswband) = mass(1:ncol,1:pver) * ext1 * ssa1
! Subprogram not used       tau_w_g(1:ncol,1:pver,iswband) = mass(1:ncol,1:pver) * ext1 * ssa1 * asm1
! Subprogram not used       tau_w_f(1:ncol,1:pver,iswband) = mass(1:ncol,1:pver) * ext1 * ssa1 * asm1 * asm1
! Subprogram not used    enddo
! Subprogram not used 
! Subprogram not used end subroutine get_nonhygro_rad_props

!==============================================================================
    
! Subprogram not used subroutine get_volcanic_radius_rad_props(lchnk, ncol, mass, pbuf,  r_ext, r_scat, r_ascat, r_mu, &
! Subprogram not used                                   tau, tau_w, tau_w_g, tau_w_f)
! Subprogram not used 
! Subprogram not used    
! Subprogram not used    use physics_buffer, only : pbuf_get_field, pbuf_get_index
! Subprogram not used 
! Subprogram not used    ! Arguments
...
! Subprogram not used    real(r8), intent(in) :: r_ext(:,:)
! Subprogram not used    real(r8), intent(in) :: r_scat(:,:)
! Subprogram not used    real(r8), intent(in) :: r_ascat(:,:)
! Subprogram not used    real(r8), intent(in) :: r_mu(:) ! log(radius) domain of mass-specific optics
! Subprogram not used 
! Subprogram not used    real(r8), intent(out) :: tau    (pcols, pver, nswbands)
! Subprogram not used    real(r8), intent(out) :: tau_w  (pcols, pver, nswbands)
! Subprogram not used    real(r8), intent(out) :: tau_w_g(pcols, pver, nswbands)
...
! Subprogram not used    ! Local variables
! Subprogram not used    integer  :: iswband
! Subprogram not used    real(r8) :: g
! Subprogram not used 
! Subprogram not used    integer  :: idx                             ! index to radius in physics buffer
! Subprogram not used    real(r8), pointer :: geometric_radius(:,:)  ! geometric mean radius of volcanic aerosol
! Subprogram not used    real(r8) :: mu(pcols,pver)                  ! log(geometric mean radius of volcanic aerosol)
! Subprogram not used    integer  :: imu                             ! index into table values of log radius
! Subprogram not used    integer  :: kmu, nmu
! Subprogram not used    real(r8) :: wmu, mutrunc, r_mu_max, r_mu_min
! Subprogram not used  
! Subprogram not used    ! interpolated values from table
! Subprogram not used    real(r8) :: ext(nswbands)
! Subprogram not used    real(r8) :: scat(nswbands)
! Subprogram not used    real(r8) :: ascat(nswbands)
! Subprogram not used 
! Subprogram not used    integer :: i, k ! column level iterator
! Subprogram not used    !-----------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used    tau    =0._r8                 
! Subprogram not used    tau_w  =0._r8                 
! Subprogram not used    tau_w_g=0._r8                 
! Subprogram not used    tau_w_f=0._r8                  
! Subprogram not used 
! Subprogram not used    ! get microphysical properties for volcanic aerosols
! Subprogram not used    idx = pbuf_get_index('VOLC_RAD_GEOM')
! Subprogram not used    call pbuf_get_field(pbuf, idx, geometric_radius )
! Subprogram not used 
! Subprogram not used    ! interpolate in radius
! Subprogram not used    ! caution: clip the table with no warning when outside bounds
! Subprogram not used    nmu = size(r_mu)
! Subprogram not used    r_mu_max = r_mu(nmu)
! Subprogram not used    r_mu_min = r_mu(1)
! Subprogram not used    do i = 1, ncol
! Subprogram not used       do k = 1, pver
! Subprogram not used          if(geometric_radius(i,k) > 0._r8) then
! Subprogram not used             mu(i,k) = log(geometric_radius(i,k))
! Subprogram not used          else
! Subprogram not used             mu(i,k) = 0._r8
! Subprogram not used          endif
! Subprogram not used          mutrunc = max(min(mu(i,k),r_mu_max),r_mu_min)
! Subprogram not used          kmu = max(min(1 + (mutrunc-r_mu_min)/(r_mu_max-r_mu_min)*(nmu-1),nmu-1._r8),1._r8)
! Subprogram not used          wmu = max(min( (mutrunc -r_mu(kmu)) / (r_mu(kmu+1) - r_mu(kmu)) ,1._r8),0._r8)
! Subprogram not used          do iswband = 1, nswbands
! Subprogram not used             ext(iswband) =  &
! Subprogram not used                ((1._r8 - wmu) * r_ext(iswband, kmu  ) + &
! Subprogram not used                (wmu) * r_ext(iswband, kmu+1))
! Subprogram not used             scat(iswband) =  &
! Subprogram not used                ((1._r8 - wmu) * r_scat(iswband, kmu  ) + &
! Subprogram not used                (wmu) * r_scat(iswband, kmu+1))
! Subprogram not used             ascat(iswband) =  &
! Subprogram not used                ((1._r8 - wmu) * r_ascat(iswband, kmu  ) + &
! Subprogram not used                (wmu) * r_ascat(iswband, kmu+1))
! Subprogram not used             if (scat(iswband).gt.0._r8) then
! Subprogram not used                g = ascat(iswband)/scat(iswband)
! Subprogram not used             else
! Subprogram not used                g=0._r8
! Subprogram not used             endif
! Subprogram not used             tau    (i,k,iswband) = mass(i,k) * ext(iswband)  
! Subprogram not used             tau_w  (i,k,iswband) = mass(i,k) * scat(iswband)  
! Subprogram not used             tau_w_g(i,k,iswband) = mass(i,k) * ascat(iswband)  
! Subprogram not used             tau_w_f(i,k,iswband) = mass(i,k) * g * ascat(iswband)  
! Subprogram not used          end do
! Subprogram not used       enddo
! Subprogram not used    enddo
! Subprogram not used 
! Subprogram not used end subroutine get_volcanic_radius_rad_props

!==============================================================================
    
! Subprogram not used subroutine get_volcanic_rad_props(ncol, mass, ext, scat, ascat, &
! Subprogram not used                                   tau, tau_w, tau_w_g, tau_w_f)
! Subprogram not used 
! Subprogram not used    ! Arguments
! Subprogram not used    integer,  intent(in) :: ncol
! Subprogram not used    real(r8), intent(in) :: mass(pcols, pver)
! Subprogram not used    real(r8), intent(in) :: ext(:)
! Subprogram not used    real(r8), intent(in) :: scat(:)
! Subprogram not used    real(r8), intent(in) :: ascat(:)
! Subprogram not used 
! Subprogram not used    real(r8), intent(out) :: tau    (pcols, pver, nswbands)
! Subprogram not used    real(r8), intent(out) :: tau_w  (pcols, pver, nswbands)
! Subprogram not used    real(r8), intent(out) :: tau_w_g(pcols, pver, nswbands)
...
! Subprogram not used          g=0._r8
! Subprogram not used       endif
! Subprogram not used       tau    (1:ncol,1:pver,iswband) = mass(1:ncol,1:pver) * ext(iswband) 
! Subprogram not used       tau_w  (1:ncol,1:pver,iswband) = mass(1:ncol,1:pver) * scat(iswband) 
! Subprogram not used       tau_w_g(1:ncol,1:pver,iswband) = mass(1:ncol,1:pver) * ascat(iswband) 
! Subprogram not used       tau_w_f(1:ncol,1:pver,iswband) = mass(1:ncol,1:pver) * g * ascat(iswband) 
! Subprogram not used    enddo
! Subprogram not used 
! Subprogram not used end subroutine get_volcanic_rad_props
