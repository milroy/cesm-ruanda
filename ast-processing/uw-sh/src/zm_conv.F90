...
! algorithm.
! 
! Author:guang jun zhang, m.lazare, n.mcfarlane. 1 Contact: P. Rasch
!
! This is contributed code not fully standardized by the 1 core group.
...
! of the 1 where it will include a more straightforward formulation
! and will make use of the standard 1 nomenclature
! 
!-----------------------------------------------------------------------
...
! Subprogram not used subroutine buoyan(lchnk   ,ncol    , &
! Subprogram not used                   q       ,t       ,p       ,z       ,pf      , &
! Subprogram not used                   tp      ,qstp    ,tl      ,rl      ,cape    , &
! Subprogram not used                   pblt    ,lcl     ,lel     ,lon     ,mx      , &
! Subprogram not used                   rd      ,grav    ,cp      ,msg     , &
! Subprogram not used                   tpert   )
! Subprogram not used !----------------------------------------------------------------------- 
! Subprogram not used ! 
! Subprogram not used ! Purpose: 
! Subprogram not used ! <Say what the routine does> 
! Subprogram not used ! 
...
! Subprogram not used ! This is contributed code not fully standardized by the CCM core group.
! Subprogram not used ! The documentation has been enhanced to the degree that we are able.
! Subprogram not used ! Reviewed:          P. Rasch, April 1996
! Subprogram not used ! 
! Subprogram not used !-----------------------------------------------------------------------
...
! Subprogram not used 
! Subprogram not used    real(r8), intent(in) :: q(pcols,pver)        ! spec. humidity
! Subprogram not used    real(r8), intent(in) :: t(pcols,pver)        ! temperature
! Subprogram not used    real(r8), intent(in) :: p(pcols,pver)        ! pressure
! Subprogram not used    real(r8), intent(in) :: z(pcols,pver)        ! height
! Subprogram not used    real(r8), intent(in) :: pf(pcols,pver+1)     ! pressure at interfaces
! Subprogram not used    real(r8), intent(in) :: pblt(pcols)          ! index of pbl depth
! Subprogram not used    real(r8), intent(in) :: tpert(pcols)         ! perturbation temperature by pbl processes
! Subprogram not used 
! Subprogram not used !
! Subprogram not used ! output arguments
! Subprogram not used !
! Subprogram not used    real(r8), intent(out) :: tp(pcols,pver)       ! parcel temperature
! Subprogram not used    real(r8), intent(out) :: qstp(pcols,pver)     ! saturation mixing ratio of parcel
! Subprogram not used    real(r8), intent(out) :: tl(pcols)            ! parcel temperature at lcl
! Subprogram not used    real(r8), intent(out) :: cape(pcols)          ! convective aval. pot. energy.
! Subprogram not used    integer lcl(pcols)        !
! Subprogram not used    integer lel(pcols)        !
! Subprogram not used    integer lon(pcols)        ! level of onset of deep convection
! Subprogram not used    integer mx(pcols)         ! level of max moist static energy
! Subprogram not used !
! Subprogram not used !--------------------------Local Variables------------------------------
! Subprogram not used !
! Subprogram not used    real(r8) capeten(pcols,5)     ! provisional value of cape
! Subprogram not used    real(r8) tv(pcols,pver)       !
! Subprogram not used    real(r8) tpv(pcols,pver)      !
! Subprogram not used    real(r8) buoy(pcols,pver)
! Subprogram not used 
! Subprogram not used    real(r8) a1(pcols)
! Subprogram not used    real(r8) a2(pcols)
! Subprogram not used    real(r8) estp(pcols)
! Subprogram not used    real(r8) pl(pcols)
! Subprogram not used    real(r8) plexp(pcols)
! Subprogram not used    real(r8) hmax(pcols)
! Subprogram not used    real(r8) hmn(pcols)
! Subprogram not used    real(r8) y(pcols)
! Subprogram not used 
! Subprogram not used    logical plge600(pcols)
! Subprogram not used    integer knt(pcols)
! Subprogram not used    integer lelten(pcols,5)
! Subprogram not used 
! Subprogram not used    real(r8) cp
! Subprogram not used    real(r8) e
! Subprogram not used    real(r8) grav
! Subprogram not used 
! Subprogram not used    integer i
! Subprogram not used    integer k
! Subprogram not used    integer msg
! Subprogram not used    integer n
! Subprogram not used 
! Subprogram not used    real(r8) rd
! Subprogram not used    real(r8) rl
! Subprogram not used 
! Subprogram not used 
! Subprogram not used 
! Subprogram not used !
...
! Subprogram not used          lelten(i,n) = pver
! Subprogram not used          capeten(i,n) = 0._r8
! Subprogram not used       end do
! Subprogram not used    end do
! Subprogram not used !
! Subprogram not used    do i = 1,ncol
! Subprogram not used       lon(i) = pver
! Subprogram not used       knt(i) = 0
! Subprogram not used       lel(i) = pver
! Subprogram not used       mx(i) = lon(i)
! Subprogram not used       cape(i) = 0._r8
! Subprogram not used       hmax(i) = 0._r8
! Subprogram not used    end do
! Subprogram not used 
! Subprogram not used    tp(:ncol,:) = t(:ncol,:)
! Subprogram not used    qstp(:ncol,:) = q(:ncol,:)
! Subprogram not used 
! Subprogram not used !!! RBN - Initialize tv and buoy for output.
! Subprogram not used !!! tv=tv : tpv=tpv : qstp=q : buoy=0.
! Subprogram not used    tv(:ncol,:) = t(:ncol,:) *(1._r8+1.608_r8*q(:ncol,:))/ (1._r8+q(:ncol,:))
! Subprogram not used    tpv(:ncol,:) = tv(:ncol,:)
! Subprogram not used    buoy(:ncol,:) = 0._r8
! Subprogram not used 
! Subprogram not used !
! Subprogram not used ! set "launching" level(mx) to be at maximum moist static energy.
! Subprogram not used ! search for this level stops at planetary boundary layer top.
! Subprogram not used !
! Subprogram not used    do k = pver,msg + 1,-1
! Subprogram not used       do i = 1,ncol
! Subprogram not used          hmn(i) = cp*t(i,k) + grav*z(i,k) + rl*q(i,k)
! Subprogram not used          if (k >= nint(pblt(i)) .and. k <= lon(i) .and. hmn(i) > hmax(i)) then
! Subprogram not used             hmax(i) = hmn(i)
! Subprogram not used             mx(i) = k
! Subprogram not used          end if
! Subprogram not used       end do
! Subprogram not used    end do
! Subprogram not used !
! Subprogram not used    do i = 1,ncol
! Subprogram not used       lcl(i) = mx(i)
! Subprogram not used       e = p(i,mx(i))*q(i,mx(i))/ (eps1+q(i,mx(i)))
! Subprogram not used       tl(i) = 2840._r8/ (3.5_r8*log(t(i,mx(i)))-log(e)-4.805_r8) + 55._r8
! Subprogram not used       if (tl(i) < t(i,mx(i))) then
! Subprogram not used          plexp(i) = (1._r8/ (0.2854_r8* (1._r8-0.28_r8*q(i,mx(i)))))
! Subprogram not used          pl(i) = p(i,mx(i))* (tl(i)/t(i,mx(i)))**plexp(i)
! Subprogram not used       else
! Subprogram not used          tl(i) = t(i,mx(i))
! Subprogram not used          pl(i) = p(i,mx(i))
! Subprogram not used       end if
! Subprogram not used    end do
! Subprogram not used 
! Subprogram not used !
! Subprogram not used ! calculate lifting condensation level (lcl).
! Subprogram not used !
! Subprogram not used    do k = pver,msg + 2,-1
! Subprogram not used       do i = 1,ncol
! Subprogram not used          if (k <= mx(i) .and. (p(i,k) > pl(i) .and. p(i,k-1) <= pl(i))) then
! Subprogram not used             lcl(i) = k - 1
! Subprogram not used          end if
! Subprogram not used       end do
! Subprogram not used    end do
! Subprogram not used !
! Subprogram not used ! if lcl is above the nominal level of non-divergence (600 mbs),
! Subprogram not used ! no deep convection is permitted (ensuing calculations
! Subprogram not used ! skipped and cape retains initialized value of zero).
! Subprogram not used !
! Subprogram not used    do i = 1,ncol
! Subprogram not used       plge600(i) = pl(i).ge.600._r8
! Subprogram not used    end do
! Subprogram not used !
! Subprogram not used ! initialize parcel properties in sub-cloud layer below lcl.
! Subprogram not used !
! Subprogram not used    do k = pver,msg + 1,-1
! Subprogram not used       do i=1,ncol
! Subprogram not used          if (k > lcl(i) .and. k <= mx(i) .and. plge600(i)) then
! Subprogram not used             tv(i,k) = t(i,k)* (1._r8+1.608_r8*q(i,k))/ (1._r8+q(i,k))
! Subprogram not used             qstp(i,k) = q(i,mx(i))
! Subprogram not used             tp(i,k) = t(i,mx(i))* (p(i,k)/p(i,mx(i)))**(0.2854_r8* (1._r8-0.28_r8*q(i,mx(i))))
! Subprogram not used !
! Subprogram not used ! buoyancy is increased by 0.5 k as in tiedtke
! Subprogram not used !
! Subprogram not used !-jjh          tpv (i,k)=tp(i,k)*(1.+1.608*q(i,mx(i)))/
! Subprogram not used !-jjh     1                     (1.+q(i,mx(i)))
! Subprogram not used             tpv(i,k) = (tp(i,k)+tpert(i))*(1._r8+1.608_r8*q(i,mx(i)))/ (1._r8+q(i,mx(i)))
! Subprogram not used             buoy(i,k) = tpv(i,k) - tv(i,k) + tiedke_add
! Subprogram not used          end if
! Subprogram not used       end do
! Subprogram not used    end do
! Subprogram not used 
! Subprogram not used !
! Subprogram not used ! define parcel properties at lcl (i.e. level immediately above pl).
! Subprogram not used !
! Subprogram not used    do k = pver,msg + 1,-1
! Subprogram not used       do i=1,ncol
! Subprogram not used          if (k == lcl(i) .and. plge600(i)) then
! Subprogram not used             tv(i,k) = t(i,k)* (1._r8+1.608_r8*q(i,k))/ (1._r8+q(i,k))
! Subprogram not used             qstp(i,k) = q(i,mx(i))
! Subprogram not used             tp(i,k) = tl(i)* (p(i,k)/pl(i))**(0.2854_r8* (1._r8-0.28_r8*qstp(i,k)))
! Subprogram not used !              estp(i)  =exp(21.656_r8 - 5418._r8/tp(i,k))
! Subprogram not used ! use of different formulas for es has about 1 g/kg difference
! Subprogram not used ! in qs at t= 300k, and 0.02 g/kg at t=263k, with the formula
! Subprogram not used ! above giving larger qs.
! Subprogram not used             call qsat_hPa(tp(i,k), p(i,k), estp(i), qstp(i,k))
! Subprogram not used             a1(i) = cp / rl + qstp(i,k) * (1._r8+ qstp(i,k) / eps1) * rl * eps1 / &
! Subprogram not used                     (rd * tp(i,k) ** 2)
! Subprogram not used             a2(i) = .5_r8* (qstp(i,k)* (1._r8+2._r8/eps1*qstp(i,k))* &
! Subprogram not used                     (1._r8+qstp(i,k)/eps1)*eps1**2*rl*rl/ &
! Subprogram not used                     (rd**2*tp(i,k)**4)-qstp(i,k)* &
! Subprogram not used                     (1._r8+qstp(i,k)/eps1)*2._r8*eps1*rl/ &
! Subprogram not used                     (rd*tp(i,k)**3))
! Subprogram not used             a1(i) = 1._r8/a1(i)
! Subprogram not used             a2(i) = -a2(i)*a1(i)**3
! Subprogram not used             y(i) = q(i,mx(i)) - qstp(i,k)
! Subprogram not used             tp(i,k) = tp(i,k) + a1(i)*y(i) + a2(i)*y(i)**2
! Subprogram not used             call qsat_hPa(tp(i,k), p(i,k), estp(i), qstp(i,k))
! Subprogram not used !
! Subprogram not used ! buoyancy is increased by 0.5 k in cape calculation.
! Subprogram not used ! dec. 9, 1994
! Subprogram not used !-jjh          tpv(i,k) =tp(i,k)*(1.+1.608*qstp(i,k))/(1.+q(i,mx(i)))
! Subprogram not used !
! Subprogram not used             tpv(i,k) = (tp(i,k)+tpert(i))* (1._r8+1.608_r8*qstp(i,k)) / (1._r8+q(i,mx(i)))
! Subprogram not used             buoy(i,k) = tpv(i,k) - tv(i,k) + tiedke_add
! Subprogram not used          end if
! Subprogram not used       end do
! Subprogram not used    end do
! Subprogram not used !
! Subprogram not used ! main buoyancy calculation.
! Subprogram not used !
! Subprogram not used    do k = pver - 1,msg + 1,-1
! Subprogram not used       do i=1,ncol
! Subprogram not used          if (k < lcl(i) .and. plge600(i)) then
! Subprogram not used             tv(i,k) = t(i,k)* (1._r8+1.608_r8*q(i,k))/ (1._r8+q(i,k))
! Subprogram not used             qstp(i,k) = qstp(i,k+1)
! Subprogram not used             tp(i,k) = tp(i,k+1)* (p(i,k)/p(i,k+1))**(0.2854_r8* (1._r8-0.28_r8*qstp(i,k)))
! Subprogram not used             call qsat_hPa(tp(i,k), p(i,k), estp(i), qstp(i,k))
! Subprogram not used             a1(i) = cp/rl + qstp(i,k)* (1._r8+qstp(i,k)/eps1)*rl*eps1/ (rd*tp(i,k)**2)
! Subprogram not used             a2(i) = .5_r8* (qstp(i,k)* (1._r8+2._r8/eps1*qstp(i,k))* &
! Subprogram not used                     (1._r8+qstp(i,k)/eps1)*eps1**2*rl*rl/ &
! Subprogram not used                     (rd**2*tp(i,k)**4)-qstp(i,k)* &
! Subprogram not used                     (1._r8+qstp(i,k)/eps1)*2._r8*eps1*rl/ &
...
! Subprogram not used             a1(i) = 1._r8/a1(i)
! Subprogram not used             a2(i) = -a2(i)*a1(i)**3
! Subprogram not used             y(i) = qstp(i,k+1) - qstp(i,k)
! Subprogram not used             tp(i,k) = tp(i,k) + a1(i)*y(i) + a2(i)*y(i)**2
! Subprogram not used             call qsat_hPa(tp(i,k), p(i,k), estp(i), qstp(i,k))
! Subprogram not used !-jjh          tpv(i,k) =tp(i,k)*(1.+1.608*qstp(i,k))/
! Subprogram not used !jt            (1.+q(i,mx(i)))
! Subprogram not used             tpv(i,k) = (tp(i,k)+tpert(i))* (1._r8+1.608_r8*qstp(i,k))/(1._r8+q(i,mx(i)))
! Subprogram not used             buoy(i,k) = tpv(i,k) - tv(i,k) + tiedke_add
! Subprogram not used          end if
! Subprogram not used       end do
...
! Subprogram not used 
! Subprogram not used !
! Subprogram not used    do k = msg + 2,pver
! Subprogram not used       do i = 1,ncol
! Subprogram not used          if (k < lcl(i) .and. plge600(i)) then
! Subprogram not used             if (buoy(i,k+1) > 0._r8 .and. buoy(i,k) <= 0._r8) then
! Subprogram not used                knt(i) = min(5,knt(i) + 1)
! Subprogram not used                lelten(i,knt(i)) = k
! Subprogram not used             end if
! Subprogram not used          end if
! Subprogram not used       end do
! Subprogram not used    end do
! Subprogram not used !
! Subprogram not used ! calculate convective available potential energy (cape).
! Subprogram not used !
! Subprogram not used    do n = 1,5
! Subprogram not used       do k = msg + 1,pver
! Subprogram not used          do i = 1,ncol
! Subprogram not used             if (plge600(i) .and. k <= mx(i) .and. k > lelten(i,n)) then
! Subprogram not used                capeten(i,n) = capeten(i,n) + rd*buoy(i,k)*log(pf(i,k+1)/pf(i,k))
! Subprogram not used             end if
! Subprogram not used          end do
! Subprogram not used       end do
! Subprogram not used    end do
! Subprogram not used !
! Subprogram not used ! find maximum cape from all possible tentative capes from
! Subprogram not used ! one sounding,
! Subprogram not used ! and use it as the final cape, april 26, 1995
! Subprogram not used !
! Subprogram not used    do n = 1,5
! Subprogram not used       do i = 1,ncol
! Subprogram not used          if (capeten(i,n) > cape(i)) then
! Subprogram not used             cape(i) = capeten(i,n)
! Subprogram not used             lel(i) = lelten(i,n)
! Subprogram not used          end if
! Subprogram not used       end do
! Subprogram not used    end do
! Subprogram not used !
! Subprogram not used ! put lower bound on cape for diagnostic purposes.
! Subprogram not used !
! Subprogram not used    do i = 1,ncol
! Subprogram not used       cape(i) = max(cape(i), 0._r8)
! Subprogram not used    end do
! Subprogram not used !
! Subprogram not used    return
! Subprogram not used end subroutine buoyan
...
! If we are to retain the fact that 1 loops over grid-points in the internal
