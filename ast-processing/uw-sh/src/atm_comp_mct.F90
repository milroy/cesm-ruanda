...
!================================================================================
  ...
  subroutine atm_init_mct( EClock, cdata_a, x2a_a, a2x_a, NLFilename )
    ...
    integer :: nstep            ! 1 nstep
    real(r8):: caldayp1         ! 1 calendar day for for next cam time step
    integer :: dtime_cam        ! Time-step increment (sec)
    integer :: ymd              ! 1 current date (YYYYMMDD)
    integer :: yr               ! 1 current year
    integer :: mon              ! 1 current month
    integer :: day              ! 1 current day
    integer :: tod              ! 1 current time of day (sec)
    ...
    if (first_time) then
       ...
       ! Set flag to indicate that 1 will provide carbon and dust deposition fluxes.
...
! Subprogram not used   subroutine atm_read_srfrest_mct( EClock, x2a_a, a2x_a)
! Subprogram not used 
! Subprogram not used     !-----------------------------------------------------------------------
! Subprogram not used     use cam_pio_utils
! Subprogram not used     !
! Subprogram not used     ! Arguments
! Subprogram not used     !
! Subprogram not used     type(ESMF_Clock),intent(inout) :: EClock
! Subprogram not used     type(mct_aVect), intent(inout) :: x2a_a
! Subprogram not used     type(mct_aVect), intent(inout) :: a2x_a
! Subprogram not used     ! 
! Subprogram not used     ! Local variables
! Subprogram not used     !
! Subprogram not used     integer               :: rcode        ! return error code
! Subprogram not used     integer               :: yr_spec      ! Current year
! Subprogram not used     integer               :: mon_spec     ! Current month
! Subprogram not used     integer               :: day_spec     ! Current day
! Subprogram not used     integer               :: sec_spec     ! Current time of day (sec)
! Subprogram not used     integer               :: nf_x2a, nf_a2x, k
! Subprogram not used     real(r8), allocatable :: tmp(:)
...
! Subprogram not used     !-----------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used     ! Determine and open surface restart dataset
! Subprogram not used 
! Subprogram not used     call seq_timemgr_EClockGetData( EClock, curr_yr=yr_spec,curr_mon=mon_spec, &
! Subprogram not used          curr_day=day_spec, curr_tod=sec_spec ) 
! Subprogram not used     fname_srf_cam = interpret_filename_spec( rsfilename_spec_cam, case=get_restcase(), &
! Subprogram not used          yr_spec=yr_spec, mon_spec=mon_spec, day_spec=day_spec, sec_spec= sec_spec )
! Subprogram not used     pname_srf_cam = trim(get_restartdir() )//fname_srf_cam
! Subprogram not used     call getfil(pname_srf_cam, fname_srf_cam)
! Subprogram not used     
! Subprogram not used     call cam_pio_openfile(File, fname_srf_cam, 0)
! Subprogram not used     call pio_initdecomp(pio_subsystem, pio_double, (/ngcols/), dof, iodesc)
! Subprogram not used     allocate(tmp(size(dof)))
! Subprogram not used     
! Subprogram not used     nf_x2a = mct_aVect_nRattr(x2a_a)
! Subprogram not used     do k=1,nf_x2a
! Subprogram not used        call mct_aVect_getRList(mstring,k,x2a_a)
...
! Subprogram not used        rcode = pio_inq_varid(File,'x2a_'//trim(itemc) ,varid)
! Subprogram not used        if (rcode == pio_noerr) then
! Subprogram not used           call pio_read_darray(File, varid, iodesc, tmp, rcode)
! Subprogram not used           x2a_a%rattr(k,:) = tmp(:)
! Subprogram not used        else
! Subprogram not used 	  if (masterproc) then
! Subprogram not used              write(iulog,*)'srfrest warning: field ',trim(itemc),' is not on restart file'
! Subprogram not used              write(iulog,*)'for backwards compatibility will set it to 0'
! Subprogram not used           end if
! Subprogram not used           x2a_a%rattr(k,:) = 0._r8
! Subprogram not used        end if
! Subprogram not used        call pio_seterrorhandling(File, pio_internal_error)
! Subprogram not used     end do
! Subprogram not used 
! Subprogram not used     nf_a2x = mct_aVect_nRattr(a2x_a)
! Subprogram not used     do k=1,nf_a2x
! Subprogram not used        call mct_aVect_getRList(mstring,k,a2x_a)
! Subprogram not used        itemc = mct_string_toChar(mstring)
! Subprogram not used        call mct_string_clean(mstring)
! Subprogram not used 
! Subprogram not used        rcode = pio_inq_varid(File,'a2x_'//trim(itemc) ,varid)
! Subprogram not used        call pio_read_darray(File, varid, iodesc, tmp, rcode)
! Subprogram not used        a2x_a%rattr(k,:) = tmp(:)
! Subprogram not used     end do
! Subprogram not used 
! Subprogram not used     call pio_freedecomp(File,iodesc)
! Subprogram not used     call pio_closefile(File)
! Subprogram not used     deallocate(tmp)
! Subprogram not used 
! Subprogram not used   end subroutine atm_read_srfrest_mct

  !===========================================================================================

! Subprogram not used   subroutine atm_write_srfrest_mct( x2a_a, a2x_a, & 
...
! Subprogram not used     ! Arguments
! Subprogram not used     !
! Subprogram not used     type(mct_aVect), intent(in) :: x2a_a
! Subprogram not used     type(mct_aVect), intent(in) :: a2x_a
...
! Subprogram not used     !
! Subprogram not used     integer                   :: rcode        ! return error code
! Subprogram not used     integer                   :: nf_x2a, nf_a2x, dimid(1), k
! Subprogram not used     type(file_desc_t)         :: file
! Subprogram not used     type(var_desc_t), pointer :: varid_x2a(:), varid_a2x(:)
! Subprogram not used     type(io_desc_t)           :: iodesc
! Subprogram not used     character(CL)             :: itemc       ! string converted to char
! Subprogram not used     type(mct_string)          :: mstring     ! mct char type
! Subprogram not used     !-----------------------------------------------------------------------
...
! Subprogram not used     ! Determine and open surface restart dataset
! Subprogram not used 
! Subprogram not used     fname_srf_cam = interpret_filename_spec( rsfilename_spec_cam, &
! Subprogram not used          yr_spec=yr_spec, mon_spec=mon_spec, day_spec=day_spec, sec_spec= sec_spec )
! Subprogram not used 
! Subprogram not used     call cam_pio_createfile(File, fname_srf_cam, 0)
! Subprogram not used     call pio_initdecomp(pio_subsystem, pio_double, (/ngcols/), dof, iodesc)
! Subprogram not used 
! Subprogram not used     nf_x2a = mct_aVect_nRattr(x2a_a)
! Subprogram not used     allocate(varid_x2a(nf_x2a))
! Subprogram not used     
! Subprogram not used     rcode = pio_def_dim(File,'x2a_nx',ngcols,dimid(1))
! Subprogram not used     do k = 1,nf_x2a
! Subprogram not used        call mct_aVect_getRList(mstring,k,x2a_a)
! Subprogram not used        itemc = mct_string_toChar(mstring)
! Subprogram not used        call mct_string_clean(mstring)
! Subprogram not used        rcode = pio_def_var(File,'x2a_'//trim(itemc),PIO_DOUBLE,dimid,varid_x2a(k))
! Subprogram not used        rcode = pio_put_att(File,varid_x2a(k),"_fillvalue",fillvalue)
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used     nf_a2x = mct_aVect_nRattr(a2x_a)
! Subprogram not used     allocate(varid_a2x(nf_a2x))
! Subprogram not used     
! Subprogram not used     rcode = pio_def_dim(File,'a2x_nx',ngcols,dimid(1))
! Subprogram not used     do k = 1,nf_a2x
! Subprogram not used        call mct_aVect_getRList(mstring,k,a2x_a)
! Subprogram not used        itemc = mct_string_toChar(mstring)
! Subprogram not used        call mct_string_clean(mstring)
! Subprogram not used        rcode = PIO_def_var(File,'a2x_'//trim(itemc),PIO_DOUBLE,dimid,varid_a2x(k))
! Subprogram not used        rcode = PIO_put_att(File,varid_a2x(k),"_fillvalue",fillvalue)
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used     rcode = pio_enddef(File)  ! don't check return code, might be enddef already
! Subprogram not used 
! Subprogram not used 
! Subprogram not used     do k=1,nf_x2a
! Subprogram not used        call pio_write_darray(File, varid_x2a(k), iodesc, x2a_a%rattr(k,:), rcode)
! Subprogram not used     end do
! Subprogram not used 
! Subprogram not used     do k=1,nf_a2x
! Subprogram not used        call pio_write_darray(File, varid_a2x(k), iodesc, a2x_a%rattr(k,:), rcode)       
! Subprogram not used     end do
! Subprogram not used 
! Subprogram not used     deallocate(varid_x2a, varid_a2x)
! Subprogram not used 
! Subprogram not used     call pio_freedecomp(File,iodesc)
! Subprogram not used     call pio_closefile(file)
! Subprogram not used 
! Subprogram not used 
! Subprogram not used   end subroutine atm_write_srfrest_mct
