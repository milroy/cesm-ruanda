...
! Subprogram not used ! Subprogram not used !           where PHI_ij = cardinal function at i,j GLL point 
! Subprogram not used ! Subprogram not used !
! Subprogram not used ! Subprogram not used !   this routine is used just to check spectral element integration by parts identities
! Subprogram not used ! Subprogram not used !
! Subprogram not used ! Subprogram not used     real(kind=real_kind), intent(in) :: v(np,np,2)  ! in lat-lon coordinates
! Subprogram not used ! Subprogram not used     type (derivative_t)              :: deriv
! Subprogram not used ! Subprogram not used     type (element_t)                 :: elem
! Subprogram not used ! Subprogram not used     real(kind=real_kind) :: result(np,np)
! Subprogram not used ! Subprogram not used 
! Subprogram not used ! Subprogram not used     ! Local
! Subprogram not used ! Subprogram not used     real(kind=real_kind) :: ucontra(np,np,2)  ! in lat-lon coordinates
! Subprogram not used ! Subprogram not used     integer i,j
! Subprogram not used ! Subprogram not used 
! Subprogram not used ! Subprogram not used     ! latlon->contra
! Subprogram not used ! Subprogram not used     do j=1,np
! Subprogram not used ! Subprogram not used        do i=1,np
! Subprogram not used ! Subprogram not used           ucontra(i,j,1)=(elem%Dinv(1,1,i,j)*v(i,j,1) + elem%Dinv(1,2,i,j)*v(i,j,2))
! Subprogram not used ! Subprogram not used           ucontra(i,j,2)=(elem%Dinv(2,1,i,j)*v(i,j,1) + elem%Dinv(2,2,i,j)*v(i,j,2))
! Subprogram not used ! Subprogram not used        enddo
! Subprogram not used ! Subprogram not used     enddo
! Subprogram not used ! Subprogram not used 
! Subprogram not used ! Subprogram not used     ! note: GLL weights  weight(i) = Mvv_twt(i,i)
! Subprogram not used ! Subprogram not used     result=0
! Subprogram not used ! Subprogram not used     j=1
! Subprogram not used ! Subprogram not used     do i=1,np
! Subprogram not used ! Subprogram not used        result(i,j)=result(i,j)-deriv%Mvv_twt(i,i)*elem%metdet(i,j)*ucontra(i,j,2)*rrearth
! Subprogram not used ! Subprogram not used     enddo
! Subprogram not used ! Subprogram not used     
! Subprogram not used ! Subprogram not used     j=np
! Subprogram not used ! Subprogram not used     do i=1,np
! Subprogram not used ! Subprogram not used        result(i,j)=result(i,j)+deriv%Mvv_twt(i,i)*elem%metdet(i,j)*ucontra(i,j,2)*rrearth
! Subprogram not used ! Subprogram not used     enddo
! Subprogram not used ! Subprogram not used     
! Subprogram not used ! Subprogram not used     i=1
! Subprogram not used ! Subprogram not used     do j=1,np
! Subprogram not used ! Subprogram not used        result(i,j)=result(i,j)-deriv%Mvv_twt(j,j)*elem%metdet(i,j)*ucontra(i,j,1)*rrearth
! Subprogram not used ! Subprogram not used     enddo
! Subprogram not used ! Subprogram not used     
! Subprogram not used ! Subprogram not used     i=np
! Subprogram not used ! Subprogram not used     do j=1,np
! Subprogram not used ! Subprogram not used        result(i,j)=result(i,j)+deriv%Mvv_twt(j,j)*elem%metdet(i,j)*ucontra(i,j,1)*rrearth
! Subprogram not used ! Subprogram not used     enddo
! Subprogram not used ! Subprogram not used   end function element_boundary_integral
