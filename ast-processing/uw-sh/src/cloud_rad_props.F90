...
! Subprogram not used 
! Subprogram not used    ! optical props for each aerosol
! Subprogram not used    real(r8), pointer :: h_ext(:,:)
! Subprogram not used    real(r8), pointer :: h_ssa(:,:)
! Subprogram not used    real(r8), pointer :: h_asm(:,:)
! Subprogram not used    real(r8), pointer :: n_ext(:)
! Subprogram not used    real(r8), pointer :: n_ssa(:)
! Subprogram not used    real(r8), pointer :: n_asm(:)
! Subprogram not used 
...
! Subprogram not used 
! Subprogram not used    ! rad properties for ice clouds
! Subprogram not used    real(r8) :: ice_tau    (nswbands,pcols,pver) ! aerosol extinction optical depth
! Subprogram not used    real(r8) :: ice_tau_w  (nswbands,pcols,pver) ! aerosol single scattering albedo * tau
! Subprogram not used    real(r8) :: ice_tau_w_g(nswbands,pcols,pver) ! aerosol assymetry parameter * tau * w
! Subprogram not used    real(r8) :: ice_tau_w_f(nswbands,pcols,pver) ! aerosol forward scattered fraction * tau * w
! Subprogram not used 
! Subprogram not used    !-----------------------------------------------------------------------------
! Subprogram not used 
...
! Subprogram not used 
! Subprogram not used 
! Subprogram not used    call get_liquid_optics_sw(state, pbuf, liq_tau, liq_tau_w, liq_tau_w_g, liq_tau_w_f)
! Subprogram not used 
! Subprogram not used    call get_ice_optics_sw   (state, pbuf, ice_tau, ice_tau_w, ice_tau_w_g, ice_tau_w_f)
! Subprogram not used 
! Subprogram not used    tau    (:,1:ncol,:) =  liq_tau    (:,1:ncol,:) + ice_tau    (:,1:ncol,:)
! Subprogram not used    tau_w  (:,1:ncol,:) =  liq_tau_w  (:,1:ncol,:) + ice_tau_w  (:,1:ncol,:)
! Subprogram not used    tau_w_g(:,1:ncol,:) =  liq_tau_w_g(:,1:ncol,:) + ice_tau_w_g(:,1:ncol,:)
! Subprogram not used    tau_w_f(:,1:ncol,:) =  liq_tau_w_f(:,1:ncol,:) + ice_tau_w_f(:,1:ncol,:)
! Subprogram not used 
...
! Subprogram not used 
! Subprogram not used    ! rad properties for ice clouds
! Subprogram not used    real(r8) :: ice_tau_abs_od(nlwbands,pcols,pver) ! ice cloud absorption optical depth
! Subprogram not used 
! Subprogram not used    !-----------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used    ncol = state%ncol
...
! Subprogram not used 
! Subprogram not used    if(present(oldcloud))then
! Subprogram not used       if(oldcloud) then
! Subprogram not used          ! make diagnostic calls to these first to output ice and liq OD's
! Subprogram not used          !call old_liq_get_rad_props_lw(state, pbuf, liq_tau_abs_od, oldliqwp=.false.)
! Subprogram not used          !call old_ice_get_rad_props_lw(state, pbuf, ice_tau_abs_od, oldicewp=.false.)
! Subprogram not used          ! This affects climate (cld_abs_od)
! Subprogram not used          call oldcloud_lw(state,pbuf,cld_abs_od,oldwp=.false.)
! Subprogram not used          return
! Subprogram not used       endif
! Subprogram not used    endif
! Subprogram not used 
! Subprogram not used    if(present(oldliq))then
! Subprogram not used       if(oldliq) then
! Subprogram not used          call old_liq_get_rad_props_lw(state, pbuf, liq_tau_abs_od, oldliqwp=.false.)
! Subprogram not used       else
! Subprogram not used          call liquid_cloud_get_rad_props_lw(state, pbuf, liq_tau_abs_od)
! Subprogram not used       endif
! Subprogram not used    else
! Subprogram not used       call liquid_cloud_get_rad_props_lw(state, pbuf, liq_tau_abs_od)
! Subprogram not used    endif
! Subprogram not used 
! Subprogram not used    if(present(oldice))then
! Subprogram not used       if(oldice) then
! Subprogram not used          call old_ice_get_rad_props_lw(state, pbuf, ice_tau_abs_od, oldicewp=.false.)
! Subprogram not used       else
! Subprogram not used          call ice_cloud_get_rad_props_lw(state, pbuf, ice_tau_abs_od)
! Subprogram not used       endif
! Subprogram not used    else
! Subprogram not used       call ice_cloud_get_rad_props_lw(state, pbuf, ice_tau_abs_od)
! Subprogram not used    endif
! Subprogram not used       
! Subprogram not used    cld_abs_od(:,1:ncol,:) = liq_tau_abs_od(:,1:ncol,:) + ice_tau_abs_od(:,1:ncol,:) 
! Subprogram not used 
