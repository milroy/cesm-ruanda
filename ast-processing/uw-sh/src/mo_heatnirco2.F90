...
! Subprogram not used       subroutine heatnirco2( co2, scco2, pmid, htng, kbot, zarg, sza )
! Subprogram not used 
...
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used       integer,intent(in)     :: kbot                    ! vertical dimension
! Subprogram not used       real(r8), intent(in)   :: sza
! Subprogram not used       real(r8), intent(in)   :: zarg(kbot)              ! midpoint geopot (m)
! Subprogram not used       real(r8), intent(in)   :: co2(kbot)               ! co2 concentration (mol/mol)
! Subprogram not used       real(r8), intent(in)   :: scco2(kbot)             ! co2 slant column (molec/cm^2)
! Subprogram not used       real(r8), intent(in)   :: pmid(kbot)              ! model midpoint pressure (Pa)
! Subprogram not used       real(r8), intent(out)  :: htng(kbot)              ! co2 near ir heating (K/day)
! Subprogram not used 
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used ! 	... local variables
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used       real(r8), parameter :: smallvalue = 1.0e-20_r8
! Subprogram not used       real(r8), parameter :: pa2hPa     = 1.e-2_r8
! Subprogram not used 
! Subprogram not used       integer  :: icolm, icolmp1
! Subprogram not used       integer  :: k, kk, kndx
! Subprogram not used       real(r8) :: reldcolm
! Subprogram not used       real(r8) :: delp
! Subprogram not used       real(r8) :: pinterp
! Subprogram not used       real(r8) :: colzpint
! Subprogram not used       real(r8) :: co2std
! Subprogram not used       real(r8) :: colparai(ncolgr)
! Subprogram not used       real(r8) :: corrnorai(ncolgr)
! Subprogram not used 
! Subprogram not used       htng(:kbot) = smallvalue
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used ! 	... vertical and column interpolation
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used level_loop : &
! Subprogram not used       do k = 1,kbot
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used ! 	... first setup pressure interpolation
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used          pinterp  = pmid(k) * pa2hPa
! Subprogram not used          colzpint = log( scco2(k) )
! Subprogram not used          if( pinterp <= xspara(ndpara) ) then
! Subprogram not used             colparai(:)  = colmpara(ndpara,:)
! Subprogram not used             corrnorai(:) = corrnormpara(ndpara,:)
! Subprogram not used             co2std       = co2stand(ndpara)
! Subprogram not used          else if( pinterp > xspara(1) ) then
! Subprogram not used             colparai(:)  = colmpara(1,:)
! Subprogram not used             corrnorai(:) = corrnormpara(1,:)
! Subprogram not used             co2std       = co2stand(1)
! Subprogram not used          else
! Subprogram not used             do kk = ndpara-1,1,-1
! Subprogram not used                if( pinterp <= xspara(kk) ) then
! Subprogram not used                   kndx = kk + 1
! Subprogram not used                   delp         = (pinterp - xspara(kndx))/(xspara(kk) - xspara(kndx))
! Subprogram not used                   colparai(:)  = colmpara(kndx,:) + delp*(colmpara(kk,:) - colmpara(kndx,:))
! Subprogram not used                   corrnorai(:) = corrnormpara(kndx,:) &
! Subprogram not used                                  + delp*(corrnormpara(kk,:) - corrnormpara(kndx,:))
! Subprogram not used                   co2std       = co2stand(kndx) + delp*(co2stand(kk) - co2stand(kndx))
...
! Subprogram not used 
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used ! Linear interpolation over column density for given altitude point
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used          if( colzpint < colparai(1) ) then
! Subprogram not used             htng(k) = corrnorai(1)
! Subprogram not used          else if( colzpint >= colparai(ncolgr) ) then
! Subprogram not used             htng(k) = corrnorai(ncolgr)
! Subprogram not used          else
! Subprogram not used loop1:      do icolm = 1,ncolgr-1
! Subprogram not used                icolmp1 = icolm + 1
! Subprogram not used                if( colzpint >= colparai(icolm) .and. &
! Subprogram not used                    colzpint <  colparai(icolmp1) ) then
! Subprogram not used                   reldcolm = (colzpint - colparai(icolm)) &
! Subprogram not used                              /(colparai(icolmp1) - colparai(icolm))
! Subprogram not used                   htng(k) = corrnorai(icolm) &
! Subprogram not used                           + (corrnorai(icolmp1) - corrnorai(icolm))*reldcolm
! Subprogram not used                   exit loop1
! Subprogram not used                end if
! Subprogram not used             end do loop1
! Subprogram not used          end if
! Subprogram not used          
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used ! From normalized value to the one corresponding to the given vmrco2
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used          htng(k) = htng(k) * co2(k)/co2std
! Subprogram not used       end do level_loop
! Subprogram not used 
! Subprogram not used       end subroutine heatnirco2
