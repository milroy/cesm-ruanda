...
!-----------------------------------------------------------------------
             ...
             if( FCA > zero ) then
!-----------------------------------------------------------------------
!  Maintain cloud core by reducing 4 and AM area going into cloud below
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
! Subprogram not used       subroutine WASHGAS (RWASH,BOXF,DTSCAV,QTRTOP,HSTAR,TM,PR,QM, &
! Subprogram not used                             QT,QTWASH,QTEVAP)
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used !---for most gases below-cloud washout assume Henry-Law equilib with precip
! Subprogram not used !---assumes that precip is liquid, if frozen, do not call this sub
! Subprogram not used !---since solubility is moderate, fraction of box with rain does not matter
! Subprogram not used !---NB this code does not consider the aqueous dissociation (eg, C-q) 
! Subprogram not used !---   that makes uptake of HNO3 and H2SO4 so complete.  To do so would
! Subprogram not used !---   require that we keep track of the pH of the falling rain.
! Subprogram not used !---THUS the Henry's Law coefficient KHA needs to be enhanced to incldue this!
! Subprogram not used !---ALSO the possible formation of other soluble species from, eg, CH2O, H2O2
! Subprogram not used !---   can be considered with enhanced values of KHA.
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used       implicit none
! Subprogram not used       real(r8), intent(in)  :: RWASH   ! precip leaving bottom of box (kg/s)
! Subprogram not used       real(r8), intent(in)  :: BOXF   ! fraction of box with washout
! Subprogram not used       real(r8), intent(in)  :: DTSCAV  ! time step (s)
! Subprogram not used       real(r8), intent(in)  :: QTRTOP  ! tracer-T in rain entering top of box 
! Subprogram not used !                                              over time step (kg)
! Subprogram not used       real(r8), intent(in)  :: HSTAR ! Henry's Law coeffs A*exp(-B/T)
! Subprogram not used       real(r8), intent(in)  :: TM      ! temperature of box (K)
! Subprogram not used       real(r8), intent(in)  :: PR      ! pressure of box (hPa)
! Subprogram not used       real(r8), intent(in)  :: QT      ! tracer in box (kg)
! Subprogram not used       real(r8), intent(in)  :: QM      ! air mass in box (kg)
! Subprogram not used       real(r8), intent(out) :: QTWASH  ! tracer picked up by precip (kg)
! Subprogram not used       real(r8), intent(out) :: QTEVAP  ! tracer evaporated from precip (kg)
! Subprogram not used       
! Subprogram not used       real(r8), parameter :: INV298 = 1._r8/298._r8
! Subprogram not used       real(r8)            :: FWASH, QTMAX, QTDIF
! Subprogram not used 
! Subprogram not used !---effective Henry's Law constant: H* = moles-T / liter-precip / press(atm-T)
! Subprogram not used !---p(atm of tracer-T) = (QT/QM) * (.029/MolWt-T) * pressr(hPa)/1000
! Subprogram not used !---limit temperature effects to T above freezing
! Subprogram not used 
! Subprogram not used !
! Subprogram not used ! jfl
! Subprogram not used !
! Subprogram not used ! added test for BOXF = 0.
! Subprogram not used !
! Subprogram not used       if ( BOXF == 0._r8 ) then
! Subprogram not used         QTWASH = 0._r8
! Subprogram not used         QTEVAP = 0._r8
! Subprogram not used         return
! Subprogram not used       end if
! Subprogram not used 
! Subprogram not used !---effective washout frequency (1/s):
! Subprogram not used         FWASH = (RWASH*HSTAR*29.e-6_r8*PR)/(QM*BOXF)
! Subprogram not used !---equilib amount of T (kg) in rain thru bottom of box over time step
! Subprogram not used         QTMAX = QT*FWASH*DTSCAV
! Subprogram not used       if (QTMAX .gt. QTRTOP) then
! Subprogram not used !---more of tracer T can go into rain
! Subprogram not used          QTDIF = min (QT, QTMAX-QTRTOP)
! Subprogram not used          QTWASH = QTDIF * (1._r8 - exp(-DTSCAV*FWASH))
! Subprogram not used          QTEVAP=0._r8
! Subprogram not used       else
! Subprogram not used !--too much of T in rain, must degas/evap T
! Subprogram not used          QTWASH = 0._r8
! Subprogram not used          QTEVAP = QTRTOP - QTMAX
! Subprogram not used       endif
! Subprogram not used      
! Subprogram not used       return
! Subprogram not used       end subroutine WASHGAS
