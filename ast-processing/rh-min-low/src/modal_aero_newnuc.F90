...
!----------------------------------------------------------------------
!----------------------------------------------------------------------
! Subprogram not used subroutine ternary_nuc_merik2007( t, rh, c2, c3, j_log, ntot, nacid, namm, r )
! Subprogram not used !subroutine ternary_fit(          t, rh, c2, c3, j_log, ntot, nacid, namm, r )
! Subprogram not used ! *************************** ternary_fit.f90 ********************************
! Subprogram not used ! joonas merikanto, 2006
! Subprogram not used !
! Subprogram not used ! fortran 90 subroutine that calculates the parameterized composition 
! Subprogram not used ! and nucleation rate of critical clusters in h2o-h2so4-nh3 vapor
! Subprogram not used !
! Subprogram not used ! warning: the fit should not be used outside its limits of validity
! Subprogram not used ! (limits indicated below)
! Subprogram not used !
! Subprogram not used ! in:
! Subprogram not used ! t:     temperature (k), limits 235-295 k
! Subprogram not used ! rh:    relative humidity as fraction (eg. 0.5=50%) limits 0.05-0.95
! Subprogram not used ! c2:    sulfuric acid concentration (molecules/cm3) limits 5x10^4 - 10^9 molecules/cm3
! Subprogram not used ! c3:    ammonia mixing ratio (ppt) limits 0.1 - 1000 ppt
! Subprogram not used !
! Subprogram not used ! out:
! Subprogram not used ! j_log: logarithm of nucleation rate (1/(s cm3))
! Subprogram not used ! ntot:  total number of molecules in the critical cluster
! Subprogram not used ! nacid: number of sulfuric acid molecules in the critical cluster
! Subprogram not used ! namm:  number of ammonia molecules in the critical cluster
! Subprogram not used ! r:     radius of the critical cluster (nm)
! Subprogram not used !  ****************************************************************************
! Subprogram not used implicit none
! Subprogram not used 
! Subprogram not used real(r8), intent(in) :: t, rh, c2, c3
! Subprogram not used real(r8), intent(out) :: j_log, ntot, nacid, namm, r
! Subprogram not used real(r8) :: j, t_onset
! Subprogram not used 
! Subprogram not used t_onset=143.6002929064716_r8 + 1.0178856665693992_r8*rh + &
! Subprogram not used    10.196398812974294_r8*log(c2) - &
! Subprogram not used    0.1849879416839113_r8*log(c2)**2 - 17.161783213150173_r8*log(c3) + &
! Subprogram not used    (109.92469248546053_r8*log(c3))/log(c2) + &
! Subprogram not used    0.7734119613144357_r8*log(c2)*log(c3) - 0.15576469879527022_r8*log(c3)**2
! Subprogram not used 
! Subprogram not used if(t_onset.gt.t) then 
! Subprogram not used 
! Subprogram not used    j_log=-12.861848898625231_r8 + 4.905527742256349_r8*c3 - 358.2337705052991_r8*rh -& 
! Subprogram not used    0.05463019231872484_r8*c3*t + 4.8630382337426985_r8*rh*t + &
! Subprogram not used    0.00020258394697064567_r8*c3*t**2 - 0.02175548069741675_r8*rh*t**2 - &
! Subprogram not used    2.502406532869512e-7_r8*c3*t**3 + 0.00003212869941055865_r8*rh*t**3 - &
! Subprogram not used    4.39129415725234e6_r8/log(c2)**2 + (56383.93843154586_r8*t)/log(c2)**2 -& 
! Subprogram not used    (239.835990963361_r8*t**2)/log(c2)**2 + &
! Subprogram not used    (0.33765136625580167_r8*t**3)/log(c2)**2 - &
! Subprogram not used    (629.7882041830943_r8*rh)/(c3**3*log(c2)) + &
! Subprogram not used    (7.772806552631709_r8*rh*t)/(c3**3*log(c2)) - &
! Subprogram not used    (0.031974053936299256_r8*rh*t**2)/(c3**3*log(c2)) + &
! Subprogram not used    (0.00004383764128775082_r8*rh*t**3)/(c3**3*log(c2)) + &
! Subprogram not used    1200.472096232311_r8*log(c2) - 17.37107890065621_r8*t*log(c2) + &
! Subprogram not used    0.08170681335921742_r8*t**2*log(c2) - 0.00012534476159729881_r8*t**3*log(c2) - &
! Subprogram not used    14.833042158178936_r8*log(c2)**2 + 0.2932631303555295_r8*t*log(c2)**2 - &
! Subprogram not used    0.0016497524241142845_r8*t**2*log(c2)**2 + &
! Subprogram not used    2.844074805239367e-6_r8*t**3*log(c2)**2 - 231375.56676032578_r8*log(c3) - &
! Subprogram not used    100.21645273730675_r8*rh*log(c3) + 2919.2852552424706_r8*t*log(c3) + &
! Subprogram not used    0.977886555834732_r8*rh*t*log(c3) - 12.286497122264588_r8*t**2*log(c3) - &
! Subprogram not used    0.0030511783284506377_r8*rh*t**2*log(c3) + &
! Subprogram not used    0.017249301826661612_r8*t**3*log(c3) + 2.967320346100855e-6_r8*rh*t**3*log(c3) + &
! Subprogram not used    (2.360931724951942e6_r8*log(c3))/log(c2) - &
! Subprogram not used    (29752.130254319443_r8*t*log(c3))/log(c2) + &
! Subprogram not used    (125.04965118142027_r8*t**2*log(c3))/log(c2) - &
! Subprogram not used    (0.1752996881934318_r8*t**3*log(c3))/log(c2) + &
! Subprogram not used    5599.912337254629_r8*log(c2)*log(c3) - 70.70896612937771_r8*t*log(c2)*log(c3) + &
! Subprogram not used    0.2978801613269466_r8*t**2*log(c2)*log(c3) - &
! Subprogram not used    0.00041866525019504_r8*t**3*log(c2)*log(c3) + 75061.15281456841_r8*log(c3)**2 - &
! Subprogram not used    931.8802278173565_r8*t*log(c3)**2 + 3.863266220840964_r8*t**2*log(c3)**2 - &
! Subprogram not used    0.005349472062284983_r8*t**3*log(c3)**2 - &
! Subprogram not used    (732006.8180571689_r8*log(c3)**2)/log(c2) + &
! Subprogram not used    (9100.06398573816_r8*t*log(c3)**2)/log(c2) - &
! Subprogram not used    (37.771091915932004_r8*t**2*log(c3)**2)/log(c2) + &
! Subprogram not used    (0.05235455395566905_r8*t**3*log(c3)**2)/log(c2) - &
! Subprogram not used    1911.0303773001353_r8*log(c2)*log(c3)**2 + &
! Subprogram not used    23.6903969622286_r8*t*log(c2)*log(c3)**2 - &
! Subprogram not used    0.09807872005428583_r8*t**2*log(c2)*log(c3)**2 + &
! Subprogram not used    0.00013564560238552576_r8*t**3*log(c2)*log(c3)**2 - &
! Subprogram not used    3180.5610833308_r8*log(c3)**3 + 39.08268568672095_r8*t*log(c3)**3 - &
! Subprogram not used    0.16048521066690752_r8*t**2*log(c3)**3 + &
! Subprogram not used    0.00022031380023793877_r8*t**3*log(c3)**3 + &
! Subprogram not used    (40751.075322248245_r8*log(c3)**3)/log(c2) - &
! Subprogram not used    (501.66977622013934_r8*t*log(c3)**3)/log(c2) + &
! Subprogram not used    (2.063469732254135_r8*t**2*log(c3)**3)/log(c2) - &
! Subprogram not used    (0.002836873785758324_r8*t**3*log(c3)**3)/log(c2) + &
! Subprogram not used    2.792313345723013_r8*log(c2)**2*log(c3)**3 - &
! Subprogram not used    0.03422552111802899_r8*t*log(c2)**2*log(c3)**3 + &
! Subprogram not used    0.00014019195277521142_r8*t**2*log(c2)**2*log(c3)**3 - &
! Subprogram not used    1.9201227328396297e-7_r8*t**3*log(c2)**2*log(c3)**3 - &
! Subprogram not used    980.923146020468_r8*log(rh) + 10.054155220444462_r8*t*log(rh) - &
! Subprogram not used    0.03306644502023841_r8*t**2*log(rh) + 0.000034274041225891804_r8*t**3*log(rh) + &
! Subprogram not used    (16597.75554295064_r8*log(rh))/log(c2) - &
! Subprogram not used    (175.2365504237746_r8*t*log(rh))/log(c2) + &
! Subprogram not used    (0.6033215603167458_r8*t**2*log(rh))/log(c2) - &
! Subprogram not used    (0.0006731787599587544_r8*t**3*log(rh))/log(c2) - &
! Subprogram not used    89.38961120336789_r8*log(c3)*log(rh) + 1.153344219304926_r8*t*log(c3)*log(rh) - &
! Subprogram not used    0.004954549700267233_r8*t**2*log(c3)*log(rh) + &
! Subprogram not used    7.096309866238719e-6_r8*t**3*log(c3)*log(rh) + &
! Subprogram not used    3.1712136610383244_r8*log(c3)**3*log(rh) - &
! Subprogram not used    0.037822330602328806_r8*t*log(c3)**3*log(rh) + &
! Subprogram not used    0.0001500555743561457_r8*t**2*log(c3)**3*log(rh) - &
! Subprogram not used    1.9828365865570703e-7_r8*t**3*log(c3)**3*log(rh)
! Subprogram not used 
! Subprogram not used    j=exp(j_log)
! Subprogram not used 
! Subprogram not used    ntot=57.40091052369212_r8 - 0.2996341884645408_r8*t + &
! Subprogram not used    0.0007395477768531926_r8*t**2 - &
! Subprogram not used    5.090604835032423_r8*log(c2) + 0.011016634044531128_r8*t*log(c2) + &
! Subprogram not used    0.06750032251225707_r8*log(c2)**2 - 0.8102831333223962_r8*log(c3) + &
! Subprogram not used    0.015905081275952426_r8*t*log(c3) - 0.2044174683159531_r8*log(c2)*log(c3) + &
! Subprogram not used    0.08918159167625832_r8*log(c3)**2 - 0.0004969033586666147_r8*t*log(c3)**2 + &
! Subprogram not used    0.005704394549007816_r8*log(c3)**3 + 3.4098703903474368_r8*log(j) - &
! Subprogram not used    0.014916956508210809_r8*t*log(j) + 0.08459090011666293_r8*log(c3)*log(j) - &
! Subprogram not used    0.00014800625143907616_r8*t*log(c3)*log(j) + 0.00503804694656905_r8*log(j)**2
! Subprogram not used  
! Subprogram not used    r=3.2888553966535506e-10_r8 - 3.374171768439839e-12_r8*t + &
! Subprogram not used    1.8347359507774313e-14_r8*t**2 + 2.5419844298881856e-12_r8*log(c2) - &
! Subprogram not used    9.498107643050827e-14_r8*t*log(c2) + 7.446266520834559e-13_r8*log(c2)**2 + &
! Subprogram not used    2.4303397746137294e-11_r8*log(c3) + 1.589324325956633e-14_r8*t*log(c3) - &
! Subprogram not used    2.034596219775266e-12_r8*log(c2)*log(c3) - 5.59303954457172e-13_r8*log(c3)**2 - &
! Subprogram not used    4.889507104645867e-16_r8*t*log(c3)**2 + 1.3847024107506764e-13_r8*log(c3)**3 + &
! Subprogram not used    4.141077193427042e-15_r8*log(j) - 2.6813110884009767e-14_r8*t*log(j) + &
! Subprogram not used    1.2879071621313094e-12_r8*log(c3)*log(j) - &
! Subprogram not used    3.80352446061867e-15_r8*t*log(c3)*log(j) - 1.8790172502456827e-14_r8*log(j)**2
! Subprogram not used  
! Subprogram not used    nacid=-4.7154180661803595_r8 + 0.13436423483953885_r8*t - & 
! Subprogram not used    0.00047184686478816176_r8*t**2 - & 
! Subprogram not used    2.564010713640308_r8*log(c2) + 0.011353312899114723_r8*t*log(c2) + &
! Subprogram not used    0.0010801941974317014_r8*log(c2)**2 + 0.5171368624197119_r8*log(c3) - &
! Subprogram not used    0.0027882479896204665_r8*t*log(c3) + 0.8066971907026886_r8*log(c3)**2 - & 
! Subprogram not used    0.0031849094214409335_r8*t*log(c3)**2 - 0.09951184152927882_r8*log(c3)**3 + &
! Subprogram not used    0.00040072788891745513_r8*t*log(c3)**3 + 1.3276469271073974_r8*log(j) - &
! Subprogram not used    0.006167654171986281_r8*t*log(j) - 0.11061390967822708_r8*log(c3)*log(j) + &
! Subprogram not used    0.0004367575329273496_r8*t*log(c3)*log(j) + 0.000916366357266258_r8*log(j)**2
! Subprogram not used  
! Subprogram not used    namm=71.20073903979772_r8 - 0.8409600103431923_r8*t + &
! Subprogram not used    0.0024803006590334922_r8*t**2 + &
! Subprogram not used    2.7798606841602607_r8*log(c2) - 0.01475023348171676_r8*t*log(c2) + &
! Subprogram not used    0.012264508212031405_r8*log(c2)**2 - 2.009926050440182_r8*log(c3) + &
! Subprogram not used    0.008689123511431527_r8*t*log(c3) - 0.009141180198955415_r8*log(c2)*log(c3) + &
! Subprogram not used    0.1374122553905617_r8*log(c3)**2 - 0.0006253227821679215_r8*t*log(c3)**2 + &
! Subprogram not used    0.00009377332742098946_r8*log(c3)**3 + 0.5202974341687757_r8*log(j) - &
! Subprogram not used    0.002419872323052805_r8*t*log(j) + 0.07916392322884074_r8*log(c3)*log(j) - &
! Subprogram not used    0.0003021586030317366_r8*t*log(c3)*log(j) + 0.0046977006608603395_r8*log(j)**2
! Subprogram not used 
! Subprogram not used else
! Subprogram not used ! nucleation rate less that 5e-6, setting j_log arbitrary small
! Subprogram not used    j_log=-300._r8
! Subprogram not used end if
! Subprogram not used 
! Subprogram not used return
! Subprogram not used 
! Subprogram not used end  subroutine ternary_nuc_merik2007
! Subprogram not used 
! Subprogram not used 
! Subprogram not used 
! Subprogram not used !----------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used    end module modal_aero_newnuc
! Subprogram not used 
! Subprogram not used 
! Subprogram not used 
