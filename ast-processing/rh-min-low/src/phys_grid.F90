...
! Subprogram not used    subroutine create_chunks(opt, chunks_per_thread)
! Subprogram not used !----------------------------------------------------------------------- 
! Subprogram not used ! 
! Subprogram not used ! Purpose: Decompose physics computational grid into chunks, for
! Subprogram not used !          improved serial efficiency and parallel load balance.
! Subprogram not used ! 
! Subprogram not used ! Method: 
! Subprogram not used ! 
! Subprogram not used ! Author: Patrick Worley
...
! Subprogram not used    use dyn_grid, only: get_block_bounds_d, get_block_gcol_cnt_d, &
! Subprogram not used                        get_gcol_block_cnt_d, get_gcol_block_d, &
! Subprogram not used                        get_block_owner_d, get_block_gcol_d
! Subprogram not used !------------------------------Arguments--------------------------------
! Subprogram not used    integer, intent(in)  :: opt           ! chunking option
! Subprogram not used       !  0: chunks may cross block boundaries, but retain same
! Subprogram not used       !     process mapping as blocks. If possible, columns assigned
! Subprogram not used       !     as day/night pairs. Columns (or pairs) are wrap-mapped.
! Subprogram not used       !     May not work with vertically decomposed blocks. (default)
! Subprogram not used       !  1: chunks may cross block boundaries, but retain same
! Subprogram not used       !     SMP-node mapping as blocks.  If possible, columns assigned
! Subprogram not used       !     as day/night pairs.  Columns (or pairs) are wrap-mapped.
! Subprogram not used       !     May not work with vertically decomposed blocks.
! Subprogram not used       !  2: 2-column day/night and season column pairs wrap-mapped
! Subprogram not used       !     to chunks to also balance assignment of polar, mid-latitude, 
! Subprogram not used       !     and equatorial columns across  chunks.
! Subprogram not used       !  3: same as 1 except that SMP defined to be pairs of consecutive
! Subprogram not used       !     processes
! Subprogram not used       !  4: chunks may cross block boundaries, but retain same
! Subprogram not used       !     process mapping as blocks. Columns assigned to chunks
! Subprogram not used       !     in block ordering.
! Subprogram not used       !     May not work with vertically decomposed blocks.
! Subprogram not used       !  5: Chunks do not cross latitude boundaries, and are block-mapped.
! Subprogram not used    integer, intent(in)  :: chunks_per_thread 
! Subprogram not used                                          ! target number of chunks per
! Subprogram not used                                          !  thread
! Subprogram not used !---------------------------Local workspace-----------------------------
! Subprogram not used    integer :: i, j, p                    ! loop indices
! Subprogram not used    integer :: nlthreads                  ! number of local OpenMP threads
! Subprogram not used    integer :: npthreads(0:npes-1)        ! number of OpenMP threads per process
! Subprogram not used    integer :: proc_smp_mapx(0:npes-1)    ! process/virtual SMP node map
! Subprogram not used    integer :: firstblock, lastblock      ! global block index bounds
! Subprogram not used    integer :: maxblksiz                  ! maximum number of columns in a dynamics block
...
! Subprogram not used    integer :: nsmpx, nsmpy               ! virtual SMP node counts and indices
! Subprogram not used    integer :: curgcol, twingcol          ! global physics and dynamics column indices
! Subprogram not used    integer :: smp                        ! SMP node index
! Subprogram not used    integer :: cid                        ! chunk id
! Subprogram not used    integer :: jb, ib                     ! global block and columns indices
! Subprogram not used    integer :: blksiz                     ! current block size
! Subprogram not used    integer :: ntmp1, ntmp2, nlchunks     ! work variables
! Subprogram not used    integer :: cbeg                       ! beginning longitude index for 
! Subprogram not used                                          !  current chunk
! Subprogram not used    integer :: max_ncols                  ! upper bound on number of columns in a block
! Subprogram not used    integer :: ncols                      ! number of columns in current chunk
! Subprogram not used    logical :: error                      ! error flag 
! Subprogram not used 
! Subprogram not used    ! indices for dynamics columns in given block
! Subprogram not used    integer, dimension(:), allocatable :: cols
! Subprogram not used 
! Subprogram not used    ! number of MPI processes per virtual SMP node (0:nsmpx-1)
! Subprogram not used    integer, dimension(:), allocatable :: nsmpprocs      
! Subprogram not used 
! Subprogram not used    ! flag indicating whether a process is busy or idle during the dynamics (0:npes-1)
! Subprogram not used    logical, dimension(:), allocatable :: proc_busy_d
! Subprogram not used 
! Subprogram not used    ! flag indicating whether any of the processes assigned to an SMP node are busy 
! Subprogram not used    ! during the dynamics, or whether all of them are idle (0:nsmps-1)
! Subprogram not used    logical, dimension(:), allocatable :: smp_busy_d
! Subprogram not used 
! Subprogram not used    ! actual SMP node/virtual SMP node map (0:nsmps-1)    
! Subprogram not used    integer, dimension(:), allocatable :: smp_smp_mapx
! Subprogram not used 
! Subprogram not used    ! column/virtual SMP node map (ngcols)
! Subprogram not used    integer, dimension(:), allocatable :: col_smp_mapx
! Subprogram not used 
! Subprogram not used    ! number of columns assigned to a given virtual SMP node (0:nsmpx-1)
! Subprogram not used    integer, dimension(:), allocatable :: nsmpcolumns
! Subprogram not used 
! Subprogram not used    ! number of OpenMP threads per virtual SMP node (0:nsmpx-1)
! Subprogram not used    integer, dimension(:), allocatable :: nsmpthreads
! Subprogram not used 
! Subprogram not used    ! number of chunks assigned to a given virtual SMP node (0:nsmpx-1)
! Subprogram not used    integer, dimension(:), allocatable :: nsmpchunks
! Subprogram not used                                          
! Subprogram not used    ! maximum number of columns assigned to a chunk in a given virtual SMP node (0:nsmpx-1)
! Subprogram not used    integer, dimension(:), allocatable :: maxcol_chk
! Subprogram not used                                          
! Subprogram not used    ! number of chunks in given virtual SMP node receiving maximum number of columns 
! Subprogram not used    ! (0:nsmpx-1)
! Subprogram not used    integer, dimension(:), allocatable :: maxcol_chks
! Subprogram not used 
! Subprogram not used    ! chunk id virtual offset (0:nsmpx-1)
! Subprogram not used    integer, dimension(:), allocatable :: cid_offset
! Subprogram not used 
! Subprogram not used    ! process-local chunk id (0:nsmpx-1)
! Subprogram not used    integer, dimension(:), allocatable :: local_cid
! Subprogram not used 
! Subprogram not used 
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used !
! Subprogram not used ! Determine number of threads per process
! Subprogram not used !
! Subprogram not used    nlthreads = 1
! Subprogram not used !
! Subprogram not used    call mpiallgatherint(nlthreads, 1, npthreads, 1, mpicom)
! Subprogram not used 
! Subprogram not used !
! Subprogram not used ! Determine index range for dynamics blocks
! Subprogram not used !
! Subprogram not used    call get_block_bounds_d(firstblock,lastblock)
! Subprogram not used 
! Subprogram not used !
! Subprogram not used ! Determine maximum number of columns in a block
! Subprogram not used !
! Subprogram not used    maxblksiz = 0
! Subprogram not used    do jb=firstblock,lastblock
! Subprogram not used       maxblksiz = max(maxblksiz,get_block_gcol_cnt_d(jb))
! Subprogram not used    enddo
! Subprogram not used 
! Subprogram not used !
! Subprogram not used !  determine which (and how many) processes are assigned
! Subprogram not used !  dynamics blocks
! Subprogram not used !
! Subprogram not used    allocate( proc_busy_d(0:npes-1) )
! Subprogram not used    proc_busy_d = .false.
! Subprogram not used    nproc_busy_d = 0
! Subprogram not used    do jb=firstblock,lastblock
! Subprogram not used       p = get_block_owner_d(jb)
! Subprogram not used       if (.not. proc_busy_d(p) ) then
! Subprogram not used          proc_busy_d(p) = .true.
! Subprogram not used          nproc_busy_d = nproc_busy_d + 1
! Subprogram not used       endif
! Subprogram not used    enddo
! Subprogram not used 
! Subprogram not used !
! Subprogram not used ! Determine virtual SMP count and processes/virtual SMP map.
! Subprogram not used !  If option 0 or >3, pretend that each SMP has only one process. 
! Subprogram not used !  If option 1, use SMP information.
! Subprogram not used !  If option 2, pretend that all processes are in one SMP node. 
! Subprogram not used !  If option 3, pretend that each SMP node is made up of two
! Subprogram not used !     processes, chosen to maximize load-balancing opportunities.
! Subprogram not used !
! Subprogram not used !  For all options < 5, if there are "idle" dynamics processes, 
! Subprogram not used !     assign them to the virtual SMP nodes in wrap fashion.
! Subprogram not used !     Communication between the active and idle dynamics 
! Subprogram not used !     processes is scatter/gather (no communications between 
! Subprogram not used !     idle dynamics processes) so there is no advantage to 
! Subprogram not used !     blocking the idle processes in these assignments.
! Subprogram not used !
! Subprogram not used    if ((opt <= 0) .or. (opt == 4)) then
! Subprogram not used 
! Subprogram not used !     assign active dynamics processes to virtual SMP nodes
! Subprogram not used       nsmpx = 0
! Subprogram not used       do p=0,npes-1
! Subprogram not used          if (proc_busy_d(p)) then
! Subprogram not used             proc_smp_mapx(p) = nsmpx
! Subprogram not used             nsmpx = nsmpx + 1
! Subprogram not used          endif
! Subprogram not used       enddo
! Subprogram not used ! 
! Subprogram not used !     assign idle dynamics processes to virtual SMP nodes (wrap map)
! Subprogram not used       nsmpy = 0
! Subprogram not used       do p=0,npes-1
! Subprogram not used          if (.not. proc_busy_d(p)) then
! Subprogram not used             proc_smp_mapx(p) = nsmpy
! Subprogram not used             nsmpy = mod(nsmpy+1,nsmpx)
! Subprogram not used          endif
! Subprogram not used       enddo
! Subprogram not used 
! Subprogram not used    elseif (opt == 1) then
! Subprogram not used 
! Subprogram not used       allocate( smp_busy_d(0:nsmps-1) )
! Subprogram not used       allocate( smp_smp_mapx(0:nsmps-1) )
! Subprogram not used 
! Subprogram not used !
! Subprogram not used !     determine SMP nodes assigned dynamics blocks
! Subprogram not used       smp_busy_d = .false.
! Subprogram not used       do p=0,npes-1
! Subprogram not used          if ( proc_busy_d(p) ) then
! Subprogram not used             smp = proc_smp_map(p)
! Subprogram not used             smp_busy_d(smp) = .true.
! Subprogram not used          endif
! Subprogram not used       enddo
! Subprogram not used 
! Subprogram not used !
! Subprogram not used !     determine number of SMP nodes assigned dynamics blocks
! Subprogram not used       nsmpx = 0
! Subprogram not used       do smp=0,nsmps-1
! Subprogram not used          if (smp_busy_d(smp)) then
! Subprogram not used             smp_smp_mapx(smp) = nsmpx
! Subprogram not used             nsmpx = nsmpx + 1
! Subprogram not used          endif
! Subprogram not used       enddo
! Subprogram not used !
! Subprogram not used !     assign processes in active dynamics SMP nodes to virtual SMP nodes
! Subprogram not used       do p=0,npes-1
! Subprogram not used          smp = proc_smp_map(p)
! Subprogram not used          if (smp_busy_d(smp)) then
! Subprogram not used             proc_smp_mapx(p) = smp_smp_mapx(smp)
! Subprogram not used          endif
! Subprogram not used       enddo
! Subprogram not used ! 
! Subprogram not used !     assign processes in idle dynamics SMP nodes to virtual SMP nodes (wrap map)
! Subprogram not used       nsmpy = 0
! Subprogram not used       do p=0,npes-1
! Subprogram not used          smp = proc_smp_map(p)
! Subprogram not used          if (.not. smp_busy_d(smp)) then
! Subprogram not used             proc_smp_mapx(p) = nsmpy
! Subprogram not used             nsmpy = mod(nsmpy+1,nsmpx)
! Subprogram not used          endif
! Subprogram not used       enddo
! Subprogram not used !
! Subprogram not used       deallocate( smp_busy_d )
! Subprogram not used       deallocate( smp_smp_mapx )
! Subprogram not used 
! Subprogram not used    elseif (opt == 2) then
! Subprogram not used 
! Subprogram not used       nsmpx = 1
! Subprogram not used       do p=0,npes-1
! Subprogram not used          proc_smp_mapx(p) = 0
! Subprogram not used       enddo
! Subprogram not used 
! Subprogram not used    elseif (opt == 3) then
! Subprogram not used 
! Subprogram not used !     find active process partners
! Subprogram not used       proc_smp_mapx = -1
! Subprogram not used       call find_partners(opt,proc_busy_d,nsmpx,proc_smp_mapx)
! Subprogram not used ! 
! Subprogram not used !     assign unassigned (idle dynamics) processes to virtual SMP nodes 
! Subprogram not used !     (wrap map)
! Subprogram not used       nsmpy = 0
! Subprogram not used       do p=0,npes-1
! Subprogram not used          if (proc_smp_mapx(p) .eq. -1) then
! Subprogram not used             proc_smp_mapx(p) = nsmpy
! Subprogram not used             nsmpy = mod(nsmpy+1,nsmpx)
! Subprogram not used          endif
! Subprogram not used       enddo
...
! Subprogram not used    else
! Subprogram not used 
! Subprogram not used       nsmpx = npes
! Subprogram not used       do p=0,npes-1
! Subprogram not used          proc_smp_mapx(p) = p
! Subprogram not used       enddo
! Subprogram not used 
! Subprogram not used    endif
! Subprogram not used !
! Subprogram not used    deallocate( proc_busy_d )
! Subprogram not used 
! Subprogram not used !
! Subprogram not used ! Determine maximum number of processes assigned to a single 
! Subprogram not used ! virtual SMP node
! Subprogram not used !
! Subprogram not used    allocate( nsmpprocs(0:nsmpx-1) )
! Subprogram not used !
! Subprogram not used    nsmpprocs(:) = 0
! Subprogram not used    do p=0,npes-1
! Subprogram not used       smp = proc_smp_mapx(p)
! Subprogram not used       nsmpprocs(smp) = nsmpprocs(smp) + 1
! Subprogram not used    enddo
! Subprogram not used    max_nproc_smpx = maxval(nsmpprocs)
! Subprogram not used !
! Subprogram not used    deallocate( nsmpprocs )
! Subprogram not used 
! Subprogram not used !
! Subprogram not used ! Determine number of columns assigned to each
! Subprogram not used ! virtual SMP in block decomposition
! Subprogram not used 
! Subprogram not used    allocate( col_smp_mapx(ngcols) )
! Subprogram not used !
! Subprogram not used    col_smp_mapx(:) = -1
! Subprogram not used    error = .false.
! Subprogram not used    do i=1,ngcols_p
! Subprogram not used       curgcol = latlon_to_dyn_gcol_map(i)
! Subprogram not used       block_cnt = get_gcol_block_cnt_d(curgcol)
! Subprogram not used       call get_gcol_block_d(curgcol,block_cnt,blockids,bcids)
! Subprogram not used       do jb=1,block_cnt
! Subprogram not used          p = get_block_owner_d(blockids(jb)) 
! Subprogram not used          if (col_smp_mapx(i) .eq. -1) then
! Subprogram not used             col_smp_mapx(i) = proc_smp_mapx(p)
! Subprogram not used          elseif (col_smp_mapx(i) .ne. proc_smp_mapx(p)) then
...
! Subprogram not used                "but vertical decomposition not limited to virtual SMP"
! Subprogram not used       call endrun()
! Subprogram not used    endif
! Subprogram not used !
! Subprogram not used    allocate( nsmpcolumns(0:nsmpx-1) )
! Subprogram not used    nsmpcolumns(:) = 0
! Subprogram not used    do i=1,ngcols_p
! Subprogram not used       curgcol = latlon_to_dyn_gcol_map(i)
! Subprogram not used       smp = col_smp_mapx(curgcol)
! Subprogram not used       nsmpcolumns(smp) = nsmpcolumns(smp) + 1
! Subprogram not used    end do
! Subprogram not used !
! Subprogram not used    deallocate( col_smp_mapx )
! Subprogram not used 
! Subprogram not used !
! Subprogram not used !  Allocate other work space
! Subprogram not used !
! Subprogram not used    allocate( nsmpthreads(0:nsmpx-1) )
! Subprogram not used    allocate( nsmpchunks (0:nsmpx-1) )
! Subprogram not used    allocate( maxcol_chk (0:nsmpx-1) )
! Subprogram not used    allocate( maxcol_chks(0:nsmpx-1) )
! Subprogram not used    allocate( cid_offset (0:nsmpx-1) )
! Subprogram not used    allocate( local_cid  (0:nsmpx-1) )
! Subprogram not used    allocate( cols(1:maxblksiz) )
! Subprogram not used !
! Subprogram not used ! Options 0-3: split local dynamics blocks into chunks,
! Subprogram not used !              using wrap-map assignment of columns and
! Subprogram not used !              day/night and north/south column pairs
! Subprogram not used !              to chunks to improve load balance
! Subprogram not used !  Option 0: local is per process
! Subprogram not used !  Option 1: local is subset of`processes assigned to same SMP node
! Subprogram not used !  Option 2: local is global
! Subprogram not used !  Option 3: local is pair of processes chosen to maximize load-balance
! Subprogram not used !            wrt restriction that only communicate with one other
! Subprogram not used !            process.
! Subprogram not used ! Option 4: split local dynamics blocks into chunks,
! Subprogram not used !           using block-map assignment of columns
! Subprogram not used !             
! Subprogram not used    if ((opt >= 0) .and. (opt <= 4)) then
! Subprogram not used !
! Subprogram not used ! Calculate number of threads available in each SMP node. 
! Subprogram not used !
! Subprogram not used       nsmpthreads(:) = 0
! Subprogram not used       do p=0,npes-1
! Subprogram not used          smp = proc_smp_mapx(p)
! Subprogram not used          nsmpthreads(smp) = nsmpthreads(smp) + npthreads(p)
! Subprogram not used       enddo
! Subprogram not used !
! Subprogram not used ! Determine number of chunks to keep all threads busy
! Subprogram not used !
! Subprogram not used       nchunks = 0
! Subprogram not used       do smp=0,nsmpx-1
! Subprogram not used          nsmpchunks(smp) = nsmpcolumns(smp)/pcols
! Subprogram not used          if (mod(nsmpcolumns(smp), pcols) .ne. 0) then
! Subprogram not used             nsmpchunks(smp) = nsmpchunks(smp) + 1
! Subprogram not used          endif
! Subprogram not used          if (nsmpchunks(smp) < chunks_per_thread*nsmpthreads(smp)) then
! Subprogram not used             nsmpchunks(smp) = chunks_per_thread*nsmpthreads(smp)
! Subprogram not used          endif
! Subprogram not used          do while (mod(nsmpchunks(smp), nsmpthreads(smp)) .ne. 0)
! Subprogram not used             nsmpchunks(smp) = nsmpchunks(smp) + 1
! Subprogram not used          enddo
! Subprogram not used          if (nsmpchunks(smp) > nsmpcolumns(smp)) then
! Subprogram not used             nsmpchunks(smp) = nsmpcolumns(smp)
! Subprogram not used          endif
! Subprogram not used          nchunks = nchunks + nsmpchunks(smp)
! Subprogram not used       enddo
! Subprogram not used !
! Subprogram not used ! Determine maximum number of columns to assign to chunks
! Subprogram not used ! in a given SMP
! Subprogram not used !
! Subprogram not used       do smp=0,nsmpx-1
! Subprogram not used          if (nsmpchunks(smp) /= 0) then
! Subprogram not used             ntmp1 = nsmpcolumns(smp)/nsmpchunks(smp)
! Subprogram not used             ntmp2 = mod(nsmpcolumns(smp),nsmpchunks(smp))
! Subprogram not used             if (ntmp2 > 0) then
! Subprogram not used                maxcol_chk(smp) = ntmp1 + 1
! Subprogram not used                maxcol_chks(smp) = ntmp2
! Subprogram not used             else
! Subprogram not used                maxcol_chk(smp) = ntmp1
! Subprogram not used                maxcol_chks(smp) = nsmpchunks(smp)
! Subprogram not used             endif
! Subprogram not used          else
! Subprogram not used             maxcol_chk(smp) = 0
! Subprogram not used             maxcol_chks(smp) = 0
! Subprogram not used          endif
! Subprogram not used       enddo
! Subprogram not used !
! Subprogram not used ! Allocate chunks and knuhcs data structures
! Subprogram not used !
! Subprogram not used       allocate( chunks(1:nchunks) )
! Subprogram not used       allocate( knuhcs(1:ngcols) )
! Subprogram not used !
! Subprogram not used ! Initialize chunks and knuhcs data structures
! Subprogram not used !
! Subprogram not used       chunks(:)%ncols = 0
! Subprogram not used       knuhcs(:)%chunkid = -1
! Subprogram not used       knuhcs(:)%col = -1
! Subprogram not used !
! Subprogram not used ! Determine chunk id ranges for each SMP
! Subprogram not used !
! Subprogram not used       cid_offset(0) = 1
! Subprogram not used       local_cid(0) = 0
! Subprogram not used       do smp=1,nsmpx-1
! Subprogram not used          cid_offset(smp) = cid_offset(smp-1) + nsmpchunks(smp-1)
! Subprogram not used          local_cid(smp) = 0
! Subprogram not used       enddo
! Subprogram not used !
! Subprogram not used ! Assign columns to chunks
! Subprogram not used !
! Subprogram not used       do jb=firstblock,lastblock
! Subprogram not used          p = get_block_owner_d(jb)
! Subprogram not used          smp = proc_smp_mapx(p)
! Subprogram not used          blksiz = get_block_gcol_cnt_d(jb)
! Subprogram not used          call get_block_gcol_d(jb,blksiz,cols)
! Subprogram not used          do ib = 1,blksiz
! Subprogram not used !
! Subprogram not used ! Assign column to a chunk if not already assigned
! Subprogram not used             curgcol = cols(ib)
! Subprogram not used             if ((dyn_to_latlon_gcol_map(curgcol) .ne. -1) .and. &
! Subprogram not used                 (knuhcs(curgcol)%chunkid == -1)) then
! Subprogram not used !
! Subprogram not used ! Find next chunk with space
! Subprogram not used ! (maxcol_chks > 0 test necessary for opt=4 block map)
! Subprogram not used                cid = cid_offset(smp) + local_cid(smp)
! Subprogram not used                if (maxcol_chks(smp) > 0) then
! Subprogram not used                   do while (chunks(cid)%ncols >=  maxcol_chk(smp))
! Subprogram not used                      local_cid(smp) = mod(local_cid(smp)+1,nsmpchunks(smp))
! Subprogram not used                      cid = cid_offset(smp) + local_cid(smp)
! Subprogram not used                   enddo
! Subprogram not used                else
! Subprogram not used                   do while (chunks(cid)%ncols >=  maxcol_chk(smp)-1)
! Subprogram not used                      local_cid(smp) = mod(local_cid(smp)+1,nsmpchunks(smp))
! Subprogram not used                      cid = cid_offset(smp) + local_cid(smp)
! Subprogram not used                   enddo
! Subprogram not used                endif
! Subprogram not used                chunks(cid)%ncols = chunks(cid)%ncols + 1
! Subprogram not used                if (chunks(cid)%ncols .eq. maxcol_chk(smp)) &
! Subprogram not used                   maxcol_chks(smp) = maxcol_chks(smp) - 1
! Subprogram not used !
! Subprogram not used                i = chunks(cid)%ncols
! Subprogram not used                chunks(cid)%gcol(i) = curgcol
! Subprogram not used                chunks(cid)%lon(i)  = lon_p(curgcol)
! Subprogram not used                chunks(cid)%lat(i)  = lat_p(curgcol)
! Subprogram not used                knuhcs(curgcol)%chunkid = cid
! Subprogram not used                knuhcs(curgcol)%col = i
! Subprogram not used !
! Subprogram not used                if (opt < 4) then
! Subprogram not used !
! Subprogram not used ! If space available, look to assign a load-balancing "twin" to same chunk
! Subprogram not used                   if ( (chunks(cid)%ncols <  maxcol_chk(smp)) .and. &
! Subprogram not used                        (maxcol_chks(smp) > 0) .and. (twin_alg > 0)) then
! Subprogram not used 
! Subprogram not used                      call find_twin(curgcol, smp, &
! Subprogram not used                                     proc_smp_mapx, twingcol)
! Subprogram not used 
! Subprogram not used                      if (twingcol > 0) then
! Subprogram not used                         chunks(cid)%ncols = chunks(cid)%ncols + 1
! Subprogram not used                         if (chunks(cid)%ncols .eq. maxcol_chk(smp)) &
! Subprogram not used                            maxcol_chks(smp) = maxcol_chks(smp) - 1
! Subprogram not used !
! Subprogram not used                         i = chunks(cid)%ncols
! Subprogram not used                         chunks(cid)%gcol(i) = twingcol
! Subprogram not used                         chunks(cid)%lon(i) = lon_p(twingcol)
! Subprogram not used                         chunks(cid)%lat(i) = lat_p(twingcol)
! Subprogram not used                         knuhcs(twingcol)%chunkid = cid
! Subprogram not used                         knuhcs(twingcol)%col = i
! Subprogram not used                      endif
! Subprogram not used !
! Subprogram not used                   endif
! Subprogram not used !
! Subprogram not used ! Move on to next chunk (wrap map)
! Subprogram not used                   local_cid(smp) = mod(local_cid(smp)+1,nsmpchunks(smp))
! Subprogram not used !
! Subprogram not used                endif
! Subprogram not used !
...
! Subprogram not used    else
! Subprogram not used !
! Subprogram not used ! Option 5: split individual dynamics blocks into chunks,
! Subprogram not used !            assigning consecutive columns to the same chunk
! Subprogram not used !
! Subprogram not used ! Determine total number of chunks and
! Subprogram not used ! number of chunks in each "SMP node"
! Subprogram not used !  (assuming no vertical decomposition)
! Subprogram not used       nchunks = 0
! Subprogram not used       nsmpchunks(:) = 0
! Subprogram not used       do j=firstblock,lastblock
! Subprogram not used          blksiz = get_block_gcol_cnt_d(j)
! Subprogram not used          nlchunks = blksiz/pcols
! Subprogram not used          if (pcols*(blksiz/pcols) /= blksiz) then
! Subprogram not used             nlchunks = nlchunks + 1
! Subprogram not used          endif
! Subprogram not used          nchunks = nchunks + nlchunks
! Subprogram not used          p = get_block_owner_d(j) 
! Subprogram not used          nsmpchunks(p) = nsmpchunks(p) + nlchunks
! Subprogram not used       enddo
! Subprogram not used !
! Subprogram not used ! Determine chunk id ranges for each SMP
! Subprogram not used !
...
! Subprogram not used          call get_block_gcol_d(jb,blksiz,cols)
! Subprogram not used 
! Subprogram not used          ib = 0
! Subprogram not used          do while (ib < blksiz)
! Subprogram not used 
! Subprogram not used             cid = cid_offset(smp) + local_cid(smp)
! Subprogram not used             max_ncols = min(pcols,blksiz-ib)
! Subprogram not used 
! Subprogram not used             ncols = 0
! Subprogram not used             do i=1,max_ncols
! Subprogram not used                ib = ib + 1
! Subprogram not used                ! check whether global index is for a column that dynamics
! Subprogram not used                ! intends to pass to the physics
! Subprogram not used                curgcol = cols(ib)
! Subprogram not used                if (dyn_to_latlon_gcol_map(curgcol) .ne. -1) then
! Subprogram not used                   ! yes - then save the information
! Subprogram not used                   ncols = ncols + 1
! Subprogram not used                   chunks(cid)%gcol(ncols) = curgcol
! Subprogram not used                   chunks(cid)%lon(ncols)  = lon_p(curgcol)
! Subprogram not used                   chunks(cid)%lat(ncols)  = lat_p(curgcol)
! Subprogram not used                   knuhcs(curgcol)%chunkid = cid
! Subprogram not used                   knuhcs(curgcol)%col = ncols
! Subprogram not used                endif
! Subprogram not used             enddo
! Subprogram not used             chunks(cid)%ncols = ncols
! Subprogram not used 
! Subprogram not used             local_cid(smp) = local_cid(smp) + 1
! Subprogram not used          enddo
! Subprogram not used       enddo
! Subprogram not used !
! Subprogram not used ! Set number of threads available in each "SMP node". 
! Subprogram not used !
! Subprogram not used       do p=0,npes-1
! Subprogram not used          nsmpthreads(p) = npthreads(p)
! Subprogram not used       enddo
! Subprogram not used !
! Subprogram not used    endif
! Subprogram not used !
! Subprogram not used ! Assign chunks to processes.
! Subprogram not used !
! Subprogram not used    call assign_chunks(npthreads, nsmpx, proc_smp_mapx, &
! Subprogram not used                       nsmpthreads, nsmpchunks)
! Subprogram not used !
! Subprogram not used ! Clean up
! Subprogram not used !
! Subprogram not used    deallocate( nsmpcolumns )
! Subprogram not used    deallocate( nsmpthreads )
! Subprogram not used    deallocate( nsmpchunks  )
! Subprogram not used    deallocate( maxcol_chk  )
! Subprogram not used    deallocate( maxcol_chks )
! Subprogram not used    deallocate( cid_offset  )
! Subprogram not used    deallocate( local_cid   )
! Subprogram not used    deallocate( cols )
! Subprogram not used    deallocate( knuhcs )
! Subprogram not used 
! Subprogram not used    return
! Subprogram not used    end subroutine create_chunks
!
!========================================================================
