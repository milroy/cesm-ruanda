...
!-------------------------------------------------------------------
! Subprogram not used subroutine add_interstitial_spec(aero_loop_end,i_in)
! Subprogram not used   implicit none
! Subprogram not used   
! Subprogram not used   !Arguments
! Subprogram not used   integer, intent(in) :: i_in, aero_loop_end
! Subprogram not used   
! Subprogram not used   !Local
! Subprogram not used   character(len=32) :: spec_a
! Subprogram not used 
! Subprogram not used   !  Replace 'c' with 'a' in species name
! Subprogram not used   call spec_c_to_a(pbuf_names(i_in), spec_a)
! Subprogram not used   pbuf_names(aero_loop_end+i_in) = spec_a
! Subprogram not used end subroutine add_interstitial_spec

!-------------------------------------------------------------------
! A generic subroutine which replaces 'c' in the cloud borne 
! species name with 'a' to make it interstital species
!-------------------------------------------------------------------
! Subprogram not used subroutine spec_c_to_a (spec_c_in,spec_a_out)
! Subprogram not used   implicit none
! Subprogram not used 
! Subprogram not used   !Arguments
! Subprogram not used   character(len=*), intent(in)  :: spec_c_in 
! Subprogram not used   character(len=*), intent(out) :: spec_a_out
! Subprogram not used 
! Subprogram not used   !Local
! Subprogram not used   character(len=32)   :: name
! Subprogram not used   character(len=1000) :: errMsg
! Subprogram not used   integer             :: k_c, k_cp1
! Subprogram not used 
! Subprogram not used   k_c = index(trim(adjustl(spec_c_in)),'_c')
! Subprogram not used   if(k_c >= 1) then
! Subprogram not used      k_cp1             = k_c + 1
! Subprogram not used      name              = trim(adjustl(spec_c_in))
! Subprogram not used      name(k_cp1:k_cp1) = 'a'
! Subprogram not used      spec_a_out        = trim(adjustl(name))
! Subprogram not used   else
! Subprogram not used      write(errMsg,*) "Input string (",trim(spec_c_in)," is not a cld borne aerosol,", &
! Subprogram not used           "cannot form interstitial species name"
! Subprogram not used      call endrun(trim(errMsg))
! Subprogram not used   endif  
! Subprogram not used end subroutine spec_c_to_a
...
!-------------------------------------------------------------------
! Subprogram not used   subroutine rand_sample_prescribed_aero(state, pbuf2d)
! Subprogram not used 
! Subprogram not used     !Purpose: Generates log normal distribution for the interstitial species.
! Subprogram not used     !Note that only the interstitial aerosols are diagnosed here
! Subprogram not used     !
! Subprogram not used     !Written by Balwinder Singh (12/14/2012)
! Subprogram not used     !Adapted from Jin-Ho Yoon
! Subprogram not used     !
! Subprogram not used     !Update log:
! Subprogram not used 
! Subprogram not used     use physics_types,  only : physics_state
! Subprogram not used     use ppgrid,         only : begchunk, endchunk, pver
! Subprogram not used     use physics_buffer, only : physics_buffer_desc, pbuf_get_field, pbuf_get_chunk, &
! Subprogram not used          pbuf_get_index
...
! Subprogram not used     
! Subprogram not used     !Local
! Subprogram not used     real(r8), parameter :: mean_max_val = 5.0_r8
! Subprogram not used     real(r8), parameter :: std_max_val  = 3.0_r8
! Subprogram not used 
! Subprogram not used     integer  :: c, ncol, i, kc, klog, logv_ndx, logm_ndx, a_ndx, icol, kpver
! Subprogram not used     real(r8) :: logm2, variance, std, mean_max, std_max, randn
! Subprogram not used     type(physics_buffer_desc), pointer :: pbuf_chnk(:)
! Subprogram not used     real(r8), pointer :: data(:,:)
! Subprogram not used     real(r8) :: logm_data(pcols,pver),logv_data(pcols,pver)
! Subprogram not used 
! Subprogram not used     randn = randn_prescribed_aero()
! Subprogram not used     do i = 1, aero_cnt
! Subprogram not used        !Species with '_a' are updated using random sampling.
! Subprogram not used        !Cloud borne species (ends with _c1,_c2, etc.) have to be skipped
! Subprogram not used        kc   = index(pbuf_names(i),'_c')
! Subprogram not used        ! if kc>1, species is cloud borne species
! Subprogram not used        if( kc > 1) cycle
! Subprogram not used        
! Subprogram not used        !If species is ending with _a1, a2 etc., then find indicies of their 
! Subprogram not used        !logv and lom conterparts
! Subprogram not used        logv_ndx = logvm_get_index(pbuf_names(i),'v')
! Subprogram not used        logm_ndx = logvm_get_index(pbuf_names(i),'m')
! Subprogram not used        a_ndx    = pbuf_get_index(trim(adjustl(pbuf_names(i))))
! Subprogram not used 
! Subprogram not used        do c = begchunk, endchunk
! Subprogram not used           ncol = state(c)%ncol
! Subprogram not used           pbuf_chnk => pbuf_get_chunk(pbuf2d, c)
! Subprogram not used 
! Subprogram not used           !Get the species mixing ratio nad its logv and logm values
! Subprogram not used           call pbuf_get_field(pbuf_chnk, a_ndx, data)
! Subprogram not used           logv_data = fields(logv_ndx)%data(:,:,c)
! Subprogram not used           logm_data = fields(logm_ndx)%data(:,:,c)
! Subprogram not used 
! Subprogram not used           do icol = 1, ncol
! Subprogram not used              do kpver = 1, pver
! Subprogram not used                 logm2    = logm_data(icol,kpver) * logm_data(icol,kpver)
! Subprogram not used 
! Subprogram not used                 !Compute variance
! Subprogram not used                 variance = max( 0.0_r8, (logv_data(icol,kpver) - logm2) )
! Subprogram not used 
! Subprogram not used                 !Standard deviation
! Subprogram not used                 std      = sqrt(variance)
! Subprogram not used 
! Subprogram not used                 !Bounds to keep mixing ratios from going unphysical
! Subprogram not used                 mean_max = exp(logm_data(icol,kpver)) * mean_max_val
! Subprogram not used                 std_max  = exp(logm_data(icol,kpver)  + std_max_val * std )
! Subprogram not used 
! Subprogram not used                 data(icol,kpver) = min(exp(logm_data(icol,kpver)+randn*std),mean_max,std_max)
! Subprogram not used 
! Subprogram not used              enddo !pver
! Subprogram not used           enddo !col
! Subprogram not used        enddo !chunk
! Subprogram not used     enddo !flds
! Subprogram not used    
! Subprogram not used   end subroutine rand_sample_prescribed_aero
!-------------------------------------------------------------------
! Subprogram not used   function randn_prescribed_aero()
! Subprogram not used     !Pupose: This function generates a new normally distributed random
! Subprogram not used     ! number at end end of each day. This random number then stays the same
! Subprogram not used     ! for the whole day
! Subprogram not used     !
! Subprogram not used     !Written by Balwinder Singh (12/14/2012)
! Subprogram not used     !Adapted from Jin-Ho Yoon
! Subprogram not used     !
! Subprogram not used     !Update log:
! Subprogram not used     
! Subprogram not used     use time_manager,   only : is_end_curr_day, is_first_step, get_nstep
! Subprogram not used     
! Subprogram not used     integer, parameter :: rconst1_1 = 5000000
! Subprogram not used     integer, parameter :: rconst1_2 = 50
! Subprogram not used     integer, parameter :: rconst2_1 = 10000000
! Subprogram not used     integer, parameter :: rconst2_2 = 10
! Subprogram not used     
! Subprogram not used     integer :: i, seed_size, nstep
! Subprogram not used     integer, allocatable :: seed(:)
! Subprogram not used 
! Subprogram not used     real(r8) :: randn_prescribed_aero
! Subprogram not used     real(r8) :: randu1, randu2
! Subprogram not used     
! Subprogram not used     !Use same random number for the entire day and generate a new normally 
! Subprogram not used     !distributed random number at the start of the new day
! Subprogram not used     if(is_first_step() .or. is_end_curr_day()) then
! Subprogram not used        !Generate two uniformly distributed random numbers (between 0 and 1)
! Subprogram not used        call random_seed(size=seed_size)
! Subprogram not used        allocate(seed(seed_size))
! Subprogram not used 
! Subprogram not used        ! Using nstep as a seed to generate same sequence
! Subprogram not used        nstep = get_nstep()
! Subprogram not used        do i = 1, seed_size
! Subprogram not used           seed(i) = rconst1_1*nstep + rconst1_2*(i-1)
! Subprogram not used        end do
! Subprogram not used        call random_seed(put=seed)
! Subprogram not used        call random_number (randu1)
! Subprogram not used 
! Subprogram not used        do i = 1, seed_size
! Subprogram not used           seed(i) = rconst2_1*nstep + rconst2_2*(i-1)
! Subprogram not used        end do
! Subprogram not used        call random_seed(put=seed)
! Subprogram not used        call random_number (randu2)
! Subprogram not used        deallocate(seed)
! Subprogram not used       
! Subprogram not used        !Normal distribution (Mean = 0, standard dev = 1)
! Subprogram not used        randn_prescribed_aero = boxMuller(randu1,randu2)
! Subprogram not used        randn_persists = randn_prescribed_aero
! Subprogram not used     else
! Subprogram not used        !Use the previously generated random number
! Subprogram not used        randn_prescribed_aero = randn_persists
! Subprogram not used     endif
! Subprogram not used   end function randn_prescribed_aero
!-------------------------------------------------------------------
! Subprogram not used   function logvm_get_index(name,type) result(index)
! Subprogram not used     implicit none
! Subprogram not used         
! Subprogram not used     !Args
! Subprogram not used     character(len=*), intent(in) :: name, type    
! Subprogram not used 
! Subprogram not used     !Loc
! Subprogram not used     character(len=64)   :: tmp_name
! Subprogram not used     character(len=1000) :: msgStr
! Subprogram not used     integer :: index, i
! Subprogram not used     
! Subprogram not used     
! Subprogram not used     index = -1
! Subprogram not used     tmp_name = trim(adjustl(name))//'_log'//trim(adjustl(type))
! Subprogram not used            
! Subprogram not used     fldloop: do i = 1, number_flds
! Subprogram not used        if(fields(i)%fldnam == tmp_name) then
! Subprogram not used           index = i       
! Subprogram not used           exit fldloop
! Subprogram not used        endif
! Subprogram not used     enddo fldloop
! Subprogram not used     
! Subprogram not used     if(index == -1) then
! Subprogram not used        write(msgStr,*) "Prescribed_aero.F90: ",tmp_name," doesn't exist in the fields%fldnam"
! Subprogram not used        call endrun(msgStr)
! Subprogram not used     endif
! Subprogram not used     
! Subprogram not used   end function logvm_get_index
!-------------------------------------------------------------------
! Subprogram not used   function boxMuller(ru1,ru2) result(rn)
! Subprogram not used     use physconst,      only : pi 
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     !Args
! Subprogram not used     real(r8), intent(in)  :: ru1, ru2
! Subprogram not used 
! Subprogram not used     !Loc
! Subprogram not used     real(r8), parameter :: pi2 = 2._r8 * pi    
! Subprogram not used     real(r8) :: ur, theta, rn
! Subprogram not used 
! Subprogram not used     !Based on Box Muller Method, generate normally distributed random numbers 
! Subprogram not used     ur    = sqrt(-2._r8 * log(ru1))
! Subprogram not used     theta = pi2 * ru2
! Subprogram not used     
! Subprogram not used     !Normal distribution (Mean = 0, standard dev = 1)
! Subprogram not used     rn = ur * cos(theta)
! Subprogram not used     
! Subprogram not used   end function boxMuller

!-------------------------------------------------------------------
!-------------------------------------------------------------------
! Subprogram not used   subroutine init_prescribed_aero_restart( piofile )
! Subprogram not used     use pio, only : file_desc_t, pio_def_var, pio_double 
...
! Subprogram not used 
! Subprogram not used     ! For allowing randn_persists to persist during reststarts
! Subprogram not used     ierr = pio_def_var(piofile, randn_persists_name, pio_double, randn_persists_desc)
! Subprogram not used 
! Subprogram not used     call init_trc_restart( 'prescribed_aero', piofile, file )
! Subprogram not used 
! Subprogram not used   end subroutine init_prescribed_aero_restart
!-------------------------------------------------------------------
! Subprogram not used   subroutine write_prescribed_aero_restart( piofile )
! Subprogram not used     use tracer_data, only : write_trc_restart
! Subprogram not used     use pio, only : file_desc_t, pio_put_var 
! Subprogram not used     implicit none
! Subprogram not used 
...
! Subprogram not used 
! Subprogram not used     !  For allowing randn_persists to persist during reststarts
! Subprogram not used     ierr = pio_put_var(piofile, randn_persists_desc, (/randn_persists/)) 
! Subprogram not used 
! Subprogram not used     call write_trc_restart( piofile, file )
! Subprogram not used 
! Subprogram not used   end subroutine write_prescribed_aero_restart

!-------------------------------------------------------------------
!-------------------------------------------------------------------
! Subprogram not used   subroutine read_prescribed_aero_restart( pioFile )
! Subprogram not used     use tracer_data, only : read_trc_restart
! Subprogram not used     use pio, only : file_desc_t, pio_inq_varid, pio_get_var 
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     type(file_desc_t) :: piofile    
! Subprogram not used     integer :: ierr
! Subprogram not used 
! Subprogram not used     ! For allowing randn_persists to persist during reststarts
! Subprogram not used     ierr = pio_inq_varid(pioFile, randn_persists_name, randn_persists_desc)
! Subprogram not used     ierr = pio_get_var(pioFile, randn_persists_desc, randn_persists)
! Subprogram not used 
! Subprogram not used     call read_trc_restart( 'prescribed_aero', piofile, file )
! Subprogram not used 
! Subprogram not used   end subroutine read_prescribed_aero_restart
