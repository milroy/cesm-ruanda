...
! SVN $URL: https://svn-ccsm-models.cgd.ucar.edu/csm_share/trunk_tags/share3_140626/shr/shr_tInterp_mod.F90 $
!===============================================================================
!BOP ===========================================================================
!
...
! Subprogram not used subroutine shr_tInterp_getAvgCosz(tavCosz,lonr,latr,ymd1,tod1,ymd2,tod2,eccen,mvelpp,lambm0,obliqr,dt,&
! Subprogram not used 	                          calendar)
! Subprogram not used 
...
! Subprogram not used 
! Subprogram not used    real(SHR_KIND_R8)   ,intent(out)  :: tavCosz(:) ! t-avg of cosz over [LB,UB]
! Subprogram not used    real(SHR_KIND_R8)   ,intent(in)   :: latr(:)    ! latitude
! Subprogram not used    real(SHR_KIND_R8)   ,intent(in)   :: lonr(:)    ! longitude
! Subprogram not used    integer(SHR_KIND_IN),intent(in)   :: ymd1,tod1  ! date of lb
! Subprogram not used    integer(SHR_KIND_IN),intent(in)   :: ymd2,tod2  ! date of ub
! Subprogram not used    real(SHR_KIND_R8)   ,intent(in)   :: eccen      ! orb param
! Subprogram not used    real(SHR_KIND_R8)   ,intent(in)   :: mvelpp     ! orb param
! Subprogram not used    real(SHR_KIND_R8)   ,intent(in)   :: lambm0     ! orb param
! Subprogram not used    real(SHR_KIND_R8)   ,intent(in)   :: obliqr     ! orb param
! Subprogram not used    integer(SHR_KIND_IN),intent(in)   :: dt         ! model time step in secs
! Subprogram not used    character(*)        ,intent(in)   :: calendar   ! calendar type
! Subprogram not used 
! Subprogram not used !EOP
! Subprogram not used 
! Subprogram not used    !----- local  ------
! Subprogram not used    integer(SHR_KIND_IN) :: lsize              ! size of local data
! Subprogram not used    type(ESMF_Time)      :: reday1, reday2     ! LB, UB time
! Subprogram not used    type(ESMF_TimeInterval) :: timeint         ! time interval
! Subprogram not used    integer(SHR_KIND_I8) :: n                  ! number of time-samples in t-avg
! Subprogram not used    type(ESMF_Time)      :: reday,reday0       ! time sample's elapsed seconds
! Subprogram not used    integer(SHR_KIND_IN) :: ymd,tod,ymd0,tod0  ! used to compute time of time-sample
! Subprogram not used    integer(SHR_KIND_IN) :: ldt                ! local dt as needed
! Subprogram not used    integer(SHR_KIND_I8) :: ldt8               ! local dt as needed in i8
! Subprogram not used    integer(SHR_KIND_I8) :: dtsec              ! delta time from timeint 
! Subprogram not used    real(SHR_KIND_R8),pointer :: cosz(:)       ! cos(zenith angle)
! Subprogram not used 
! Subprogram not used    !----- formats -----
! Subprogram not used    character(*),parameter :: subName = "(shr_tInterp_getAvgCosz) "
! Subprogram not used    character(*),parameter :: F00   = "('(shr_tInterp_getAvgCosz) ',8a)" 
! Subprogram not used 
! Subprogram not used !-------------------------------------------------------------------------------
! Subprogram not used ! Computes time avg cosz over interval [LB,UB]
! Subprogram not used !-------------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used    lsize = size(lonr)
! Subprogram not used    allocate(cosz(lsize))
! Subprogram not used    if (lsize < 1 .or. size(latr) /= lsize .or. size(tavCosz) /= lsize) then
! Subprogram not used       call shr_sys_abort(subname//' ERROR: lon lat tavCosz sizes disagree')
! Subprogram not used    endif
! Subprogram not used 
! Subprogram not used    ldt = dt
! Subprogram not used 
! Subprogram not used    !--- get LB & UB dates ---
! Subprogram not used    call shr_cal_timeSet(reday1,ymd1,tod1,calendar)
! Subprogram not used    call shr_cal_timeSet(reday2,ymd2,tod2,calendar)
! Subprogram not used    if (reday1 > reday2) call shr_sys_abort(subname//'ERROR: lower-bound > upper-bound')
! Subprogram not used 
! Subprogram not used    timeint = reday2-reday1
! Subprogram not used    call ESMF_TimeIntervalGet(timeint,s_i8=dtsec)
! Subprogram not used    ldt8 = ldt
! Subprogram not used    if (mod(dtsec,ldt8) /= 0) then
! Subprogram not used       ldt8 = (dtsec)/((dtsec)/ldt8+1)
! Subprogram not used       ldt = ldt8
! Subprogram not used    endif
! Subprogram not used 
! Subprogram not used    !--- compute time average ---
! Subprogram not used    tavCosz = 0.0_SHR_KIND_R8 ! initialize partial sum
! Subprogram not used    n       = 0               ! dt weighted average in t-avg
! Subprogram not used    reday   = reday1          ! mid [LB,UB] interval t-step starts at LB 
! Subprogram not used    ymd     = ymd1
! Subprogram not used    tod     = tod1
! Subprogram not used    do while( reday < reday2) ! mid-interval t-steps thru interval [LB,UB]
! Subprogram not used 
! Subprogram not used       !--- advance to next time in [LB,UB] ---
! Subprogram not used       ymd0 = ymd
! Subprogram not used       tod0 = tod
! Subprogram not used       reday0 = reday
! Subprogram not used       call shr_cal_advDateInt(ldt,'seconds',ymd0,tod0,ymd,tod,calendar)
! Subprogram not used       call shr_cal_timeSet(reday,ymd,tod,calendar)
! Subprogram not used 
! Subprogram not used       if (reday > reday2) then
! Subprogram not used          ymd = ymd2
! Subprogram not used          tod = tod2
! Subprogram not used          timeint = reday2-reday0
! Subprogram not used          call ESMF_TimeIntervalGet(timeint,s_i8=dtsec)
! Subprogram not used          ldt = dtsec
! Subprogram not used       endif
! Subprogram not used 
! Subprogram not used       !--- get next cosz value for t-avg ---
! Subprogram not used       call shr_tInterp_getCosz(cosz,lonr,latr,ymd,tod,eccen,mvelpp,lambm0,obliqr,calendar)
! Subprogram not used       n = n + ldt
! Subprogram not used       tavCosz = tavCosz + cosz*real(ldt,SHR_KIND_R8)  ! add to partial sum
! Subprogram not used 
! Subprogram not used    end do
! Subprogram not used    tavCosz = tavCosz/real(n,SHR_KIND_R8) ! form t-avg
! Subprogram not used 
! Subprogram not used    deallocate( cosz )
! Subprogram not used 
! Subprogram not used end subroutine shr_tInterp_getAvgCosz
...
! Subprogram not used subroutine shr_tInterp_getCosz(cosz,lonr,latr,ymd,tod,eccen,mvelpp,lambm0,obliqr,calendar)
! Subprogram not used 
! Subprogram not used    implicit none
! Subprogram not used 
! Subprogram not used ! !USES:
...
! Subprogram not used ! !INPUT/OUTPUT PARAMETERS:
! Subprogram not used 
! Subprogram not used    real(SHR_KIND_R8)   ,intent(out)   :: cosz(:)    ! cos(zenith angle)
! Subprogram not used    real(SHR_KIND_R8)   ,intent(in)    :: latr(:)    ! latitude
! Subprogram not used    real(SHR_KIND_R8)   ,intent(in)    :: lonr(:)    ! longitude
! Subprogram not used    integer(SHR_KIND_IN),intent(in)    :: ymd,tod    ! date of interest
! Subprogram not used    real(SHR_KIND_R8)   ,intent(in)    :: eccen      ! orb param
! Subprogram not used    real(SHR_KIND_R8)   ,intent(in)    :: mvelpp     ! orb param
! Subprogram not used    real(SHR_KIND_R8)   ,intent(in)    :: lambm0     ! orb param
! Subprogram not used    real(SHR_KIND_R8)   ,intent(in)    :: obliqr     ! orb param
...
! Subprogram not used    real(SHR_KIND_R8)    :: calday      ! julian days
! Subprogram not used    real(SHR_KIND_R8)    :: declin,eccf ! orb params
! Subprogram not used 
! Subprogram not used    real(SHR_KIND_R8),parameter :: solZenMin = 0.001_SHR_KIND_R8 ! min solar zenith angle
! Subprogram not used 
! Subprogram not used    !----- formats -----
! Subprogram not used    character(*),parameter :: subName = "(shr_tInterp_getCosz) "
! Subprogram not used    character(*),parameter :: F00   = "('(shr_tInterp_getCosz) ',8a)" 
! Subprogram not used 
! Subprogram not used !-------------------------------------------------------------------------------
! Subprogram not used ! Returns cos(zenith angle)
! Subprogram not used !-------------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used    lsize = size(lonr)
! Subprogram not used    if (lsize < 1 .or. size(latr) /= lsize .or. size(cosz) /= lsize) then
! Subprogram not used       call shr_sys_abort(subname//' ERROR: lon lat cosz sizes disagree')
! Subprogram not used    endif
! Subprogram not used 
! Subprogram not used    call shr_cal_date2julian(ymd,tod,calday,calendar)
! Subprogram not used    call shr_orb_decl( calday, eccen, mvelpp, lambm0, obliqr, declin, eccf)
! Subprogram not used    do n = 1,lsize
! Subprogram not used       cosz(n) = max(solZenMin, shr_orb_cosz( calday, latr(n), lonr(n), declin ))
! Subprogram not used    end do
! Subprogram not used 
! Subprogram not used end subroutine shr_tInterp_getCosz
...
! Subprogram not used subroutine shr_tInterp_setAbort(flag)
! Subprogram not used 
! Subprogram not used   implicit none
! Subprogram not used 
! Subprogram not used ! !INPUT/OUTPUT PARAMETERS:
...
! Subprogram not used 
! Subprogram not used   !--- formats ---
! Subprogram not used   character(*),parameter :: subName =   "(shr_tInterp_setAbort) "
! Subprogram not used   character(*),parameter :: F00     = "('(shr_tInterp_setAbort) ',a) "
! Subprogram not used 
! Subprogram not used !-------------------------------------------------------------------------------
! Subprogram not used !
! Subprogram not used !-------------------------------------------------------------------------------
...
! Subprogram not used 
! Subprogram not used end subroutine shr_tInterp_setAbort
...
! Subprogram not used subroutine shr_tInterp_abort(string)
! Subprogram not used 
! Subprogram not used   implicit none
! Subprogram not used 
! Subprogram not used ! !INPUT/OUTPUT PARAMETERS:
...
! Subprogram not used   character(*),parameter :: subName =   "(shr_tInterp_abort) "
! Subprogram not used   character(*),parameter :: F00     = "('(shr_tInterp_abort) ',a) "
! Subprogram not used 
! Subprogram not used !-------------------------------------------------------------------------------
! Subprogram not used !
! Subprogram not used !-------------------------------------------------------------------------------
...
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used end subroutine shr_tInterp_abort
...
! Subprogram not used subroutine shr_tInterp_getDebug(level)
! Subprogram not used 
! Subprogram not used   implicit none
! Subprogram not used 
! Subprogram not used ! !INPUT/OUTPUT PARAMETERS:
...
! Subprogram not used 
! Subprogram not used   !--- formats ---
! Subprogram not used   character(*),parameter :: subName =   "(shr_tInterp_getDebug) "
! Subprogram not used   character(*),parameter :: F00     = "('(shr_tInterp_getDebug) ',a) "
! Subprogram not used 
! Subprogram not used !-------------------------------------------------------------------------------
! Subprogram not used !
! Subprogram not used !-------------------------------------------------------------------------------
...
! Subprogram not used 
! Subprogram not used end subroutine shr_tInterp_getDebug
...
! Subprogram not used subroutine shr_tInterp_setDebug(iflag)
! Subprogram not used 
! Subprogram not used   implicit none
! Subprogram not used 
! Subprogram not used ! !INPUT/OUTPUT PARAMETERS:
...
! Subprogram not used 
! Subprogram not used   !--- formats ---
! Subprogram not used   character(*),parameter :: subName =   "(shr_tInterp_setDebug) "
! Subprogram not used   character(*),parameter :: F01     = "('(shr_tInterp_setDebug) ',a,i3) "
! Subprogram not used 
! Subprogram not used !-------------------------------------------------------------------------------
! Subprogram not used !
! Subprogram not used !-------------------------------------------------------------------------------
...
! Subprogram not used   if (debug>0 .and. s_loglev > 0) write(s_logunit,F01) "DEBUG: level changed to ",debug
! Subprogram not used 
! Subprogram not used end subroutine shr_tInterp_setDebug
