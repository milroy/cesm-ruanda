...
! Subprogram not used 
! Subprogram not used       subroutine aurora_inti
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used ! 	... initialize aurora module
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used       use ppgrid,       only : pver
! Subprogram not used       use pmgrid,       only : plev, plevp
! Subprogram not used       use constituents, only : cnst_get_ind, cnst_mw
...
! Subprogram not used       real(r8), parameter :: e       = 1.e-10_r8
! Subprogram not used       real(r8), parameter :: convert = 3.1211e8_r8
! Subprogram not used 
! Subprogram not used       real(r8) :: plb
! Subprogram not used       real(r8) :: alfa_1, alfa_2, alfa21, alfa22
! Subprogram not used       real(r8) :: e21, e22
! Subprogram not used 
! Subprogram not used       integer :: op_ndx,o2p_ndx,np_ndx,n2p_ndx,e_ndx
! Subprogram not used 
! Subprogram not used       op_ndx   = get_spc_ndx( 'Op' )
! Subprogram not used       o2p_ndx  = get_spc_ndx( 'O2p' )
! Subprogram not used       np_ndx   = get_spc_ndx( 'Np' )
! Subprogram not used       n2p_ndx  = get_spc_ndx( 'N2p' )
! Subprogram not used       e_ndx    = get_spc_ndx( 'e' )
! Subprogram not used 
! Subprogram not used       has_ions = op_ndx > 0 .and. o2p_ndx > 0 .and. np_ndx > 0 .and. n2p_ndx > 0 .and. e_ndx > 0
! Subprogram not used 
! Subprogram not used       if (.not. has_ions) return
! Subprogram not used 
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used !	... initialize module variables
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used       twopi  = 2._r8*pi
! Subprogram not used       dtr    = pi/180._r8
! Subprogram not used 
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used !	... set molecular weights
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used       call cnst_get_ind( 'O2', m )
! Subprogram not used       rmass_o2    = cnst_mw(m)
! Subprogram not used       rmassinv_o2 = 1._r8/rmass_o2
! Subprogram not used       call cnst_get_ind( 'O', m )
! Subprogram not used       rmass_o1    = cnst_mw(m)
! Subprogram not used       rmassinv_o1 = 1._r8/rmass_o1
! Subprogram not used       call cnst_get_ind( 'N', m )
! Subprogram not used       rmass_n2    = 2._r8*cnst_mw(m)
! Subprogram not used       rmassinv_n2 = 1._r8/rmass_n2
! Subprogram not used 
! Subprogram not used       offa(isouth)   = 4.3_r8*dtr
! Subprogram not used       offa(inorth)   = 3.7_r8*dtr
! Subprogram not used       phid(isouth)   = 0._r8
! Subprogram not used       phid(inorth)   = 0._r8
! Subprogram not used       alfa_1         = 2._r8
! Subprogram not used       alfa_2         = 2._r8
! Subprogram not used 
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used ! Values from 10/05/94 HPI estimates (50% or more higher than old estimates):
! Subprogram not used !     alfa_1 = amin1(1.5,1.25+0.05*plevel)
! Subprogram not used !     alfa_2 = 1.2 + 0.095*plevel
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used       alfa0  = 0.5_r8*(alfa_1 + alfa_2)
! Subprogram not used       ralfa  = (alfa_2 - alfa_1) / (alfa_1 + alfa_2 + e)
! Subprogram not used       alfa21 = 0.075_r8
! Subprogram not used       alfa22 = 0.075_r8
! Subprogram not used       alfa20 = 0.5_r8 * (alfa21 + alfa22)
! Subprogram not used       ralfa2 = (alfa22 - alfa21) / (alfa21 + alfa22 + e)
! Subprogram not used       e21    = 1.e-80_r8
! Subprogram not used       e22    = 1.e-80_r8
! Subprogram not used       e20    = 0.5_r8 * (e21 + e22)
! Subprogram not used       re2    = (e22 - e21) / (e21 + e22)
! Subprogram not used 
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used ! Set cusp and drizzle parameters:
! Subprogram not used ! (conversion between particle number density and characteristic
! Subprogram not used !  energy and column energy input)
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used       fc = convert * ec / alfac
! Subprogram not used       fd = convert * ed / alfad
! Subprogram not used 
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used ! Solar proton flux:
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used       flx_sp = e_sp/1.602e-6_r8
! Subprogram not used 
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used ! 	... set auroral lower bndy index
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used       plb = 5.e-4_r8*exp( 7._r8 ) * .1_r8             ! Pa
! Subprogram not used       do k = 1,pver
! Subprogram not used 	 if( pref_mid(k) >= plb ) then
! Subprogram not used 	    lev1 = k-1
! Subprogram not used 	    exit
! Subprogram not used 	 end if
! Subprogram not used       end do
! Subprogram not used 
! Subprogram not used       if (masterproc) write(iulog,*) ' '
! Subprogram not used       if (masterproc) write(iulog,*) 'aurora_inti: aurora will go down to lev,p = ',lev1,pref_mid(lev1)
! Subprogram not used       if (masterproc) write(iulog,*) ' '
! Subprogram not used 
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used ! Report to stdout:
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used         call addfld('ALATM   ','RADIANS ',1,'I',&
! Subprogram not used              'Magnetic latitude at each geographic coordinate',phys_decomp)
! Subprogram not used         call addfld('ALONM   ','RADIANS ',1,'I',&
! Subprogram not used              'Magnetic longitude at each geographic coordinate',phys_decomp)
! Subprogram not used         call addfld( 'QSUM', '/s ', pver, 'I', &
! Subprogram not used              'total ion production', phys_decomp )
! Subprogram not used 
! Subprogram not used       end subroutine aurora_inti
