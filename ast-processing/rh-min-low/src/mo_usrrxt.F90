...
! Subprogram not used       subroutine usrrxt_hrates( rxt, tempn, tempi, tempe, invariants, &
! Subprogram not used 				h2ovmr, pmid, m, ncol, kbot )
! Subprogram not used !-----------------------------------------------------------------
! Subprogram not used !        ... set the user specified reaction rates for heating
! Subprogram not used !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used       use shr_kind_mod,  only : r8 => shr_kind_r8
! Subprogram not used       use chem_mods,     only : nfs, rxntot
...
! Subprogram not used       real(r8), intent(in)    :: tempn(pcols,pver)            ! neutral temperature (K)
! Subprogram not used       real(r8), intent(in)    :: tempi(pcols,pver)            ! ion temperature (K)
! Subprogram not used       real(r8), intent(in)    :: tempe(pcols,pver)            ! electron temperature (K)
! Subprogram not used       real(r8), intent(in)    :: m(ncol,pver)                 ! total atm density (1/cm^3)
! Subprogram not used       real(r8), intent(in)    :: h2ovmr(ncol,pver)            ! water vapor (vmr)
! Subprogram not used       real(r8), intent(in)    :: pmid(pcols,pver)             ! midpoint pressure (Pa)
! Subprogram not used       real(r8), intent(in)    :: invariants(ncol,pver,nfs)    ! invariants density (1/cm^3)
! Subprogram not used       real(r8), intent(inout) :: rxt(ncol,pver,rxntot)        ! gas phase rates
! Subprogram not used       
! Subprogram not used !-----------------------------------------------------------------
...
! Subprogram not used                    tp, &
! Subprogram not used                    tinv, &
! Subprogram not used                    ko, &
! Subprogram not used                    kinf, &
! Subprogram not used                    fc, &
! Subprogram not used                    xr                       ! factor to increase particle radii depending on rel hum
! Subprogram not used 
! Subprogram not used !-----------------------------------------------------------------
! Subprogram not used !	... o + o2 + m --> o3 + m
! Subprogram not used !-----------------------------------------------------------------
! Subprogram not used       do k = 1,kbot
! Subprogram not used          tinv(:ncol)       = 1._r8 / tempn(:ncol,k)
! Subprogram not used          tp(:)             = 300._r8 * tinv(:)
! Subprogram not used          rxt(:,k,usr_O_O2_ndx) = 6.e-34_r8 * tp(:)**2.4_r8
! Subprogram not used 
! Subprogram not used !-----------------------------------------------------------------
! Subprogram not used !	... o + o + m -> o2 + m
! Subprogram not used !-----------------------------------------------------------------
! Subprogram not used          rxt(:,k,usr_O_O_ndx) = 2.76e-34_r8 * exp( 720.0_r8*tinv(:) )
! Subprogram not used 
! Subprogram not used !-----------------------------------------------------------------
! Subprogram not used !	... ho2 + ho2 --> h2o2
! Subprogram not used !	Note: this rate involves the water vapor number density
! Subprogram not used !-----------------------------------------------------------------
! Subprogram not used          ko(:)   = 3.0e-13_r8  * exp( 460._r8*tinv(:) )
! Subprogram not used          kinf(:) = 2.1e-33_r8 * m(:,k) * exp( 920._r8*tinv(:) )
! Subprogram not used          fc(:)   = 1._r8 + 1.4e-21_r8 * m(:,k) * h2ovmr(:,k) * exp( 2200._r8*tinv(:) )
! Subprogram not used          rxt(:,k,usr_HO2_HO2_ndx) = (ko(:) + kinf(:)) * fc(:)
! Subprogram not used 
! Subprogram not used       end do
! Subprogram not used 
! Subprogram not used !-----------------------------------------------------------------
! Subprogram not used ! 	... the ionic rates
! Subprogram not used !-----------------------------------------------------------------
! Subprogram not used       if ( has_ion_rxts ) then
! Subprogram not used          level_loop2 :  do k = 1,kbot
! Subprogram not used             tp(:ncol)         = (2._r8*tempi(:ncol,k) + tempn(:ncol,k)) / ( 3._r8 * t0 )
! Subprogram not used             tp(:)             = max( min( tp(:),20._r8 ),1._r8 )
! Subprogram not used             rxt(:,k,ion1_ndx) = 2.82e-11_r8 + tp(:)*(-7.74e-12_r8 + tp(:)*(1.073e-12_r8  &
! Subprogram not used                  + tp(:)*(-5.17e-14_r8 + 9.65e-16_r8*tp(:))))
! Subprogram not used             tp(:ncol)         = (.6363_r8*tempi(:ncol,k) + .3637_r8*tempn(:ncol,k)) / t0
! Subprogram not used             tp(:)             = max( min( tp(:),trlim2 ),1._r8 )
! Subprogram not used             rxt(:,k,ion2_ndx) = 1.533e-12_r8 + tp(:)*(-5.92e-13_r8 + tp(:)*8.6e-14_r8)
! Subprogram not used             tp(:ncol)         = 2._r8 * t0 /(tempi(:ncol,k) + tempn(:ncol,k))
! Subprogram not used             where( tp(:ncol) < trlim3 )
! Subprogram not used                rxt(:,k,ion3_ndx)  = 1.4e-10_r8 * tp(:)**.44_r8
! Subprogram not used             elsewhere
! Subprogram not used                rxt(:,k,ion3_ndx)  = 5.2e-11_r8 / tp(:)**.2_r8
! Subprogram not used             endwhere
! Subprogram not used             tp(:ncol)          = t0 / tempe(:ncol,k)
! Subprogram not used             rxt(:,k,elec1_ndx) = 4.e-7_r8 * tp(:)**.85_r8
! Subprogram not used             rxt(:,k,elec3_ndx) = 1.8e-7_r8 * tp(:)**.39_r8
! Subprogram not used             where( tp(:ncol) < 4._r8 )
! Subprogram not used                rxt(:,k,elec2_ndx) = 2.7e-7_r8 * tp(:)**.7_r8
! Subprogram not used             elsewhere
! Subprogram not used                rxt(:,k,elec2_ndx) = 1.6e-7_r8 * tp(:)**.55_r8
! Subprogram not used             endwhere
! Subprogram not used          end do level_loop2
! Subprogram not used       endif
! Subprogram not used       end subroutine usrrxt_hrates
...
!-------------------------------------------------------------------------
  ...
  !  Heterogeneous reaction rates for uptake of a gas on an aerosol:
  !-------------------------------------------------------------------------
! Subprogram not used   function hetrxtrate( sfc, dm_aer, dg_gas, c_gas, gamma_gas ) result(rate)
! Subprogram not used 
! Subprogram not used     real(r8), intent(in) :: sfc(:)
! Subprogram not used     real(r8), intent(in) :: dm_aer(:)
! Subprogram not used     real(r8), intent(in) :: dg_gas
! Subprogram not used     real(r8), intent(in) :: c_gas
! Subprogram not used     real(r8), intent(in) :: gamma_gas
! Subprogram not used     real(r8) :: rate
! Subprogram not used 
! Subprogram not used     real(r8),allocatable :: rxt(:)
! Subprogram not used     integer :: n, i
! Subprogram not used 
! Subprogram not used     n = size(sfc)
! Subprogram not used 
! Subprogram not used     allocate(rxt(n))
! Subprogram not used     do i=1,n
! Subprogram not used        rxt(i) = sfc(i) / (0.5_r8*dm_aer(i)/dg_gas + (4._r8/(c_gas*gamma_gas)))
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used     rate = sum(rxt)
! Subprogram not used 
! Subprogram not used     deallocate(rxt)
! Subprogram not used 
! Subprogram not used   endfunction hetrxtrate
! Subprogram not used 
! Subprogram not used end module mo_usrrxt
