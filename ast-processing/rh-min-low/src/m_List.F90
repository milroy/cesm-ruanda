...
!\begin{tabular}{|c|c|c|}
!\hline
!{\bf Item} & {\bf Starting Point in Buffer} & {\bf Length} \!\hline

!{\tt latitude} & 1 & 8 \!\hline

!{\tt longitude} & 9 & 9 \!\hline

!{\tt pressure} & 20 & 8\!\hline
...
! Subprogram not used  subroutine initStr1_(aList, strs)
! Subprogram not used 
! Subprogram not used ! !USES:
! Subprogram not used !
! Subprogram not used       use m_String, only : String,toChar
! Subprogram not used       use m_String, only : len
! Subprogram not used       use m_String, only : ptr_chars
...
! Subprogram not used !
! Subprogram not used       type(String),dimension(:),intent(in)	  :: strs
! Subprogram not used 
! Subprogram not used ! !OUTPUT PARAMETERS:   
! Subprogram not used !
! Subprogram not used       type(List),intent(out)	  :: aList  ! an indexed string values
! Subprogram not used 
! Subprogram not used 
! Subprogram not used ! !REVISION HISTORY:
! Subprogram not used ! 23Apr98 - Jing Guo <guo@thunder> - initial prototype/prolog/code
! Subprogram not used !EOP ___________________________________________________________________
! Subprogram not used 
! Subprogram not used   character(len=*),parameter :: myname_=myname//'::initStr1_'
! Subprogram not used   character(len=1),allocatable,dimension(:) :: ch1
! Subprogram not used   integer :: ier
! Subprogram not used   integer :: n,i,lc,le
! Subprogram not used 
! Subprogram not used   n=size(strs)
! Subprogram not used   le=0
! Subprogram not used   do i=1,n
! Subprogram not used     le=le+len(strs(i))
! Subprogram not used   end do
! Subprogram not used   le=le+n-1	! for n-1 ":"s
! Subprogram not used 
! Subprogram not used 	allocate(ch1(le),stat=ier)
! Subprogram not used 		if(ier/=0) call die(myname_,'allocate()',ier)
! Subprogram not used 
! Subprogram not used   le=0
! Subprogram not used   do i=1,n
! Subprogram not used     if(i>1) then
! Subprogram not used       le=le+1
! Subprogram not used       ch1(le)=':'
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used     lc=le+1
! Subprogram not used     le=le+len(strs(i))
! Subprogram not used     ch1(lc:le)=ptr_chars(strs(i))
! Subprogram not used   end do
! Subprogram not used     
! Subprogram not used   call init_(aList,toChar(ch1))
! Subprogram not used 
! Subprogram not used 	deallocate(ch1,stat=ier)
! Subprogram not used 		if(ier/=0) call die(myname_,'deallocate()',ier)
! Subprogram not used 
! Subprogram not used  end subroutine initStr1_
...
! Subprogram not used  function exportToString_(inList)
! Subprogram not used 
! Subprogram not used ! !USES:
! Subprogram not used !
! Subprogram not used       use m_die,    only : die
...
! Subprogram not used ! ! INPUT PARAMETERS:
! Subprogram not used 
! Subprogram not used       type(List),       intent(in) :: inList
! Subprogram not used 
! Subprogram not used ! ! OUTPUT PARAMETERS:
! Subprogram not used 
! Subprogram not used       type(String)                 :: exportToString_
! Subprogram not used 
! Subprogram not used ! !REVISION HISTORY:
! Subprogram not used ! 14Aug02 - J. Larson <larson@mcs.anl.gov> - initial version.
! Subprogram not used !EOP ___________________________________________________________________
! Subprogram not used 
! Subprogram not used   character(len=*),parameter :: myname_=myname//'::exportToString_'
! Subprogram not used 
! Subprogram not used   if(allocated_(inList)) then
! Subprogram not used      call getall_(exportToString_, inList)
! Subprogram not used   else
! Subprogram not used      call String_init(exportToString_, 'NOTHING')
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used  end function exportToString_
...
! Subprogram not used  subroutine getrange_(itemStr, i1, i2, aList)
! Subprogram not used 
! Subprogram not used ! !USES:
! Subprogram not used !
! Subprogram not used       use m_die,    only : die
! Subprogram not used       use m_stdio,  only : stderr
! Subprogram not used       use m_String, only : String,init,toChar
! Subprogram not used 
! Subprogram not used       implicit none
! Subprogram not used 
! Subprogram not used ! !INPUT PARAMETERS: 
! Subprogram not used !
! Subprogram not used       integer,     intent(in)  :: i1
! Subprogram not used       integer,     intent(in)  :: i2
! Subprogram not used       type(List),  intent(in)  :: aList
! Subprogram not used 
! Subprogram not used ! !OUTPUT PARAMETERS:   
! Subprogram not used !
! Subprogram not used       type(String),intent(out) :: itemStr
! Subprogram not used 
! Subprogram not used ! !REVISION HISTORY:
! Subprogram not used ! 23Apr98 - Jing Guo <guo@thunder> - initial prototype/prolog/code
! Subprogram not used ! 26Jul02 - J. Larson - Added argument checks.
! Subprogram not used !EOP ___________________________________________________________________
! Subprogram not used 
! Subprogram not used   character(len=*),parameter :: myname_=myname//'::getrange_'
! Subprogram not used   integer :: lb,le,ni
! Subprogram not used 
! Subprogram not used        ! Argument Sanity Checks:
! Subprogram not used 
! Subprogram not used   if(.not. allocated_(aList)) then
! Subprogram not used      write(stderr,'(2a)') myname_, &
! Subprogram not used 	  ':: FATAL--List argument aList is not initialized.'
! Subprogram not used      call die(myname_)
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used        ! is i2 >= i1 as we assume?
! Subprogram not used 
! Subprogram not used   if(i1 > i2) then
! Subprogram not used      write(stderr,'(2a,2(a,i8))') myname_, &
! Subprogram not used 	  ':: FATAL.  Starting/Ending item ranks are out of order; ', &
! Subprogram not used 	  'i2 must be greater or equal to i1.  i1 =',i1,' i2 = ',i2
! Subprogram not used      call die(myname_)
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used   ni=size(aList%lc,2) ! the number of items in aList...
! Subprogram not used 
! Subprogram not used        ! is i1 or i2 too big?
! Subprogram not used 
! Subprogram not used   if(i1 > ni) then
! Subprogram not used      write(stderr,'(2a,2(a,i8))') myname_, &
! Subprogram not used 	  ':: FATAL--i1 is greater than the number of items in ', &
! Subprogram not used 	  'The List argument aList: i1 =',i1,' ni = ',ni
! Subprogram not used      call die(myname_)
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used   if(i2 > ni) then
! Subprogram not used      write(stderr,'(2a,2(a,i8))') myname_, &
! Subprogram not used 	  ':: FATAL--i2 is greater than the number of items in ', &
! Subprogram not used 	  'The List argument aList: i2 =',i2,' ni = ',ni
! Subprogram not used      call die(myname_)
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used        ! End of Argument Sanity Checks.
! Subprogram not used 
! Subprogram not used   lb=aList%lc(0,max(1,i1))
! Subprogram not used   le=aList%lc(1,min(ni,i2))
! Subprogram not used   call init(itemStr,toChar(aList%bf(lb:le)))
! Subprogram not used 
! Subprogram not used  end subroutine getrange_
...
! Subprogram not used  subroutine test_indices_(indices, aList, Values)
! Subprogram not used 
! Subprogram not used ! !USES:
! Subprogram not used !
...
! Subprogram not used ! !INPUT PARAMETERS: 
! Subprogram not used !
! Subprogram not used       type(List),            intent(in)	:: aList  ! an indexed string values
! Subprogram not used       character(len=*),      intent(in) :: Values ! ":" delimited names
! Subprogram not used 
! Subprogram not used ! !OUTPUT PARAMETERS:   
! Subprogram not used !
! Subprogram not used       integer, dimension(:), pointer    :: indices
! Subprogram not used 
! Subprogram not used ! !REVISION HISTORY:
! Subprogram not used ! 12Feb03 - J. Larson <larson@mcs.anl.gov> Working refactored version  
! Subprogram not used !EOP ___________________________________________________________________
! Subprogram not used 
! Subprogram not used   character(len=*),parameter :: myname_=myname//'::test_indices_'
! Subprogram not used   type(List)   :: tList
! Subprogram not used   type(String) :: tStr
! Subprogram not used   integer :: i, ierr, n
! Subprogram not used 
! Subprogram not used        ! Create working list based on input colon-delimited string
! Subprogram not used 
! Subprogram not used   call init_(tList, values)
! Subprogram not used 
! Subprogram not used 
! Subprogram not used        ! Count items in tList and allocate indices(:) accordingly
! Subprogram not used 
! Subprogram not used   n = nitem_(tList)
! Subprogram not used   allocate(indices(n), stat=ierr)
! Subprogram not used   if(ierr /= 0) then
! Subprogram not used      write(stderr,'(2a,i8,a)') myname_, &
! Subprogram not used 	  ':: FATAL--allocate(indices(...) failed with stat=',ierr,&
! Subprogram not used 	  '.  On entry to this routine, this pointer must be NULL.'
! Subprogram not used      call die(myname_)
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used        ! Retrieve each item from tList as a String and index it
! Subprogram not used 
! Subprogram not used   do i=1,n
! Subprogram not used     call get_(tStr,i,tList)
! Subprogram not used     indices(i) = indexStr_(aList,tStr)
! Subprogram not used     call String_clean(tStr)
! Subprogram not used   end do
! Subprogram not used 
! Subprogram not used        ! Clean up temporary List tList
! Subprogram not used 
! Subprogram not used   call clean_(tList)
! Subprogram not used 
! Subprogram not used  end subroutine test_indices_
...
! Subprogram not used     subroutine send_(inList, dest, TagBase, comm, status)
! Subprogram not used !
! Subprogram not used ! !USES:
! Subprogram not used !
! Subprogram not used       use m_stdio
...
! Subprogram not used       use m_String, only:  String
! Subprogram not used       use m_String, only:  String_toChar => toChar
! Subprogram not used       use m_String, only:  String_len
! Subprogram not used       use m_String, only:  String_clean => clean
! Subprogram not used 
! Subprogram not used       implicit none
! Subprogram not used 
...
! Subprogram not used 
! Subprogram not used ! !REVISION HISTORY:
! Subprogram not used !  6Jun01 - J.W. Larson - initial version.
...
! Subprogram not used 
! Subprogram not used  type(String) :: DummStr
! Subprogram not used  integer :: ierr, length
! Subprogram not used 
! Subprogram not used        ! Set status flag to zero (success) if present:
! Subprogram not used 
! Subprogram not used  if(present(status)) status = 0
! Subprogram not used 
! Subprogram not used        ! Step 1.  Extract CHARACTER buffer from inList and store it
! Subprogram not used        ! in String variable DummStr, determine its length.
! Subprogram not used 
! Subprogram not used  call getall_(DummStr, inList)
! Subprogram not used  length = String_len(DummStr)
! Subprogram not used 
! Subprogram not used        ! Step 2.  Send Length of String DummStr to process dest.
! Subprogram not used 
! Subprogram not used  call MPI_SEND(length, 1, MP_type(length), dest, TagBase, comm, ierr)
! Subprogram not used   if(ierr /= 0) then
! Subprogram not used      if(present(status)) then
! Subprogram not used         write(stderr,'(2a,i8)') myname_, &
! Subprogram not used 	     ':: MPI_SEND(length...) failed.  ierror=', ierr
! Subprogram not used         status = ierr
! Subprogram not used         return
! Subprogram not used      else
! Subprogram not used         call MP_perr_die(myname_,':: MPI_SEND(length...) failed',ierr)
! Subprogram not used      endif
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used        ! Step 3.  Send CHARACTER portion of String DummStr 
! Subprogram not used        ! to process dest.
! Subprogram not used 
! Subprogram not used  call MPI_SEND(DummStr%c(1), length, MP_CHARACTER, dest, TagBase+1, &
! Subprogram not used                comm, ierr)
! Subprogram not used   if(ierr /= 0) then
! Subprogram not used      if(present(status)) then
! Subprogram not used         write(stderr,'(2a,i8)') myname_, &
! Subprogram not used 	     ':: MPI_SEND(DummStr%c...) failed.  ierror=', ierr
! Subprogram not used         status = ierr
! Subprogram not used         return
! Subprogram not used      else
! Subprogram not used         call MP_perr_die(myname_,':: MPI_SEND(DummStr%c...) failed',ierr)
! Subprogram not used      endif
! Subprogram not used   endif
! Subprogram not used 
...
! Subprogram not used     subroutine recv_(outList, source, TagBase, comm, status)
! Subprogram not used !
! Subprogram not used ! !USES:
! Subprogram not used !
! Subprogram not used       use m_stdio, only : stderr
...
! Subprogram not used ! !OUTPUT PARAMETERS: 
! Subprogram not used !
! Subprogram not used       type(List),         intent(out) :: outList  
! Subprogram not used       integer, optional,  intent(out) :: status
! Subprogram not used 
! Subprogram not used ! !REVISION HISTORY:
! Subprogram not used !  6Jun01 - J.W. Larson - initial version.
! Subprogram not used ! 11Jun01 - R. Jacob - small bug fix; status in MPI_RECV
! Subprogram not used ! 13Jun01 - J.W. Larson <larson@mcs.anl.gov> - Initialize status
! Subprogram not used !           (if present).
! Subprogram not used !EOP ___________________________________________________________________
! Subprogram not used 
...
! Subprogram not used  type(String) :: DummStr
! Subprogram not used 
! Subprogram not used        ! Initialize status to zero (success), if present.
! Subprogram not used 
! Subprogram not used   if(present(status)) status = 0
! Subprogram not used 
! Subprogram not used        ! Step 1.  Receive Length of String DummStr from process source.
! Subprogram not used 
! Subprogram not used  call MPI_RECV(length, 1, MP_type(length), source, TagBase, comm, &
! Subprogram not used                MPstatus, ierr)
! Subprogram not used   if(ierr /= 0) then
! Subprogram not used      if(present(status)) then
! Subprogram not used         write(stderr,'(2a,i8)') myname_, &
! Subprogram not used 	     ':: MPI_RECV(length...) failed.  ierror=', ierr
! Subprogram not used         status = ierr
! Subprogram not used         return
! Subprogram not used      else
! Subprogram not used         call MP_perr_die(myname_,':: MPI_RECV(length...) failed',ierr)
! Subprogram not used      endif
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used  allocate(DummStr%c(length), stat=ierr)
! Subprogram not used 
! Subprogram not used        ! Step 2.  Send CHARACTER portion of String DummStr 
! Subprogram not used        ! to process dest.
! Subprogram not used 
! Subprogram not used  call MPI_RECV(DummStr%c(1), length, MP_CHARACTER, source, TagBase+1, &
! Subprogram not used                comm, MPstatus, ierr)
! Subprogram not used   if(ierr /= 0) then
! Subprogram not used      if(present(status)) then
! Subprogram not used         write(stderr,'(2a,i8)') myname_, &
! Subprogram not used 	     ':: MPI_RECV(DummStr%c...) failed.  ierror=', ierr
! Subprogram not used         status = ierr
! Subprogram not used         return
! Subprogram not used      else
! Subprogram not used         call MP_perr_die(myname_,':: MPI_RECV(DummStr%c...) failed',ierr)
! Subprogram not used      endif
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used        ! Step 3.  Initialize outList.
! Subprogram not used 
! Subprogram not used  call initStr_(outList, DummStr)
! Subprogram not used 
