...
! temperature and humidity. NOTE: \! (1) all fluxes are positive downward,  \! (2) here, tstar = (WT)/U*, and qstar = (WQ)/U*,  \! (3a) wind speeds should all be above a minimum speed (eg. 1.0 m/s). \! (3b) (wind-ice) speeds should all be above a minimum speed (eg. 1.0 m/s). \!
...
! !INTERFACE:
!
! Subprogram not used       subroutine atmo_boundary_const (nx_block, ny_block, &
! Subprogram not used                                       sfctype,  icells,   &
! Subprogram not used                                       indxi,    indxj,    & 
! Subprogram not used                                       uatm,     vatm,     &  
! Subprogram not used                                       wind,     rhoa,     &
! Subprogram not used                                       strx,     stry,     &   
! Subprogram not used                                       lhcoef,   shcoef)
! Subprogram not used 
! Subprogram not used ! !DESCRIPTION:
! Subprogram not used !
! Subprogram not used ! Compute coefficients for atm/ice fluxes, stress
! Subprogram not used ! NOTE: \! (1) all fluxes are positive downward,  \! (2) reference temperature and humidity are NOT computed
! Subprogram not used 
! Subprogram not used 
! Subprogram not used !
! Subprogram not used ! !REVISION HISTORY: same as module
! Subprogram not used !
...
! Subprogram not used          icells                ! number of cells that require atmo fluxes
! Subprogram not used 
! Subprogram not used       integer (kind=int_kind), dimension(nx_block*ny_block), &
! Subprogram not used          intent(in) :: &
! Subprogram not used          indxi, indxj    ! compressed i and j indices
! Subprogram not used 
! Subprogram not used       character (len=3), intent(in) :: &
! Subprogram not used          sfctype      ! ice or ocean
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), dimension (nx_block,ny_block), intent(in) :: &
! Subprogram not used          uatm     , & ! x-direction wind speed (m/s)
! Subprogram not used          vatm     , & ! y-direction wind speed (m/s)
! Subprogram not used          wind     , & ! wind speed (m/s)
...
! Subprogram not used          strx     , & ! x surface stress (N)
! Subprogram not used          stry         ! y surface stress (N)
! Subprogram not used 
...
! Subprogram not used          ij      ! combined ij index
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind) :: &
! Subprogram not used          tau, &  ! stress at zlvl
! Subprogram not used          Lheat   ! Lvap or Lsub, depending on surface type
! Subprogram not used 
! Subprogram not used       !------------------------------------------------------------
...
! Subprogram not used          shcoef(i,j) = c0
! Subprogram not used          lhcoef(i,j) = c0
! Subprogram not used       enddo
! Subprogram not used       enddo
! Subprogram not used 
! Subprogram not used       if (calc_strair) then
! Subprogram not used 
! Subprogram not used       do j = 1, ny_block
! Subprogram not used       do i = 1, nx_block
! Subprogram not used          strx(i,j) = c0
! Subprogram not used          stry(i,j) = c0
! Subprogram not used       enddo
! Subprogram not used       enddo
! Subprogram not used 
...
! Subprogram not used       !------------------------------------------------------------
! Subprogram not used 
! Subprogram not used          tau = rhoa(i,j) * 0.0012_dbl_kind * wind(i,j)
! Subprogram not used !AOMIP         tau = rhoa(i,j) * (1.10_dbl_kind + c4*p01*wind(i,j)) &
! Subprogram not used !AOMIP                         * wind(i,j) * p001
! Subprogram not used          strx(i,j) = tau * uatm(i,j)
! Subprogram not used          stry(i,j) = tau * vatm(i,j)
! Subprogram not used 
! Subprogram not used       enddo                     ! ij
! Subprogram not used 
! Subprogram not used       endif                     ! calc_strair
! Subprogram not used 
! Subprogram not used       !------------------------------------------------------------
! Subprogram not used       ! define variables that depend on surface type
! Subprogram not used       !------------------------------------------------------------
! Subprogram not used 
! Subprogram not used       if (sfctype(1:3)=='ice') then
! Subprogram not used          Lheat = Lsub           ! ice to vapor
! Subprogram not used       elseif (sfctype(1:3)=='ocn') then
! Subprogram not used          Lheat = Lvap           ! liquid to vapor
! Subprogram not used       endif   ! sfctype
! Subprogram not used 
! Subprogram not used !DIR$ CONCURRENT !Cray
! Subprogram not used !cdir nodep      !NEC
! Subprogram not used !ocl novrec      !Fujitsu
...
! Subprogram not used 
! Subprogram not used       !------------------------------------------------------------
! Subprogram not used       ! coefficients for turbulent flux calculation
! Subprogram not used       !------------------------------------------------------------
! Subprogram not used 
! Subprogram not used          shcoef(i,j) = (1.20e-3_dbl_kind)*cp_air*rhoa(i,j)*wind(i,j)
! Subprogram not used          lhcoef(i,j) = (1.50e-3_dbl_kind)*Lheat *rhoa(i,j)*wind(i,j)
! Subprogram not used 
! Subprogram not used       enddo                     ! ij
! Subprogram not used 
! Subprogram not used       end subroutine atmo_boundary_const
