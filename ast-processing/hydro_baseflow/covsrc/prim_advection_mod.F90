...
! Subprogram not used subroutine remap_calc_grids( hvcoord , ps , dt , eta_dot_dpdn , p_lag , p_ref , dp_lag , dp_ref )
! Subprogram not used   implicit none
! Subprogram not used   type(hvcoord_t)      , intent(in   ) :: hvcoord               !Derived type to hold vertical sigma grid parameters
! Subprogram not used   real(kind=real_kind) , intent(in   ) :: ps                    !Surface pressure for this column
! Subprogram not used   real(kind=real_kind) , intent(in   ) :: dt                    !Time step
! Subprogram not used   real(kind=real_kind) , intent(in   ) :: eta_dot_dpdn(nlev+1)  !Looks like a vertical pressure flux
! Subprogram not used                                                                 !to compute deformed grid spacing
! Subprogram not used   real(kind=real_kind) , intent(  out) :: p_lag(nlev+1)         !Pressures at interfaces of the Lagrangian deformed grid
! Subprogram not used   real(kind=real_kind) , intent(  out) :: p_ref(nlev+1)         !Pressures at interfaces of the reference grid
! Subprogram not used   real(kind=real_kind) , intent(  out) :: dp_lag(nlev)          !Pressure differences on Lagrangian deformed grid
! Subprogram not used   real(kind=real_kind) , intent(  out) :: dp_ref(nlev)          !Pressure differences on reference grid
! Subprogram not used   integer :: k                                                  !Iterator
! Subprogram not used   p_ref(1) = 0  !Both grids have a model top pressure of zero
! Subprogram not used   p_lag(1) = 0  !Both grids have a model top pressure of zero
! Subprogram not used   do k = 1 , nlev
! Subprogram not used     dp_ref(k) = ( hvcoord%hyai(k+1) - hvcoord%hyai(k) ) * hvcoord%ps0 + &
! Subprogram not used          ( hvcoord%hybi(k+1) - hvcoord%hybi(k) ) * ps  !Reference pressure difference
! Subprogram not used     ! Lagrangian pressure difference (flux in - flux out over the time step)
! Subprogram not used     dp_lag(k) = dp_ref(k) + dt * ( eta_dot_dpdn(k+1) - eta_dot_dpdn(k) )
! Subprogram not used     p_ref(k+1) = p_ref(k) + dp_ref(k) !Pressure at interfaces accumulated using difference over each cell
! Subprogram not used     p_lag(k+1) = p_lag(k) + dp_lag(k) !Pressure at interfaces accumulated using difference over each cell
! Subprogram not used   enddo
! Subprogram not used end subroutine remap_calc_grids
...
! Subprogram not used subroutine remap1_nofilter(Qdp,nx,qsize,dp1,dp2)
! Subprogram not used   ! remap 1 field
! Subprogram not used   ! input:  Qdp   field to be remapped (NOTE: MASS, not MIXING RATIO)
! Subprogram not used   !         dp1   layer thickness (source)
! Subprogram not used   !         dp2   layer thickness (target)
! Subprogram not used   !
! Subprogram not used   ! output: remaped Qdp, conserving mass
! Subprogram not used   !
! Subprogram not used   implicit none
! Subprogram not used   integer, intent(in) :: nx,qsize 
! Subprogram not used   real (kind=real_kind), intent(inout) :: Qdp(nx,nx,nlev,qsize)
! Subprogram not used   real (kind=real_kind), intent(in) :: dp1(nx,nx,nlev),dp2(nx,nx,nlev)
! Subprogram not used   ! ========================
! Subprogram not used   ! Local Variables
! Subprogram not used   ! ========================
! Subprogram not used 
! Subprogram not used   real (kind=real_kind), dimension(nlev+1)    :: rhs,lower_diag,diag,upper_diag,q_diag,zgam,z1c,z2c,zv
! Subprogram not used   real (kind=real_kind), dimension(nlev)      :: h,Qcol,dy,za0,za1,za2,zarg,zhdp,dp_star,dp_np1
! Subprogram not used   real (kind=real_kind)  :: f_xm,level1,level2,level3,level4,level5, &
! Subprogram not used                             peaks_min,peaks_max,tmp_cal,xm,xm_d,zv1,zv2, &
! Subprogram not used                             zero = 0,one = 1,tiny = 1e-12,qmax = 1d50
! Subprogram not used   integer(kind=int_kind) :: zkr(nlev+1),filter_code(nlev),peaks,im1,im2,im3,ip1,ip2, & 
! Subprogram not used                             lt1,lt2,lt3,t0,t1,t2,t3,t4,tm,tp,ie,i,ilev,j,jk,k,q
! Subprogram not used   logical :: abort=.false.  
! Subprogram not used !   call t_startf('remap1_nofilter')
! Subprogram not used 
! Subprogram not used   do q=1,qsize
! Subprogram not used   do i=1,nx
! Subprogram not used     do j=1,nx
! Subprogram not used 
! Subprogram not used       z1c(1)=0 ! source grid
! Subprogram not used       z2c(1)=0 ! target grid
! Subprogram not used       do k=1,nlev
! Subprogram not used          z1c(k+1)=z1c(k)+dp1(i,j,k)
! Subprogram not used          z2c(k+1)=z2c(k)+dp2(i,j,k)
! Subprogram not used       enddo
! Subprogram not used 
! Subprogram not used       zv(1)=0
! Subprogram not used       do k=1,nlev
! Subprogram not used         Qcol(k)=Qdp(i,j,k,q)!  *(z1c(k+1)-z1c(k)) input is mass
! Subprogram not used         zv(k+1) = zv(k)+Qcol(k)
! Subprogram not used       enddo
! Subprogram not used       
! Subprogram not used       if (ABS(z2c(nlev+1)-z1c(nlev+1)).GE.0.000001) then
! Subprogram not used         write(6,*) 'SURFACE PRESSURE IMPLIED BY ADVECTION SCHEME'
! Subprogram not used         write(6,*) 'NOT CORRESPONDING TO SURFACE PRESSURE IN    '
! Subprogram not used         write(6,*) 'DATA FOR MODEL LEVELS'
! Subprogram not used         write(6,*) 'PLEVMODEL=',z2c(nlev+1)
! Subprogram not used         write(6,*) 'PLEV     =',z1c(nlev+1)
! Subprogram not used         write(6,*) 'DIFF     =',z2c(nlev+1)-z1c(nlev+1)
! Subprogram not used         abort=.true.
! Subprogram not used       endif
! Subprogram not used       
! Subprogram not used       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subprogram not used       !! quadratic splies with UK met office monotonicity constraints  !!
! Subprogram not used       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subprogram not used       
! Subprogram not used       zkr  = 99
! Subprogram not used       ilev = 2
! Subprogram not used       zkr(1) = 1
! Subprogram not used       zkr(nlev+1) = nlev
! Subprogram not used       kloop: do k = 2,nlev
! Subprogram not used         do jk = ilev,nlev+1
! Subprogram not used           if (z1c(jk).ge.z2c(k)) then
! Subprogram not used             ilev      = jk
! Subprogram not used             zkr(k)   = jk-1
! Subprogram not used             cycle kloop
! Subprogram not used           endif
! Subprogram not used         enddo
! Subprogram not used       enddo kloop
! Subprogram not used       
! Subprogram not used       zgam  = (z2c(1:nlev+1)-z1c(zkr)) / (z1c(zkr+1)-z1c(zkr))
! Subprogram not used       zgam(1)      = 0.0
! Subprogram not used       zgam(nlev+1) = 1.0
! Subprogram not used       zhdp = z1c(2:nlev+1)-z1c(1:nlev)
! Subprogram not used       
! Subprogram not used       
! Subprogram not used       h = 1/zhdp
! Subprogram not used       zarg = Qcol * h
! Subprogram not used       rhs = 0
! Subprogram not used       lower_diag = 0
! Subprogram not used       diag = 0
! Subprogram not used       upper_diag = 0
! Subprogram not used       
! Subprogram not used       rhs(1)=3*zarg(1)
! Subprogram not used       rhs(2:nlev) = 3*(zarg(2:nlev)*h(2:nlev) + zarg(1:nlev-1)*h(1:nlev-1)) 
! Subprogram not used       rhs(nlev+1)=3*zarg(nlev)
! Subprogram not used       
! Subprogram not used       lower_diag(1)=1
! Subprogram not used       lower_diag(2:nlev) = h(1:nlev-1)
! Subprogram not used       lower_diag(nlev+1)=1
! Subprogram not used       
! Subprogram not used       diag(1)=2
! Subprogram not used       diag(2:nlev) = 2*(h(2:nlev) + h(1:nlev-1))
! Subprogram not used       diag(nlev+1)=2
! Subprogram not used       
! Subprogram not used       upper_diag(1)=1
! Subprogram not used       upper_diag(2:nlev) = h(2:nlev)
! Subprogram not used       upper_diag(nlev+1)=0
! Subprogram not used       
! Subprogram not used       q_diag(1)=-upper_diag(1)/diag(1)
! Subprogram not used       rhs(1)= rhs(1)/diag(1)
! Subprogram not used       
! Subprogram not used       do k=2,nlev+1
! Subprogram not used         tmp_cal    =  1/(diag(k)+lower_diag(k)*q_diag(k-1))
! Subprogram not used         q_diag(k) = -upper_diag(k)*tmp_cal
! Subprogram not used         rhs(k) =  (rhs(k)-lower_diag(k)*rhs(k-1))*tmp_cal
! Subprogram not used       enddo
! Subprogram not used       do k=nlev,1,-1
! Subprogram not used         rhs(k)=rhs(k)+q_diag(k)*rhs(k+1)
! Subprogram not used       enddo
! Subprogram not used       
! Subprogram not used       za0 = rhs(1:nlev) 
! Subprogram not used       za1 = -4*rhs(1:nlev) - 2*rhs(2:nlev+1) + 6*zarg  
! Subprogram not used       za2 =  3*rhs(1:nlev) + 3*rhs(2:nlev+1) - 6*zarg 
! Subprogram not used       
! Subprogram not used       
! Subprogram not used       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subprogram not used       !! start iteration from top to bottom of atmosphere !! 
! Subprogram not used       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subprogram not used       
! Subprogram not used       zv1 = 0
! Subprogram not used       do k=1,nlev
! Subprogram not used         if (zgam(k+1)>1d0) then
! Subprogram not used           WRITE(*,*) 'r not in [0:1]', zgam(k+1)
! Subprogram not used           abort=.true.
! Subprogram not used         endif
! Subprogram not used         zv2 = zv(zkr(k+1))+(za0(zkr(k+1))*zgam(k+1)+(za1(zkr(k+1))/2)*(zgam(k+1)**2)+ &
! Subprogram not used              (za2(zkr(k+1))/3)*(zgam(k+1)**3))*zhdp(zkr(k+1))
! Subprogram not used         Qdp(i,j,k,q) = (zv2 - zv1) ! / (z2c(k+1)-z2c(k) ) dont convert back to mixing ratio
! Subprogram not used         zv1 = zv2
! Subprogram not used       enddo
! Subprogram not used     enddo
! Subprogram not used   enddo
! Subprogram not used   enddo ! q loop 
! Subprogram not used   if (abort) call abortmp('Bad levels in remap1_nofilter.  usually CFL violatioin')
! Subprogram not used !   call t_stopf('remap1_nofilter')
! Subprogram not used end subroutine remap1_nofilter
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subprogram not used   subroutine Prim_Advec_Tracers_spelt(elem, spelt, deriv,hvcoord,hybrid,&
! Subprogram not used         dt,tl,nets,nete)
! Subprogram not used     use perf_mod, only : t_startf, t_stopf            ! _EXTERNAL
! Subprogram not used     use spelt_mod, only : spelt_run, spelt_runpos, spelt_runair, spelt_runlimit, edgeveloc, spelt_mcgregordss, spelt_rkdss
! Subprogram not used     use derivative_mod, only : interpolate_gll2spelt_points
! Subprogram not used     use vertremap_mod, only: remap1_nofilter ! _EXTERNAL (actually INTERNAL)
! Subprogram not used     
! Subprogram not used     implicit none
! Subprogram not used     type (element_t), intent(inout)   :: elem(:)
! Subprogram not used     type (spelt_struct), intent(inout)  :: spelt(:)
...
! Subprogram not used     type (TimeLevel_t)                :: tl
! Subprogram not used 
! Subprogram not used     real(kind=real_kind) , intent(in) :: dt
! Subprogram not used     integer,intent(in)                :: nets,nete
! Subprogram not used 
! Subprogram not used     real (kind=real_kind), dimension(np,np,nlev)    :: dp_star
! Subprogram not used     real (kind=real_kind), dimension(np,np,nlev)    :: dp
! Subprogram not used    
! Subprogram not used     integer :: np1,ie,k, i, j
! Subprogram not used     
! Subprogram not used 
! Subprogram not used     call t_barrierf('sync_prim_advec_tracers_spelt', hybrid%par%comm)
! Subprogram not used     call t_startf('prim_advec_tracers_spelt')
! Subprogram not used     np1 = tl%np1
! Subprogram not used 
! Subprogram not used 
! Subprogram not used     ! interpolate t+1 velocity from reference levels to lagrangian levels
! Subprogram not used     ! For rsplit=0, we need to first compute lagrangian levels based on vertical velocity
! Subprogram not used     ! which requires we first DSS mean vertical velocity from dynamics
! Subprogram not used     ! 
! Subprogram not used     if (rsplit==0) then
! Subprogram not used        do ie=nets,nete
! Subprogram not used           do k=1,nlev
! Subprogram not used              elem(ie)%derived%eta_dot_dpdn(:,:,k) = elem(ie)%spheremp(:,:)*elem(ie)%derived%eta_dot_dpdn(:,:,k) 
! Subprogram not used           enddo
! Subprogram not used           call edgeVpack(edgeAdv1,elem(ie)%derived%eta_dot_dpdn(:,:,1:nlev),nlev,0,elem(ie)%desc)
! Subprogram not used        enddo
! Subprogram not used        call bndry_exchangeV(hybrid,edgeAdv1)
! Subprogram not used        do ie=nets,nete
! Subprogram not used           call edgeVunpack(edgeAdv1,elem(ie)%derived%eta_dot_dpdn(:,:,1:nlev),nlev,0,elem(ie)%desc)
! Subprogram not used           do k=1,nlev
! Subprogram not used              elem(ie)%derived%eta_dot_dpdn(:,:,k)=elem(ie)%derived%eta_dot_dpdn(:,:,k)*elem(ie)%rspheremp(:,:)
! Subprogram not used           enddo
! Subprogram not used           ! SET VERTICAL VELOCITY TO ZERO FOR DEBUGGING
! Subprogram not used           elem(ie)%derived%eta_dot_dpdn(:,:,:)=0
! Subprogram not used           
! Subprogram not used           ! elem%state%u(np1)  = velocity at time t+1 on reference levels
! Subprogram not used           ! elem%derived%vstar = velocity at t+1 on floating levels (computed below) using eta_dot_dpdn
! Subprogram not used !           call remap_UV_ref2lagrange(np1,dt,elem,hvcoord,ie)
! Subprogram not used           do k=1,nlev
! Subprogram not used              dp(:,:,k) = ( hvcoord%hyai(k+1) - hvcoord%hyai(k) )*hvcoord%ps0 + &
! Subprogram not used                   ( hvcoord%hybi(k+1) - hvcoord%hybi(k) )*elem(ie)%state%ps_v(:,:,np1)
! Subprogram not used              dp_star(:,:,k) = dp(:,:,k) + dt*(elem(ie)%derived%eta_dot_dpdn(:,:,k+1) -&
! Subprogram not used                   elem(ie)%derived%eta_dot_dpdn(:,:,k)) 
! Subprogram not used           enddo
! Subprogram not used           elem(ie)%derived%vstar=elem(ie)%state%v(:,:,:,:,np1)
! Subprogram not used           call remap1_nofilter(elem(ie)%derived%vstar,np,1,dp,dp_star)
! Subprogram not used           !take the average on level, should be improved later, because we know the SE velocity at t+1/2
! Subprogram not used           spelt(ie)%vn12=(spelt(ie)%vn0+elem(ie)%derived%vstar)/2.0D0
! Subprogram not used        end do
! Subprogram not used     else
! Subprogram not used        ! for rsplit>0:  dynamics is also vertically lagrangian, so we do not need to
! Subprogram not used        ! remap the velocities
! Subprogram not used        stop 'FVM need to use lagrangian winds here' 
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used 
! Subprogram not used     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subprogram not used     ! 2D advection step
! Subprogram not used     ! 
! Subprogram not used     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subprogram not used !------------------------------------------------------------------------------------      
! Subprogram not used     
! Subprogram not used !     call t_startf('spelt_depalg')
! Subprogram not used !     call spelt_mcgregordss(elem,spelt,nets,nete, hybrid, deriv, dt, 3)
! Subprogram not used     call spelt_rkdss(elem,spelt,nets,nete, hybrid, deriv, dt, 3)
! Subprogram not used !     call t_stopf('spelt_depalg')
! Subprogram not used     
! Subprogram not used     ! ! end mcgregordss
! Subprogram not used     ! spelt departure calcluation should use vstar.
! Subprogram not used     ! from c(n0) compute c(np1):
! Subprogram not used !     call spelt_run(elem,spelt,hybrid,deriv,dt,tl,nets,nete)
! Subprogram not used     
! Subprogram not used     call spelt_runair(elem,spelt,hybrid,deriv,dt,tl,nets,nete)
! Subprogram not used !     call spelt_runpos(elem,spelt,hybrid,deriv,dt,tl,nets,nete)
! Subprogram not used !       call spelt_runlimit(elem,spelt,hybrid,deriv,dt,tl,nets,nete)
! Subprogram not used     call t_stopf('prim_advec_tracers_spelt')
! Subprogram not used   end subroutine Prim_Advec_Tracers_spelt
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subprogram not used   subroutine Prim_Advec_Tracers_fvm(elem, fvm, deriv,hvcoord,hybrid,&
! Subprogram not used         dt,tl,nets,nete)
! Subprogram not used     use perf_mod, only : t_startf, t_stopf            ! _EXTERNAL
! Subprogram not used     use vertremap_mod, only: remap1_nofilter  ! _EXTERNAL (actually INTERNAL)
! Subprogram not used !    use fvm_mod, only : cslam_run, cslam_runairdensity, edgeveloc, fvm_mcgregor, fvm_mcgregordss
! Subprogram not used     use fvm_mod, only : cslam_run, cslam_runairdensity, edgeveloc, fvm_mcgregor, fvm_mcgregordss, fvm_rkdss
! Subprogram not used     
! Subprogram not used     implicit none
! Subprogram not used     type (element_t), intent(inout)   :: elem(:)
...
! Subprogram not used     real (kind=real_kind), dimension(np,np,nlev)    :: dp
! Subprogram not used    
! Subprogram not used     integer :: np1,ie,k
! Subprogram not used     
! Subprogram not used     real (kind=real_kind)  :: vstar(np,np,2)
! Subprogram not used     real (kind=real_kind)  :: vhat(np,np,2)
! Subprogram not used     real (kind=real_kind), dimension(np, np) :: v1, v2
! Subprogram not used     
! Subprogram not used 
! Subprogram not used     call t_barrierf('sync_prim_advec_tracers_fvm', hybrid%par%comm)
! Subprogram not used     call t_startf('prim_advec_tracers_fvm')
! Subprogram not used     np1 = tl%np1
! Subprogram not used 
! Subprogram not used     ! interpolate t+1 velocity from reference levels to lagrangian levels
! Subprogram not used     ! For rsplit=0, we need to first compute lagrangian levels based on vertical velocity
...
! Subprogram not used           !        elem(ie)%derived%eta_dot_dpdn(:,:,:)=0
! Subprogram not used           ! elem%state%u(np1)  = velocity at time t+1 on reference levels
! Subprogram not used           ! elem%derived%vstar = velocity at t+1 on floating levels (computed below)
! Subprogram not used !           call remap_UV_ref2lagrange(np1,dt,elem,hvcoord,ie)
! Subprogram not used           do k=1,nlev
! Subprogram not used              dp(:,:,k) = ( hvcoord%hyai(k+1) - hvcoord%hyai(k) )*hvcoord%ps0 + &
! Subprogram not used                   ( hvcoord%hybi(k+1) - hvcoord%hybi(k) )*elem(ie)%state%ps_v(:,:,np1)
...
! Subprogram not used     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subprogram not used !------------------------------------------------------------------------------------    
! Subprogram not used !     call t_startf('fvm_depalg')
! Subprogram not used 
! Subprogram not used !     call fvm_mcgregordss(elem,fvm,nets,nete, hybrid, deriv, dt, 3)
! Subprogram not used     call fvm_rkdss(elem,fvm,nets,nete, hybrid, deriv, dt, 3)
! Subprogram not used !     call t_stopf('fvm_depalg')
! Subprogram not used 
! Subprogram not used !------------------------------------------------------------------------------------    
! Subprogram not used 
! Subprogram not used     ! fvm departure calcluation should use vstar.
! Subprogram not used     ! from c(n0) compute c(np1): 
! Subprogram not used     call cslam_runairdensity(elem,fvm,hybrid,deriv,dt,tl,nets,nete)
! Subprogram not used !     call cslam_run(elem,fvm,hybrid,deriv,dt,tl,nets,nete)
! Subprogram not used 
! Subprogram not used     call t_stopf('prim_advec_tracers_fvm')
! Subprogram not used   end subroutine Prim_Advec_Tracers_fvm
...
! Subprogram not used   subroutine euler_step_dg(np1_qdp, n0_qdp, dt,elem,hvcoord,hybrid,deriv,nets,nete,&
! Subprogram not used       DSSopt,rhs_multiplier)
! Subprogram not used   ! ===================================
! Subprogram not used   ! This routine is the basic foward
! Subprogram not used   ! euler component used to construct RK SSP methods
! Subprogram not used   !
! Subprogram not used   !           u(np1) = u(n0) + dt2*DSS[ RHS(u(n0)) ]
! Subprogram not used   !
! Subprogram not used   ! n0 can be the same as np1.  
! Subprogram not used   !
! Subprogram not used   ! DSSopt = DSSeta or DSSomega:   also DSS eta_dot_dpdn or omega
! Subprogram not used   !
! Subprogram not used   ! ===================================
! Subprogram not used   use kinds, only : real_kind
! Subprogram not used   use dimensions_mod, only : np, npdg, nlev
! Subprogram not used   use hybrid_mod, only : hybrid_t
! Subprogram not used   use element_mod, only : element_t
! Subprogram not used   use derivative_mod, only : derivative_t, divergence_sphere_wk, edge_flux_u_cg, gll_to_dgmodal, dgmodal_to_gll
! Subprogram not used   use edge_mod, only : edgevpack, edgevunpack, edgedgvunpack
! Subprogram not used   use bndry_mod, only : bndry_exchangev
! Subprogram not used   use hybvcoord_mod, only : hvcoord_t
! Subprogram not used 
! Subprogram not used   implicit none
! Subprogram not used   integer :: np1_qdp, n0_qdp, nets, nete, DSSopt, rhs_multiplier
! Subprogram not used   real (kind=real_kind), intent(in)  :: dt
! Subprogram not used 
...
! Subprogram not used   real (kind=real_kind), dimension(np,np)    :: divdp
! Subprogram not used   real (kind=real_kind), dimension(npdg,npdg)    :: pshat
! Subprogram not used   real (kind=real_kind), dimension(0:np+1,0:np+1,nlev,qsize)    :: qedges
! Subprogram not used   real (kind=real_kind), dimension(np,np,2)    :: vtemp
! Subprogram not used   real(kind=real_kind), dimension(np,np,nlev) :: dp,dp_star
! Subprogram not used   real(kind=real_kind), dimension(np,np,2,nlev) :: Vstar
! Subprogram not used   real (kind=real_kind), pointer, dimension(:,:,:)   :: DSSvar
! Subprogram not used ! nelemd
! Subprogram not used 
! Subprogram not used   real(kind=real_kind) :: dp0
! Subprogram not used   integer :: ie,q,i,j,k
! Subprogram not used   integer :: rhs_viss=0
! Subprogram not used 
! Subprogram not used   call t_barrierf('sync_euler_step_dg', hybrid%par%comm)
! Subprogram not used   call t_startf('euler_step_dg')
! Subprogram not used 
! Subprogram not used 
! Subprogram not used   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subprogram not used   !   compute Q min/max values for lim8
! Subprogram not used   !   compute biharmonic mixing term f
! Subprogram not used   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subprogram not used   rhs_viss=0
! Subprogram not used   if (limiter_option == 8) then
! Subprogram not used      call abortmp('limiter_opiton=8 not supported for dg advection')
! Subprogram not used      ! todo:  we need to track a 'dg' mass, and use that to back out Q
! Subprogram not used      ! then compute Qmin/Qmax here
! Subprogram not used   endif  
! Subprogram not used 
! Subprogram not used 
! Subprogram not used 
...
! Subprogram not used   do ie=nets,nete
! Subprogram not used 
! Subprogram not used      ! note: eta_dot_dpdn is actually dimension nlev+1, but nlev+1 data is
! Subprogram not used      ! all zero so we only have to DSS 1:nlev
! Subprogram not used      if ( DSSopt == DSSeta) DSSvar => elem(ie)%derived%eta_dot_dpdn(:,:,:)
! Subprogram not used      if ( DSSopt == DSSomega) DSSvar => elem(ie)%derived%omega_p(:,:,:)
! Subprogram not used      if ( DSSopt == DSSdiv_vdp_ave) DSSvar => elem(ie)%derived%divdp_proj(:,:,:)
! Subprogram not used 
! Subprogram not used      if(DSSopt==DSSno_var)then
! Subprogram not used 	call edgeVpack(edgeAdv,elem(ie)%state%Qdp(:,:,:,:,n0_qdp),nlev*qsize,0,elem(ie)%desc)
! Subprogram not used      else
! Subprogram not used 	call edgeVpack(edgeAdv_p1,elem(ie)%state%Qdp(:,:,:,:,n0_qdp),nlev*qsize,0,elem(ie)%desc)
! Subprogram not used 	! also DSS extra field
! Subprogram not used 	do k=1,nlev
! Subprogram not used 	    DSSvar(:,:,k) = elem(ie)%spheremp(:,:)*DSSvar(:,:,k) 
! Subprogram not used 	enddo
! Subprogram not used 	call edgeVpack(edgeAdv_p1,DSSvar(:,:,1:nlev),nlev,nlev*qsize,elem(ie)%desc)
! Subprogram not used      endif
! Subprogram not used 
! Subprogram not used   end do
! Subprogram not used 
! Subprogram not used   if(DSSopt==DSSno_var)then
! Subprogram not used      call bndry_exchangeV(hybrid,edgeAdv)
! Subprogram not used   else
! Subprogram not used      call bndry_exchangeV(hybrid,edgeAdv_p1)
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used   do ie=nets,nete
! Subprogram not used 
...
! Subprogram not used 
! Subprogram not used      if(DSSopt==DSSno_var)then
! Subprogram not used 	call edgeDGVunpack(edgeAdv,qedges,nlev*qsize,0,elem(ie)%desc)
! Subprogram not used      else
! Subprogram not used 	call edgeDGVunpack(edgeAdv_p1,qedges,nlev*qsize,0,elem(ie)%desc)
! Subprogram not used 	call edgeVunpack(edgeAdv_p1,DSSvar(:,:,1:nlev),nlev,qsize*nlev,elem(ie)%desc)
! Subprogram not used 	do k=1,nlev
! Subprogram not used 	  DSSvar(:,:,k)=DSSvar(:,:,k)*elem(ie)%rspheremp(:,:)
! Subprogram not used 	enddo
! Subprogram not used      endif
! Subprogram not used 
! Subprogram not used      ! compute flux and advection term
! Subprogram not used      do k=1,nlev
! Subprogram not used         dp(:,:,k) = elem(ie)%derived%dp(:,:,k) - &
! Subprogram not used              rhs_multiplier*dt*elem(ie)%derived%divdp_proj(:,:,k) 
! Subprogram not used         Vstar(:,:,1,k) = elem(ie)%derived%vn0(:,:,1,k)/dp(:,:,k)
! Subprogram not used         Vstar(:,:,2,k) = elem(ie)%derived%vn0(:,:,2,k)/dp(:,:,k)
! Subprogram not used      enddo
! Subprogram not used 
! Subprogram not used      do q=1,qsize
! Subprogram not used         do k=1,nlev
! Subprogram not used            vtemp(:,:,1)=elem(ie)%state%Qdp(:,:,k,q,n0_qdp)*Vstar(:,:,1,k)
! Subprogram not used            vtemp(:,:,2)=elem(ie)%state%Qdp(:,:,k,q,n0_qdp)*Vstar(:,:,2,k)
! Subprogram not used            
! Subprogram not used            divdp = divergence_sphere_wk(vtemp,deriv,elem(ie)) + &
! Subprogram not used                 edge_flux_u_cg( Vstar(:,:,:,k), elem(ie)%state%Qdp(:,:,k,q,n0_qdp),qedges(:,:,k,q),&
! Subprogram not used                 deriv, elem(ie), u_is_contra=.false.)
! Subprogram not used 
! Subprogram not used            ! advance in time. GLL quadrature, cardinal function basis, under-integrated.  
! Subprogram not used            ! local mass matrix is diagonal, with entries elem(ie)%spheremp(),
! Subprogram not used            ! so we divide through by elem(ie)%spheremp().
! Subprogram not used            elem(ie)%state%Qdp(:,:,k,q,np1_qdp)=elem(ie)%state%Qdp(:,:,k,q,n0_qdp) - dt*divdp/elem(ie)%spheremp
! Subprogram not used            
! Subprogram not used            if (npdg<np) then
! Subprogram not used               ! modal timestep, with exact integration.  using prognostic variable: p*metdet
! Subprogram not used               ! local mass matrix is diagonal assuming npdg<np so that GLL quadrature is exact)
! Subprogram not used               ! (note: GLL/modal conversion comutes with time-stepping)
! Subprogram not used               
! Subprogram not used               ! compute modal coefficients of p*metdet
! Subprogram not used               ! (spherical inner-product of Legendre polynomial and p)
! Subprogram not used               pshat = gll_to_dgmodal(elem(ie)%state%Qdp(:,:,k,q,np1_qdp)*elem(ie)%metdet(:,:),deriv)
! Subprogram not used 
! Subprogram not used               ! modal based limiter goes here
! Subprogram not used               ! apply a little dissipation to last mode:
! Subprogram not used               do j=1,npdg
! Subprogram not used               do i=1,npdg
! Subprogram not used                  !if ( (i-1)+(j-1) == 4) pshat(i,j)=pshat(i,j)*.75
! Subprogram not used                  !if ( (i-1)+(j-1) == 3) pshat(i,j)=pshat(i,j)*.90
! Subprogram not used                  if ( i==npdg) pshat(i,j)=pshat(i,j)*.90
! Subprogram not used                  if ( j==npdg) pshat(i,j)=pshat(i,j)*.90
! Subprogram not used               enddo
! Subprogram not used               enddo
! Subprogram not used 
! Subprogram not used 
! Subprogram not used               ! evalute modal expanion of p*metdet on GLL points
! Subprogram not used               divdp=dgmodal_to_gll(pshat,deriv)  
! Subprogram not used 
! Subprogram not used               ! convert from p*metdet back to p:
! Subprogram not used               elem(ie)%state%Qdp(:,:,k,q,np1_qdp)=divdp/elem(ie)%metdet(:,:)
! Subprogram not used            endif
! Subprogram not used         enddo
! Subprogram not used         if(limiter_option == 4)then
! Subprogram not used            ! reuse CG limiter, which wants Qdp*spheremp:
! Subprogram not used            do k=1,nlev
! Subprogram not used               elem(ie)%state%Qdp(:,:,k,q,np1_qdp)=elem(ie)%state%Qdp(:,:,k,q,np1_qdp)*elem(ie)%spheremp(:,:)
! Subprogram not used            enddo
! Subprogram not used            call limiter2d_zero(elem(ie)%state%Qdp(:,:,:,q,np1_qdp),hvcoord)
! Subprogram not used            do k=1,nlev
! Subprogram not used               elem(ie)%state%Qdp(:,:,k,q,np1_qdp)=elem(ie)%state%Qdp(:,:,k,q,np1_qdp)/elem(ie)%spheremp(:,:)
! Subprogram not used            enddo
! Subprogram not used         endif
! Subprogram not used      end do
! Subprogram not used   end do
! Subprogram not used   call t_stopf('euler_step_dg')
! Subprogram not used   
! Subprogram not used   end subroutine euler_step_dg
...
! Subprogram not used   subroutine limiter2d_minmax(Q,dp,hvcoord,spheremp,qmin,qmax)
! Subprogram not used !
! Subprogram not used ! mass conserving limiter (2D only).  to be called just before DSS
! Subprogram not used !
! Subprogram not used ! in pure 2D advection, the element mass will not be negative before DSS
! Subprogram not used ! this routine will redistribute to remove negative values (conservative)
! Subprogram not used !
! Subprogram not used ! if used in 3D, should be applied with 2D/vertical split advection
! Subprogram not used ! 
! Subprogram not used ! call with Qdp and assocated dp
! Subprogram not used !
! Subprogram not used !
! Subprogram not used   implicit none
! Subprogram not used   real (kind=real_kind), intent(inout) :: Q(np,np,nlev)
! Subprogram not used   real (kind=real_kind), intent(in   ) :: spheremp(np,np)
! Subprogram not used   real (kind=real_kind), intent(in   ) ::  dp(np,np,nlev)
! Subprogram not used   type (hvcoord_t)     , intent(in   ) :: hvcoord
! Subprogram not used 
! Subprogram not used   ! local
! Subprogram not used   integer i,j,k
! Subprogram not used   real (kind=real_kind) :: mass,mass_new,area,qmin(nlev),qmax(nlev),mass2
! Subprogram not used 
! Subprogram not used   do k = 1 , nlev
! Subprogram not used     mass = sum( Q(:,:,k)*spheremp(:,:) )
! Subprogram not used     area = sum( dp(:,:,k)*spheremp(:,:) )
! Subprogram not used 
! Subprogram not used     Q(:,:,k) = Q(:,:,k) / dp(:,:,k)  ! % convert to concentration
! Subprogram not used 
! Subprogram not used !   if (mass>0) print *,k,mass/area,qmin(k),qmax(k)
! Subprogram not used     ! max limiter
! Subprogram not used     if ( maxval(Q(:,:,k)) > qmax(k) ) then
! Subprogram not used       Q(:,:,k) = qmax(k) - Q(:,:,k)      ! some of these will be negative
! Subprogram not used       mass2 = area * qmax(k) - mass
! Subprogram not used 
! Subprogram not used       if (mass2 < 0) Q(:,:,k) = -Q(:,:,k) 
! Subprogram not used       mass_new = 0
! Subprogram not used       do j = 1 , np
! Subprogram not used         do i = 1 , np
! Subprogram not used           if ( Q(i,j,k) < 0 ) then
! Subprogram not used             Q(i,j,k) = 0
! Subprogram not used           else
! Subprogram not used             mass_new = mass_new + Q(i,j,k) * dp(i,j,k) * spheremp(i,j)
! Subprogram not used           endif
! Subprogram not used         enddo
! Subprogram not used       enddo
! Subprogram not used     
! Subprogram not used       ! now scale the all positive values to restore mass
! Subprogram not used       if ( mass_new > 0 ) Q(:,:,k) = Q(:,:,k) * abs(mass2) / mass_new
! Subprogram not used       if ( mass2    < 0 ) Q(:,:,k) = -Q(:,:,k) 
! Subprogram not used       
! Subprogram not used       Q(:,:,k) = qmax(k) - Q(:,:,k)
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used     ! min limiter
! Subprogram not used     if ( minval(Q(:,:,k)) < qmin(k) ) then
! Subprogram not used       Q(:,:,k) = Q(:,:,k) - qmin(k)
! Subprogram not used       mass2 = mass - area * qmin(k)
! Subprogram not used       ! negative mass.  so reduce all postive values to zero 
! Subprogram not used       ! then increase negative values as much as possible
! Subprogram not used       if ( mass2 < 0 ) Q(:,:,k) = -Q(:,:,k) 
! Subprogram not used       mass_new = 0
! Subprogram not used       do j = 1 , np
! Subprogram not used         do i = 1 , np
...
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used     Q(:,:,k) = Q(:,:,k) * dp(:,:,k)
! Subprogram not used   enddo
! Subprogram not used   end subroutine limiter2d_minmax

!-----------------------------------------------------------------------------
!-----------------------------------------------------------------------------

! Subprogram not used   subroutine limiter2d_zero(Q,hvcoord)
! Subprogram not used   ! mass conserving zero limiter (2D only).  to be called just before DSS
! Subprogram not used   !
! Subprogram not used   ! this routine is called inside a DSS loop, and so Q had already
! Subprogram not used   ! been multiplied by the mass matrix.  Thus dont include the mass
! Subprogram not used   ! matrix when computing the mass = integral of Q over the element
! Subprogram not used   !
! Subprogram not used   ! ps is only used when advecting Q instead of Qdp
! Subprogram not used   ! so ps should be at one timelevel behind Q
! Subprogram not used   implicit none
! Subprogram not used   real (kind=real_kind), intent(inout) :: Q(np,np,nlev)
! Subprogram not used   type (hvcoord_t)     , intent(in   ) :: hvcoord
! Subprogram not used 
! Subprogram not used   ! local
! Subprogram not used   real (kind=real_kind) :: dp(np,np)
! Subprogram not used   real (kind=real_kind) :: mass,mass_new,ml
! Subprogram not used   integer i,j,k
! Subprogram not used 
! Subprogram not used   do k = nlev , 1 , -1
! Subprogram not used     mass = 0
! Subprogram not used     do j = 1 , np
! Subprogram not used       do i = 1 , np
! Subprogram not used         !ml = Q(i,j,k)*dp(i,j)*spheremp(i,j)  ! see above
! Subprogram not used         ml = Q(i,j,k)
! Subprogram not used         mass = mass + ml
! Subprogram not used       enddo
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used     ! negative mass.  so reduce all postive values to zero 
! Subprogram not used     ! then increase negative values as much as possible
! Subprogram not used     if ( mass < 0 ) Q(:,:,k) = -Q(:,:,k) 
! Subprogram not used     mass_new = 0
! Subprogram not used     do j = 1 , np
! Subprogram not used       do i = 1 , np
! Subprogram not used         if ( Q(i,j,k) < 0 ) then
...
! Subprogram not used         else
! Subprogram not used           ml = Q(i,j,k)
! Subprogram not used           mass_new = mass_new + ml
! Subprogram not used         endif
! Subprogram not used       enddo
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used     ! now scale the all positive values to restore mass
! Subprogram not used     if ( mass_new > 0 ) Q(:,:,k) = Q(:,:,k) * abs(mass) / mass_new
! Subprogram not used     if ( mass     < 0 ) Q(:,:,k) = -Q(:,:,k) 
! Subprogram not used   enddo
! Subprogram not used   end subroutine limiter2d_zero

!-----------------------------------------------------------------------------
!-----------------------------------------------------------------------------

! Subprogram not used   subroutine advance_hypervis_scalar( edgeAdv , elem , hvcoord , hybrid , deriv , nt , nt_qdp , nets , nete , dt2 )
! Subprogram not used   !  hyperviscsoity operator for foward-in-time scheme
! Subprogram not used   !  take one timestep of:  
! Subprogram not used   !          Q(:,:,:,np) = Q(:,:,:,np) +  dt2*nu*laplacian**order ( Q )
! Subprogram not used   !
...
! Subprogram not used   use hybrid_mod     , only : hybrid_t
! Subprogram not used   use element_mod    , only : element_t
! Subprogram not used   use derivative_mod , only : derivative_t
! Subprogram not used   use edge_mod       , only : EdgeBuffer_t, edgevpack, edgevunpack
! Subprogram not used   use bndry_mod      , only : bndry_exchangev
! Subprogram not used   use perf_mod       , only : t_startf, t_stopf                          ! _EXTERNAL
! Subprogram not used   implicit none
! Subprogram not used   type (EdgeBuffer_t)  , intent(inout)         :: edgeAdv
! Subprogram not used   type (element_t)     , intent(inout), target :: elem(:)
! Subprogram not used   type (hvcoord_t)     , intent(in   )         :: hvcoord
! Subprogram not used   type (hybrid_t)      , intent(in   )         :: hybrid
! Subprogram not used   type (derivative_t)  , intent(in   )         :: deriv
! Subprogram not used   integer              , intent(in   )         :: nt
! Subprogram not used   integer              , intent(in   )         :: nt_qdp
...
! Subprogram not used   real (kind=real_kind), dimension(np,np,nlev                ) :: dp
! Subprogram not used   real (kind=real_kind), dimension(      nlev,qsize,nets:nete) :: min_neigh
! Subprogram not used   real (kind=real_kind), dimension(      nlev,qsize,nets:nete) :: max_neigh
...
! Subprogram not used   real (kind=real_kind) :: v1,v2,dt,dp0
! Subprogram not used   integer :: density_scaling = 0
! Subprogram not used   if ( nu_q           == 0 ) return
! Subprogram not used   if ( hypervis_order /= 2 ) return
! Subprogram not used !   call t_barrierf('sync_advance_hypervis_scalar', hybrid%par%comm)
! Subprogram not used   call t_startf('advance_hypervis_scalar')
! Subprogram not used   
! Subprogram not used   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subprogram not used   !  hyper viscosity  
! Subprogram not used   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subprogram not used   dt = dt2 / hypervis_subcycle_q
! Subprogram not used 
! Subprogram not used   do ic = 1 , hypervis_subcycle_q
! Subprogram not used     do ie = nets , nete
! Subprogram not used       ! Qtens = Q/dp   (apply hyperviscsoity to dp0 * Q, not Qdp)
! Subprogram not used       do k = 1 , nlev
! Subprogram not used          ! various options:
! Subprogram not used          !   1)  biharmonic( Qdp )
! Subprogram not used          !   2)  dp0 * biharmonic( Qdp/dp )    
! Subprogram not used          !   3)  dpave * biharmonic(Q/dp)
! Subprogram not used          ! For trace mass / mass consistenciy, we use #2 when nu_p=0
! Subprogram not used          ! and #e when nu_p>0, where dpave is the mean mass flux from the nu_p
! Subprogram not used          ! contribution from dynamics.
! Subprogram not used          dp0 = ( hvcoord%hyai(k+1) - hvcoord%hyai(k) ) * hvcoord%ps0 + &
! Subprogram not used               ( hvcoord%hybi(k+1) - hvcoord%hybi(k) ) * hvcoord%ps0
! Subprogram not used          dp(:,:,k) = elem(ie)%derived%dp(:,:,k) - dt2*elem(ie)%derived%divdp_proj(:,:,k)
! Subprogram not used          if (nu_p>0) then
! Subprogram not used             do q = 1 , qsize
! Subprogram not used                Qtens(:,:,k,q,ie) = elem(ie)%derived%dpdiss_ave(:,:,k)*&
! Subprogram not used                     elem(ie)%state%Qdp(:,:,k,q,nt_qdp) / dp(:,:,k) 
! Subprogram not used             enddo
! Subprogram not used          else
! Subprogram not used             do q = 1 , qsize
! Subprogram not used                Qtens(:,:,k,q,ie) = dp0*elem(ie)%state%Qdp(:,:,k,q,nt_qdp) / dp(:,:,k)
! Subprogram not used             enddo
! Subprogram not used          endif
! Subprogram not used       enddo
! Subprogram not used    enddo
! Subprogram not used 
! Subprogram not used     ! compute biharmonic operator. Qtens = input and output 
! Subprogram not used     call biharmonic_wk_scalar( elem , Qtens , deriv , edgeAdv , hybrid , nets , nete )
! Subprogram not used     do ie = nets , nete
! Subprogram not used       !spheremp     => elem(ie)%spheremp
! Subprogram not used       do q = 1 , qsize
! Subprogram not used         do k = 1 , nlev
! Subprogram not used           dp0 = ( hvcoord%hyai(k+1) - hvcoord%hyai(k) ) * hvcoord%ps0 + &
! Subprogram not used                 ( hvcoord%hybi(k+1) - hvcoord%hybi(k) ) * hvcoord%ps0
! Subprogram not used           do j = 1 , np
! Subprogram not used             do i = 1 , np
! Subprogram not used               ! advection Qdp.  For mass advection consistency:
! Subprogram not used               ! DIFF( Qdp) ~   dp0 DIFF (Q)  =  dp0 DIFF ( Qdp/dp )  
! Subprogram not used               elem(ie)%state%Qdp(i,j,k,q,nt_qdp) = elem(ie)%state%Qdp(i,j,k,q,nt_qdp) * elem(ie)%spheremp(i,j) &
! Subprogram not used                                                    - dt * nu_q * Qtens(i,j,k,q,ie)
! Subprogram not used             enddo
! Subprogram not used           enddo
! Subprogram not used         enddo
! Subprogram not used 
! Subprogram not used         ! smooth some of the negativities introduced by diffusion:
! Subprogram not used         call limiter2d_zero( elem(ie)%state%Qdp(:,:,:,q,nt_qdp) , hvcoord )
! Subprogram not used       enddo
! Subprogram not used       call edgeVpack  ( edgeAdv , elem(ie)%state%Qdp(:,:,:,:,nt_qdp) , qsize*nlev , 0 , elem(ie)%desc )
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used     call bndry_exchangeV( hybrid , edgeAdv )
! Subprogram not used     
! Subprogram not used     do ie = nets , nete
! Subprogram not used       call edgeVunpack( edgeAdv , elem(ie)%state%Qdp(:,:,:,:,nt_qdp) , qsize*nlev , 0 , elem(ie)%desc )
! Subprogram not used       !rspheremp     => elem(ie)%rspheremp
! Subprogram not used       do q = 1 , qsize    
! Subprogram not used         ! apply inverse mass matrix
! Subprogram not used         do k = 1 , nlev
! Subprogram not used           elem(ie)%state%Qdp(:,:,k,q,nt_qdp) = elem(ie)%rspheremp(:,:) * elem(ie)%state%Qdp(:,:,k,q,nt_qdp)
! Subprogram not used         enddo
! Subprogram not used       enddo
! Subprogram not used     enddo
! Subprogram not used   enddo
! Subprogram not used   call t_stopf('advance_hypervis_scalar')
! Subprogram not used   end subroutine advance_hypervis_scalar
  ...
  use kinds, only : real_kind
  use hybvcoord_mod, only : hvcoord_t
  ! Next line uses module in same file, but remap1_nofilter never used and removed by codecov
  ! use vertremap_mod, only : remap1, remap1_nofilter, remap_q_ppm ! _EXTERNAL (actually INTERNAL)
  use vertremap_mod, only : remap1, remap_q_ppm ! _EXTERNAL (actually INTERNAL)
