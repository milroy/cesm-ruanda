...
! Subprogram not used   subroutine rebin( nsrc, ntrg, src_x, trg_x, src, trg )
! Subprogram not used     !---------------------------------------------------------------
! Subprogram not used     !	... rebin src to trg
! Subprogram not used     !---------------------------------------------------------------
! Subprogram not used 
...
! Subprogram not used     real(r8), intent(in)  :: src_x(nsrc+1)         ! source coordinates
! Subprogram not used     real(r8), intent(in)  :: trg_x(ntrg+1)         ! target coordinates
! Subprogram not used     real(r8), intent(in)  :: src(nsrc)             ! source array
! Subprogram not used     real(r8), intent(out) :: trg(ntrg)             ! target array
! Subprogram not used 
! Subprogram not used     !---------------------------------------------------------------
! Subprogram not used     !	... local variables
! Subprogram not used     !---------------------------------------------------------------
! Subprogram not used     integer  :: i, l
! Subprogram not used     integer  :: si, si1
! Subprogram not used     integer  :: sil, siu
! Subprogram not used     real(r8) :: y
! Subprogram not used     real(r8) :: sl, su
! Subprogram not used     real(r8) :: tl, tu
! Subprogram not used 
! Subprogram not used     !---------------------------------------------------------------
! Subprogram not used     !	... check interval overlap
! Subprogram not used     !---------------------------------------------------------------
! Subprogram not used     !     if( trg_x(1) < src_x(1) .or. trg_x(ntrg+1) > src_x(nsrc+1) ) then
! Subprogram not used     !        write(iulog,*) 'rebin: target grid is outside source grid'
! Subprogram not used     !        write(iulog,*) '       target grid from ',trg_x(1),' to ',trg_x(ntrg+1)
! Subprogram not used     !        write(iulog,*) '       source grid from ',src_x(1),' to ',src_x(nsrc+1)
! Subprogram not used     !        call endrun
! Subprogram not used     !     end if
! Subprogram not used 
! Subprogram not used     do i = 1,ntrg
! Subprogram not used        tl = trg_x(i)
! Subprogram not used        if( tl < src_x(nsrc+1) ) then
! Subprogram not used           do sil = 1,nsrc+1
! Subprogram not used              if( tl <= src_x(sil) ) then
! Subprogram not used                 exit
! Subprogram not used              end if
! Subprogram not used           end do
! Subprogram not used           tu = trg_x(i+1)
! Subprogram not used           do siu = 1,nsrc+1
! Subprogram not used              if( tu <= src_x(siu) ) then
! Subprogram not used                 exit
! Subprogram not used              end if
! Subprogram not used           end do
! Subprogram not used           y   = 0._r8
! Subprogram not used           sil = max( sil,2 )
! Subprogram not used           siu = min( siu,nsrc+1 )
! Subprogram not used           do si = sil,siu
! Subprogram not used              si1 = si - 1
! Subprogram not used              sl  = max( tl,src_x(si1) )
! Subprogram not used              su  = min( tu,src_x(si) )
! Subprogram not used              y   = y + (su - sl)*src(si1)
! Subprogram not used           end do
! Subprogram not used           trg(i) = y/(trg_x(i+1) - trg_x(i))
! Subprogram not used        else
! Subprogram not used           trg(i) = 0._r8
! Subprogram not used        end if
! Subprogram not used     end do
! Subprogram not used 
