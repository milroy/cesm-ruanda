...
!                call pbuf_allocate_field_long(pbuf2d, i)
  ...
  subroutine pbuf1d_print(pbuf)
    ...
    print *,"physics_buffer.F90.in",727,currentpbufflds,size(pbuf)
    do i=1,currentpbufflds
       pbufPtr => pbuf(i)
       print *,"physics_buffer.F90.in",730,i,trim(pbufPtr%hdr%name),pbufPtr%hdr%dtype,pbufPtr%hdr%persistence,pbufPtr%hdr%dimsizes
    end do
  ...
  subroutine get_pbuf1d_field_by_index_1d_int(pbuf, index, field, start,kount, col_type, copy_if_needed, errcode)
    ...
    if(index<1 .or. index>size(pbuf)) then
      print *,"physics_buffer.F90.in",799,index
