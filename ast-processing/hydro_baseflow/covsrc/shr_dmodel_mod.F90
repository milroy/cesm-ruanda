...
! Subprogram not used    real(R8) function rdiff(v1,v2) ! internal function
! Subprogram not used       !------------------------------------------
! Subprogram not used       real(R8),intent(in) :: v1,v2                 ! two values to compare
! Subprogram not used       real(R8),parameter  :: c0           = 0.0_R8 ! zero
! Subprogram not used       real(R8),parameter  :: large_number = 1.0e20_R8 ! infinity
! Subprogram not used       !------------------------------------------
! Subprogram not used       if (v1 == v2) then
! Subprogram not used          rdiff = c0
! Subprogram not used       elseif (v1 == c0 .and. v2 /= c0) then
! Subprogram not used          rdiff = large_number
! Subprogram not used       elseif (v2 == c0 .and. v1 /= c0) then
! Subprogram not used          rdiff = large_number
! Subprogram not used       else
! Subprogram not used !        rdiff = abs((v2-v1)/v1)   ! old version, but rdiff(v1,v2) /= vdiff(v2,v1)
! Subprogram not used          rdiff = abs(2.0_R8*(v2-v1)/(v1+v2))
! Subprogram not used       endif
! Subprogram not used       !------------------------------------------
! Subprogram not used    end function rdiff
...
! Subprogram not used subroutine shr_dmodel_mapSet_dest(smatp,ggridS,gsmapS,nxgS,nygS,ggridD,gsmapD,nxgD,nygD, &
! Subprogram not used            name,type,algo,mask,vect,compid,mpicom,strategy)
! Subprogram not used 
! Subprogram not used     use shr_map_mod
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     !----- arguments -----
! Subprogram not used     type(mct_sMatP), intent(inout) :: smatp
! Subprogram not used     type(mct_gGrid), intent(in)    :: ggridS
! Subprogram not used     type(mct_gsmap), intent(in)    :: gsmapS
! Subprogram not used     integer(IN)    , intent(in)    :: nxgS
! Subprogram not used     integer(IN)    , intent(in)    :: nygS
! Subprogram not used     type(mct_gGrid), intent(in)    :: ggridD
! Subprogram not used     type(mct_gsmap), intent(in)    :: gsmapD
! Subprogram not used     integer(IN)    , intent(in)    :: nxgD
! Subprogram not used     integer(IN)    , intent(in)    :: nygD
! Subprogram not used     character(len=*),intent(in)    :: name
! Subprogram not used     character(len=*),intent(in)    :: type
! Subprogram not used     character(len=*),intent(in)    :: algo
! Subprogram not used     character(len=*),intent(in)    :: mask
! Subprogram not used     character(len=*),intent(in)    :: vect
! Subprogram not used     integer(IN)    , intent(in)    :: compid
! Subprogram not used     integer(IN)    , intent(in)    :: mpicom
! Subprogram not used     character(len=*),intent(in),optional :: strategy
! Subprogram not used 
! Subprogram not used     !----- local -----
! Subprogram not used 
! Subprogram not used     integer(IN) :: n,i,j
! Subprogram not used     integer(IN) :: lsizeS,gsizeS,lsizeD,gsizeD
! Subprogram not used     integer(IN) :: nlon,nlat,nmsk
! Subprogram not used     integer(IN) :: my_task,master_task,ierr
! Subprogram not used 
! Subprogram not used     real(R8)   , pointer :: Xsrc(:,:)
! Subprogram not used     real(R8)   , pointer :: Ysrc(:,:)
! Subprogram not used     integer(IN), pointer :: Msrc(:,:)
! Subprogram not used     real(R8)   , pointer :: Xdst(:)
! Subprogram not used     real(R8)   , pointer :: Ydst(:)
! Subprogram not used     integer(IN), pointer :: Mdst(:)
! Subprogram not used     type(shr_map_mapType) :: shrmap
! Subprogram not used     type(mct_aVect) :: AVl
! Subprogram not used     type(mct_aVect) :: AVg
! Subprogram not used 
! Subprogram not used     character(len=32) :: lstrategy
! Subprogram not used     integer(IN) :: nsrc,ndst,nwts
! Subprogram not used     integer(IN), pointer :: points(:)
! Subprogram not used     integer(IN), pointer :: isrc(:)
! Subprogram not used     integer(IN), pointer :: idst(:)
! Subprogram not used     real(R8)   , pointer :: wgts(:)
! Subprogram not used     type(mct_sMat) :: sMat0
! Subprogram not used 
! Subprogram not used     character(*), parameter :: subname = '(shr_dmodel_mapSet_dest) '
! Subprogram not used     character(*), parameter :: F00   = "('(shr_dmodel_mapSet_dest) ',8a)"
! Subprogram not used     character(*), parameter :: F01   = "('(shr_dmodel_mapSet_dest) ',a,5i8)"
! Subprogram not used 
! Subprogram not used !-------------------------------------------------------------------------------
! Subprogram not used ! PURPOSE:  Initialize sMatP from mct gGrid
! Subprogram not used !-------------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used     call MPI_COMM_RANK(mpicom,my_task,ierr)
! Subprogram not used     master_task = 0
! Subprogram not used 
! Subprogram not used     !--- get sizes and allocate for SRC ---
! Subprogram not used 
! Subprogram not used     lsizeS = mct_aVect_lsize(ggridS%data)
! Subprogram not used     call mct_avect_init(AVl,rList='lon:lat:mask',lsize=lsizeS)
! Subprogram not used     call mct_avect_copy(ggridS%data,AVl,rList='lon:lat:mask')
! Subprogram not used 
! Subprogram not used     call mct_avect_gather(AVl,AVg,gsmapS,master_task,mpicom)
! Subprogram not used 
! Subprogram not used     allocate(Xsrc(nxgS,nygS),Ysrc(nxgS,nygS),Msrc(nxgS,nygS))
! Subprogram not used     if (my_task == master_task) then
! Subprogram not used        gsizeS = mct_aVect_lsize(AVg)
! Subprogram not used        if (gsizeS /= nxgS*nygS) then
! Subprogram not used           write(logunit,F01) ' ERROR: gsizeS ',gsizeS,nxgS,nygS
! Subprogram not used           call shr_sys_abort(subname//' ERROR gsizeS')
! Subprogram not used        endif
! Subprogram not used 
! Subprogram not used        nlon = mct_avect_indexRA(AVg,'lon')
! Subprogram not used        nlat = mct_avect_indexRA(AVg,'lat')
! Subprogram not used        nmsk = mct_avect_indexRA(AVg,'mask')
! Subprogram not used 
! Subprogram not used        n = 0
! Subprogram not used        Msrc = 1
! Subprogram not used        do j = 1,nygS
! Subprogram not used        do i = 1,nxgS
! Subprogram not used           n = n + 1
! Subprogram not used           Xsrc(i,j) = AVg%rAttr(nlon,n)
! Subprogram not used           Ysrc(i,j) = AVg%rAttr(nlat,n)
! Subprogram not used           if (abs(AVg%rAttr(nmsk,n)) < 0.5_R8) Msrc(i,j) = 0
! Subprogram not used        enddo
! Subprogram not used        enddo
! Subprogram not used     endif
! Subprogram not used     call shr_mpi_bcast(Xsrc,mpicom)
! Subprogram not used     call shr_mpi_bcast(Ysrc,mpicom)
! Subprogram not used     call shr_mpi_bcast(Msrc,mpicom)
! Subprogram not used 
! Subprogram not used     if (my_task == master_task) call mct_aVect_clean(AVg)
! Subprogram not used     call mct_aVect_clean(AVl)
! Subprogram not used 
! Subprogram not used     !--- get sizes and allocate for DST ---
! Subprogram not used 
! Subprogram not used     lsizeD = mct_aVect_lsize(ggridD%data)
! Subprogram not used     call mct_avect_init(AVl,rList='lon:lat:mask',lsize=lsizeD)
! Subprogram not used     call mct_avect_copy(ggridD%data,AVl,rList='lon:lat:mask')
! Subprogram not used 
! Subprogram not used 
! Subprogram not used     allocate(Xdst(lsizeD),Ydst(lsizeD),Mdst(lsizeD))
! Subprogram not used 
! Subprogram not used     nlon = mct_avect_indexRA(AVl,'lon')
! Subprogram not used     nlat = mct_avect_indexRA(AVl,'lat')
! Subprogram not used     nmsk = mct_avect_indexRA(AVl,'mask')
! Subprogram not used 
! Subprogram not used     Mdst = 1
! Subprogram not used     do n = 1,lsizeD
! Subprogram not used        Xdst(n) = AVl%rAttr(nlon,n)
! Subprogram not used        Ydst(n) = AVl%rAttr(nlat,n)
! Subprogram not used        if (abs(AVl%rAttr(nmsk,n)) < 0.5_R8) Mdst(n) = 0
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used     call mct_aVect_clean(AVl)
! Subprogram not used 
! Subprogram not used     !--- set map ---
! Subprogram not used 
! Subprogram not used     nsrc = nxgS*nygS
! Subprogram not used     ndst = nxgD*nygD
! Subprogram not used     call mct_gsmap_orderedPoints(gsmapD,my_task,points)
! Subprogram not used     if (size(points) /= size(Xdst)) then
! Subprogram not used        write(logunit,F01) ' ERROR: gsizeD ',size(points),size(Xdst)
! Subprogram not used        call shr_sys_abort(subname//' ERROR points size')
! Subprogram not used     endif
! Subprogram not used     call shr_map_mapSet(shrmap,Xsrc,Ysrc,Msrc,Xdst,Ydst,Mdst,ndst,points, &
! Subprogram not used        trim(name),trim(type),trim(algo),trim(mask),trim(vect))
! Subprogram not used     deallocate(points)
! Subprogram not used     deallocate(Xsrc,Ysrc,Msrc)
! Subprogram not used     deallocate(Xdst,Ydst,Mdst)
! Subprogram not used 
! Subprogram not used     !--- convert map to sMatP ---
! Subprogram not used 
! Subprogram not used     lstrategy = 'Xonly'
! Subprogram not used     if (present(strategy)) then
! Subprogram not used        lstrategy = trim(strategy)
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used     call shr_map_get(shrmap,shr_map_fs_nwts,nwts)
! Subprogram not used     allocate(isrc(nwts),idst(nwts),wgts(nwts))
! Subprogram not used     call shr_map_get(shrmap,isrc,idst,wgts)
! Subprogram not used     call shr_map_clean(shrmap)
! Subprogram not used 
! Subprogram not used     call mct_sMat_init(sMat0,ndst,nsrc,nwts)
! Subprogram not used 
! Subprogram not used     call mct_sMat_ImpLColI (sMat0,isrc,nwts)
! Subprogram not used     call mct_sMat_ImpLRowI (sMat0,idst,nwts)
! Subprogram not used     call mct_sMat_ImpMatrix(sMat0,wgts,nwts)
! Subprogram not used     deallocate(isrc,idst,wgts)
! Subprogram not used 
! Subprogram not used     call mct_sMatP_Init(sMatP,sMat0,gsmapS,gsmapD,master_task,mpicom,compid)
! Subprogram not used 
! Subprogram not used     call mct_sMat_clean(sMat0)
! Subprogram not used 
! Subprogram not used end subroutine shr_dmodel_mapSet_dest
...
! Subprogram not used subroutine shr_dmodel_translate_list( avi, avo, avifld, avofld, ilist, olist, cnt) 
! Subprogram not used 
! Subprogram not used   implicit none
! Subprogram not used 
! Subprogram not used   !----- arguments -----
! Subprogram not used   type(mct_aVect), intent(in)    :: avi       ! input av
! Subprogram not used   type(mct_aVect), intent(inout) :: avo       ! output av
! Subprogram not used   character(len=*),intent(in)    :: avifld(:) ! input field names for translation
! Subprogram not used   character(len=*),intent(in)    :: avofld(:) ! output field names for translation
! Subprogram not used   character(CL)   ,intent(out)   :: ilist     ! input list for translation
! Subprogram not used   character(CL)   ,intent(out)   :: olist     ! output list for translation
! Subprogram not used   integer(IN)     ,intent(out)   :: cnt       ! indices  
! Subprogram not used 
! Subprogram not used 
! Subprogram not used   !----- local -----
! Subprogram not used   integer(IN)      :: n,k,ka,kb,kc ! indices  
! Subprogram not used   integer(IN)      :: lsize        ! lsize
! Subprogram not used   integer(IN)      :: nflds        ! number of fields in avi
! Subprogram not used   character(CL)    :: cfld         ! character field name
! Subprogram not used   type(mct_string) :: sfld         ! string field
! Subprogram not used   integer(IN)      :: ktrans
! Subprogram not used   character(*), parameter :: subname = '(shr_dmodel_translateAV) '
! Subprogram not used 
! Subprogram not used !-------------------------------------------------------------------------------
! Subprogram not used ! PURPOSE:  Fill avo from avi
! Subprogram not used !-------------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used   if (size(avifld) /= size(avofld)) then
! Subprogram not used      write(logunit,*) subname,' ERROR": avi and avo fld list ',size(avifld),size(avofld)
! Subprogram not used      call shr_sys_flush(logunit)
! Subprogram not used   endif
! Subprogram not used   ktrans = size(avifld)
! Subprogram not used 
! Subprogram not used   ! generate fld lists
! Subprogram not used   nflds = mct_aVect_nRattr(avi)
! Subprogram not used   cnt = 0
! Subprogram not used   do ka = 1,nflds
! Subprogram not used      call mct_aVect_getRList(sfld,ka,avi)
! Subprogram not used      cfld = mct_string_toChar(sfld)
! Subprogram not used      call mct_string_clean(sfld)
! Subprogram not used 
! Subprogram not used      k = 0
! Subprogram not used      kb = 0
! Subprogram not used      kc = 0
! Subprogram not used      do while (kb == 0 .and. k < ktrans)
! Subprogram not used         k = k + 1
! Subprogram not used         if (trim(avifld(k)) == trim(cfld)) then
! Subprogram not used            kb = k
! Subprogram not used            kc = mct_aVect_indexRA(avo,trim(avofld(kb)),perrWith='quiet')
! Subprogram not used            if (ka > 0 .and. kc > 0) then
! Subprogram not used               cnt = cnt + 1
! Subprogram not used               if (cnt == 1) then
! Subprogram not used                  ilist = trim(avifld(kb))
! Subprogram not used                  olist = trim(avofld(kb))
! Subprogram not used               else
! Subprogram not used                  ilist = trim(ilist)//':'//trim(avifld(kb))
! Subprogram not used                  olist = trim(olist)//':'//trim(avofld(kb))
! Subprogram not used               endif
! Subprogram not used            endif
! Subprogram not used         endif
! Subprogram not used      enddo
! Subprogram not used   enddo
! Subprogram not used 
! Subprogram not used end subroutine shr_dmodel_translate_list

!===============================================================================

! Subprogram not used subroutine shr_dmodel_translateAV_list( avi, avo, ilist, olist, rearr )
! Subprogram not used 
! Subprogram not used   implicit none
! Subprogram not used 
! Subprogram not used   !----- arguments -----
! Subprogram not used   type(mct_aVect), intent(in)    :: avi       ! input av
! Subprogram not used   type(mct_aVect), intent(inout) :: avo       ! output av
! Subprogram not used   character(CL)   ,intent(in)    :: ilist     ! input list for translation
! Subprogram not used   character(CL)   ,intent(in)    :: olist     ! output list for translation
! Subprogram not used   type(mct_rearr), intent(in),optional :: rearr  ! rearranger for diff decomp
! Subprogram not used 
! Subprogram not used   !----- local -----
! Subprogram not used   integer(IN)      :: lsize       ! lsize
! Subprogram not used   type(mct_aVect)  :: avtri,avtro ! translated av on input/output grid
! Subprogram not used   character(*), parameter :: subname = '(shr_dmodel_translateAV) '
! Subprogram not used 
! Subprogram not used !-------------------------------------------------------------------------------
! Subprogram not used ! PURPOSE:  Fill avo from avi
! Subprogram not used !-------------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used   lsize = mct_avect_lsize(avi)
! Subprogram not used   call mct_avect_init(avtri,rlist=olist,lsize=lsize)
! Subprogram not used   call mct_avect_Copy(avi,avtri,rList=ilist,TrList=olist)
! Subprogram not used   
! Subprogram not used   if (present(rearr)) then
! Subprogram not used      lsize = mct_avect_lsize(avo)
! Subprogram not used      call mct_avect_init(avtro,rlist=olist,lsize=lsize)
! Subprogram not used      call mct_avect_zero(avtro)
! Subprogram not used      call mct_rearr_rearrange(avtri, avtro, rearr)
! Subprogram not used      call mct_avect_Copy(avtro,avo)
! Subprogram not used      call mct_aVect_clean(avtro)
! Subprogram not used   else
! Subprogram not used      call mct_avect_Copy(avtri,avo)
! Subprogram not used   endif
! Subprogram not used   
! Subprogram not used   call mct_aVect_clean(avtri)
! Subprogram not used   
! Subprogram not used end subroutine shr_dmodel_translateAV_list
