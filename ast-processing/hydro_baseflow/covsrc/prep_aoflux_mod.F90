...
! Subprogram not used   subroutine prep_aoflux_calc_xao_ox(timer)
! Subprogram not used     !---------------------------------------------------------------
! Subprogram not used     ! Description
! Subprogram not used     ! Create xao_ox 
! Subprogram not used     !
! Subprogram not used     ! Uses
! Subprogram not used     use prep_ocn_mod, only: prep_ocn_get_mapper_Fa2o
! Subprogram not used     !
! Subprogram not used     ! Arguments
! Subprogram not used     character(len=*), intent(in)    :: timer
! Subprogram not used     !
! Subprogram not used     ! Local Variables
! Subprogram not used     type(seq_map), pointer :: mapper_Fa2o
! Subprogram not used     integer :: exi
! Subprogram not used     character(*), parameter :: subname = '(prep_aoflux_calc_xao_ax)'
! Subprogram not used     character(*), parameter :: F00 = "('"//subname//" : ', 4A )"
! Subprogram not used     !---------------------------------------------------------------
! Subprogram not used 
! Subprogram not used     ! this mapping has to be done with area overlap mapping for all fields 
! Subprogram not used     ! due to the masking of the xao_ax data and the fact that a2oS is bilinear
! Subprogram not used 
! Subprogram not used     call t_drvstartf (trim(timer),barrier=mpicom_CPLID)
! Subprogram not used     do exi = 1,num_inst_xao 
! Subprogram not used        if (iamroot_CPLID .and. exi == 1) then 
! Subprogram not used           write(logunit,F00) 'Calling map_atm2ocn_mct for mapping xao_ax to xao_ox'
! Subprogram not used        end if
! Subprogram not used 
! Subprogram not used        mapper_Fa2o => prep_ocn_get_mapper_Fa2o()
! Subprogram not used        call seq_map_map(mapper_Fa2o, xao_ax(exi), xao_ox(exi), norm=.true.)
! Subprogram not used     enddo
! Subprogram not used     call t_drvstopf  (trim(timer))
! Subprogram not used 
! Subprogram not used   end subroutine prep_aoflux_calc_xao_ox
...
! Subprogram not used   function prep_aoflux_get_o2x_ax()
! Subprogram not used     type(mct_aVect), pointer :: prep_aoflux_get_o2x_ax(:)
! Subprogram not used     prep_aoflux_get_o2x_ax => o2x_ax(:)   
! Subprogram not used   end function prep_aoflux_get_o2x_ax
