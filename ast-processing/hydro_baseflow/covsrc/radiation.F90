...
! Subprogram not used subroutine radiation_get(iradsw_out, iradlw_out, iradae_out, irad_always_out, spectralflux_out)
! Subprogram not used !----------------------------------------------------------------------- 
! Subprogram not used ! Purpose: Provide access to private module data.  (This should be eliminated.)
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used    integer, intent(out), optional :: iradsw_out
! Subprogram not used    integer, intent(out), optional :: iradlw_out
! Subprogram not used    integer, intent(out), optional :: iradae_out
! Subprogram not used    integer, intent(out), optional :: irad_always_out
! Subprogram not used    logical, intent(out), optional :: spectralflux_out
! Subprogram not used    !-----------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used    if ( present(iradsw_out) )      iradsw_out = iradsw
! Subprogram not used    if ( present(iradlw_out) )      iradlw_out = iradlw
! Subprogram not used    if ( present(iradae_out) )      iradae_out = -999
! Subprogram not used    if ( present(irad_always_out) ) irad_always_out = irad_always
! Subprogram not used    if ( present(spectralflux_out) ) spectralflux_out = spectralflux_out
! Subprogram not used 
! Subprogram not used end subroutine radiation_get
...
!===============================================================================
  ...
  subroutine radiation_tend(state,ptend, pbuf, &
    ...
    ! 2007-12-27  M. Iacono        Modify to use 1 cloud optical properties with rrtmg.
    !-----------------------------------------------------------------------
...
! Subprogram not used subroutine calc_col_mean(state, mmr_pointer, mean_value)
! Subprogram not used !----------------------------------------------------------------------- 
! Subprogram not used ! 
! Subprogram not used ! Compute the column mean mass mixing ratio.  
! Subprogram not used !
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used 
...
! Subprogram not used    real(r8), dimension(:,:),   pointer     :: mmr_pointer  ! mass mixing ratio (lev)
! Subprogram not used    real(r8), dimension(pcols), intent(out) :: mean_value   ! column mean mmr
! Subprogram not used 
! Subprogram not used    integer  :: i, k, ncol
! Subprogram not used    real(r8) :: ptot(pcols)
! Subprogram not used    !-----------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used    ncol         = state%ncol
! Subprogram not used    mean_value   = 0.0_r8
! Subprogram not used    ptot         = 0.0_r8
! Subprogram not used 
! Subprogram not used    do k=1,pver
! Subprogram not used       do i=1,ncol
! Subprogram not used          mean_value(i) = mean_value(i) + mmr_pointer(i,k)*state%pdeldry(i,k)
! Subprogram not used          ptot(i)         = ptot(i) + state%pdeldry(i,k)
! Subprogram not used       end do
! Subprogram not used    end do
! Subprogram not used    do i=1,ncol
! Subprogram not used       mean_value(i) = mean_value(i) / ptot(i)
! Subprogram not used    end do
! Subprogram not used 
! Subprogram not used end subroutine calc_col_mean
