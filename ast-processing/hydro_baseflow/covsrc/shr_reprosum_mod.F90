...
! Subprogram not used    subroutine shr_reprosum_ddpdd (arr, arr_gsum, nsummands, dsummands,  &
! Subprogram not used                                   nflds, mpi_comm                       )
! Subprogram not used !----------------------------------------------------------------------
! Subprogram not used ! 
! Subprogram not used ! Purpose: 
! Subprogram not used ! Compute the global sum of each field in "arr" using the indicated 
! Subprogram not used ! communicator with a reproducible yet scalable implementation based 
! Subprogram not used ! on He and Ding's implementation of the double-double algorithm.
! Subprogram not used ! 
! Subprogram not used !----------------------------------------------------------------------
! Subprogram not used !
! Subprogram not used ! Arguments
! Subprogram not used !
! Subprogram not used       integer,  intent(in) :: nsummands  ! number of local summands
! Subprogram not used       integer,  intent(in) :: dsummands  ! declared first dimension
! Subprogram not used       integer,  intent(in) :: nflds      ! number of fields
! Subprogram not used       real(r8), intent(in) :: arr(dsummands,nflds) 
! Subprogram not used                                          ! input array
! Subprogram not used       integer,  intent(in) :: mpi_comm   ! MPI subcommunicator
! Subprogram not used 
! Subprogram not used       real(r8), intent(out):: arr_gsum(nflds)      
! Subprogram not used                                          ! global sums
! Subprogram not used 
! Subprogram not used !
! Subprogram not used ! Local workspace
! Subprogram not used !
! Subprogram not used       integer :: old_cw                  ! for x86 processors, save
! Subprogram not used                                          !  current arithmetic mode
! Subprogram not used       integer :: ifld, isum              ! loop variables
! Subprogram not used       integer :: ierr                    ! MPI error return
! Subprogram not used 
! Subprogram not used       real(r8)    :: e, t1, t2           ! temporaries
! Subprogram not used       complex(r8) :: arr_lsum_dd(nflds)  ! local sums (in double-double
! Subprogram not used                                          !  format)
! Subprogram not used       complex(r8) :: arr_gsum_dd(nflds)  ! global sums (in double-double
! Subprogram not used                                          !  format)
! Subprogram not used 
! Subprogram not used       integer, save :: mpi_sumdd
! Subprogram not used       logical, save :: first_time = .true.
! Subprogram not used 
! Subprogram not used !
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used !
! Subprogram not used       call shr_reprosumx86_fix_start (old_cw)
! Subprogram not used 
! Subprogram not used       if (first_time) then
! Subprogram not used          call mpi_op_create(ddpdd, .true., mpi_sumdd, ierr)
! Subprogram not used          first_time = .false.
! Subprogram not used       endif
! Subprogram not used 
! Subprogram not used       do ifld=1,nflds
! Subprogram not used          arr_lsum_dd(ifld) = (0.0_r8,0.0_r8)
! Subprogram not used 
! Subprogram not used          do isum=1,nsummands
! Subprogram not used                         
! Subprogram not used             ! Compute arr(isum,ifld) + arr_lsum_dd(ifld) using Knuth''s 
! Subprogram not used             ! trick.
! Subprogram not used             t1 = arr(isum,ifld) + real(arr_lsum_dd(ifld))
! Subprogram not used             e  = t1 - arr(isum,ifld)
! Subprogram not used             t2 = ((real(arr_lsum_dd(ifld)) - e) &
! Subprogram not used                   + (arr(isum,ifld) - (t1 - e))) &
! Subprogram not used                  + aimag(arr_lsum_dd(ifld))
! Subprogram not used 
! Subprogram not used             ! The result is t1 + t2, after normalization.
! Subprogram not used             arr_lsum_dd(ifld) = cmplx ( t1 + t2, t2 - ((t1 + t2) - t1), r8 )
! Subprogram not used          enddo
! Subprogram not used 
! Subprogram not used       enddo
! Subprogram not used 
! Subprogram not used       call mpi_allreduce (arr_lsum_dd, arr_gsum_dd, nflds, &
! Subprogram not used                           MPI_COMPLEX16, mpi_sumdd, mpi_comm, ierr)
! Subprogram not used       do ifld=1,nflds
! Subprogram not used          arr_gsum(ifld) = real(arr_gsum_dd(ifld))
! Subprogram not used       enddo
! Subprogram not used 
! Subprogram not used       call shr_reprosumx86_fix_end (old_cw)
! Subprogram not used 
! Subprogram not used    end subroutine shr_reprosum_ddpdd
!
!-----------------------------------------------------------------------
!
! Subprogram not used    subroutine DDPDD (dda, ddb, len, itype)
! Subprogram not used !----------------------------------------------------------------------
! Subprogram not used ! 
! Subprogram not used ! Purpose: 
! Subprogram not used ! Modification of original codes written by David H. Bailey    
! Subprogram not used ! This subroutine computes ddb(i) = dda(i)+ddb(i)
! Subprogram not used ! 
! Subprogram not used !----------------------------------------------------------------------
! Subprogram not used !
! Subprogram not used ! Arguments
! Subprogram not used !
! Subprogram not used       integer, intent(in)        :: len       ! array length
! Subprogram not used       complex(r8), intent(in)    :: dda(len)  ! input
! Subprogram not used       complex(r8), intent(inout) :: ddb(len)  ! result
! Subprogram not used       integer, intent(in)        :: itype     ! unused
! Subprogram not used !
! Subprogram not used ! Local workspace
! Subprogram not used !
! Subprogram not used       real(r8) e, t1, t2
! Subprogram not used       integer i
! Subprogram not used !
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used !
! Subprogram not used       do i = 1, len
! Subprogram not used !   Compute dda + ddb using Knuth's trick.
! Subprogram not used          t1 = real(dda(i)) + real(ddb(i))
! Subprogram not used          e  = t1 - real(dda(i))
! Subprogram not used          t2 = ((real(ddb(i)) - e) + (real(dda(i)) - (t1 - e))) &
! Subprogram not used               + aimag(dda(i)) + aimag(ddb(i))
! Subprogram not used 
! Subprogram not used !   The result is t1 + t2, after normalization.
! Subprogram not used          ddb(i) = cmplx ( t1 + t2, t2 - ((t1 + t2) - t1), r8 )
! Subprogram not used       enddo
! Subprogram not used 
! Subprogram not used 
! Subprogram not used    end subroutine DDPDD
!
!-----------------------------------------------------------------------
!
