...
! Subprogram not used       subroutine jlong_hrates( nlev, sza_in, alb_in, p_in, t_in, &
! Subprogram not used                                mw, o2_vmr, o3_vmr, colo3_in, qrl_col, &
! Subprogram not used                                cparg, kbot )
! Subprogram not used !==============================================================================
! Subprogram not used !   Purpose:                                                                   
! Subprogram not used !     To calculate the thermal heating rates longward of 200nm.        
! Subprogram not used !==============================================================================
! Subprogram not used !   Approach:
! Subprogram not used !     1) Reads the Cross Section*QY NetCDF file
! Subprogram not used !     2) Given a temperature profile, derives the appropriate XS*QY
! Subprogram not used !
! Subprogram not used !     3) Reads the Radiative Source function (RSF) NetCDF file
! Subprogram not used !        Units = quanta cm-2 sec-1
! Subprogram not used !
! Subprogram not used !     4) Indices are supplied to select a RSF that is consistent with
! Subprogram not used !        the reference atmosphere in TUV (for direct comparision of J's).
! Subprogram not used !        This approach will be replaced in the global model. Here colo3, zenith
! Subprogram not used !        angle, and altitude will be inputed and the correct entry in the table
! Subprogram not used !        will be derived.
! Subprogram not used !==============================================================================
! Subprogram not used 
! Subprogram not used 	use physconst,       only : avogad
! Subprogram not used         use error_messages, only : alloc_err
! Subprogram not used 
...
! Subprogram not used !------------------------------------------------------------------------------
! Subprogram not used       integer, intent (in)     :: nlev               ! number vertical levels
! Subprogram not used       integer, intent (in)     :: kbot               ! heating levels
! Subprogram not used       real(r8), intent(in)     :: o2_vmr(nlev)       ! o2 conc (mol/mol)
! Subprogram not used       real(r8), intent(in)     :: o3_vmr(nlev)       ! o3 conc (mol/mol)
! Subprogram not used       real(r8), intent(in)     :: sza_in             ! solar zenith angle (degrees)
! Subprogram not used       real(r8), intent(in)     :: alb_in(nlev)       ! albedo
! Subprogram not used       real(r8), intent(in)     :: p_in(nlev)         ! midpoint pressure (hPa)
! Subprogram not used       real(r8), intent(in)     :: t_in(nlev)         ! Temperature profile (K)
! Subprogram not used       real(r8), intent(in)     :: colo3_in(nlev)     ! o3 column density (molecules/cm^3)
! Subprogram not used       real(r8), intent(in)     :: mw(nlev)           ! atms molecular weight
! Subprogram not used       real(r8), intent(in)     :: cparg(nlev)        ! specific heat capacity
! Subprogram not used       real(r8), intent(inout)  :: qrl_col(:,:)	     ! heating rates
! Subprogram not used 
! Subprogram not used !----------------------------------------------------------------------
! Subprogram not used !  	... local variables
! Subprogram not used !----------------------------------------------------------------------
! Subprogram not used       integer  ::  astat
! Subprogram not used       integer  ::  k, km, m
! Subprogram not used       integer  ::  t_index					! Temperature index
! Subprogram not used       integer  ::  pndx
! Subprogram not used       real(r8) ::  ptarget
! Subprogram not used       real(r8) ::  delp
! Subprogram not used       real(r8) ::  hfactor
! Subprogram not used       real(r8), allocatable                :: rsf(:,:)	        ! Radiative source function
! Subprogram not used       real(r8), allocatable                :: xswk(:,:)	        ! working xsection array
! Subprogram not used       real(r8), allocatable                :: wrk(:)	        ! work vector
! Subprogram not used 
! Subprogram not used !----------------------------------------------------------------------
! Subprogram not used !        ... allocate variables
! Subprogram not used !----------------------------------------------------------------------
! Subprogram not used       allocate( rsf(nw,kbot),stat=astat )
! Subprogram not used       if( astat /= 0 ) then
! Subprogram not used          call alloc_err( astat, 'jlong_hrates', 'rsf', nw*nlev )
! Subprogram not used       end if
! Subprogram not used       allocate( xswk(nw,3),wrk(nw),stat=astat )
! Subprogram not used       if( astat /= 0 ) then
! Subprogram not used          call alloc_err( astat, 'jlong_hrates', 'xswk,wrk', 3*nw )
! Subprogram not used       end if
! Subprogram not used 
! Subprogram not used !----------------------------------------------------------------------
! Subprogram not used !        ... interpolate table rsf to model variables
! Subprogram not used !----------------------------------------------------------------------
! Subprogram not used       call interpolate_rsf( alb_in, sza_in, p_in, colo3_in, kbot, rsf )
! Subprogram not used 
! Subprogram not used !------------------------------------------------------------------------------
! Subprogram not used !     ... calculate thermal heating rates for wavelengths >200nm
! Subprogram not used !------------------------------------------------------------------------------
! Subprogram not used !------------------------------------------------------------------------------
! Subprogram not used !     LLNL LUT approach to finding temperature index...
! Subprogram not used !     Calculate the temperature index into the cross section
! Subprogram not used !     data which lists coss sections for temperatures from
! Subprogram not used !     150 to 350 degrees K.  Make sure the index is a value
! Subprogram not used !     between 1 and 201.
! Subprogram not used !------------------------------------------------------------------------------
! Subprogram not used       qrl_col(kbot+1:nlev,:) = 0._r8
! Subprogram not used level_loop_1 : &
! Subprogram not used       do k = 1,kbot
! Subprogram not used !----------------------------------------------------------------------
! Subprogram not used !   	... get index into xsqy
! Subprogram not used !----------------------------------------------------------------------
! Subprogram not used         t_index = t_in(k) - 148.5_r8
! Subprogram not used         t_index = min( 201,max( t_index,1) )
! Subprogram not used !----------------------------------------------------------------------
! Subprogram not used !   	... find pressure level
! Subprogram not used !----------------------------------------------------------------------
! Subprogram not used 	ptarget = p_in(k)
! Subprogram not used 	if( ptarget >= prs(1) ) then
! Subprogram not used 	   xswk(:,1) = xs_o2b(:,t_index,1)
! Subprogram not used 	   xswk(:,2) = xs_o3a(:,t_index,1)
! Subprogram not used 	   xswk(:,3) = xs_o3b(:,t_index,1)
! Subprogram not used 	else if( ptarget <= prs(np_xs) ) then
! Subprogram not used 	   xswk(:,1) = xs_o2b(:,t_index,np_xs)
! Subprogram not used 	   xswk(:,2) = xs_o3a(:,t_index,np_xs)
! Subprogram not used 	   xswk(:,3) = xs_o3b(:,t_index,np_xs)
! Subprogram not used 	else
! Subprogram not used 	   do km = 2,np_xs
! Subprogram not used 	      if( ptarget >= prs(km) ) then
! Subprogram not used 		 pndx = km - 1
! Subprogram not used 		 delp = (prs(pndx) - ptarget)*dprs(pndx)
! Subprogram not used 		 exit
! Subprogram not used 	      end if
! Subprogram not used 	   end do
! Subprogram not used 	   xswk(:,1) = xs_o2b(:,t_index,pndx) &
! Subprogram not used                        + delp*(xs_o2b(:,t_index,pndx+1) - xs_o2b(:,t_index,pndx))
! Subprogram not used 	   xswk(:,2) = xs_o3a(:,t_index,pndx) &
! Subprogram not used                        + delp*(xs_o3a(:,t_index,pndx+1) - xs_o3a(:,t_index,pndx))
! Subprogram not used 	   xswk(:,3) = xs_o3b(:,t_index,pndx) &
! Subprogram not used                        + delp*(xs_o3b(:,t_index,pndx+1) - xs_o3b(:,t_index,pndx))
! Subprogram not used 	end if
! Subprogram not used 	hfactor      = avogad/(cparg(k)*mw(k))
! Subprogram not used 	wrk(:)       = xswk(:,1)*bde_o2_b(:)
! Subprogram not used 	qrl_col(k,2) = dot_product( wrk(:),rsf(:,k) ) * o2_vmr(k) * hfactor
! Subprogram not used 	wrk(:)       = xswk(:,2)*bde_o3_a(:)
! Subprogram not used 	qrl_col(k,3) = dot_product( wrk(:),rsf(:,k) ) * o3_vmr(k) * hfactor
! Subprogram not used 	wrk(:)       = xswk(:,3)*bde_o3_b(:)
! Subprogram not used 	qrl_col(k,4) = dot_product( wrk(:),rsf(:,k) ) * o3_vmr(k) * hfactor
! Subprogram not used       end do level_loop_1
! Subprogram not used 
! Subprogram not used       deallocate( rsf, xswk, wrk )
! Subprogram not used 
! Subprogram not used       end subroutine jlong_hrates
