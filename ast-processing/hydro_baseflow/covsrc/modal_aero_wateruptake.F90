...
!-----------------------------------------------------------------------
!
! 1 specific driver for modal aerosol water uptake code.
!
...
!-----------------------------------------------------------------------
! Subprogram not used       subroutine makoh_cubic( cx, p2, p1, p0, im )
! Subprogram not used !
! Subprogram not used !     solves  x**3 + p2 x**2 + p1 x + p0 = 0
! Subprogram not used !     where p0, p1, p2 are real
! Subprogram not used !
! Subprogram not used       integer, parameter :: imx=200
! Subprogram not used       integer :: im
! Subprogram not used       real(r8) :: p0(imx), p1(imx), p2(imx)
! Subprogram not used       complex(r8) :: cx(3,imx)
! Subprogram not used 
! Subprogram not used       integer :: i
! Subprogram not used       real(r8) :: eps, q(imx), r(imx), sqrt3, third
! Subprogram not used       complex(r8) :: ci, cq, crad(imx), cw, cwsq, cy(imx), cz(imx)
! Subprogram not used 
! Subprogram not used       save eps
! Subprogram not used       data eps/1.e-20_r8/
! Subprogram not used 
! Subprogram not used       third=1._r8/3._r8
! Subprogram not used       ci=cmplx(0._r8,1._r8,r8)
! Subprogram not used       sqrt3=sqrt(3._r8)
! Subprogram not used       cw=0.5_r8*(-1+ci*sqrt3)
! Subprogram not used       cwsq=0.5_r8*(-1-ci*sqrt3)
! Subprogram not used 
! Subprogram not used       do i=1,im
! Subprogram not used       if(p1(i).eq.0._r8)then
! Subprogram not used !        completely insoluble particle
! Subprogram not used          cx(1,i)=(-p0(i))**third
! Subprogram not used          cx(2,i)=cx(1,i)
! Subprogram not used          cx(3,i)=cx(1,i)
! Subprogram not used       else
! Subprogram not used          q(i)=p1(i)/3._r8
! Subprogram not used          r(i)=p0(i)/2._r8
! Subprogram not used          crad(i)=r(i)*r(i)+q(i)*q(i)*q(i)
! Subprogram not used          crad(i)=sqrt(crad(i))
! Subprogram not used 
! Subprogram not used          cy(i)=r(i)-crad(i)
! Subprogram not used          if (abs(cy(i)).gt.eps) cy(i)=cy(i)**third
! Subprogram not used          cq=q(i)
! Subprogram not used          cz(i)=-cq/cy(i)
! Subprogram not used 
! Subprogram not used          cx(1,i)=-cy(i)-cz(i)
! Subprogram not used          cx(2,i)=-cw*cy(i)-cwsq*cz(i)
! Subprogram not used          cx(3,i)=-cwsq*cy(i)-cw*cz(i)
! Subprogram not used       endif
! Subprogram not used       enddo
! Subprogram not used 
! Subprogram not used       return
! Subprogram not used       end subroutine makoh_cubic
