...
! Subprogram not used       logical function is_leap_year(year)
! Subprogram not used         ! returns .true. if year is a leap year
! Subprogram not used 
! Subprogram not used         ! Input/output paramters
! Subprogram not used         integer, intent(in) :: year
! Subprogram not used 
! Subprogram not used         is_leap_year = .false.
! Subprogram not used         if (mod(year,  4) == 0) is_leap_year = .true.
! Subprogram not used         if (mod(year,100) == 0) is_leap_year = .false.
! Subprogram not used         if (mod(year,400) == 0) is_leap_year = .true.
! Subprogram not used 
! Subprogram not used         end function is_leap_year
