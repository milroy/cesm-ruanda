...
! Subprogram not used  subroutine clean_(ReArr, status)
! Subprogram not used 
! Subprogram not used !
! Subprogram not used ! !USES:
! Subprogram not used !
! Subprogram not used    use m_Router,only : Router     
! Subprogram not used    use m_Router,only : Router_clean => clean
...
! Subprogram not used !
! Subprogram not used    type(Rearranger),    intent(inout)           :: ReArr
! Subprogram not used 
...
! Subprogram not used ! 31Jan02 - E.T. Ong <eong@mcs.anl.gov> - initial prototype
! Subprogram not used ! 20Mar02 - E.T. Ong <eong@mcs.anl.gov> - working code
...
! Subprogram not used 
! Subprogram not used         ! Set output status flag (if present) to zero, which assumes
! Subprogram not used         ! success.
! Subprogram not used 
! Subprogram not used    if(present(status)) status = 0
! Subprogram not used 
! Subprogram not used         ! Clean up send and receive Routers:
! Subprogram not used 
! Subprogram not used    call Router_clean(ReArr%SendRouter,ier)
...
! Subprogram not used 	   ':: ERROR--Router_clean(ReArr%SendRouter) failed with ier=',ier
! Subprogram not used       endif
! Subprogram not used    endif
! Subprogram not used 
! Subprogram not used    call Router_clean(ReArr%RecvRouter,ier)
! Subprogram not used    if(ier /= 0) then
! Subprogram not used       if(present(status)) then
! Subprogram not used 	 status = ier
! Subprogram not used 	 return
! Subprogram not used       else
! Subprogram not used          write(stderr,'(2a,i8)') myname_, &
! Subprogram not used 	   ':: ERROR--Router_clean(ReArr%RecvRouter) failed with ier=',ier
! Subprogram not used       endif
! Subprogram not used    endif
! Subprogram not used 
! Subprogram not used        ! Clean up Local on-PE copy buffer:
! Subprogram not used 
! Subprogram not used    if(associated(ReArr%LocalPack)) then
! Subprogram not used       deallocate(ReArr%LocalPack, stat=ier)
! Subprogram not used       if(ier /= 0) then
! Subprogram not used 	 if(present(status)) then
! Subprogram not used 	    status=ier
! Subprogram not used 	 else
! Subprogram not used 	    write(stderr,'(2a,i8)') myname_, &
! Subprogram not used 	      ':: ERROR--deallocate(ReArr%LocalPack) failed with stat=',ier
! Subprogram not used 	 endif
! Subprogram not used       endif
! Subprogram not used    endif
! Subprogram not used 
...
! Subprogram not used     subroutine print_(rearr,mycomm,lun)
! Subprogram not used !
! Subprogram not used ! !USES:
! Subprogram not used !
! Subprogram not used       use m_die
! Subprogram not used       use m_Router, only: router_print => print
! Subprogram not used 
! Subprogram not used       implicit none
! Subprogram not used 
! Subprogram not used !INPUT/OUTPUT PARAMETERS:
! Subprogram not used       type(Rearranger),      intent(in) :: rearr
...
! Subprogram not used 
! Subprogram not used 
! Subprogram not used       call router_print(rearr%SendRouter,mycomm,lun)
! Subprogram not used       call router_print(rearr%RecvRouter,mycomm,lun)
! Subprogram not used 
