...
!------------------------------------------------------------------
!  Reads in:
!     file from which to read aerosol Masses on 1 grid. Currently
!        assumed to be MATCH ncep runs, averaged by month.
!     NOTE (Data have been externally interpolated onto 1 grid 
...
!  Method:
!    read data from file
!    allocate memory for storage of aerosol data on 1 horizontal grid
...
! 
   ...
   if ( ierr==NF90_NOERR ) then
      ...
      call handle_ncerr(nf90_close(aernid),subname, 350)
      ...
      if (masterproc) then
         ...
         ! First ensure dataset is 1-ready
...
!--------------------------------------------------------------------
   ...
   real(r8), intent(in) :: pint(pcols,pverp)              ! interface pressure from 1
