...
subroutine box_rearrange_comp2io_real (IOsystem, ioDesc, s1, src, niodof, &
  ...
  if ( present( comm_option ) ) then
     if ((comm_option == 0) &
         .or. (comm_option == 1) &
         .or. (comm_option == 2)) then
         pio_option = comm_option
      endif
  else
     pio_option = 2
  endif
  if (pio_option == 2) then
    pio_hs     = .true.
    pio_isend  = .false.
    pio_maxreq = 64
  ...
  if (s1 > 0 .and. s1<ndof) &
    call piodie( "box_rearrange.F90.in",249, &
  ...
  if (pio_option /= 1) then
    ...
    do i=1,num_iotasks
      if (scount(i) /= 0) then
        ...
        if(Debug) print *, "box_rearrange.F90.in",337,myrank,': send posted dest=',io_comprank,' count=',scount(i), stype(i)

        call MPI_ISEND( src, 1, stype(i),    &                 ! buf, count, type
                        io_comprank,102,    &                 ! destination,tag
...
subroutine box_rearrange_io2comp_real (IOsystem,ioDesc,s1, iobuf,s2, compbuf, &
  ...
  if (size(compbuf) > 0 .and. size(compbuf)<ndof) &
    call piodie( "box_rearrange.F90.in",515, &
    ...
    loop_ioproc: do while(.not. found.and.k<1004)
       ...
       do j=1,ndim
          if ( gcoord(j) < lb(j,i) ) then
             ...
             if(k>1000) print *,"/glade/u/home/milroy/cesm/cesm1_3_beta11/models/utils/pio/box_rearrange.F90.in",748,i,gcoord(:),lb(:,i),ub(:,i)
             cycle loop_ioproc
          else  if(gcoord(j) >= ub(j,i) ) then
             i = min(nioproc,i+decompstep(j))
             if(k>1000) print *,"/glade/u/home/milroy/cesm/cesm1_3_beta11/models/utils/pio/box_rearrange.F90.in",752,i,gcoord(:),lb(:,i),ub(:,i)
             cycle loop_ioproc
          endif
       end do
...
!    if(Debug) print *,"box_rearrange.F90.in",845,minval(compdof), maxval(compdof)
    do i=1,ndof
       ...
       if (compdof(i)==0) then             ! sender hole
          ...
          if (.not. find_ioproc(gcoord, lb, ub, lstride, ndim, nioproc, &
             ...
             call piodie( "box_rearrange.F90.in",878, 'quitting' )
          endif
...
!!!!!!
    ...
    if(ndim.ne.size(iodesc%start)) then
       print *,"box_rearrange.F90.in",936,ndim, size(iodesc%start)
       call piodie("box_rearrange.F90.in",937,'bad ndim size',ndim)
    end if
    ...
    if (Iosystem%IOproc) then
       ...
       if(Debug) then
          print *, "box_rearrange.F90.in",965,iodesc%start, iodesc%count
          if(iosystem%io_rank==0) &
               print *,"box_rearrange.F90.in",967,ndim,(i,' :', &
    ...
    do i=1,ndof
       ...
       if (iorank /= -1) then                       ! not a sender hole
          if (iorank<1 .or. iorank>num_iotasks) &
               call piodie("box_rearrange.F90.in",1146,'io destination out of range',iorank)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    do i=2,num_iotasks
       ...
       if (scount(i)/=0 .and. spos(i) > ndof) & 
            call piodie("box_rearrange.F90.in",1290,'spos=',int(spos(i)),'> ndof=',ndof)
    end do
    ...
    do i=1,ndof
       ...
       if (iorank /= -1) then                                  ! skip sender hole
          ...
          if (tempcount(iorank) > scount(iorank)) &
               call piodie("box_rearrange.F90.in",1305,'tempcount>scount')
       endif
    end do
