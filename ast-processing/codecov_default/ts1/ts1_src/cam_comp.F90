...
!-----------------------------------------------------------------------
!
! Purpose:  The 1 Community Atmosphere Model component. Interfaces with 
...
!-----------------------------------------------------------------------
   ...
   !
   public cam_init      ! First phase of 1 initialization
   public cam_run1      ! 1 run method phase 1
   public cam_run2      ! 1 run method phase 2
   public cam_run3      ! 1 run method phase 3
   public cam_run4      ! 1 run method phase 4
   public cam_final     ! 1 Finalization
   !
   ! Private module data
   !
...
!   use shr_orb_mod,      only: shr_orb_params
   ...
   ! Arguments
   !
   type(cam_out_t), pointer        :: cam_out(:)       ! Output from 1 to surface
   type(cam_in_t) , pointer        :: cam_in(:)        ! Merged input state to 1
   integer            , intent(in) :: mpicom_atm       ! 1 MPI communicator
   ...
   etamid = nan
   !
   ! Initialize 1 MPI communicator, number of processors, master processors 
   !	
...
!-----------------------------------------------------------------------
   ...
   !
   type(cam_out_t), pointer :: cam_out(:) ! Output from 1 to surface
   type(cam_in_t), pointer :: cam_in(:)   ! Input from merged surface to 1
!-----------------------------------------------------------------------
   !
   ! Local variables
   !
   ...
   integer :: iu              ! 1 Statistics output unit number
   character*24 :: filenam    ! 1 Stats output filename
...
!------------------------------Externals--------------------------------
   ...
   ! output log to figure out if 1 completed successfully.  This problem has 
