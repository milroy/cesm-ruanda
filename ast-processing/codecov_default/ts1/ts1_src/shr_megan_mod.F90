...
333 format ('Fall_voc',i3.3)
  ...
  !-------------------------------------------------------------------------
  !-------------------------------------------------------------------------
! Subprogram not used   function get_parser_items( spec_entry ) result(items)
! Subprogram not used 
! Subprogram not used     character(len=*), intent(in) :: spec_entry
! Subprogram not used 
! Subprogram not used     type(parser_items_t), pointer :: items ! items returned
! Subprogram not used 
! Subprogram not used     integer :: ndxs(512)
! Subprogram not used     integer :: nelem, j, i
! Subprogram not used     character(len=CL) :: tmp_str
! Subprogram not used 
! Subprogram not used     j = scan( spec_entry, '=' )
! Subprogram not used 
! Subprogram not used     nelem = 1
! Subprogram not used     ndxs(nelem) = j
! Subprogram not used 
! Subprogram not used     tmp_str = trim( spec_entry(j+1:) )
! Subprogram not used     j = scan( tmp_str, '+' )
! Subprogram not used 
! Subprogram not used     do while(j>0)
! Subprogram not used 
! Subprogram not used        nelem = nelem+1
! Subprogram not used        ndxs(nelem) = ndxs(nelem-1) + j
! Subprogram not used 
! Subprogram not used        tmp_str = tmp_str(j+1:)
! Subprogram not used        j = scan( tmp_str, '+' )
! Subprogram not used 
! Subprogram not used     enddo
! Subprogram not used     ndxs(nelem+1) = len(spec_entry)+1
! Subprogram not used 
! Subprogram not used     allocate(items)
! Subprogram not used     allocate(items%megan_comp_names(nelem))
! Subprogram not used     items%mech_comp_name = trim(adjustl( spec_entry(:ndxs(1)-1)))
! Subprogram not used     items%n_megan_comps = nelem
! Subprogram not used     do i = 1,nelem 
! Subprogram not used        items%megan_comp_names(i) = trim(adjustl( spec_entry(ndxs(i)+1:ndxs(i+1)-1)))
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used   endfunction get_parser_items

  !-------------------------------------------------------------------------
  !-------------------------------------------------------------------------
! Subprogram not used   subroutine destroy_parser_items( items )
! Subprogram not used     type(parser_items_t), pointer :: items
! Subprogram not used 
! Subprogram not used     deallocate( items%megan_comp_names )
! Subprogram not used     deallocate( items )
! Subprogram not used     nullify( items )
! Subprogram not used   endsubroutine destroy_parser_items

  !-------------------------------------------------------------------------
  !-------------------------------------------------------------------------
! Subprogram not used   function add_megan_comp( name ) result(megan_comp)
! Subprogram not used 
! Subprogram not used     character(len=16), intent(in) :: name
! Subprogram not used     type(shr_megan_megcomp_t), pointer :: megan_comp
! Subprogram not used 
! Subprogram not used     megan_comp => get_megan_comp_by_name(shr_megan_linkedlist, name)
! Subprogram not used     if(associated(megan_comp)) then
! Subprogram not used        ! already in the list so return...
! Subprogram not used        return
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used     ! create new megan compound and add it to the list
! Subprogram not used     allocate(megan_comp)
! Subprogram not used 
! Subprogram not used     !    element%index = lookup_element( name )
! Subprogram not used     !    element%emis_factors = get_factors( list_elem%index )
! Subprogram not used 
! Subprogram not used     megan_comp%index = shr_megan_megcomps_n+1
! Subprogram not used 
! Subprogram not used     megan_comp%name = trim(name)
! Subprogram not used     nullify(megan_comp%next_megcomp)
! Subprogram not used 
! Subprogram not used     call add_megan_comp_to_list(megan_comp)
! Subprogram not used 
! Subprogram not used   end function add_megan_comp

  !-------------------------------------------------------------------------
  !-------------------------------------------------------------------------
! Subprogram not used   recursive function get_megan_comp_by_name(list_comp, name) result(megan_comp)
! Subprogram not used 
! Subprogram not used     type(shr_megan_megcomp_t), pointer  :: list_comp
! Subprogram not used     character(len=*), intent(in) :: name  ! variable name
! Subprogram not used     type(shr_megan_megcomp_t), pointer  :: megan_comp ! returned object
! Subprogram not used 
! Subprogram not used     if(associated(list_comp)) then
! Subprogram not used        if(list_comp%name .eq. name) then
! Subprogram not used           megan_comp => list_comp
! Subprogram not used        else
! Subprogram not used           megan_comp => get_megan_comp_by_name(list_comp%next_megcomp, name)
! Subprogram not used        end if
! Subprogram not used     else
! Subprogram not used        nullify(megan_comp)
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used   end function get_megan_comp_by_name

  !-------------------------------------------------------------------------
  !-------------------------------------------------------------------------
! Subprogram not used   subroutine add_megan_comp_to_list( new_megan_comp )
! Subprogram not used 
! Subprogram not used     type(shr_megan_megcomp_t), target, intent(in) :: new_megan_comp
! Subprogram not used 
! Subprogram not used     type(shr_megan_megcomp_t), pointer :: list_comp
! Subprogram not used 
! Subprogram not used     if(associated(shr_megan_linkedlist)) then
! Subprogram not used        list_comp => shr_megan_linkedlist
! Subprogram not used        do while(associated(list_comp%next_megcomp))
! Subprogram not used           list_comp => list_comp%next_megcomp
! Subprogram not used        end do
! Subprogram not used        list_comp%next_megcomp => new_megan_comp
! Subprogram not used     else
! Subprogram not used        shr_megan_linkedlist => new_megan_comp
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used     shr_megan_megcomps_n = shr_megan_megcomps_n + 1
! Subprogram not used 
! Subprogram not used   end subroutine add_megan_comp_to_list
