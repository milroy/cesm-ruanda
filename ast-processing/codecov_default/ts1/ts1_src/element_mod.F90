...
! Subprogram not used   function GetColumnIdP(elem,i,j) result(col_id)
! Subprogram not used 
! Subprogram not used     ! Get unique identifier for a Physics column on the P-grid
! Subprogram not used 
! Subprogram not used     type(element_t), intent(in) :: elem
! Subprogram not used     integer, intent(in) :: i,j
! Subprogram not used     integer :: col_id
! Subprogram not used     col_id = elem%gdofP(i,j)
! Subprogram not used   end function GetColumnIdP

  !___________________________________________________________________
! Subprogram not used   function GetColumnIdV(elem,i,j) result(col_id)
! Subprogram not used 
! Subprogram not used     !  Get unique identifier for a Physics column on the V-grid
! Subprogram not used 
! Subprogram not used     type(element_t), intent(in) :: elem
! Subprogram not used     integer, intent(in) :: i,j
! Subprogram not used     integer :: col_id
! Subprogram not used     col_id = elem%gdofP(i,j)
! Subprogram not used   end function GetColumnIdV

  !___________________________________________________________________
! Subprogram not used   function element_coordinates(start,end,points) result(cart)
! Subprogram not used 
! Subprogram not used     ! Initialize 2D rectilinear element colocation points
! Subprogram not used 
! Subprogram not used     use kinds, only : longdouble_kind
! Subprogram not used     type (cartesian2D_t), intent(in) :: start
! Subprogram not used     type (cartesian2D_t), intent(in) :: end
! Subprogram not used     real (kind=longdouble_kind), intent(in) :: points(:)
! Subprogram not used 
! Subprogram not used     type (cartesian2D_t) :: cart(SIZE(points),SIZE(points))
! Subprogram not used     type (cartesian2D_t) :: length, centroid
! Subprogram not used     real (kind=longdouble_kind) :: y 
! Subprogram not used     integer i,j
! Subprogram not used 
! Subprogram not used     length%x   = 0.50D0*(end%x-start%x)
! Subprogram not used     length%y   = 0.50D0*(end%y-start%y)
! Subprogram not used     centroid%x = 0.50D0*(end%x+start%x) 
! Subprogram not used     centroid%y = 0.50D0*(end%y+start%y) 
! Subprogram not used     do j=1,SIZE(points)
! Subprogram not used        y = centroid%y + length%y*points(j)
! Subprogram not used        do i=1,SIZE(points)
! Subprogram not used           cart(i,j)%x = centroid%x + length%x*points(i)
! Subprogram not used           cart(i,j)%y = y
! Subprogram not used        end do
! Subprogram not used     end do
! Subprogram not used   end function element_coordinates
  ...
  !___________________________________________________________________
! Subprogram not used   function element_var_coordinates3d(c,points) result(cart)
! Subprogram not used 
! Subprogram not used     use kinds, only : longdouble_kind
! Subprogram not used     type (cartesian3D_t), intent(in) :: c(4)
! Subprogram not used     real (kind=longdouble_kind), intent(in) :: points(:)
! Subprogram not used     type (cartesian3D_t) :: cart(SIZE(points),SIZE(points))
! Subprogram not used 
! Subprogram not used     real (kind=longdouble_kind) :: p(size(points))
! Subprogram not used     real (kind=longdouble_kind) :: q(size(points)),r
! Subprogram not used     integer i,j
! Subprogram not used 
! Subprogram not used     p(:) = (1.0D0-points(:))/2.0D0
! Subprogram not used     q(:) = (1.0D0+points(:))/2.0D0
! Subprogram not used 
! Subprogram not used     do j=1,SIZE(points)
! Subprogram not used        do i=1,SIZE(points)
! Subprogram not used           cart(i,j)%x = p(i)*p(j)*c(1)%x &
! Subprogram not used                       + q(i)*p(j)*c(2)%x &
! Subprogram not used                       + q(i)*q(j)*c(3)%x &
! Subprogram not used                       + p(i)*q(j)*c(4)%x 
! Subprogram not used           cart(i,j)%y = p(i)*p(j)*c(1)%y &
! Subprogram not used                       + q(i)*p(j)*c(2)%y &
! Subprogram not used                       + q(i)*q(j)*c(3)%y &
! Subprogram not used                       + p(i)*q(j)*c(4)%y 
! Subprogram not used           cart(i,j)%z = p(i)*p(j)*c(1)%z &
! Subprogram not used                       + q(i)*p(j)*c(2)%z &
! Subprogram not used                       + q(i)*q(j)*c(3)%z &
! Subprogram not used                       + p(i)*q(j)*c(4)%z 
! Subprogram not used 
! Subprogram not used           ! project back to sphere:
! Subprogram not used           r = distance(cart(i,j))
! Subprogram not used           cart(i,j)%x = cart(i,j)%x/r
! Subprogram not used           cart(i,j)%y = cart(i,j)%y/r
! Subprogram not used           cart(i,j)%z = cart(i,j)%z/r
! Subprogram not used        end do
! Subprogram not used     end do
! Subprogram not used   end function element_var_coordinates3d
