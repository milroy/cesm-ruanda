...
! Subprogram not used   function Temp2PotTemp(pr3d,t3d) result(pt3d)
! Subprogram not used     real (kind=real_kind), parameter  :: c_p = 1004.64D0    ! Cp 
! Subprogram not used     real (kind=real_kind), parameter  :: p_0 = 100000.0D0   ! Initial Surface pressure
! Subprogram not used     real (kind=real_kind), parameter  :: r_d = 287.04D0      ! Gas const (dry)
! Subprogram not used     real (kind=real_kind),intent(in) :: pr3d(np,np,nlev),t3d(np,np,nlev)
! Subprogram not used     real (kind=real_kind)            :: pt3d(np,np,nlev)
! Subprogram not used     integer:: i,j,k    
! Subprogram not used     real (kind=real_kind):: rdcp,pp
! Subprogram not used     
! Subprogram not used     rdcp = r_d/c_p
! Subprogram not used     do k=1,nlev
! Subprogram not used        do j=1,np
! Subprogram not used           do i=1,np
! Subprogram not used              pp = (pr3d(i,j,k) + pr3d(i,j,k+1))*0.5D0
! Subprogram not used              pt3d(i,j,k)=  t3d(i,j,k)*(p_0/pp)**rdcp 
! Subprogram not used           enddo
! Subprogram not used        enddo
! Subprogram not used     enddo
! Subprogram not used   end function Temp2PotTemp

! Subprogram not used   function getsurfpress(lnps) result (press)
! Subprogram not used     real (kind=real_kind) :: press(np,np)
! Subprogram not used     real (kind=real_kind) :: lnps(np,np)
! Subprogram not used 
! Subprogram not used     press(:,:) = 0.0
! Subprogram not used     
! Subprogram not used   end function getsurfpress
