...
! Subprogram not used    integer(SHR_KIND_IN),external    :: system ! function to envoke shell command
! Subprogram not used 
...
! Subprogram not used 
! Subprogram not used    rcode = system(str)
! Subprogram not used 
! Subprogram not used 
...
! Subprogram not used    integer(SHR_KIND_IN),external    :: chdir   ! AIX system call
! Subprogram not used 
! Subprogram not used    !----- formats -----
...
! Subprogram not used 
! Subprogram not used 
! Subprogram not used    rcode=chdir(path(1:lenpath))
! Subprogram not used 
! Subprogram not used 
...
! Subprogram not used integer(SHR_KIND_I8) FUNCTION shr_sys_irtc( rate )
! Subprogram not used 
! Subprogram not used    IMPLICIT none
! Subprogram not used 
! Subprogram not used    !----- arguments -----
! Subprogram not used    integer(SHR_KIND_I8), optional :: rate
! Subprogram not used 
! Subprogram not used    !----- local -----
! Subprogram not used    integer(SHR_KIND_IN)      :: count
! Subprogram not used    integer(SHR_KIND_IN)      :: count_rate
! Subprogram not used    integer(SHR_KIND_IN)      :: count_max
! Subprogram not used    integer(SHR_KIND_IN),save :: last_count   = -1
! Subprogram not used    integer(SHR_KIND_I8),save :: count_offset =  0
! Subprogram not used 
! Subprogram not used    !----- formats -----
! Subprogram not used    character(*),parameter :: subName =   '(shr_sys_irtc) '
! Subprogram not used    character(*),parameter :: F00     = "('(shr_sys_irtc) ',4a)"
! Subprogram not used 
! Subprogram not used !-------------------------------------------------------------------------------
! Subprogram not used ! emulates Cray/SGI irtc function (returns clock tick since last reboot)
! Subprogram not used !-------------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used    call system_clock(count=count,count_rate=count_rate, count_max=count_max)
! Subprogram not used    if ( present(rate) ) rate = count_rate
! Subprogram not used    shr_sys_irtc = count
! Subprogram not used 
! Subprogram not used    !--- adjust for clock wrap-around ---
! Subprogram not used    if ( last_count /= -1 ) then
! Subprogram not used      if ( count < last_count ) count_offset = count_offset + count_max
! Subprogram not used    end if
! Subprogram not used    shr_sys_irtc = shr_sys_irtc + count_offset
! Subprogram not used    last_count   = count
! Subprogram not used 
! Subprogram not used END FUNCTION shr_sys_irtc
...
! Subprogram not used subroutine shr_sys_backtrace()
! Subprogram not used 
! Subprogram not used   ! This routine uses compiler-specific facilities to print a backtrace to
! Subprogram not used   ! error_unit (standard error, usually unit 0).
! Subprogram not used 
! Subprogram not used 
! Subprogram not used   ! tracebackqq uses optional arguments, so *must* have an explicit
! Subprogram not used   ! interface.
! Subprogram not used   use ifcore, only: tracebackqq
! Subprogram not used 
! Subprogram not used   ! An exit code of -1 is a special value that prevents this subroutine
! Subprogram not used   ! from aborting the run.
! Subprogram not used   call tracebackqq(user_exit_code=-1)
! Subprogram not used 
! Subprogram not used 
! Subprogram not used   flush(error_unit)
! Subprogram not used 
! Subprogram not used end subroutine shr_sys_backtrace
...
!
! Subprogram not used subroutine print_error_to_logs(error_type, message)
! Subprogram not used   character(len=*), intent(in) :: error_type, message
! Subprogram not used 
! Subprogram not used   integer, allocatable :: log_units(:)
! Subprogram not used 
! Subprogram not used   integer :: i
! Subprogram not used 
! Subprogram not used   if (s_logunit == output_unit .or. s_logunit == error_unit) then
! Subprogram not used      ! If the log unit number is standard output or standard error, just
! Subprogram not used      ! print to that.
! Subprogram not used      allocate(log_units(1), source=[s_logunit])
! Subprogram not used   else
! Subprogram not used      ! Otherwise print the same message to both the log unit and standard
! Subprogram not used      ! error.
! Subprogram not used      allocate(log_units(2), source=[error_unit, s_logunit])
! Subprogram not used   end if
! Subprogram not used 
! Subprogram not used   do i = 1, size(log_units)
! Subprogram not used      write(log_units(i),*) trim(error_type), ": ", trim(message)
! Subprogram not used      flush(log_units(i))
! Subprogram not used   end do
! Subprogram not used 
! Subprogram not used end subroutine print_error_to_logs
