...
! Subprogram not used   subroutine CheckSchedule()
! Subprogram not used     implicit none 
! Subprogram not used 
! Subprogram not used     integer                             :: i,nSched,nbufferwords_1,nbufferwords_2
! Subprogram not used     type (Schedule_t), pointer          :: pSchedule
! Subprogram not used 
! Subprogram not used     nSched = SIZE(Schedule)
! Subprogram not used 
! Subprogram not used     do i=1,nSched
! Subprogram not used        pSchedule => Schedule(i)
! Subprogram not used        nbufferwords_1 = SUM(pSchedule%SendCycle(:)%lengthP)
! Subprogram not used        nbufferwords_2 = SUM(pSchedule%RecvCycle(:)%lengthP)
! Subprogram not used        if(nbufferwords_1 .ne. nbufferwords_2) then 
! Subprogram not used           write (*,100) i,nbufferwords_1, nbufferwords_2
! Subprogram not used        endif
! Subprogram not used     enddo
! Subprogram not used 100 format('CheckSchedule: ERR IAM:',I3,' SIZEOF(SendBuffer):',I10,' != SIZEOF(RecvBuffer) :',I10)
! Subprogram not used 110 format('CheckSchedule: ERR IAM:',I3,' LENGTH(SendBuffer):',I10,' != LENGTH(RecvBuffer) :',I10)
! Subprogram not used 
! Subprogram not used   end subroutine CheckSchedule
! Subprogram not used   subroutine PrintSchedule(Schedule)
! Subprogram not used     use gridgraph_mod, only : printgridedge
! Subprogram not used 
! Subprogram not used     implicit none
! Subprogram not used     type (Schedule_t),intent(in),target   :: Schedule(:)
! Subprogram not used     type (Schedule_t), pointer            :: pSchedule
! Subprogram not used     type (Cycle_t),pointer                :: pCycle
! Subprogram not used 
! Subprogram not used     integer               :: i,j,nSched
! Subprogram not used 
! Subprogram not used     nSched = SIZE(Schedule)
! Subprogram not used 
! Subprogram not used     write(*,*) '------NEW SCHEDULE FORMAT---------------------'
! Subprogram not used     do i=1,nSched
! Subprogram not used        pSchedule => Schedule(i)
! Subprogram not used        write(*,*)
! Subprogram not used        write(*,*) '----------------------------------------------'
! Subprogram not used        write(*,90) i,pSchedule%ncycles
! Subprogram not used        write(*,*) '----------------------------------------------'
! Subprogram not used        write(*,*) '-----------SEND-------------------------------'
! Subprogram not used        do j=1,pSchedule%nSendCycles
! Subprogram not used           pCycle => pSchedule%SendCycle(j)
! Subprogram not used           call PrintCycle(pCycle)
! Subprogram not used           call PrintGridEdge(pCycle%edge%members)
! Subprogram not used        enddo
! Subprogram not used        write(*,*) '-----------RECV-------------------------------'
! Subprogram not used        do j=1,pSchedule%nRecvCycles
! Subprogram not used           pCycle => pSchedule%RecvCycle(j)
! Subprogram not used           call PrintCycle(pCycle)
! Subprogram not used           call PrintGridEdge(pCycle%edge%members)
! Subprogram not used        enddo
! Subprogram not used        write(*,*) '-----------MOVE-------------------------------'
! Subprogram not used        pCycle => pSchedule%MoveCycle(1)
! Subprogram not used        call PrintCycle(pCycle)
! Subprogram not used        call PrintGridEdge(pCycle%edge%members)
! Subprogram not used     enddo
! Subprogram not used 90  format('NODE # ',I2,2x,'NCYCLES ',I2)
! Subprogram not used 97  format(10x,'EDGE #',I2,2x,'TYPE ',I1,2x,'G.EDGES',I4,2x,'WORDS ',I5,2x, &
! Subprogram not used          'SRC ',I3,2x,'DEST ',I3,2x,'PTR ',I4)
! Subprogram not used 100 format(15x,I4,5x,I3,1x,'(',I1,') --',I1,'--> ',I3,1x,'(',I1,')')
! Subprogram not used 
! Subprogram not used 
! Subprogram not used   end subroutine PrintSchedule

! Subprogram not used   subroutine PrintCycle(Cycle)
! Subprogram not used 
! Subprogram not used     implicit none 
! Subprogram not used     type (Cycle_t),intent(in),target  ::  Cycle
! Subprogram not used 
! Subprogram not used     write(*,97) Cycle%edge%number,Cycle%type,Cycle%edge%nmembers, &
! Subprogram not used          Cycle%lengthP,Cycle%source, Cycle%dest,Cycle%ptrP
! Subprogram not used 
! Subprogram not used 97  format(5x,'METAEDGE #',I2,2x,'TYPE ',I1,2x,'G.EDGES',I4,2x,'WORDS ',I5,2x, &
! Subprogram not used          'SRC ',I3,2x,'DEST ',I3,2x,'PTR ',I5)
! Subprogram not used 
! Subprogram not used   end subroutine PrintCycle
    ...
    do i=1,Edge%nmembers
       ...
       if (face.ge.5) then !its a corner
          ...
          if(loc>max_neigh_edges) then
             print *,"<stdin>",1049,iam,face,i,max_corner_elem,edge%members(i)%head_face
