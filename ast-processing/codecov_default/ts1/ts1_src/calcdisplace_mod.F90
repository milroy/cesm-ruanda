...
CONTAINS
  ...
  subroutine calcdisplace_box(gsize,lenblock,start,count,ndim,displace)
    ...
    if (mod(iosize,ndisp) /= 0) then
       print *,"calcdisplace_mod.F90",90,ndisp,mod(iosize,ndisp)
       call piodie("calcdisplace_mod.F90",91,'mod(iosize,ndisp)/=0 iosize=',iosize)
    endif
       ...
       if (i<ndisp) then
          do while (.not. done)
             if (myloc(idim)<ub(idim)) then
                ...
                idim=idim+1
                if (idim>ndim) call piodie("calcdisplace_mod.F90",142,'dim overflow')
             endif
          end do
       endif
    ...
    do i=fdim+1,ndim
       if (myloc(i) /= ub(i)) then
          print *,'myloc=',myloc
          print *,'ub=',ub
          call piodie( "calcdisplace_mod.F90",153,'myloc/=ub')
       endif
    end do
...
!          silently fail by deallocating the displace array
          ...
          exit
!          call piodie("calcdisplace_mod.F90",167,'displace is not increasing')
       endif
    enddo
...
! Subprogram not used   integer function gcd_array_i4(ain) result(bsize)
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used 
! Subprogram not used     integer(i4), intent(in),dimension(:) :: ain
! Subprogram not used 
! Subprogram not used     ! locals
! Subprogram not used     integer(i4) :: i,n
! Subprogram not used 
! Subprogram not used     bsize=1
! Subprogram not used     n = size(ain) 
! Subprogram not used     ! First check, if an element is 1, then 1 is the gcd (i.e bsize)
! Subprogram not used     if(n==0 .or. any(ain <= 1)) return
! Subprogram not used 
! Subprogram not used     ! Calculate GCD using GCD(a,b,c,...) = GCD(a,GCD(b,c...))
! Subprogram not used     ! otherwise gcd = 1.
! Subprogram not used     ! Done by calling the external function that is below.
! Subprogram not used 
! Subprogram not used     bsize = ain(1)
! Subprogram not used     do i = 2,n
! Subprogram not used        bsize = gcd_pair(bsize,ain(i))
! Subprogram not used        if (bsize == 1) exit
! Subprogram not used     end do
! Subprogram not used 
! Subprogram not used   end function gcd_array_i4
...
! Subprogram not used   integer FUNCTION gcd_pair_i4(u,v) result(gcd)
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     integer(i4),intent(in) :: u,v
! Subprogram not used 
! Subprogram not used     ! locals
! Subprogram not used     integer(i4) :: x,a,b
! Subprogram not used 
! Subprogram not used     a = u
! Subprogram not used     b = v
! Subprogram not used 
! Subprogram not used     if(a < b) then
! Subprogram not used        x = a
! Subprogram not used        a = b
! Subprogram not used        b = x
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used     do 
! Subprogram not used        x = mod(a,b)
! Subprogram not used        if ( x == 0 ) EXIT
! Subprogram not used        a = b 
! Subprogram not used        b = x 
! Subprogram not used     end do
! Subprogram not used 
! Subprogram not used     gcd = b 
! Subprogram not used 
! Subprogram not used   end FUNCTION gcd_pair_i4
