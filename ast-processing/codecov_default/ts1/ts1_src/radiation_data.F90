!================================================================================================
! output/input data necessary to drive radiation offline
! Francis Vitt -- Created 15 Dec 2009
!================================================================================================
module radiation_data

  use shr_kind_mod,     only: r8=>shr_kind_r8
  use ppgrid,           only : pcols, pver, pverp, begchunk, endchunk
  use cam_history,      only: addfld, add_default, phys_decomp, outfld
  use rad_constituents, only: rad_cnst_get_info, rad_cnst_get_gas, rad_cnst_get_aer_mmr
  use radconstants,     only: nradgas, gaslist
  use cam_history_support, only: fieldname_len, fillvalue
  use spmd_utils,       only: masterproc
  use abortutils,       only: endrun

  use drv_input_data,only: drv_input_data_t
  use drv_input_data,only: drv_input_data_get, drv_input_2d_t, drv_input_3d_t, drv_input_4d_t, drv_input_2di_t
  use physics_types, only: physics_state
  use physics_buffer,only: physics_buffer_desc, pbuf_get_chunk

  implicit none
  private

  public :: rad_data_readnl
  public :: rad_data_register
  public :: rad_data_init
  public :: rad_data_write
  public :: rad_data_read

  integer :: cld_ifld,rel_ifld,rei_ifld
  integer :: dei_ifld,mu_ifld,lambdac_ifld,iciwp_ifld,iclwp_ifld
  integer :: des_ifld,icswp_ifld,cldfsnow_ifld
  integer :: cldemis_ifld, cldtau_ifld, cicewp_ifld, cliqwp_ifld, nmxrgn_ifld, pmxrgn_ifld
  integer :: qrs_ifld, qrl_ifld
  integer :: dgnumwet_ifld, qaerwat_ifld

  character(len=fieldname_len), parameter :: &
       lndfrc_fldn    = 'rad_lndfrc      ' , &
       icefrc_fldn    = 'rad_icefrc      ' , &
       snowh_fldn     = 'rad_snowh       ' , &
       asdir_fldn     = 'rad_asdir       ' , &
       asdif_fldn     = 'rad_asdif       ' , &
       aldir_fldn     = 'rad_aldir       ' , &
       aldif_fldn     = 'rad_aldif       ' , &
       coszen_fldn    = 'rad_coszen      ' , &
       asdir_pos_fldn = 'rad_asdir_pos   ' , &
       asdif_pos_fldn = 'rad_asdif_pos   ' , &
       aldir_pos_fldn = 'rad_aldir_pos   ' , &
       aldif_pos_fldn = 'rad_aldif_pos   ' , &
       lwup_fldn      = 'rad_lwup        ' , &
       ts_fldn        = 'rad_ts          ' , &
       temp_fldn      = 'rad_temp        ' , &
       pdel_fldn      = 'rad_pdel        ' , &
       pdeldry_fldn   = 'rad_pdeldry     ' , &
       pmid_fldn      = 'rad_pmid        ' , &
       watice_fldn    = 'rad_watice      ' , &
       watliq_fldn    = 'rad_watliq      ' , &
       watvap_fldn    = 'rad_watvap      ' , &
       zint_fldn      = 'rad_zint        ' , &
       pint_fldn      = 'rad_pint        ' , &
       cld_fldn       = 'rad_cld         ' , &
       cldemis_fldn   = 'rad_cldemis     ' , &
       cldtau_fldn    = 'rad_cldtau      ' , &
       cicewp_fldn    = 'rad_cicewp      ' , &
       cliqwp_fldn    = 'rad_cliqwp      ' , &
       nmxrgn_fldn    = 'rad_nmxrgn      ' , &
       pmxrgn_fldn    = 'rad_pmxrgn      ' , &
       cldfsnow_fldn  = 'rad_cldfsnow    ' , &
       rel_fldn       = 'rad_rel         ' , &
       rei_fldn       = 'rad_rei         ' , &
       dei_fldn       = 'rad_dei         ' , &
       des_fldn       = 'rad_des         ' , &
       mu_fldn        = 'rad_mu          ' , &
       lambdac_fldn   = 'rad_lambdac     ' , &
       iciwp_fldn     = 'rad_iciwp       ' , &
       iclwp_fldn     = 'rad_iclwp       ' , &
       icswp_fldn     = 'rad_icswp       ' , &
       qrs_fldn       = 'rad_qrs         ' , &
       qrl_fldn       = 'rad_qrl         ' 

  ! for modal aerosols
  character(len=fieldname_len), allocatable :: dgnumwet_fldn(:)
  character(len=fieldname_len), allocatable :: qaerwat_fldn(:)
  integer :: nmodes=0

  ! rad constituents mixing ratios
  integer :: ngas, naer=0
  character(len=64), allocatable :: gasnames(:)
  character(len=64), allocatable :: aernames(:)
 
  ! control options  
  logical          :: rad_data_output = .false.
  integer          :: rad_data_histfile_num = 2
  character(len=1) :: rad_data_avgflag = 'I'

  ! MG microphys check
  logical :: mg_microphys

  logical :: fixed_dyn_heating = .false.

  ! for fixed dynamical heating ...
  logical :: do_fdh = .false.

  integer :: tcorr_idx = -1
  integer :: qrsin_idx = -1
  integer :: qrlin_idx = -1
  integer :: tropp_idx = -1

contains


!================================================================================================
!================================================================================================
  subroutine rad_data_register
    use physics_buffer,  only: pbuf_add_field, dtype_r8

    if ( do_fdh ) then
      call pbuf_add_field('tcorr', 'global',  dtype_r8, (/pcols,pver/), tcorr_idx)
      call pbuf_add_field('qrsin', 'physpkg', dtype_r8, (/pcols,pver/), qrsin_idx) 
      call pbuf_add_field('qrlin', 'physpkg', dtype_r8, (/pcols,pver/), qrlin_idx) 
      call pbuf_add_field('tropp', 'physpkg', dtype_r8, (/pcols/),      tropp_idx) 
    end if 

  end subroutine rad_data_register

!================================================================================================
!================================================================================================
  subroutine rad_data_readnl(nlfile)

    ! Read rad_data_nl namelist group.  Parse input.

    use namelist_utils,  only: find_group_name
    use units,           only: getunit, freeunit
    use mpishorthand

    character(len=*), intent(in) :: nlfile  ! filepath for file containing namelist input
    logical :: rad_data_fdh = .false.

    ! Local variables
    integer :: unitn, ierr, i
    character(len=*), parameter :: subname = 'rad_data_readnl'

    namelist /rad_data_nl/ rad_data_output, rad_data_histfile_num, rad_data_avgflag, rad_data_fdh

    !-----------------------------------------------------------------------------

    if (masterproc) then
       unitn = getunit()
       open( unitn, file=trim(nlfile), status='old' )
       call find_group_name(unitn, 'rad_data_nl', status=ierr)
       if (ierr == 0) then
          read(unitn, rad_data_nl, iostat=ierr)
          if (ierr /= 0) then
             call endrun(subname // ':: ERROR reading namelist')
          end if
       end if
       close(unitn)
       call freeunit(unitn)
    end if

    ! Broadcast namelist variables
    call mpibcast (rad_data_output,       1,   mpilog ,  0, mpicom)
    call mpibcast (rad_data_fdh,          1,   mpilog ,  0, mpicom)
    call mpibcast (rad_data_histfile_num, 1,   mpiint ,  0, mpicom)
    call mpibcast (rad_data_avgflag,      1,   mpichar , 0, mpicom)
    do_fdh = rad_data_fdh

  end subroutine rad_data_readnl

  !================================================================================================
  !================================================================================================
  subroutine rad_data_init
    use phys_control,     only: phys_getopts
    use physics_buffer, only: pbuf_get_index
    implicit none
    
    integer :: i
    integer :: m,l, nspec
    character(len=64) :: name
    character(len=32) :: aername =  ' '
    character(len=128):: long_name
    character(len=64) :: long_name_description
    character(len=16)  :: microp_scheme  ! microphysics scheme
    character(len=16)  :: rad_scheme
   
    call phys_getopts(microp_scheme_out=microp_scheme, radiation_scheme_out=rad_scheme)
    mg_microphys =  (trim(microp_scheme) == 'MG')

    cld_ifld    = pbuf_get_index('CLD')
    rel_ifld    = pbuf_get_index('REL')
    rei_ifld    = pbuf_get_index('REI')
    qrs_ifld    = pbuf_get_index('QRS')
    qrl_ifld    = pbuf_get_index('QRL')
    if (mg_microphys) then
       dei_ifld      = pbuf_get_index('DEI')
       des_ifld      = pbuf_get_index('DES')
       mu_ifld       = pbuf_get_index('MU')
       lambdac_ifld  = pbuf_get_index('LAMBDAC')
       iciwp_ifld    = pbuf_get_index('ICIWP')
       iclwp_ifld    = pbuf_get_index('ICLWP')
       icswp_ifld    = pbuf_get_index('ICSWP')
       cldfsnow_ifld = pbuf_get_index('CLDFSNOW')
    else
       cldemis_ifld= pbuf_get_index('CLDEMIS')
       cldtau_ifld = pbuf_get_index('CLDTAU')
       cicewp_ifld = pbuf_get_index('CICEWP')
       cliqwp_ifld = pbuf_get_index('CLIQWP')
       nmxrgn_ifld = pbuf_get_index('NMXRGN')
       pmxrgn_ifld = pbuf_get_index('PMXRGN')
    endif
    
    if ( do_fdh ) then
       call addfld('rad_TROP_P','Pa',    1, 'A','Pressure at which tropopause is defined for radiation',phys_decomp )
       call addfld('rad_TCORR','K',    pver, 'I','Fixed dynamical heating temperature correction ',phys_decomp )
    endif

    call rad_cnst_get_info(0, ngas=ngas, naero=naer, nmodes=nmodes)

    ! The code to output the gases assumes that the rad_constituents module has
    ! ordered them in the same way that they are ordered in the "gaslist" array
    ! in module radconstants, and that there are nradgas of them.  This ordering 
    ! is performed in the internal init_lists routine in rad_constituents.
    if (ngas /= nradgas) then
       call endrun('rad_data_init: ERROR: ngas /= nradgas')
    end if

    allocate( gasnames(ngas) )
    call rad_cnst_get_info(0, gasnames=gasnames)

    if (naer > 0) then
       allocate( aernames(naer) )
       call rad_cnst_get_info(0, aernames=aernames)
    endif

    if (nmodes>0) then
       allocate(dgnumwet_fldn(nmodes),qaerwat_fldn(nmodes))
       dgnumwet_ifld = pbuf_get_index('DGNUMWET')
       qaerwat_ifld  = pbuf_get_index('QAERWAT')
       do m = 1, nmodes
          write(dgnumwet_fldn(m), 1001 ) m
          write(qaerwat_fldn(m), 1003 ) m
       enddo
    endif

    if (.not.rad_data_output) return

    call addfld (lndfrc_fldn, 'fraction', 1,    rad_data_avgflag,&
         'radiation input: land fraction',phys_decomp)
    call addfld (icefrc_fldn, 'fraction', 1,    rad_data_avgflag,&
         'radiation input: ice fraction',phys_decomp)
    call addfld (snowh_fldn,  'm',        1,    rad_data_avgflag,&
         'radiation input: water equivalent snow depth',phys_decomp)
    call addfld (asdir_fldn,  '1',        1,    rad_data_avgflag,&
         'radiation input: short wave direct albedo',phys_decomp, flag_xyfill=.true.)
    call addfld (asdif_fldn,  '1',        1,    rad_data_avgflag,&
         'radiation input: short wave difuse albedo',phys_decomp, flag_xyfill=.true.)
    call addfld (aldir_fldn,  '1',        1,    rad_data_avgflag,&
         'radiation input: long wave direct albedo', phys_decomp, flag_xyfill=.true.)
    call addfld (aldif_fldn,  '1',        1,    rad_data_avgflag,&
         'radiation input: long wave difuse albedo', phys_decomp, flag_xyfill=.true.)

    call addfld (coszen_fldn,     '1', 1,    rad_data_avgflag,&
         'radiation input: cosine solar zenith when positive', phys_decomp, flag_xyfill=.true.)
    call addfld (asdir_pos_fldn,  '1', 1,    rad_data_avgflag,&
         'radiation input: short wave direct albedo weighted by coszen', phys_decomp, flag_xyfill=.true.)
    call addfld (asdif_pos_fldn,  '1', 1,    rad_data_avgflag,&
         'radiation input: short wave difuse albedo weighted by coszen', phys_decomp, flag_xyfill=.true.)
    call addfld (aldir_pos_fldn,  '1', 1,    rad_data_avgflag,&
         'radiation input: long wave direct albedo weighted by coszen', phys_decomp, flag_xyfill=.true.)
    call addfld (aldif_pos_fldn,  '1', 1,    rad_data_avgflag,&
         'radiation input: long wave difuse albedo weighted by coszen', phys_decomp, flag_xyfill=.true.)
    
    call addfld (lwup_fldn,   'W/m2',     1,    rad_data_avgflag,&
         'radiation input: long wave up radiation flux ',phys_decomp)
    call addfld (ts_fldn,     'K',        1,    rad_data_avgflag,&
         'radiation input: surface temperature',phys_decomp)

    call addfld (temp_fldn,   'K',        pver, rad_data_avgflag,&
         'radiation input: midpoint temperature',phys_decomp)
    call addfld (pdel_fldn,   'Pa',       pver, rad_data_avgflag,&
         'radiation input: pressure layer thickness',phys_decomp)
    call addfld (pdeldry_fldn,'Pa',       pver, rad_data_avgflag,&
         'radiation input: dry pressure layer thickness',phys_decomp)
    call addfld (pmid_fldn,   'Pa',       pver, rad_data_avgflag,&
         'radiation input: midpoint pressure',phys_decomp)
    call addfld (watice_fldn, 'kg/kg',    pver, rad_data_avgflag,&
         'radiation input: cloud ice',phys_decomp)
    call addfld (watliq_fldn, 'kg/kg',    pver, rad_data_avgflag,&
         'radiation input: cloud liquid water',phys_decomp)
    call addfld (watvap_fldn, 'kg/kg',    pver, rad_data_avgflag,&
         'radiation input: water vapor',phys_decomp)

    call addfld (zint_fldn,   'km',       pverp,rad_data_avgflag,&
         'radiation input: interface height',phys_decomp)
    call addfld (pint_fldn,   'Pa',       pverp,rad_data_avgflag,&
         'radiation input: interface pressure',phys_decomp)

    call addfld (cld_fldn,    'fraction', pver, rad_data_avgflag,&
         'radiation input: cloud fraction',phys_decomp)
    call addfld (rel_fldn,    'micron',   pver, rad_data_avgflag,&
         'radiation input: effective liquid drop radius',phys_decomp)
    call addfld (rei_fldn,    'micron',   pver, rad_data_avgflag,&
         'radiation input: effective ice partical radius',phys_decomp)
    call addfld (qrs_fldn,    'J/s/kg',      pver, rad_data_avgflag,&
         'radiation input: solar heating rate',phys_decomp)
    call addfld (qrl_fldn,    'J/s/kg',      pver, rad_data_avgflag,&
         'radiation input: longwave heating rate',phys_decomp)

    if (mg_microphys) then
       call addfld (dei_fldn,    'micron',   pver, rad_data_avgflag,&
            'radiation input: effective ice partical diameter',phys_decomp)
       call addfld (des_fldn,    'micron',   pver, rad_data_avgflag,&
            'radiation input: effective snow partical diameter',phys_decomp)
       call addfld (mu_fldn,     ' ',        pver, rad_data_avgflag,&
            'radiation input: ice gamma parameter for optics (radiation)',phys_decomp)
       call addfld (lambdac_fldn,' ',        pver, rad_data_avgflag,&
            'radiation input: slope of droplet distribution for optics (radiation)',phys_decomp)
       call addfld (iciwp_fldn,  'kg/m2',    pver, rad_data_avgflag,&
            'radiation input: In-cloud ice water path',phys_decomp)
       call addfld (iclwp_fldn,  'kg/m2',    pver, rad_data_avgflag,&
            'radiation input: In-cloud liquid water path',phys_decomp)
       call addfld (icswp_fldn,  'kg/m2',    pver, rad_data_avgflag,&
            'radiation input: In-cloud snow water path',phys_decomp)
       call addfld (cldfsnow_fldn, 'fraction', pver, rad_data_avgflag,&
            'radiation input: cloud liquid drops + snow',phys_decomp)
    else

      call addfld (nmxrgn_fldn, ' ',        1,    rad_data_avgflag,&
         'radiation input: ',phys_decomp)
      call addfld (pmxrgn_fldn, 'Pa',       pverp, rad_data_avgflag,&
         'radiation input: ',phys_decomp)
      call addfld (cldemis_fldn, ' ',       pver, rad_data_avgflag,&
         'radiation input: cloud property ',phys_decomp)
      call addfld (cldtau_fldn, ' ',        pver, rad_data_avgflag,&
         'radiation input: cloud property ',phys_decomp)
      call addfld (cicewp_fldn, ' ',        pver, rad_data_avgflag,&
         'radiation input: cloud property ',phys_decomp)
      call addfld (cliqwp_fldn, ' ',        pver, rad_data_avgflag,&
         'radiation input: cloud property ',phys_decomp)
    endif

    call add_default (lndfrc_fldn,    rad_data_histfile_num, ' ')
    call add_default (icefrc_fldn,    rad_data_histfile_num, ' ')
    call add_default (snowh_fldn,     rad_data_histfile_num, ' ')
    call add_default (asdir_fldn,     rad_data_histfile_num, ' ')
    call add_default (asdif_fldn,     rad_data_histfile_num, ' ')
    call add_default (aldir_fldn,     rad_data_histfile_num, ' ')
    call add_default (aldif_fldn,     rad_data_histfile_num, ' ')

    call add_default (coszen_fldn,    rad_data_histfile_num, ' ')
    call add_default (asdir_pos_fldn, rad_data_histfile_num, ' ')
    call add_default (asdif_pos_fldn, rad_data_histfile_num, ' ')
    call add_default (aldir_pos_fldn, rad_data_histfile_num, ' ')
    call add_default (aldif_pos_fldn, rad_data_histfile_num, ' ')

    call add_default (lwup_fldn,      rad_data_histfile_num, ' ')
    call add_default (ts_fldn,        rad_data_histfile_num, ' ')
    call add_default (temp_fldn,      rad_data_histfile_num, ' ')
    call add_default (pdel_fldn,      rad_data_histfile_num, ' ')
    call add_default (pdeldry_fldn,   rad_data_histfile_num, ' ')
    call add_default (pmid_fldn,      rad_data_histfile_num, ' ')
    call add_default (watice_fldn,    rad_data_histfile_num, ' ')
    call add_default (watliq_fldn,    rad_data_histfile_num, ' ')
    call add_default (watvap_fldn,    rad_data_histfile_num, ' ')
    call add_default (zint_fldn,      rad_data_histfile_num, ' ')
    call add_default (pint_fldn,      rad_data_histfile_num, ' ')

    call add_default (cld_fldn,       rad_data_histfile_num, ' ')
    call add_default (rel_fldn,       rad_data_histfile_num, ' ')
    call add_default (rei_fldn,       rad_data_histfile_num, ' ')
    call add_default (qrs_fldn,       rad_data_histfile_num, ' ')
    call add_default (qrl_fldn,       rad_data_histfile_num, ' ')
    
    if (mg_microphys) then
       call add_default (dei_fldn,       rad_data_histfile_num, ' ')
       call add_default (des_fldn,       rad_data_histfile_num, ' ')
       call add_default (mu_fldn,        rad_data_histfile_num, ' ')
       call add_default (lambdac_fldn,   rad_data_histfile_num, ' ')
       call add_default (iciwp_fldn,     rad_data_histfile_num, ' ')
       call add_default (iclwp_fldn,     rad_data_histfile_num, ' ')
       call add_default (icswp_fldn,     rad_data_histfile_num, ' ')
       call add_default (cldfsnow_fldn,  rad_data_histfile_num, ' ')
    else
       call add_default (cldemis_fldn,   rad_data_histfile_num, ' ')
       call add_default (cldtau_fldn,    rad_data_histfile_num, ' ')
       call add_default (cicewp_fldn,    rad_data_histfile_num, ' ')
       call add_default (cliqwp_fldn,    rad_data_histfile_num, ' ')
       call add_default (nmxrgn_fldn,    rad_data_histfile_num, ' ')
       call add_default (pmxrgn_fldn,    rad_data_histfile_num, ' ')
    endif
    
    ! rad constituents

    long_name_description = ' mass mixing ratio used in rad climate calculation'

    do i = 1, ngas
       long_name = trim(gasnames(i))//trim(long_name_description)
       name = 'rad_'//gasnames(i)
       call addfld(trim(name), 'kg/kg', pver, rad_data_avgflag, trim(long_name), phys_decomp)
       call add_default (trim(name), rad_data_histfile_num, ' ')
    end do
    
    if (naer > 0) then

       do i = 1, naer
          long_name = trim(aernames(i))//trim(long_name_description)
          name = 'rad_'//aernames(i)
          call addfld(trim(name), 'kg/kg', pver, rad_data_avgflag, trim(long_name), phys_decomp)
          call add_default (trim(name), rad_data_histfile_num, ' ')
       end do
    endif
    if (nmodes>0) then

       ! for modal aerosol model
       do m = 1, nmodes
          write(long_name, 1002) m
          call addfld ( dgnumwet_fldn(m), '',  pver, rad_data_avgflag, trim(long_name), phys_decomp )
          call add_default (dgnumwet_fldn(m), rad_data_histfile_num, ' ')

          write(long_name, 1004) m
          call addfld ( qaerwat_fldn(m), '',  pver, rad_data_avgflag, trim(long_name), phys_decomp )
          call add_default (qaerwat_fldn(m), rad_data_histfile_num, ' ')

          ! get mode info
          call rad_cnst_get_info(0, m, nspec=nspec)
          ! aerosol species loop
          do l = 1, nspec
             call rad_cnst_get_info(0,m,l, spec_name=aername)
             name = 'rad_'//trim(aername)
             call addfld(trim(name), 'kg/kg', pver, rad_data_avgflag, trim(long_name), phys_decomp)
             call add_default (trim(name), rad_data_histfile_num, ' ')
          end do
       end do
    endif

 1001 format ( 'rad_dgnumwet_', I1 )
 1002 format ( 'radiation input: DGNUMWET for mode ', I1 )
 1003 format ( 'rad_qaerwat_', I1 )
 1004 format ( 'radiation input: QAERWAT for mode ', I1 )

  end subroutine rad_data_init

  !================================================================================================
  !================================================================================================
  subroutine rad_data_write(  pbuf, state, cam_in, coszen )

    use physics_types,    only: physics_state
    use camsrfexch,       only: cam_in_t     
    use constituents,     only: cnst_get_ind
    use physics_buffer,   only: pbuf_get_field, pbuf_old_tim_idx
    use drv_input_data,   only: drv_input_data_freq
    use physconst,        only: cpair

    implicit none
    type(physics_buffer_desc), pointer :: pbuf(:)
    
    type(physics_state), intent(in), target :: state
    type(cam_in_t),      intent(in) :: cam_in
    real(r8),            intent(in) :: coszen(pcols)

    ! Local variables
    integer :: i, k
    integer :: m,l, nspec
    character(len=32) :: name, aername
    real(r8), pointer :: mmr(:,:)

    integer :: lchnk, itim_old, ifld
    integer :: ixcldice              ! cloud ice water index
    integer :: ixcldliq              ! cloud liquid water index
    integer :: icol
    integer :: ncol
    
    ! surface albedoes weighted by (positive cosine zenith angle)
    real(r8):: coszrs_pos(pcols)    ! = max(coszrs,0)
    real(r8):: asdir_pos (pcols)    !
    real(r8):: asdif_pos (pcols)    !
    real(r8):: aldir_pos (pcols)    !
    real(r8):: aldif_pos (pcols)    !

    real(r8), pointer, dimension(:,:)  :: ptr
    integer , pointer, dimension(:)    :: iptr1d
    real(r8),          dimension(pcols) :: rptr1d

    ! for Fixed Dynamical Heating
    real(r8), pointer, dimension(:,:) :: tcorr
    real(r8), pointer, dimension(:,:) :: qrsin
    real(r8), pointer, dimension(:,:) :: qrlin
    real(r8), pointer, dimension(:)   :: tropp
    real(r8), pointer, dimension(:,:) :: qrs
    real(r8), pointer, dimension(:,:) :: qrl
    real(r8), pointer, dimension(:,:,:) :: dgnumwet_ptr
    real(r8), pointer, dimension(:,:,:) :: qaerwat_ptr

    call pbuf_get_field(pbuf, qrs_ifld, qrs )    
    call pbuf_get_field(pbuf, qrl_ifld, qrl )

    lchnk = state%lchnk
    ncol = state%ncol

   ! store temperature correction above tropopause in pbuf for Fixed Dynamical Heating
    if(do_fdh) then
       
       call pbuf_get_field(pbuf, tcorr_idx, tcorr)
       call pbuf_get_field(pbuf, qrsin_idx, qrsin)
       call pbuf_get_field(pbuf, qrlin_idx, qrlin)
       call pbuf_get_field(pbuf, tropp_idx, tropp)
       do k = 1, pver
          do i = 1, ncol
             if ( state%pmid(i,k) < tropp(i)) then
                tcorr(i,k) = tcorr(i,k) + drv_input_data_freq*(qrs(i,k) - qrsin(i,k) + qrl(i,k) - qrlin(i,k)) /cpair 
             endif
          enddo
       enddo

       call outfld('rad_TROP_P', tropp(:ncol), ncol, lchnk)
       call outfld('rad_TCORR', tcorr(:ncol,:), ncol, lchnk)

    end if

    if (.not.rad_data_output) return

    call outfld(qrs_fldn, qrs, pcols, lchnk )
    call outfld(qrl_fldn, qrl, pcols, lchnk )

    ! get index of (liquid+ice) cloud water
    call cnst_get_ind('CLDICE', ixcldice)
    call cnst_get_ind('CLDLIQ', ixcldliq)

    do icol = 1, ncol
       coszrs_pos(icol)  = max(coszen(icol),0._r8)
    enddo
    asdir_pos(:ncol)  = cam_in%asdir(:ncol) * coszrs_pos(:ncol)
    asdif_pos(:ncol)  = cam_in%asdif(:ncol) * coszrs_pos(:ncol)
    aldir_pos(:ncol)  = cam_in%aldir(:ncol) * coszrs_pos(:ncol)
    aldif_pos(:ncol)  = cam_in%aldif(:ncol) * coszrs_pos(:ncol)

    call outfld(lndfrc_fldn, cam_in%landfrac,  pcols, lchnk)
    call outfld(icefrc_fldn, cam_in%icefrac,   pcols, lchnk)
    call outfld(snowh_fldn,  cam_in%snowhland, pcols, lchnk)
    call outfld(temp_fldn,   state%t,               pcols, lchnk   )
    call outfld(pdel_fldn,   state%pdel,            pcols, lchnk   )
    call outfld(pdeldry_fldn,state%pdeldry,         pcols, lchnk   )
    call outfld(watice_fldn, state%q(:,:,ixcldice), pcols, lchnk   )
    call outfld(watliq_fldn, state%q(:,:,ixcldliq), pcols, lchnk   )
    call outfld(watvap_fldn, state%q(:,:,1),        pcols, lchnk   )
    call outfld(zint_fldn,   state%zi,              pcols, lchnk   )
    call outfld(pint_fldn,   state%pint,            pcols, lchnk   )
    call outfld(pmid_fldn,   state%pmid,            pcols, lchnk   )

    call outfld(asdir_fldn, cam_in%asdir, pcols, lchnk   )
    call outfld(asdif_fldn, cam_in%asdif, pcols, lchnk   )
    call outfld(aldir_fldn, cam_in%aldir, pcols, lchnk   )
    call outfld(aldif_fldn, cam_in%aldif, pcols, lchnk   )

    call outfld(coszen_fldn, coszrs_pos, pcols, lchnk   )
    call outfld(asdir_pos_fldn, asdir_pos, pcols, lchnk   )
    call outfld(asdif_pos_fldn, asdif_pos, pcols, lchnk   )
    call outfld(aldir_pos_fldn, aldir_pos, pcols, lchnk   )
    call outfld(aldif_pos_fldn, aldif_pos, pcols, lchnk   )

    call outfld(lwup_fldn,  cam_in%lwup,  pcols, lchnk   )
    call outfld(ts_fldn,    cam_in%ts,    pcols, lchnk   )

    itim_old = pbuf_old_tim_idx()

    call pbuf_get_field(pbuf, cld_ifld,    ptr, start=(/1,1,itim_old/), kount=(/pcols,pver,1/) )
    call outfld(cld_fldn,    ptr, pcols, lchnk )

    call pbuf_get_field(pbuf, rel_ifld,    ptr )
    call outfld(rel_fldn,    ptr, pcols, lchnk )

    call pbuf_get_field(pbuf, rei_ifld,    ptr )
    call outfld(rei_fldn,    ptr, pcols, lchnk )
    
    if (mg_microphys) then

       call pbuf_get_field(pbuf,  dei_ifld, ptr     )
       call outfld(dei_fldn,      ptr, pcols, lchnk   )       

       call pbuf_get_field(pbuf,  des_ifld, ptr     )
       call outfld(des_fldn,      ptr, pcols, lchnk   )       

       call pbuf_get_field(pbuf,  mu_ifld, ptr      )
       call outfld(mu_fldn,       ptr, pcols, lchnk   ) 

       call pbuf_get_field(pbuf,  lambdac_ifld, ptr )
       call outfld(lambdac_fldn,  ptr, pcols, lchnk   )       

       call pbuf_get_field(pbuf,  iciwp_ifld, ptr   )
       call outfld(iciwp_fldn,    ptr, pcols, lchnk   )       

       call pbuf_get_field(pbuf,  iclwp_ifld, ptr   )
       call outfld(iclwp_fldn,    ptr, pcols, lchnk   )       

       call pbuf_get_field(pbuf,  icswp_ifld, ptr   )
       call outfld(icswp_fldn,    ptr, pcols, lchnk   )       

       call pbuf_get_field(pbuf,  cldfsnow_ifld, ptr, start=(/1,1,itim_old/), kount=(/pcols,pver,1/) )
       call outfld(cldfsnow_fldn, ptr, pcols, lchnk   )

    else

       call pbuf_get_field(pbuf, cldemis_ifld,    ptr )
       call outfld(cldemis_fldn,   ptr, pcols, lchnk )

       call pbuf_get_field(pbuf, cldtau_ifld,    ptr )
       call outfld(cldtau_fldn,    ptr, pcols, lchnk )

       call pbuf_get_field(pbuf, cicewp_ifld,    ptr )
       call outfld(cicewp_fldn,    ptr, pcols, lchnk )

       call pbuf_get_field(pbuf, cliqwp_ifld,    ptr )
       call outfld(cliqwp_fldn,    ptr, pcols, lchnk )

       call pbuf_get_field(pbuf, nmxrgn_ifld,    iptr1d )
       rptr1d = dble(iptr1d)
       call outfld(nmxrgn_fldn,    rptr1d, pcols, lchnk )

       call pbuf_get_field(pbuf, pmxrgn_ifld,    ptr )
       call outfld(pmxrgn_fldn,    ptr, pcols, lchnk )

    endif

    ! output mixing ratio of rad constituents 

    do i = 1, ngas
       name = 'rad_'//gasnames(i)
       call rad_cnst_get_gas(0, gaslist(i), state, pbuf, mmr)
       call outfld(trim(name), mmr, pcols, lchnk)
    end do

    if ( naer>0 ) then
       ! bulk aerosol model
       do i = 1, naer
          name = 'rad_'//aernames(i)
          call rad_cnst_get_aer_mmr(0, i, state, pbuf, mmr)
          call outfld(trim(name), mmr, pcols, lchnk)
       end do
    endif
    if (nmodes>0) then

       call pbuf_get_field(pbuf, dgnumwet_ifld, dgnumwet_ptr)
       call pbuf_get_field(pbuf, qaerwat_ifld, qaerwat_ptr)

       ! for modal aerosol model
       do m = 1, nmodes
          ptr => dgnumwet_ptr(:,:,m)
          call outfld( dgnumwet_fldn(m), ptr, pcols, lchnk )
          ptr => qaerwat_ptr(:,:,m)
          call outfld( qaerwat_fldn(m),  ptr, pcols, lchnk )

          ! get mode info
          call rad_cnst_get_info(0, m, nspec=nspec)
          ! aerosol species loop
          do l = 1, nspec
             call rad_cnst_get_info(0,m,l, spec_name=aername)
             call rad_cnst_get_aer_mmr(0, m, l, 'a', state, pbuf, mmr)
             name = 'rad_'//aername
             call outfld(trim(name), mmr, pcols, lchnk)
          enddo
       enddo
    endif

  end subroutine rad_data_write

!=================================================================================
!=================================================================================
! Subprogram not used   subroutine rad_data_read(indata, phys_state, pbuf2d, cam_in, recno )
! Subprogram not used 
! Subprogram not used     use camsrfexch,       only: cam_in_t
! Subprogram not used     use physics_buffer,   only: pbuf_get_field, pbuf_old_tim_idx
! Subprogram not used     use constituents,     only: cnst_get_ind
! Subprogram not used     use tropopause,       only: tropopause_find, TROP_ALG_HYBSTOB, TROP_ALG_CLIMATE
! Subprogram not used 
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     type(drv_input_data_t), intent(inout) :: indata
! Subprogram not used     type(physics_buffer_desc), pointer         :: pbuf2d(:,:)
! Subprogram not used     type(physics_state), target, intent(inout) :: phys_state(begchunk:endchunk)
! Subprogram not used     type(cam_in_t),      target, intent(inout) :: cam_in(begchunk:endchunk)
! Subprogram not used     integer,             intent(in)            :: recno
! Subprogram not used 
! Subprogram not used ! local vars
! Subprogram not used     type(physics_buffer_desc), pointer :: pbuf(:)
! Subprogram not used 
! Subprogram not used     type(drv_input_3d_t) :: cld_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: cldfsnow_ptrs(begchunk:endchunk)
! Subprogram not used 
! Subprogram not used     type(drv_input_3d_t) :: rel_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: rei_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: dei_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: des_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: mu_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: lambdac_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: iciwp_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: iclwp_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: icswp_ptrs(begchunk:endchunk)
! Subprogram not used 
! Subprogram not used     type(drv_input_3d_t) :: qrs_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: qrl_ptrs(begchunk:endchunk)
! Subprogram not used 
! Subprogram not used     type(drv_input_3d_t) :: watvap_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: watliq_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: watice_ptrs(begchunk:endchunk)
! Subprogram not used 
! Subprogram not used     type(drv_input_3d_t) :: zi_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: pint_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: lnpint_ptrs(begchunk:endchunk)
! Subprogram not used 
! Subprogram not used     type(drv_input_3d_t) :: temp_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: pdel_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: pdeldry_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: lnpmid_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: pmid_ptrs(begchunk:endchunk)
! Subprogram not used 
! Subprogram not used     type(drv_input_2d_t) :: lndfrac_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_2d_t) :: icefrac_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_2d_t) :: snowhland_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_2d_t) :: asdir_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_2d_t) :: asdif_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_2d_t) :: aldir_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_2d_t) :: aldif_ptrs(begchunk:endchunk)
! Subprogram not used 
! Subprogram not used     type(drv_input_2di_t):: nmxrgn_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: pmxrgn_ptrs(begchunk:endchunk)
! Subprogram not used 
! Subprogram not used     type(drv_input_3d_t) :: cldemis_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: cldtau_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: cicewp_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_3d_t) :: cliqwp_ptrs(begchunk:endchunk)
! Subprogram not used 
! Subprogram not used     type(drv_input_2d_t) :: lwup_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_2d_t) :: ts_ptrs(begchunk:endchunk)
! Subprogram not used 
! Subprogram not used     integer :: i, k, c, ncol, itim
! Subprogram not used 
! Subprogram not used     integer :: ixcldice   ! cloud ice water index
! Subprogram not used     integer :: ixcldliq   ! cloud liquid water index
! Subprogram not used 
! Subprogram not used     ! for Fixed Dynamical Heating
! Subprogram not used     real(r8), pointer, dimension(:,:) :: tcorr
! Subprogram not used     real(r8), pointer, dimension(:,:) :: qrsin
! Subprogram not used     real(r8), pointer, dimension(:,:) :: qrlin
! Subprogram not used     real(r8), pointer, dimension(:)   :: tropp
! Subprogram not used     integer :: troplev(pcols)
! Subprogram not used 
! Subprogram not used     ! for modal aerosols
! Subprogram not used     type(drv_input_4d_t) :: dgnumwet_ptrs(begchunk:endchunk)
! Subprogram not used     type(drv_input_4d_t) :: qaerwat_ptrs(begchunk:endchunk)
! Subprogram not used 
! Subprogram not used     ! get index of (liquid+ice) cloud water
! Subprogram not used     call cnst_get_ind('CLDICE', ixcldice)
! Subprogram not used     call cnst_get_ind('CLDLIQ', ixcldliq)
! Subprogram not used 
! Subprogram not used     ! phys buffer time index
! Subprogram not used     itim = pbuf_old_tim_idx()
! Subprogram not used 
! Subprogram not used     ! setup the data pointers
! Subprogram not used !$OMP PARALLEL DO PRIVATE (C,pbuf)
! Subprogram not used     do c=begchunk,endchunk
! Subprogram not used        pbuf => pbuf_get_chunk(pbuf2d, c)
! Subprogram not used 
! Subprogram not used        watvap_ptrs (c)%array => phys_state(c)%q(:,:,1)
! Subprogram not used        watliq_ptrs (c)%array => phys_state(c)%q(:,:,ixcldliq)
! Subprogram not used        watice_ptrs (c)%array => phys_state(c)%q(:,:,ixcldice)
! Subprogram not used 
! Subprogram not used        zi_ptrs     (c)%array => phys_state(c)%zi
! Subprogram not used        pint_ptrs   (c)%array => phys_state(c)%pint
! Subprogram not used        lnpint_ptrs (c)%array => phys_state(c)%lnpint
! Subprogram not used 
! Subprogram not used        temp_ptrs   (c)%array => phys_state(c)%t
! Subprogram not used        pdel_ptrs   (c)%array => phys_state(c)%pdel
! Subprogram not used        pdeldry_ptrs(c)%array => phys_state(c)%pdeldry
! Subprogram not used        lnpmid_ptrs (c)%array => phys_state(c)%lnpmid
! Subprogram not used        pmid_ptrs   (c)%array => phys_state(c)%pmid
! Subprogram not used 
! Subprogram not used        lndfrac_ptrs  (c)%array => cam_in(c)%landfrac
! Subprogram not used        icefrac_ptrs  (c)%array => cam_in(c)%icefrac
! Subprogram not used        snowhland_ptrs(c)%array => cam_in(c)%snowhland
! Subprogram not used        asdir_ptrs    (c)%array => cam_in(c)%asdir
! Subprogram not used        asdif_ptrs    (c)%array => cam_in(c)%asdif
! Subprogram not used        aldir_ptrs    (c)%array => cam_in(c)%aldir
! Subprogram not used        aldif_ptrs    (c)%array => cam_in(c)%aldif
! Subprogram not used        lwup_ptrs     (c)%array => cam_in(c)%lwup
! Subprogram not used        ts_ptrs       (c)%array => cam_in(c)%ts
! Subprogram not used 
! Subprogram not used        call pbuf_get_field(pbuf, cld_ifld, cld_ptrs   (c)%array, start=(/1,1,itim/), kount=(/pcols,pver,1/) )
! Subprogram not used        call pbuf_get_field(pbuf, rel_ifld,   rel_ptrs(c)%array )
! Subprogram not used        call pbuf_get_field(pbuf, rei_ifld,   rei_ptrs(c)%array )
! Subprogram not used 
! Subprogram not used        call pbuf_get_field(pbuf, qrs_ifld,   qrs_ptrs(c)%array )
! Subprogram not used        call pbuf_get_field(pbuf, qrl_ifld,   qrl_ptrs(c)%array )
! Subprogram not used 
! Subprogram not used        if (mg_microphys) then
! Subprogram not used           call pbuf_get_field(pbuf, dei_ifld,   dei_ptrs(c)%array )
! Subprogram not used           call pbuf_get_field(pbuf, des_ifld,   des_ptrs(c)%array )
! Subprogram not used           call pbuf_get_field(pbuf, mu_ifld,    mu_ptrs     (c)%array )
! Subprogram not used           call pbuf_get_field(pbuf, lambdac_ifld, lambdac_ptrs(c)%array )
! Subprogram not used           call pbuf_get_field(pbuf, iciwp_ifld, iciwp_ptrs  (c)%array )
! Subprogram not used           call pbuf_get_field(pbuf, iclwp_ifld, iclwp_ptrs  (c)%array )
! Subprogram not used           call pbuf_get_field(pbuf, icswp_ifld, icswp_ptrs  (c)%array )
! Subprogram not used           call pbuf_get_field(pbuf, cldfsnow_ifld, cldfsnow_ptrs(c)%array, start=(/1,1,itim/), kount=(/pcols,pver,1/) )
! Subprogram not used        else
! Subprogram not used           call pbuf_get_field(pbuf, nmxrgn_ifld,   nmxrgn_ptrs(c)%array )
! Subprogram not used           call pbuf_get_field(pbuf, pmxrgn_ifld,   pmxrgn_ptrs(c)%array )
! Subprogram not used           call pbuf_get_field(pbuf, cldemis_ifld,  cldemis_ptrs(c)%array )
! Subprogram not used           call pbuf_get_field(pbuf, cldtau_ifld,   cldtau_ptrs(c)%array )
! Subprogram not used           call pbuf_get_field(pbuf, cicewp_ifld,   cicewp_ptrs(c)%array )
! Subprogram not used           call pbuf_get_field(pbuf, cliqwp_ifld,   cliqwp_ptrs(c)%array )
! Subprogram not used        endif
! Subprogram not used 
! Subprogram not used        if (nmodes>0) then
! Subprogram not used           call pbuf_get_field(pbuf, dgnumwet_ifld, dgnumwet_ptrs(c)%array )
! Subprogram not used           call pbuf_get_field(pbuf, qaerwat_ifld, qaerwat_ptrs(c)%array )
! Subprogram not used        endif
! Subprogram not used 
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used     
! Subprogram not used     if (nmodes>0) then
! Subprogram not used        call get_aeromode_data( indata, dgnumwet_fldn, recno, dgnumwet_ptrs )
! Subprogram not used        call get_aeromode_data( indata, qaerwat_fldn, recno, qaerwat_ptrs )
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used     ! get the 2D data
! Subprogram not used 
! Subprogram not used     call drv_input_data_get( indata, lndfrc_fldn, recno, lndfrac_ptrs )
! Subprogram not used     call drv_input_data_get( indata, icefrc_fldn, recno, icefrac_ptrs )
! Subprogram not used     call drv_input_data_get( indata, snowh_fldn,  recno, snowhland_ptrs )
! Subprogram not used     call drv_input_data_get( indata, asdir_fldn,  recno, asdir_ptrs )
! Subprogram not used     call drv_input_data_get( indata, asdif_fldn,  recno, asdif_ptrs )
! Subprogram not used     call drv_input_data_get( indata, aldir_fldn,  recno, aldir_ptrs )
! Subprogram not used     call drv_input_data_get( indata, aldif_fldn,  recno, aldif_ptrs )
! Subprogram not used     call drv_input_data_get( indata, lwup_fldn,   recno, lwup_ptrs )
! Subprogram not used     call drv_input_data_get( indata, ts_fldn,     recno, ts_ptrs )
! Subprogram not used 
! Subprogram not used     ! get the 3D data
! Subprogram not used 
! Subprogram not used     call drv_input_data_get( indata, cld_fldn,    'lev', pver, recno, cld_ptrs )
! Subprogram not used     call drv_input_data_get( indata, rel_fldn,    'lev', pver, recno, rel_ptrs )
! Subprogram not used     call drv_input_data_get( indata, rei_fldn,    'lev', pver, recno, rei_ptrs )
! Subprogram not used     call drv_input_data_get( indata, qrs_fldn,    'lev', pver, recno, qrs_ptrs )
! Subprogram not used     call drv_input_data_get( indata, qrl_fldn,    'lev', pver, recno, qrl_ptrs )
! Subprogram not used 
! Subprogram not used     if (mg_microphys) then
! Subprogram not used        call drv_input_data_get( indata, dei_fldn,    'lev', pver, recno, dei_ptrs )
! Subprogram not used        call drv_input_data_get( indata, des_fldn,    'lev', pver, recno, des_ptrs )
! Subprogram not used        call drv_input_data_get( indata, mu_fldn,     'lev', pver, recno, mu_ptrs )
! Subprogram not used        call drv_input_data_get( indata, lambdac_fldn,'lev', pver, recno, lambdac_ptrs )
! Subprogram not used        call drv_input_data_get( indata, iciwp_fldn,  'lev', pver, recno, iciwp_ptrs )
! Subprogram not used        call drv_input_data_get( indata, iclwp_fldn,  'lev', pver, recno, iclwp_ptrs )
! Subprogram not used        call drv_input_data_get( indata, icswp_fldn,  'lev', pver, recno, icswp_ptrs )
! Subprogram not used        call drv_input_data_get( indata, cldfsnow_fldn,'lev',pver, recno, cldfsnow_ptrs )
! Subprogram not used     else
! Subprogram not used        call drv_input_data_get( indata, nmxrgn_fldn, recno, nmxrgn_ptrs )
! Subprogram not used        call drv_input_data_get( indata, pmxrgn_fldn, 'ilev',pverp,recno, pmxrgn_ptrs )
! Subprogram not used        call drv_input_data_get( indata, cldemis_fldn,'lev', pver, recno, cldemis_ptrs )
! Subprogram not used        call drv_input_data_get( indata, cldtau_fldn, 'lev', pver, recno, cldtau_ptrs )
! Subprogram not used        call drv_input_data_get( indata, cicewp_fldn, 'lev', pver, recno, cicewp_ptrs )
! Subprogram not used        call drv_input_data_get( indata, cliqwp_fldn, 'lev', pver, recno, cliqwp_ptrs )
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used     call drv_input_data_get( indata, watvap_fldn, 'lev', pver, recno, watvap_ptrs )
! Subprogram not used     call drv_input_data_get( indata, watliq_fldn, 'lev', pver, recno, watliq_ptrs )
! Subprogram not used     call drv_input_data_get( indata, watice_fldn, 'lev', pver, recno, watice_ptrs )
! Subprogram not used     call drv_input_data_get( indata, temp_fldn,   'lev', pver, recno, temp_ptrs )
! Subprogram not used     call drv_input_data_get( indata, pdel_fldn,   'lev', pver, recno, pdel_ptrs )
! Subprogram not used     call drv_input_data_get( indata, pdeldry_fldn,'lev', pver, recno, pdeldry_ptrs )
! Subprogram not used     call drv_input_data_get( indata, pmid_fldn,   'lev', pver, recno, pmid_ptrs )
! Subprogram not used 
! Subprogram not used     call drv_input_data_get( indata, zint_fldn,   'ilev',pverp,recno, zi_ptrs )
! Subprogram not used     call drv_input_data_get( indata, pint_fldn,   'ilev',pverp,recno, pint_ptrs )
! Subprogram not used 
! Subprogram not used     do c=begchunk,endchunk
! Subprogram not used        ncol = phys_state(c)%ncol
! Subprogram not used        if (all(pmid_ptrs(c)%array(:ncol,:) > 0._r8 )) then
! Subprogram not used           do i=1,ncol
! Subprogram not used              lnpmid_ptrs(c)%array(i,:) = log(pmid_ptrs(c)%array(i,:))
! Subprogram not used              lnpint_ptrs(c)%array(i,:) = log(pint_ptrs(c)%array(i,:))
! Subprogram not used           enddo
! Subprogram not used        endif
! Subprogram not used 
! Subprogram not used        ! adjust temperatue input above tropopause for Fixed Dynamical Heating ....
! Subprogram not used        if (do_fdh) then
! Subprogram not used           pbuf => pbuf_get_chunk(pbuf2d, c)
! Subprogram not used 
! Subprogram not used           call pbuf_get_field(pbuf, tropp_idx, tropp)
! Subprogram not used           call pbuf_get_field(pbuf, tcorr_idx, tcorr)
! Subprogram not used           call pbuf_get_field(pbuf, qrsin_idx, qrsin)
! Subprogram not used           call pbuf_get_field(pbuf, qrlin_idx, qrlin)
! Subprogram not used 
! Subprogram not used           call tropopause_find(phys_state(c), troplev, tropP=tropp(:), primary=TROP_ALG_HYBSTOB, backup=TROP_ALG_CLIMATE)
! Subprogram not used 
! Subprogram not used           qrsin(:,:) = qrs_ptrs(c)%array(:,:)
! Subprogram not used           qrlin(:,:) = qrl_ptrs(c)%array(:,:)
! Subprogram not used 
! Subprogram not used           do k = 1, pver
! Subprogram not used              do i = 1, ncol
! Subprogram not used                 if ( phys_state(c)%pmid(i,k) < tropp(i) ) then
! Subprogram not used                    temp_ptrs(c)%array(i,k) = temp_ptrs(c)%array(i,k) + tcorr(i,k)
! Subprogram not used                 endif
! Subprogram not used              enddo
! Subprogram not used           enddo
! Subprogram not used        endif
! Subprogram not used 
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used     call get_rad_cnst_data(indata, phys_state, pbuf2d, recno)
! Subprogram not used 
! Subprogram not used   end subroutine rad_data_read

!================================================================================================
! Private routines
!================================================================================================


  !================================================================================================
  !================================================================================================
! Subprogram not used   subroutine get_aeromode_data(indata, infld_names, recno, chunk_ptrs)
! Subprogram not used     use drv_input_data, only: drv_input_data_read
! Subprogram not used 
! Subprogram not used     type(drv_input_data_t), intent(inout) :: indata
! Subprogram not used     character(len=*),    intent(in)    :: infld_names(nmodes)
! Subprogram not used     integer,             intent(in)    :: recno
! Subprogram not used     type(drv_input_4d_t),   intent(inout) :: chunk_ptrs(begchunk:endchunk)
! Subprogram not used 
! Subprogram not used     real(r8) :: data (pcols, pver, begchunk:endchunk)
! Subprogram not used 
! Subprogram not used     integer :: c, n
! Subprogram not used 
! Subprogram not used     do n = 1,nmodes
! Subprogram not used 
! Subprogram not used        data = drv_input_data_read( indata, infld_names(n), 'lev', pver, recno )
! Subprogram not used        do c=begchunk,endchunk
! Subprogram not used           chunk_ptrs(c)%array(:,:,n) = data(:,:,c)
! Subprogram not used        enddo
! Subprogram not used 
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used   end subroutine get_aeromode_data

  !================================================================================================
  !================================================================================================
! Subprogram not used   subroutine get_rad_cnst_data(indata, state, pbuf2d, recno)
! Subprogram not used 
! Subprogram not used     use physics_types,    only: physics_state
! Subprogram not used     use physics_buffer,   only: physics_buffer_desc
! Subprogram not used     use rad_constituents, only: rad_cnst_get_info
! Subprogram not used 
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     ! Arguments
! Subprogram not used     type(drv_input_data_t), intent(inout) :: indata
! Subprogram not used     type(physics_buffer_desc), pointer :: pbuf2d(:,:)
! Subprogram not used     type(physics_state), intent(inout) :: state(begchunk:endchunk)
! Subprogram not used     integer,             intent(in)    :: recno
! Subprogram not used 
! Subprogram not used     ! Local variables
! Subprogram not used     integer :: i, ncol
! Subprogram not used     integer :: m, l, nspec
! Subprogram not used     integer :: idx
! Subprogram not used     character(len=32) :: aername
! Subprogram not used 
! Subprogram not used     !-----------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used     ! read in mixing ratio of rad constituents 
! Subprogram not used 
! Subprogram not used     do i = 1,ngas
! Subprogram not used        call read_rad_gas_data(indata, gasnames(i), i, state, pbuf2d, recno )
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used     do i = 1,naer
! Subprogram not used        call read_rad_aer_data(indata, aernames(i), i, state, pbuf2d, recno )
! Subprogram not used     enddo
! Subprogram not used     
! Subprogram not used     ! aerosol mode loop
! Subprogram not used     do m = 1, nmodes
! Subprogram not used 
! Subprogram not used        ! get mode info
! Subprogram not used        call rad_cnst_get_info(0, m, nspec=nspec)
! Subprogram not used        ! aerosol species loop
! Subprogram not used        do l = 1, nspec
! Subprogram not used           call rad_cnst_get_info(0,m,l, spec_name=aername)
! Subprogram not used           call read_rad_mam_data( indata, aername, m, l, state, pbuf2d, recno )
! Subprogram not used        end do
! Subprogram not used     end do
! Subprogram not used 
! Subprogram not used   end subroutine get_rad_cnst_data

  !=================================================================================
  !=================================================================================
! Subprogram not used   subroutine read_rad_gas_data(indata, name, idx, state, pbuf2d, recno )
! Subprogram not used     use rad_constituents, only: rad_cnst_get_gas
! Subprogram not used     use drv_input_data,   only: drv_input_data_read
! Subprogram not used 
! Subprogram not used     type(drv_input_data_t), intent(inout) :: indata
! Subprogram not used     character(len=32),   intent(in)    :: name
! Subprogram not used     integer,             intent(in)    :: idx
! Subprogram not used     type(physics_state),target, intent(inout) :: state(begchunk:endchunk)
! Subprogram not used     type(physics_buffer_desc), pointer, dimension(:,:) :: pbuf2d
! Subprogram not used     integer,             intent(in)    :: recno
! Subprogram not used 
! Subprogram not used     type(physics_buffer_desc), pointer, dimension(:) :: phys_buffer_chunk
! Subprogram not used     character(len=32) :: radname
! Subprogram not used     integer            :: c, ncol
! Subprogram not used     real(r8)          :: mass(pcols,pver,begchunk:endchunk)
! Subprogram not used     real(r8), pointer :: mmr_ptr(:,:)
! Subprogram not used 
! Subprogram not used     radname = 'rad_'//trim(name)
! Subprogram not used     mass = drv_input_data_read( indata, radname, 'lev', pver, recno )
! Subprogram not used 
! Subprogram not used     do c = begchunk,endchunk
! Subprogram not used        ncol = state(c)%ncol
! Subprogram not used        phys_buffer_chunk => pbuf_get_chunk(pbuf2d, c)
! Subprogram not used        call rad_cnst_get_gas(0, gaslist(idx), state(c), phys_buffer_chunk, mmr_ptr)
! Subprogram not used        mmr_ptr(:ncol,:) = mass(:ncol,:,c)
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used   end subroutine read_rad_gas_data

  !=================================================================================
  !=================================================================================
! Subprogram not used   subroutine read_rad_aer_data(indata, name, idx, state, pbuf2d, recno )
! Subprogram not used     use rad_constituents, only: rad_cnst_get_aer_mmr
! Subprogram not used     use drv_input_data,   only: drv_input_data_read
! Subprogram not used 
! Subprogram not used     type(drv_input_data_t), intent(inout) :: indata
! Subprogram not used     character(len=32),   intent(in)    :: name
! Subprogram not used     integer,             intent(in)    :: idx
! Subprogram not used     type(physics_state),target, intent(inout) :: state(begchunk:endchunk)
! Subprogram not used     type(physics_buffer_desc), pointer, dimension(:,:) :: pbuf2d
! Subprogram not used     integer,             intent(in)    :: recno
! Subprogram not used 
! Subprogram not used     type(physics_buffer_desc), pointer, dimension(:) :: phys_buffer_chunk
! Subprogram not used     character(len=32) :: radname
! Subprogram not used     integer :: c, ncol
! Subprogram not used     real(r8)          :: mass(pcols,pver,begchunk:endchunk)
! Subprogram not used     real(r8), pointer :: mmr_ptr(:,:)
! Subprogram not used 
! Subprogram not used     radname = 'rad_'//trim(name)
! Subprogram not used     mass = drv_input_data_read( indata, radname, 'lev', pver, recno )
! Subprogram not used 
! Subprogram not used     do c = begchunk,endchunk
! Subprogram not used        ncol = state(c)%ncol
! Subprogram not used        phys_buffer_chunk => pbuf_get_chunk(pbuf2d, c)
! Subprogram not used        call rad_cnst_get_aer_mmr(0, idx, state(c), phys_buffer_chunk, mmr_ptr)
! Subprogram not used        mmr_ptr(:ncol,:) = mass(:ncol,:,c)
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used   end subroutine read_rad_aer_data

  !=================================================================================
  !=================================================================================
! Subprogram not used   subroutine read_rad_mam_data(indata, name, mode_idx, spec_idx, state, pbuf2d, recno )
! Subprogram not used     use rad_constituents, only: rad_cnst_get_aer_mmr
! Subprogram not used     use drv_input_data,   only: drv_input_data_read
! Subprogram not used 
! Subprogram not used     type(drv_input_data_t), intent(inout) :: indata
! Subprogram not used     character(len=32),   intent(in)    :: name
! Subprogram not used     integer,             intent(in)    :: mode_idx
! Subprogram not used     integer,             intent(in)    :: spec_idx
! Subprogram not used     type(physics_state),target, intent(inout) :: state(begchunk:endchunk)
! Subprogram not used     type(physics_buffer_desc), pointer, dimension(:,:) :: pbuf2d
! Subprogram not used     integer,             intent(in)    :: recno
! Subprogram not used 
! Subprogram not used     type(physics_buffer_desc), pointer, dimension(:) :: phys_buffer_chunk
! Subprogram not used     character(len=32) :: radname
! Subprogram not used     integer :: c, ncol
! Subprogram not used     real(r8)          :: mass(pcols,pver,begchunk:endchunk)
! Subprogram not used     real(r8), pointer :: mmr_ptr(:,:)
! Subprogram not used 
! Subprogram not used     radname = 'rad_'//trim(name)
! Subprogram not used     mass = drv_input_data_read( indata, radname, 'lev', pver, recno )
! Subprogram not used 
! Subprogram not used     do c = begchunk,endchunk
! Subprogram not used        ncol = state(c)%ncol
! Subprogram not used        phys_buffer_chunk => pbuf_get_chunk(pbuf2d, c)
! Subprogram not used        call rad_cnst_get_aer_mmr(0, mode_idx, spec_idx, 'a', state(c), phys_buffer_chunk, mmr_ptr)
! Subprogram not used        mmr_ptr(:ncol,:) = mass(:ncol,:,c)
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used   end subroutine read_rad_mam_data

end module radiation_data
