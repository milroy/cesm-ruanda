...
! Subprogram not used    subroutine double_capacity(array)
! Subprogram not used      character(len=256), intent(inout), allocatable :: array(:)
! Subprogram not used 
! Subprogram not used      character(len=256), allocatable :: tmp(:)
! Subprogram not used      integer :: ierr
! Subprogram not used 
! Subprogram not used      allocate(tmp(size(array)*2), stat=ierr)
! Subprogram not used      if ( ierr /= 0 ) then
! Subprogram not used         call endrun('physprop_accum_unique_files: Allocation error.')
! Subprogram not used      end if
! Subprogram not used 
! Subprogram not used      tmp(:size(array)) = array
! Subprogram not used 
! Subprogram not used      deallocate(array, stat=ierr)
! Subprogram not used      if ( ierr /= 0 ) then
! Subprogram not used         call endrun('physprop_accum_unique_files: Deallocation error.')
! Subprogram not used      end if
! Subprogram not used 
! Subprogram not used      call move_alloc(tmp, array)
! Subprogram not used 
! Subprogram not used    end subroutine double_capacity
...
! Subprogram not used subroutine hygro_optics_init(phys_prop, nc_id)
! Subprogram not used 
! Subprogram not used    ! Read optics data of type 'hygro' and interpolate it to 1's rh mesh.
! Subprogram not used 
! Subprogram not used    type (physprop_type), intent(inout) :: phys_prop  ! data after interp onto cam rh mesh
! Subprogram not used    type (file_desc_t),   intent(inout) :: nc_id      ! indentifier for netcdf file
! Subprogram not used 
! Subprogram not used    ! Local variables
! Subprogram not used    integer :: ierr ! error flag
! Subprogram not used 
! Subprogram not used    integer :: rh_idx_id, lw_band_id, sw_band_id
! Subprogram not used    integer :: kbnd, krh
! Subprogram not used    integer :: rh_id, sw_ext_id, sw_ssa_id, sw_asm_id, lw_ext_id
! Subprogram not used    integer :: nbnd, swbands
! Subprogram not used 
! Subprogram not used    ! temp data from hygroscopic file before interpolation onto cam-rh-mesh
! Subprogram not used    integer  :: nfilerh ! number of rh values in file
! Subprogram not used    real(r8), allocatable, dimension(:) :: frh
! Subprogram not used    real(r8), allocatable, dimension(:,:)  :: fsw_ext
! Subprogram not used    real(r8), allocatable, dimension(:,:)  :: fsw_ssa
! Subprogram not used    real(r8), allocatable, dimension(:,:)  :: fsw_asm
! Subprogram not used 
! Subprogram not used    real(r8) :: rh ! real rh value on cam rh mesh (indexvalue)
! Subprogram not used    !------------------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used    allocate(phys_prop%sw_hygro_ext(nrh,nswbands))
! Subprogram not used    allocate(phys_prop%sw_hygro_ssa(nrh,nswbands))
! Subprogram not used    allocate(phys_prop%sw_hygro_asm(nrh,nswbands))
! Subprogram not used    allocate(phys_prop%lw_abs(nlwbands))
! Subprogram not used 
! Subprogram not used    ierr = pio_inq_dimid(nc_id, 'rh_idx', rh_idx_id)
! Subprogram not used 
! Subprogram not used    ierr = pio_inq_dimlen(nc_id, rh_idx_id, nfilerh)
! Subprogram not used 
! Subprogram not used    ierr = pio_inq_dimid(nc_id, 'lw_band', lw_band_id)
! Subprogram not used 
! Subprogram not used    ierr = pio_inq_dimid(nc_id, 'sw_band', sw_band_id)
! Subprogram not used 
! Subprogram not used    ierr = pio_inq_dimlen(nc_id, lw_band_id, nbnd)
! Subprogram not used 
! Subprogram not used    if (nbnd .ne. nlwbands) call endrun(phys_prop%sourcefile// &
! Subprogram not used         ' has the wrong number of lwbands')
! Subprogram not used 
! Subprogram not used    ierr = pio_inq_dimlen(nc_id, sw_band_id, swbands)
! Subprogram not used 
! Subprogram not used    if(swbands .ne. nswbands) call endrun(phys_prop%sourcefile// &
! Subprogram not used          ' has the wrong number of sw bands')
! Subprogram not used 
! Subprogram not used    ierr = pio_inq_varid(nc_id, 'rh', rh_id)
! Subprogram not used 
! Subprogram not used    ierr = pio_inq_varid(nc_id, 'ext_sw', sw_ext_id)
! Subprogram not used 
! Subprogram not used    ierr = pio_inq_varid(nc_id, 'ssa_sw', sw_ssa_id)
! Subprogram not used 
! Subprogram not used    ierr = pio_inq_varid(nc_id, 'asm_sw', sw_asm_id)
! Subprogram not used 
! Subprogram not used    ierr = pio_inq_varid(nc_id, 'abs_lw', lw_ext_id)
! Subprogram not used 
! Subprogram not used    ! specific optical properties on file's rh mesh
! Subprogram not used    allocate(fsw_ext(nfilerh,nswbands))
! Subprogram not used    allocate(fsw_asm(nfilerh,nswbands))
! Subprogram not used    allocate(fsw_ssa(nfilerh,nswbands))
! Subprogram not used    allocate(frh(nfilerh))
! Subprogram not used 
! Subprogram not used    ierr = pio_get_var(nc_id, rh_id, frh)
! Subprogram not used 
! Subprogram not used    ierr = pio_get_var(nc_id, sw_ext_id, fsw_ext)
! Subprogram not used 
! Subprogram not used    ierr = pio_get_var(nc_id, sw_ssa_id, fsw_ssa)
! Subprogram not used 
! Subprogram not used    ierr = pio_get_var(nc_id, sw_asm_id, fsw_asm)
! Subprogram not used 
! Subprogram not used    ierr = pio_get_var(nc_id, lw_ext_id, phys_prop%lw_abs)
! Subprogram not used 
! Subprogram not used    ! interpolate onto cam's rh mesh
! Subprogram not used    do kbnd = 1,nswbands
! Subprogram not used       do krh = 1, nrh
! Subprogram not used          rh = 1.0_r8 / nrh * (krh - 1)
! Subprogram not used          phys_prop%sw_hygro_ext(krh,kbnd) = &
! Subprogram not used             exp_interpol( frh, fsw_ext(:,kbnd) / fsw_ext(1,kbnd), rh ) &
! Subprogram not used             * fsw_ext(1, kbnd)
! Subprogram not used          phys_prop%sw_hygro_ssa(krh,kbnd) = &
! Subprogram not used             lin_interpol( frh, fsw_ssa(:,kbnd) / fsw_ssa(1,kbnd), rh ) &
! Subprogram not used             * fsw_ssa(1, kbnd)
! Subprogram not used          phys_prop%sw_hygro_asm(krh,kbnd) = &
! Subprogram not used             lin_interpol( frh, fsw_asm(:,kbnd) / fsw_asm(1,kbnd), rh ) &
! Subprogram not used             * fsw_asm(1, kbnd)
! Subprogram not used       enddo
! Subprogram not used    enddo
! Subprogram not used 
! Subprogram not used    deallocate (fsw_ext, fsw_asm, fsw_ssa, frh)
! Subprogram not used 
! Subprogram not used    ! read refractive index data if available
! Subprogram not used    call refindex_aer_init(phys_prop, nc_id)
! Subprogram not used 
! Subprogram not used    ! read bulk aero props
! Subprogram not used    call bulk_props_init(phys_prop, nc_id)
! Subprogram not used 
! Subprogram not used end subroutine hygro_optics_init

!================================================================================================

! Subprogram not used subroutine zero_optics_init(phys_prop, nc_id)
! Subprogram not used 
! Subprogram not used    ! Read optics data of type 'nonhygro'
! Subprogram not used 
! Subprogram not used    type (physprop_type), intent(inout) :: phys_prop  ! storage for file data
! Subprogram not used    type (file_desc_t),   intent(inout) :: nc_id      ! indentifier for netcdf file
! Subprogram not used 
! Subprogram not used    ! Local variables
! Subprogram not used    integer :: lw_band_id, sw_band_id
! Subprogram not used    integer :: sw_ext_id, sw_ssa_id, sw_asm_id, lw_ext_id
! Subprogram not used    integer :: swbands, nbnd
! Subprogram not used    integer :: ierr ! error flag
! Subprogram not used    !------------------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used    ! perhaps this doesn't even need allocated.
! Subprogram not used    allocate (phys_prop%sw_nonhygro_ext(nswbands))
! Subprogram not used    allocate (phys_prop%sw_nonhygro_ssa(nswbands))
! Subprogram not used    allocate (phys_prop%sw_nonhygro_asm(nswbands))
! Subprogram not used    allocate (phys_prop%lw_abs(nlwbands))
! Subprogram not used 
! Subprogram not used    phys_prop%sw_nonhygro_ext = 0._r8
! Subprogram not used    phys_prop%sw_nonhygro_ssa = 0._r8
! Subprogram not used    phys_prop%sw_nonhygro_asm = 0._r8
! Subprogram not used    phys_prop%lw_abs = 0._r8
! Subprogram not used 
! Subprogram not used end subroutine zero_optics_init
...
! Subprogram not used subroutine volcanic_radius_optics_init(phys_prop, nc_id)
! Subprogram not used 
! Subprogram not used    ! Read optics data of type 'volcanic_radius'
! Subprogram not used 
! Subprogram not used    type (physprop_type), intent(inout) :: phys_prop  ! storage for file data
! Subprogram not used    type (file_desc_t),   intent(inout) :: nc_id      ! indentifier for netcdf file
! Subprogram not used 
! Subprogram not used    ! Local variables
! Subprogram not used    integer :: lw_band_id, sw_band_id, mu_id, mu_did
! Subprogram not used    integer :: sw_ext_id, sw_scat_id, sw_ascat_id, lw_abs_id
! Subprogram not used    integer :: swbands, nbnd, n_mu_samples
! Subprogram not used    integer :: ierr ! error flag
! Subprogram not used    !------------------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used    ierr = pio_inq_dimid(nc_id, 'mu_samples', mu_did)
! Subprogram not used    ierr = pio_inq_dimlen(nc_id, mu_did, n_mu_samples)
! Subprogram not used 
! Subprogram not used    allocate (phys_prop%r_sw_ext(nswbands,n_mu_samples))
! Subprogram not used    allocate (phys_prop%r_sw_scat(nswbands,n_mu_samples))
! Subprogram not used    allocate (phys_prop%r_sw_ascat(nswbands,n_mu_samples))
! Subprogram not used    allocate (phys_prop%r_lw_abs(nlwbands,n_mu_samples))
! Subprogram not used    allocate (phys_prop%mu(n_mu_samples))
! Subprogram not used 
! Subprogram not used    ierr = pio_inq_dimid(nc_id, 'lw_band', lw_band_id)
! Subprogram not used 
! Subprogram not used    ierr = pio_inq_dimid(nc_id, 'sw_band', sw_band_id)
...
! Subprogram not used         ' has the wrong number of sw bands')
! Subprogram not used 
! Subprogram not used    ! read file data
! Subprogram not used    ierr = pio_inq_varid(nc_id, 'bext_sw', sw_ext_id)
! Subprogram not used    ierr = pio_inq_varid(nc_id, 'bsca_sw', sw_scat_id)
! Subprogram not used    ierr = pio_inq_varid(nc_id, 'basc_sw', sw_ascat_id)
! Subprogram not used    ierr = pio_inq_varid(nc_id, 'babs_lw', lw_abs_id)
! Subprogram not used    ierr = pio_inq_varid(nc_id, 'mu_samples', mu_id)
! Subprogram not used 
! Subprogram not used    ierr = pio_get_var(nc_id, sw_ext_id, phys_prop%r_sw_ext)
! Subprogram not used    ierr = pio_get_var(nc_id, sw_scat_id, phys_prop%r_sw_scat)
! Subprogram not used    ierr = pio_get_var(nc_id, sw_ascat_id, phys_prop%r_sw_ascat)
! Subprogram not used    ierr = pio_get_var(nc_id, lw_abs_id, phys_prop%r_lw_abs)
! Subprogram not used    ierr = pio_get_var(nc_id, mu_id, phys_prop%mu)
! Subprogram not used 
! Subprogram not used    ! read bulk aero props
! Subprogram not used    call bulk_props_init(phys_prop, nc_id)
! Subprogram not used 
! Subprogram not used end subroutine volcanic_radius_optics_init

!================================================================================================

! Subprogram not used subroutine volcanic_optics_init(phys_prop, nc_id)
! Subprogram not used 
! Subprogram not used    ! Read optics data of type 'volcanic'
! Subprogram not used 
! Subprogram not used    type (physprop_type), intent(inout) :: phys_prop  ! storage for file data
! Subprogram not used    type (file_desc_t)  , intent(inout) :: nc_id      ! indentifier for netcdf file
! Subprogram not used 
...
! Subprogram not used 
! Subprogram not used    allocate (phys_prop%sw_nonhygro_ext(nswbands))
! Subprogram not used    allocate (phys_prop%sw_nonhygro_scat(nswbands))
! Subprogram not used    allocate (phys_prop%sw_nonhygro_ascat(nswbands))
! Subprogram not used    allocate (phys_prop%lw_abs(nlwbands))
! Subprogram not used 
! Subprogram not used    ierr = pio_inq_dimid(nc_id, 'lw_band', lw_band_id)
! Subprogram not used    ierr = pio_inq_dimid(nc_id, 'sw_band', sw_band_id)
...
! Subprogram not used 
! Subprogram not used    ierr = pio_inq_dimlen(nc_id, sw_band_id, swbands)
! Subprogram not used    if(masterproc) write(iulog,*) 'swbands',swbands
! Subprogram not used 
! Subprogram not used    if (swbands .ne. nswbands) call endrun(phys_prop%sourcefile// &
! Subprogram not used         ' has the wrong number of sw bands')
! Subprogram not used 
...
! Subprogram not used    ierr = pio_inq_varid(nc_id, 'babs_lw', lw_abs_id)
! Subprogram not used 
! Subprogram not used    ierr = pio_get_var(nc_id, sw_ext_id, phys_prop%sw_nonhygro_ext)
! Subprogram not used    ierr = pio_get_var(nc_id, sw_scat_id, phys_prop%sw_nonhygro_scat)
! Subprogram not used    ierr = pio_get_var(nc_id, sw_ascat_id, phys_prop%sw_nonhygro_ascat)
! Subprogram not used    ierr = pio_get_var(nc_id, lw_abs_id, phys_prop%lw_abs)
! Subprogram not used 
! Subprogram not used    ! read bulk aero props
! Subprogram not used    call bulk_props_init(phys_prop, nc_id)
! Subprogram not used 
! Subprogram not used end subroutine volcanic_optics_init

!================================================================================================

subroutine hygroscopic_optics_init(phys_prop, nc_id)

   ! Read optics data of type 'hygroscopic' and interpolate it to 1's rh mesh.
...
! Subprogram not used subroutine nonhygro_optics_init(phys_prop, nc_id)
! Subprogram not used 
! Subprogram not used    ! Read optics data of type 'nonhygro'
! Subprogram not used 
! Subprogram not used    type (physprop_type), intent(inout) :: phys_prop  ! storage for file data
...
! Subprogram not used 
! Subprogram not used    ierr = pio_get_var(nc_id, sw_ext_id, phys_prop%sw_nonhygro_ext)
! Subprogram not used    ierr = pio_get_var(nc_id, sw_ssa_id, phys_prop%sw_nonhygro_ssa)
! Subprogram not used    ierr = pio_get_var(nc_id, sw_asm_id, phys_prop%sw_nonhygro_asm)
! Subprogram not used    ierr = pio_get_var(nc_id, lw_ext_id, phys_prop%lw_abs)
! Subprogram not used 
! Subprogram not used    ! read refractive index data if available
! Subprogram not used    call refindex_aer_init(phys_prop, nc_id)
...
! Subprogram not used    call bulk_props_init(phys_prop, nc_id)
! Subprogram not used 
! Subprogram not used end subroutine nonhygro_optics_init
...
! Subprogram not used subroutine aer_optics_log(name, ext, ssa, asm)
! Subprogram not used 
! Subprogram not used    ! Purpose:
! Subprogram not used    !   write aerosol optical constants to log file
! Subprogram not used 
! Subprogram not used    ! Author: D. Fillmore
! Subprogram not used 
! Subprogram not used    character(len=*), intent(in) :: name
! Subprogram not used    real(r8), intent(in) :: ext(:)
! Subprogram not used    real(r8), intent(in) :: ssa(:)
! Subprogram not used    real(r8), intent(in) :: asm(:)
! Subprogram not used 
! Subprogram not used    integer :: kbnd, nbnd
! Subprogram not used    !------------------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used    nbnd = ubound(ext, 1)
! Subprogram not used 
! Subprogram not used    write(iulog, '(2x, a)') name
! Subprogram not used    write(iulog, '(2x, a, 4x, a, 4x, a, 4x, a)') 'SW band', 'ext (m^2 kg^-1)', ' ssa', ' asm'
! Subprogram not used    do kbnd = 1, nbnd
! Subprogram not used       write(iulog, '(2x, i7, 4x, f13.2, 4x, f4.2, 4x, f4.2)') kbnd, ext(kbnd), ssa(kbnd), asm(kbnd)
! Subprogram not used    end do
! Subprogram not used 
! Subprogram not used end subroutine aer_optics_log
