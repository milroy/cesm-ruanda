...
contains
  ...
  !------------------------------------------------------------------------------
! Subprogram not used    subroutine get_clump_bounds_new (n, bounds)
! Subprogram not used      !
! Subprogram not used      ! !DESCRIPTION:
! Subprogram not used      ! Determine clump bounds
! Subprogram not used      !
! Subprogram not used      ! !ARGUMENTS:
! Subprogram not used      implicit none
! Subprogram not used      integer, intent(in)  :: n                ! processor clump index
! Subprogram not used      type(bounds_type), intent(out) :: bounds ! clump bounds
! Subprogram not used      !
! Subprogram not used      ! !LOCAL VARIABLES:
! Subprogram not used      character(len=32), parameter :: subname = 'get_clump_bounds'  ! Subroutine name
! Subprogram not used      integer :: cid                                                ! clump id
! Subprogram not used      !------------------------------------------------------------------------------
! Subprogram not used      !    Make sure this IS being called from a threaded region
! Subprogram not used 
! Subprogram not used      cid  = procinfo%cid(n)
! Subprogram not used      bounds%begp = clumps(cid)%begp
! Subprogram not used      bounds%endp = clumps(cid)%endp
! Subprogram not used      bounds%begc = clumps(cid)%begc
! Subprogram not used      bounds%endc = clumps(cid)%endc
! Subprogram not used      bounds%begl = clumps(cid)%begl
! Subprogram not used      bounds%endl = clumps(cid)%endl
! Subprogram not used      bounds%begg = clumps(cid)%begg
! Subprogram not used      bounds%endg = clumps(cid)%endg
! Subprogram not used    end subroutine get_clump_bounds_new
   ...
   !------------------------------------------------------------------------------
! Subprogram not used    subroutine get_proc_total(pid, ncells, nlunits, ncols, npfts)
! Subprogram not used      !
! Subprogram not used      ! !DESCRIPTION:
! Subprogram not used      ! Count up gridcells, landunits, columns, and pfts on process.
! Subprogram not used      !
! Subprogram not used      ! !ARGUMENTS:
! Subprogram not used      implicit none
! Subprogram not used      integer, intent(in)  :: pid     ! proc id
! Subprogram not used      integer, intent(out) :: ncells  ! total number of gridcells on the processor
! Subprogram not used      integer, intent(out) :: nlunits ! total number of landunits on the processor
! Subprogram not used      integer, intent(out) :: ncols   ! total number of columns on the processor
! Subprogram not used      integer, intent(out) :: npfts   ! total number of pfts on the processor
! Subprogram not used      !
! Subprogram not used      ! !LOCAL VARIABLES:
! Subprogram not used      integer :: cid       ! clump index
! Subprogram not used      !------------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used      npfts   = 0
! Subprogram not used      nlunits = 0
! Subprogram not used      ncols   = 0
! Subprogram not used      ncells  = 0
! Subprogram not used      do cid = 1,nclumps
! Subprogram not used         if (clumps(cid)%owner == pid) then
! Subprogram not used            ncells  = ncells  + clumps(cid)%ncells
! Subprogram not used            nlunits = nlunits + clumps(cid)%nlunits
! Subprogram not used            ncols   = ncols   + clumps(cid)%ncols
! Subprogram not used            npfts   = npfts   + clumps(cid)%npfts
! Subprogram not used         end if
! Subprogram not used      end do
! Subprogram not used    end subroutine get_proc_total
