...
! Subprogram not used   subroutine positive_moisture( cp, xlv, xls, ncol, mkx, dt, qvmin, qlmin, qimin, & 
! Subprogram not used                                 dp, qv, ql, qi, t, s, qvten, qlten, qiten, sten )
! Subprogram not used   ! ------------------------------------------------------------------------------- !
! Subprogram not used   ! If any 'ql < qlmin, qi < qimin, qv < qvmin' are developed in any layer,         !
! Subprogram not used   ! force them to be larger than minimum value by (1) condensating water vapor      !
! Subprogram not used   ! into liquid or ice, and (2) by transporting water vapor from the very lower     !
! Subprogram not used   ! layer. '2._r8' is multiplied to the minimum values for safety.                  !
! Subprogram not used   ! Update final state variables and tendencies associated with this correction.    !
! Subprogram not used   ! If any condensation happens, update (s,t) too.                                  !
! Subprogram not used   ! Note that (qv,ql,qi,t,s) are final state variables after applying corresponding !
! Subprogram not used   ! input tendencies.                                                               !
! Subprogram not used   ! Be careful the order of k : '1': near-surface layer, 'mkx' : top layer          ! 
! Subprogram not used   ! ------------------------------------------------------------------------------- !
! Subprogram not used     implicit none
! Subprogram not used     integer,  intent(in)     :: ncol, mkx
! Subprogram not used     real(r8), intent(in)     :: cp, xlv, xls
! Subprogram not used     real(r8), intent(in)     :: dt, qvmin, qlmin, qimin
! Subprogram not used     real(r8), intent(in)     :: dp(ncol,mkx)
! Subprogram not used     real(r8), intent(inout)  :: qv(ncol,mkx), ql(ncol,mkx), qi(ncol,mkx), t(ncol,mkx), s(ncol,mkx)
! Subprogram not used     real(r8), intent(inout)  :: qvten(ncol,mkx), qlten(ncol,mkx), qiten(ncol,mkx), sten(ncol,mkx)
! Subprogram not used     integer   i, k
! Subprogram not used     real(r8)  dql, dqi, dqv, sum, aa, dum 
! Subprogram not used 
! Subprogram not used   ! Modification : I should check whether this is exactly same as the one used in
! Subprogram not used   !                shallow convection and cloud macrophysics.
! Subprogram not used 
! Subprogram not used     do i = 1, ncol
! Subprogram not used        do k = mkx, 1, -1    ! From the top to the 1st (lowest) layer from the surface
! Subprogram not used           dql        = max(0._r8,1._r8*qlmin-ql(i,k))
! Subprogram not used           dqi        = max(0._r8,1._r8*qimin-qi(i,k))
! Subprogram not used           qlten(i,k) = qlten(i,k) +  dql/dt
! Subprogram not used           qiten(i,k) = qiten(i,k) +  dqi/dt
! Subprogram not used           qvten(i,k) = qvten(i,k) - (dql+dqi)/dt
! Subprogram not used           sten(i,k)  = sten(i,k)  + xlv * (dql/dt) + xls * (dqi/dt)
! Subprogram not used           ql(i,k)    = ql(i,k) +  dql
! Subprogram not used           qi(i,k)    = qi(i,k) +  dqi
! Subprogram not used           qv(i,k)    = qv(i,k) -  dql - dqi
! Subprogram not used           s(i,k)     = s(i,k)  +  xlv * dql + xls * dqi
! Subprogram not used           t(i,k)     = t(i,k)  + (xlv * dql + xls * dqi)/cp
! Subprogram not used           dqv        = max(0._r8,1._r8*qvmin-qv(i,k))
! Subprogram not used           qvten(i,k) = qvten(i,k) + dqv/dt
! Subprogram not used           qv(i,k)    = qv(i,k)    + dqv
! Subprogram not used           if( k .ne. 1 ) then 
! Subprogram not used               qv(i,k-1)    = qv(i,k-1)    - dqv*dp(i,k)/dp(i,k-1)
! Subprogram not used               qvten(i,k-1) = qvten(i,k-1) - dqv*dp(i,k)/dp(i,k-1)/dt
! Subprogram not used           endif
! Subprogram not used           qv(i,k) = max(qv(i,k),qvmin)
! Subprogram not used           ql(i,k) = max(ql(i,k),qlmin)
! Subprogram not used           qi(i,k) = max(qi(i,k),qimin)
! Subprogram not used        end do
! Subprogram not used        ! Extra moisture used to satisfy 'qv(i,1)=qvmin' is proportionally 
! Subprogram not used        ! extracted from all the layers that has 'qv > 2*qvmin'. This fully
! Subprogram not used        ! preserves column moisture. 
! Subprogram not used        if( dqv .gt. 1.e-20_r8 ) then
! Subprogram not used            sum = 0._r8
! Subprogram not used            do k = 1, mkx
! Subprogram not used               if( qv(i,k) .gt. 2._r8*qvmin ) sum = sum + qv(i,k)*dp(i,k)
! Subprogram not used            enddo
! Subprogram not used            aa = dqv*dp(i,1)/max(1.e-20_r8,sum)
! Subprogram not used            if( aa .lt. 0.5_r8 ) then
! Subprogram not used                do k = 1, mkx
! Subprogram not used                   if( qv(i,k) .gt. 2._r8*qvmin ) then
! Subprogram not used                       dum        = aa*qv(i,k)
! Subprogram not used                       qv(i,k)    = qv(i,k) - dum
! Subprogram not used                       qvten(i,k) = qvten(i,k) - dum/dt
! Subprogram not used                   endif
! Subprogram not used                enddo 
! Subprogram not used            else 
! Subprogram not used                write(iulog,*) 'Full positive_moisture is impossible in vertical_diffusion'
! Subprogram not used            endif
! Subprogram not used        endif 
! Subprogram not used     end do
! Subprogram not used     return
! Subprogram not used 
! Subprogram not used   end subroutine positive_moisture
