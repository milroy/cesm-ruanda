...
!    print*, "Occurs at elem ", max_dx_at_min_dx_id, " where dx_long = ", max_dx_at_min_dx
    ...
    ! 1 26 level AMIP simulation: max gravity wave speed 341.75 m/s
     ...
     if (hybrid%masterthread) then
       ...
       if (nu>0) then
          ...
          if (hypervis_order==2) then
             ...
             ! should be fixed - but need all 1 defaults adjusted, 
             ! so we have to coordiante with 1
...
! Subprogram not used   function global_maximum(h,hybrid,npts,nets,nete) result(Max_sphere)
! Subprogram not used 
! Subprogram not used     use kinds, only : real_kind
! Subprogram not used     use hybrid_mod, only : hybrid_t
! Subprogram not used     use reduction_mod, only : red_max, pmax_mt
! Subprogram not used 
! Subprogram not used     integer              , intent(in) :: npts,nets,nete     
! Subprogram not used     real (kind=real_kind), intent(in) :: h(npts,npts,nets:nete)
! Subprogram not used     type (hybrid_t)      , intent(in) :: hybrid
! Subprogram not used 
! Subprogram not used     real (kind=real_kind) :: Max_sphere
! Subprogram not used 
! Subprogram not used     ! Local variables
! Subprogram not used 
! Subprogram not used     real (kind=real_kind) :: redp(1)
! Subprogram not used 
! Subprogram not used     Max_sphere = MAXVAL(h(:,:,nets:nete))
! Subprogram not used 
! Subprogram not used     redp(1) = Max_sphere
! Subprogram not used     call pmax_mt(red_max,redp,1,hybrid)
! Subprogram not used     Max_sphere = red_max%buf(1)
! Subprogram not used 
! Subprogram not used   end function global_maximum
...
! Subprogram not used   function l1_snorm(elem, h,ht,hybrid,npts,nets,nete) result(l1)
! Subprogram not used 
! Subprogram not used     use kinds, only : real_kind
...
! Subprogram not used     integer              , intent(in) :: npts,nets,nete
! Subprogram not used     real (kind=real_kind), intent(in) :: h(npts,npts,nets:nete)  ! computed soln
! Subprogram not used     real (kind=real_kind), intent(in) :: ht(npts,npts,nets:nete) ! true soln
! Subprogram not used     type (hybrid_t)      , intent(in) :: hybrid
! Subprogram not used     real (kind=real_kind)             :: l1     
! Subprogram not used 
! Subprogram not used     ! Local variables
! Subprogram not used 
! Subprogram not used     real (kind=real_kind) :: dhabs(npts,npts,nets:nete)
! Subprogram not used     real (kind=real_kind) :: htabs(npts,npts,nets:nete)
! Subprogram not used     real (kind=real_kind) :: dhabs_int
! Subprogram not used     real (kind=real_kind) :: htabs_int
...
! Subprogram not used              dhabs(i,j,ie) = ABS(h(i,j,ie)-ht(i,j,ie))
! Subprogram not used              htabs(i,j,ie) = ABS(ht(i,j,ie))
! Subprogram not used           end do
! Subprogram not used        end do
! Subprogram not used     end do
! Subprogram not used 
! Subprogram not used     dhabs_int = global_integral(elem, dhabs(:,:,nets:nete),hybrid,npts,nets,nete)
! Subprogram not used     htabs_int = global_integral(elem, htabs(:,:,nets:nete),hybrid,npts,nets,nete)
! Subprogram not used 
! Subprogram not used     l1 = dhabs_int/htabs_int
! Subprogram not used 
! Subprogram not used   end function l1_snorm
...
! Subprogram not used   function l1_vnorm(elem, v,vt,hybrid,npts,nets,nete) result(l1)
! Subprogram not used     use kinds, only : real_kind
! Subprogram not used     use element_mod, only : element_t
! Subprogram not used     use hybrid_mod, only : hybrid_t
! Subprogram not used 
...
! Subprogram not used     integer              , intent(in) :: npts,nets,nete
! Subprogram not used     real (kind=real_kind), intent(in) :: v(npts,npts,2,nets:nete)  ! computed soln
! Subprogram not used     real (kind=real_kind), intent(in) :: vt(npts,npts,2,nets:nete) ! true soln
! Subprogram not used     type (hybrid_t)      , intent(in) :: hybrid
! Subprogram not used     real (kind=real_kind)             :: l1     
! Subprogram not used 
! Subprogram not used     ! Local variables
...
! Subprogram not used     real (kind=real_kind) :: dvsq(npts,npts,nets:nete)
! Subprogram not used     real (kind=real_kind) :: vtsq(npts,npts,nets:nete)
! Subprogram not used     real (kind=real_kind) :: dvco(npts,npts,2)         ! covariant velocity
! Subprogram not used     real (kind=real_kind) :: vtco(npts,npts,2)         ! covariant velocity
! Subprogram not used     real (kind=real_kind) :: dv1,dv2
! Subprogram not used     real (kind=real_kind) :: vt1,vt2
! Subprogram not used     real (kind=real_kind) :: dvsq_int
! Subprogram not used     real (kind=real_kind) :: vtsq_int
! Subprogram not used 
! Subprogram not used     integer i,j,ie
! Subprogram not used 
! Subprogram not used     do ie=nets,nete
...
! Subprogram not used           do i=1,npts
! Subprogram not used 
! Subprogram not used              dv1     = v(i,j,1,ie)-vt(i,j,1,ie)
! Subprogram not used              dv2     = v(i,j,2,ie)-vt(i,j,2,ie)
! Subprogram not used 
! Subprogram not used              vt1     = vt(i,j,1,ie)
! Subprogram not used              vt2     = vt(i,j,2,ie)
! Subprogram not used 
! Subprogram not used              dvco(i,j,1) = met(1,1,i,j)*dv1 + met(1,2,i,j)*dv2
! Subprogram not used              dvco(i,j,2) = met(2,1,i,j)*dv1 + met(2,2,i,j)*dv2
! Subprogram not used 
! Subprogram not used              vtco(i,j,1) = met(1,1,i,j)*vt1 + met(1,2,i,j)*vt2
! Subprogram not used              vtco(i,j,2) = met(2,1,i,j)*vt1 + met(2,2,i,j)*vt2
! Subprogram not used 
! Subprogram not used              dvsq(i,j,ie) = SQRT(dvco(i,j,1)*dv1 + dvco(i,j,2)*dv2)
! Subprogram not used              vtsq(i,j,ie) = SQRT(vtco(i,j,1)*vt1 + vtco(i,j,2)*vt2)
! Subprogram not used 
! Subprogram not used           end do
! Subprogram not used        end do
! Subprogram not used     end do
! Subprogram not used 
! Subprogram not used     dvsq_int = global_integral(elem, dvsq(:,:,nets:nete),hybrid,npts,nets,nete)
! Subprogram not used     vtsq_int = global_integral(elem, vtsq(:,:,nets:nete),hybrid,npts,nets,nete)
! Subprogram not used 
! Subprogram not used     l1 = dvsq_int/vtsq_int
! Subprogram not used 
! Subprogram not used   end function l1_vnorm
...
! Subprogram not used   function l2_snorm(elem, h,ht,hybrid,npts,nets,nete) result(l2)
! Subprogram not used     use kinds, only : real_kind
! Subprogram not used     use element_mod, only : element_t
! Subprogram not used     use hybrid_mod, only : hybrid_t
! Subprogram not used 
...
! Subprogram not used     real (kind=real_kind), intent(in) :: ht(npts,npts,nets:nete) ! true soln
! Subprogram not used     type (hybrid_t)      , intent(in) :: hybrid
! Subprogram not used     real (kind=real_kind)             :: l2   
! Subprogram not used 
! Subprogram not used     ! Local variables
! Subprogram not used 
! Subprogram not used     real (kind=real_kind) :: dh2(npts,npts,nets:nete)
! Subprogram not used     real (kind=real_kind) :: ht2(npts,npts,nets:nete)
! Subprogram not used     real (kind=real_kind) :: dh2_int
! Subprogram not used     real (kind=real_kind) :: ht2_int
! Subprogram not used     integer i,j,ie
! Subprogram not used 
! Subprogram not used     do ie=nets,nete
! Subprogram not used        do j=1,npts
! Subprogram not used           do i=1,npts
! Subprogram not used              dh2(i,j,ie)=(h(i,j,ie)-ht(i,j,ie))**2
! Subprogram not used              ht2(i,j,ie)=ht(i,j,ie)**2
! Subprogram not used           end do
! Subprogram not used        end do
! Subprogram not used     end do
! Subprogram not used 
! Subprogram not used     dh2_int = global_integral(elem,dh2(:,:,nets:nete),hybrid,npts,nets,nete)
! Subprogram not used     ht2_int = global_integral(elem,ht2(:,:,nets:nete),hybrid,npts,nets,nete)
! Subprogram not used 
! Subprogram not used     l2 = SQRT(dh2_int)/SQRT(ht2_int)
! Subprogram not used 
! Subprogram not used   end function l2_snorm
...
! Subprogram not used   function l2_vnorm(elem, v,vt,hybrid,npts,nets,nete) result(l2)
! Subprogram not used     use kinds, only : real_kind
! Subprogram not used     use element_mod, only : element_t
! Subprogram not used     use hybrid_mod, only : hybrid_t
! Subprogram not used 
...
! Subprogram not used              vtco(i,j,2) = met(2,1,i,j)*vt1 + met(2,2,i,j)*vt2
! Subprogram not used 
! Subprogram not used              dvsq(i,j,ie) = dvco(i,j,1)*dv1 + dvco(i,j,2)*dv2
! Subprogram not used              vtsq(i,j,ie) = vtco(i,j,1)*vt1 + vtco(i,j,2)*vt2
! Subprogram not used 
! Subprogram not used           end do
! Subprogram not used        end do
! Subprogram not used     end do
...
! Subprogram not used     vtsq_int = global_integral(elem, vtsq(:,:,nets:nete),hybrid,npts,nets,nete)
! Subprogram not used 
! Subprogram not used     l2 = SQRT(dvsq_int)/SQRT(vtsq_int)
! Subprogram not used 
! Subprogram not used   end function l2_vnorm
...
! Subprogram not used   function linf_snorm(h,ht,hybrid,npts,nets,nete) result(linf)
! Subprogram not used     use kinds, only : real_kind
! Subprogram not used     use hybrid_mod, only : hybrid_t
! Subprogram not used     integer              , intent(in) :: npts,nets,nete
! Subprogram not used     real (kind=real_kind), intent(in) :: h(npts,npts,nets:nete)  ! computed soln
! Subprogram not used     real (kind=real_kind), intent(in) :: ht(npts,npts,nets:nete) ! true soln
! Subprogram not used     type (hybrid_t)      , intent(in) :: hybrid
! Subprogram not used     real (kind=real_kind)             :: linf    
! Subprogram not used 
! Subprogram not used     ! Local variables
! Subprogram not used 
! Subprogram not used     real (kind=real_kind) :: dhabs(npts,npts,nets:nete)
! Subprogram not used     real (kind=real_kind) :: htabs(npts,npts,nets:nete)
! Subprogram not used     real (kind=real_kind) :: dhabs_max
! Subprogram not used     real (kind=real_kind) :: htabs_max
! Subprogram not used     integer i,j,ie
! Subprogram not used 
! Subprogram not used     do ie=nets,nete
! Subprogram not used        do j=1,npts
...
! Subprogram not used     end do
! Subprogram not used 
! Subprogram not used     dhabs_max = global_maximum(dhabs(:,:,nets:nete),hybrid,npts,nets,nete)
! Subprogram not used     htabs_max = global_maximum(htabs(:,:,nets:nete),hybrid,npts,nets,nete)
! Subprogram not used 
! Subprogram not used     linf = dhabs_max/htabs_max
! Subprogram not used 
! Subprogram not used   end function linf_snorm
...
! Subprogram not used   function linf_vnorm(elem,v,vt,hybrid,npts,nets,nete) result(linf)
! Subprogram not used     use kinds, only : real_kind
! Subprogram not used     use hybrid_mod, only : hybrid_t
! Subprogram not used     use element_mod, only : element_t
! Subprogram not used 
...
! Subprogram not used     real (kind=real_kind) :: dv1,dv2
! Subprogram not used     real (kind=real_kind) :: vt1,vt2
! Subprogram not used     real (kind=real_kind) :: dvsq_max
! Subprogram not used     real (kind=real_kind) :: vtsq_max
! Subprogram not used     integer i,j,ie
! Subprogram not used 
! Subprogram not used     do ie=nets,nete
! Subprogram not used        met => elem(ie)%met
...
! Subprogram not used     end do
! Subprogram not used 
! Subprogram not used     dvsq_max = global_maximum(dvsq(:,:,nets:nete),hybrid,npts,nets,nete)
! Subprogram not used     vtsq_max = global_maximum(vtsq(:,:,nets:nete),hybrid,npts,nets,nete)
! Subprogram not used 
! Subprogram not used     linf = dvsq_max/vtsq_max
! Subprogram not used 
! Subprogram not used   end function linf_vnorm
