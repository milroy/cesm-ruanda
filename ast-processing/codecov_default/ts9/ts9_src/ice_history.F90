...
! Subprogram not used       subroutine define_hist_field(id, vname, vunit, vcoord, vcellmeas, &
! Subprogram not used                                    vdesc, vcomment, cona, conb, &
! Subprogram not used                                    ns1, vhistfreq)
! Subprogram not used 
! Subprogram not used !     !DESCRIPTION:
! Subprogram not used !     Initializes description of an available field and returns location
! Subprogram not used !     in the available fields array for use in later calls.
! Subprogram not used !
! Subprogram not used !     !REVISION HISTORY:
! Subprogram not used !     !REVISION HISTORY:
! Subprogram not used !     2009 Created by D. Bailey following POP
! Subprogram not used 
! Subprogram not used !     !USES:
! Subprogram not used       use ice_exit
! Subprogram not used       use ice_calendar, only: histfreq, histfreq_n, nstreams
! Subprogram not used 
! Subprogram not used !     !OUTPUT PARAMETERS:
! Subprogram not used 
! Subprogram not used       integer (int_kind), dimension(max_nstrm), intent(out) :: &
! Subprogram not used          id                ! location in avail_fields array for use in
! Subprogram not used                            ! later routines
! Subprogram not used 
! Subprogram not used !     !INPUT PARAMETERS
! Subprogram not used 
! Subprogram not used       character (len=*), intent(in) :: &
! Subprogram not used          vname      , & ! variable names
! Subprogram not used          vunit      , & ! variable units
! Subprogram not used          vcoord     , & ! variable coordinates
! Subprogram not used          vcellmeas  , & ! variables cell measures
! Subprogram not used          vdesc      , & ! variable descriptions
! Subprogram not used          vcomment       ! variable comments
! Subprogram not used 
! Subprogram not used       real (kind=dbl_kind), intent(in) :: &
! Subprogram not used          cona       , & ! multiplicative conversion factor
! Subprogram not used          conb           ! additive conversion factor
! Subprogram not used 
! Subprogram not used       character (len=*), intent(in) :: &
! Subprogram not used          vhistfreq      ! history frequency
! Subprogram not used  
! Subprogram not used       integer (kind=int_kind), intent(in) :: &
! Subprogram not used          ns1            ! stream index
! Subprogram not used 
! Subprogram not used       integer (kind=int_kind) :: &
! Subprogram not used          ns         , & ! loop index
! Subprogram not used          lenf           ! length of namelist string
! Subprogram not used 
! Subprogram not used       character (len=40) :: stmp
! Subprogram not used 
! Subprogram not used       lenf = len(trim(vhistfreq))
! Subprogram not used       if (ns1 == 1) id(:) = 0
! Subprogram not used 
! Subprogram not used       do ns = 1, nstreams
! Subprogram not used          if (vhistfreq(ns1:ns1) == histfreq(ns)) then
! Subprogram not used 
! Subprogram not used             num_avail_hist_fields = num_avail_hist_fields + 1
! Subprogram not used 
! Subprogram not used             if (num_avail_hist_fields > max_avail_hist_fields) &
! Subprogram not used                call abort_ice("Need to increase max_avail_hist_fields")
! Subprogram not used 
! Subprogram not used             id(ns) = num_avail_hist_fields
! Subprogram not used 
! Subprogram not used             stmp = vname
! Subprogram not used             if (lenf > 1 .and. ns1 > 1) &
! Subprogram not used                write(stmp,'(a,a1,a1)') trim(stmp),'_',vhistfreq(ns1:ns1)
! Subprogram not used 
! Subprogram not used             avail_hist_fields(id(ns))%vname = trim(stmp)
! Subprogram not used             avail_hist_fields(id(ns))%vunit = trim(vunit)
! Subprogram not used             avail_hist_fields(id(ns))%vcoord = trim(vcoord)
! Subprogram not used             avail_hist_fields(id(ns))%vcellmeas = trim(vcellmeas)
! Subprogram not used             avail_hist_fields(id(ns))%vdesc = trim(vdesc)
! Subprogram not used             avail_hist_fields(id(ns))%vcomment = trim(vcomment)
! Subprogram not used             avail_hist_fields(id(ns))%cona = cona
! Subprogram not used             avail_hist_fields(id(ns))%conb = conb
! Subprogram not used             avail_hist_fields(id(ns))%vhistfreq = vhistfreq(ns1:ns1)
! Subprogram not used             avail_hist_fields(id(ns))%vhistfreq_n = histfreq_n(ns)
! Subprogram not used 
! Subprogram not used          endif
! Subprogram not used       enddo
! Subprogram not used 
! Subprogram not used       end subroutine define_hist_field
