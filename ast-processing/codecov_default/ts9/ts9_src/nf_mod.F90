...
!! @retval ierr @copydoc error_return
!>
! Subprogram not used   integer function inq_att_vid(File,varid,name,xtype,len) result(ierr)
! Subprogram not used 
! Subprogram not used 
! Subprogram not used     type (File_desc_t), intent(inout) :: File
! Subprogram not used     integer(i4), intent(in)           :: varid
! Subprogram not used     character(len=*), intent(in)      :: name
! Subprogram not used     integer, intent(out)              :: xtype
! Subprogram not used     integer, intent(out)              :: len !Attribute length
! Subprogram not used 
! Subprogram not used     !------------------
! Subprogram not used     ! Local variables
! Subprogram not used     !------------------
! Subprogram not used     integer :: iotype, mpierr, msg, nlen
! Subprogram not used     integer(kind=PIO_Offset) :: clen
! Subprogram not used     type(iosystem_desc_t), pointer :: ios
! Subprogram not used 
! Subprogram not used     ios => File%iosystem
! Subprogram not used     iotype = File%iotype
! Subprogram not used     ierr=PIO_noerr
! Subprogram not used     nlen = len_trim(name)
! Subprogram not used 
! Subprogram not used     if(ios%async_interface) then
! Subprogram not used        if(.not. ios%ioproc ) then
! Subprogram not used           msg=PIO_MSG_INQ_ATT
! Subprogram not used           if(ios%comp_rank==0) call mpi_send(msg, 1, mpi_integer, ios%ioroot, 1, ios%union_comm, ierr)
! Subprogram not used           call MPI_BCAST(file%fh,1,MPI_INTEGER,ios%CompMaster, ios%my_comm , mpierr)
! Subprogram not used        end if
! Subprogram not used        call MPI_BCAST(varid,1,MPI_INTEGER,ios%CompMaster, ios%my_comm , mpierr)
! Subprogram not used        call MPI_BCAST(nlen,1,MPI_INTEGER,ios%CompMaster, ios%my_comm , mpierr)
! Subprogram not used        call MPI_BCAST(name,nlen,MPI_CHARACTER,ios%CompMaster, ios%my_comm , mpierr)
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used 
! Subprogram not used 
! Subprogram not used     if(ios%IOproc) then
! Subprogram not used        select case(iotype)
! Subprogram not used 
! Subprogram not used        case(pio_iotype_pnetcdf)
! Subprogram not used           ierr=nfmpi_inq_att(File%fh,varid,name(1:nlen),xtype,clen)
! Subprogram not used 
! Subprogram not used           len = INT(clen,kind=i4)
! Subprogram not used 
! Subprogram not used        case(pio_iotype_netcdf4p)
! Subprogram not used           ierr=nf90_inquire_attribute( File%fh,varid,name(1:nlen), &
! Subprogram not used                xtype=xtype,len=len)          
! Subprogram not used        case(pio_iotype_netcdf, pio_iotype_netcdf4c)
! Subprogram not used 
! Subprogram not used           if (ios%io_rank==0) then
! Subprogram not used              ierr=nf90_inquire_attribute( File%fh,varid,name(1:nlen), &
! Subprogram not used                   xtype=xtype,len=len)
! Subprogram not used           endif
! Subprogram not used 
! Subprogram not used           if(.not.ios%async_interface .and. ios%num_tasks==ios%num_iotasks) then
! Subprogram not used              call MPI_BCAST(xtype,1,MPI_INTEGER,0,ios%IO_comm, mpierr)
! Subprogram not used              call CheckMPIReturn('nf_mod',mpierr)
! Subprogram not used              call MPI_BCAST(len,1,MPI_INTEGER,0,ios%IO_comm, mpierr)
! Subprogram not used              call CheckMPIReturn('nf_mod',mpierr)
! Subprogram not used           end if
! Subprogram not used 
! Subprogram not used        case default
! Subprogram not used           call bad_iotype(iotype,"nf_mod.F90",372)
! Subprogram not used 
! Subprogram not used        end select
! Subprogram not used     endif
! Subprogram not used     call check_netcdf(File, ierr,"nf_mod.F90",376)
! Subprogram not used     if(ios%async_interface .or. ios%num_tasks>ios%num_iotasks) then
! Subprogram not used        call MPI_BCAST(xtype,1,MPI_INTEGER,ios%IOMaster, ios%my_comm , mpierr)
! Subprogram not used        call CheckMPIReturn('nf_mod',mpierr)
! Subprogram not used        call MPI_BCAST(len,1,MPI_INTEGER,ios%IOMaster, ios%my_comm  , mpierr)
! Subprogram not used        call CheckMPIReturn('nf_mod',mpierr)
! Subprogram not used     end if
! Subprogram not used   end function inq_att_vid
...
!! @retval ierr @copydoc error_return
!>
! Subprogram not used   integer function inq_att_vardesc(File,vardesc,name,xtype,len) result(ierr)
! Subprogram not used 
! Subprogram not used     type (File_desc_t), intent(inout) :: File
! Subprogram not used     type(var_desc_t), intent(in)           :: vardesc
! Subprogram not used     character(len=*), intent(in)      :: name
! Subprogram not used     integer, intent(out)              :: xtype
! Subprogram not used     integer, intent(out)              :: len !Attribute length
! Subprogram not used 
! Subprogram not used     ierr = pio_inq_att(file, vardesc%varid, name, xtype, len)
! Subprogram not used 
! Subprogram not used   end function inq_att_vardesc
...
!! @retval ierr @copydoc error_return
!>
! Subprogram not used   integer function inq_attlen_vid(File,varid,name,len) result(ierr)
! Subprogram not used 
! Subprogram not used     type (File_desc_t), intent(inout) :: File
! Subprogram not used     integer(i4), intent(in)            :: varid
! Subprogram not used     character(len=*), intent(in)      :: name
...
! Subprogram not used     if(ios%async_interface) then
! Subprogram not used        if(.not. ios%ioproc ) then
! Subprogram not used           msg=PIO_MSG_INQ_ATTLEN
! Subprogram not used           if(ios%comp_rank==0) call mpi_send(msg, 1, mpi_integer, ios%ioroot, 1, ios%union_comm, ierr)
! Subprogram not used           call MPI_BCAST(file%fh,1,MPI_INTEGER,ios%CompMaster, ios%my_comm , mpierr)
! Subprogram not used        end if
! Subprogram not used        call MPI_BCAST(varid,1,MPI_INTEGER,ios%CompMaster, ios%my_comm , mpierr)
...
! Subprogram not used 
! Subprogram not used        case(pio_iotype_pnetcdf)
! Subprogram not used           ierr=nfmpi_inq_attlen(File%fh,varid,name(1:nlen),clen)
! Subprogram not used           len = INT(clen,kind=i4)
! Subprogram not used 
! Subprogram not used        case(pio_iotype_netcdf4p)
! Subprogram not used              ierr=nf90_inquire_attribute( File%fh,varid,name(1:nlen), &
! Subprogram not used                   len=len)
! Subprogram not used        case(pio_iotype_netcdf, pio_iotype_netcdf4c)
! Subprogram not used           if (ios%io_rank==0) then
! Subprogram not used              ierr=nf90_inquire_attribute( File%fh,varid,name(1:nlen), &
! Subprogram not used                   len=len)
...
! Subprogram not used 
! Subprogram not used        case default
! Subprogram not used           call bad_iotype(iotype,"nf_mod.F90",481)
! Subprogram not used 
! Subprogram not used        end select
! Subprogram not used     endif
! Subprogram not used     call check_netcdf(File, ierr,"nf_mod.F90",485)
! Subprogram not used     if(ios%async_interface.or.ios%num_tasks>ios%num_iotasks) then
! Subprogram not used        call MPI_BCAST(len,1,MPI_INTEGER,ios%IOMaster,ios%my_comm, mpierr)
! Subprogram not used        call CheckMPIReturn('nf_mod',mpierr)
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used   end function inq_attlen_vid
...
!! @retval ierr @copydoc error_return
!>
! Subprogram not used   integer function inq_attlen_vardesc(File,vardesc,name,len) result(ierr)
! Subprogram not used 
! Subprogram not used     type (File_desc_t), intent(inout) :: File
! Subprogram not used     type (Var_desc_t), intent(in)            :: vardesc
! Subprogram not used     character(len=*), intent(in)      :: name
! Subprogram not used     integer, intent(out),optional     :: len !Attribute length
! Subprogram not used 
! Subprogram not used     ierr = pio_inq_attlen(file, vardesc%varid, name, len)
! Subprogram not used 
! Subprogram not used   end function inq_attlen_vardesc
...
!! @retval ierr @copydoc error_return
!<
! Subprogram not used   integer function inq_attname_vid(File,varid,attnum,name) result(ierr)
! Subprogram not used 
! Subprogram not used     type (File_desc_t), intent(inout) :: File
! Subprogram not used     integer(i4), intent(in)           :: varid
! Subprogram not used     integer, intent(in)              :: attnum !Attribute number
! Subprogram not used     character(len=*), intent(out)     :: name
! Subprogram not used 
! Subprogram not used 
! Subprogram not used     !------------------
! Subprogram not used     ! Local variables
! Subprogram not used     !------------------
! Subprogram not used     integer :: iotype, mpierr, msg
! Subprogram not used     type(iosystem_desc_t), pointer :: ios
! Subprogram not used     character(len=PIO_MAX_NAME) :: tmpname
! Subprogram not used 
! Subprogram not used     ios => File%iosystem
! Subprogram not used 
! Subprogram not used     iotype = File%iotype
...
! Subprogram not used     if(ios%async_interface) then
! Subprogram not used        if(.not. ios%ioproc ) then
! Subprogram not used           msg=PIO_MSG_INQ_ATTNAME
! Subprogram not used           if(ios%comp_rank==0) call mpi_send(msg, 1, mpi_integer, ios%ioroot, 1, ios%union_comm, ierr)
! Subprogram not used           call MPI_BCAST(file%fh,1,MPI_INTEGER,ios%CompMaster, ios%my_comm , mpierr)
! Subprogram not used        end if
! Subprogram not used        call MPI_BCAST(varid,1,MPI_INTEGER,ios%CompMaster, ios%my_comm , mpierr)
! Subprogram not used        call MPI_BCAST(attnum,1,MPI_INTEGER,ios%CompMaster, ios%my_comm , mpierr)
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used     if(ios%IOproc) then
! Subprogram not used        select case(iotype)
! Subprogram not used 
! Subprogram not used        case(pio_iotype_pnetcdf)
! Subprogram not used           ierr=nfmpi_inq_attname(File%fh,varid,attnum,tmpname)
! Subprogram not used 
! Subprogram not used 
! Subprogram not used        case(pio_iotype_netcdf4p)
! Subprogram not used           ierr=nf90_inq_attname(File%fh,varid,attnum,tmpname)
! Subprogram not used        case(pio_iotype_netcdf, pio_iotype_netcdf4c)
! Subprogram not used           if (ios%io_rank==0) then
! Subprogram not used              ierr=nf90_inq_attname(File%fh,varid,attnum,tmpname)
! Subprogram not used              if(Debug) print *,"nf_mod.F90",570,name
! Subprogram not used           endif
! Subprogram not used           if(.not.ios%async_interface .and. ios%num_tasks==ios%num_iotasks) then
! Subprogram not used              call MPI_BCAST(tmpname,PIO_MAX_NAME,MPI_CHARACTER,0,ios%IO_comm, mpierr)
! Subprogram not used              call CheckMPIReturn('nf_mod',mpierr)
! Subprogram not used           end if
! Subprogram not used 
! Subprogram not used 
! Subprogram not used        case default
! Subprogram not used           call bad_iotype(iotype,"nf_mod.F90",580)
! Subprogram not used 
! Subprogram not used        end select
! Subprogram not used     endif
! Subprogram not used     call check_netcdf(File, ierr,"nf_mod.F90",584)
! Subprogram not used     if(ios%async_interface .or. ios%num_tasks>ios%num_iotasks) then
! Subprogram not used        call MPI_BCAST(tmpname,PIO_MAX_NAME,MPI_CHARACTER,ios%IOMaster,ios%my_comm, mpierr)
! Subprogram not used        call CheckMPIReturn('nf_mod',mpierr)
! Subprogram not used     end if
! Subprogram not used     name = tmpname(1:len_trim(tmpname))
! Subprogram not used   end function inq_attname_vid
...
!! @retval ierr @copydoc error_return
!<
! Subprogram not used   integer function inq_attname_vardesc(File,vardesc,attnum,name) result(ierr)
! Subprogram not used     type (File_desc_t), intent(inout) :: File
! Subprogram not used     type(var_desc_t), intent(in)           :: vardesc
! Subprogram not used     integer, intent(in)              :: attnum !Attribute number
! Subprogram not used     character(len=*), intent(out)     :: name
! Subprogram not used 
! Subprogram not used     ierr = pio_inq_attname(file, vardesc%varid, attnum, name)
! Subprogram not used 
! Subprogram not used   end function inq_attname_vardesc
...
!! @retval ierr @copydoc error_return
!>
! Subprogram not used   integer function inq_varname_vdesc(File,vardesc,name) result(ierr)
! Subprogram not used 
! Subprogram not used     type (File_desc_t), intent(in)   :: File
! Subprogram not used     type (Var_desc_t), intent(in)    :: vardesc
! Subprogram not used     character(len=*), intent(out)    :: name
! Subprogram not used     
! Subprogram not used     ierr = pio_inq_varname(file,vardesc%varid,name)
! Subprogram not used 
! Subprogram not used   end function inq_varname_vdesc
...
!! @retval ierr @copydoc error_return
!>
! Subprogram not used   integer function inq_varnatts_vid(File,varid,natts) result(ierr)
! Subprogram not used 
! Subprogram not used     type (File_desc_t), intent(in)   :: File
! Subprogram not used     integer           , intent(in) :: varid
! Subprogram not used     integer(i4), intent(out)         :: natts
! Subprogram not used 
! Subprogram not used 
! Subprogram not used     !------------------
! Subprogram not used     ! Local variables
...
! Subprogram not used     if(ios%async_interface) then
! Subprogram not used        if( .not. ios%ioproc ) then
! Subprogram not used           msg=PIO_MSG_INQ_VARNATTS
! Subprogram not used           if(ios%comp_rank==0) call mpi_send(msg, 1, mpi_integer, ios%ioroot, 1, ios%union_comm, ierr)
! Subprogram not used           call MPI_BCAST(file%fh,1,MPI_INTEGER,ios%CompMaster, ios%my_comm , mpierr)
! Subprogram not used        end if
! Subprogram not used        call MPI_BCAST(varid,1,MPI_INTEGER,ios%CompMaster, ios%my_comm , mpierr)
...
! Subprogram not used 
! Subprogram not used        case(pio_iotype_pnetcdf)
! Subprogram not used           ierr=nfmpi_inq_varnatts(File%fh,varid,natts)
! Subprogram not used 
! Subprogram not used        case(pio_iotype_netcdf4p)
! Subprogram not used           ierr=nf90_inquire_variable(File%fh,varid,nAtts=natts)
! Subprogram not used        case(pio_iotype_netcdf, pio_iotype_netcdf4c)
! Subprogram not used           if (ios%io_rank==0) then
! Subprogram not used              ierr=nf90_inquire_variable(File%fh,varid,nAtts=natts)
! Subprogram not used           endif
! Subprogram not used 
! Subprogram not used           call MPI_BCAST(natts,1,MPI_INTEGER,0,ios%IO_comm, mpierr)
! Subprogram not used           call CheckMPIReturn('nf_mod',mpierr)
! Subprogram not used 
! Subprogram not used        case default
! Subprogram not used           call bad_iotype(iotype,"nf_mod.F90",1136)
! Subprogram not used 
! Subprogram not used        end select
! Subprogram not used     endif
! Subprogram not used     call check_netcdf(File, ierr,"nf_mod.F90",1140)
! Subprogram not used     if(ios%async_interface .or. ios%num_tasks>ios%num_iotasks) then
! Subprogram not used        call MPI_BCAST(natts,1,MPI_INTEGER,ios%IOMaster,ios%My_comm, mpierr)
! Subprogram not used        call CheckMPIReturn('nf_mod',mpierr)
! Subprogram not used     end if
! Subprogram not used   end function inq_varnatts_vid
...
!! @retval ierr @copydoc error_return
!>
! Subprogram not used   integer function inq_varnatts_vdesc(File,vardesc,natts) result(ierr)
! Subprogram not used 
! Subprogram not used     type (File_desc_t), intent(in)   :: File
! Subprogram not used     type (Var_desc_t), intent(in)    :: vardesc
! Subprogram not used     integer(i4), intent(out)         :: natts
! Subprogram not used 
! Subprogram not used 
! Subprogram not used     ierr = pio_inq_varnatts(file, vardesc%varid, natts)
! Subprogram not used   end function inq_varnatts_vdesc
...
!! @retval ierr @copydoc error_return
!<
! Subprogram not used   integer function PIO_redef(File) result(ierr)
! Subprogram not used     type (File_desc_t), intent(inout) :: File
! Subprogram not used 
! Subprogram not used     !------------------
! Subprogram not used     ! Local variables
...
! Subprogram not used     ios => file%iosystem
! Subprogram not used     ierr=PIO_noerr
! Subprogram not used     if(ios%async_interface .and. .not. ios%ioproc) then
! Subprogram not used        msg = PIO_MSG_REDEF
! Subprogram not used        if(ios%comp_rank==0) call mpi_send(msg, 1, mpi_integer, ios%ioroot, 1, ios%union_comm, ierr)
! Subprogram not used        call mpi_bcast(file%fh, 1, mpi_integer, ios%compmaster, ios%intercomm, ierr)
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used     if(ios%IOproc) then
! Subprogram not used        select case(iotype)
...
! Subprogram not used        case(pio_iotype_pnetcdf)
! Subprogram not used 
! Subprogram not used           ierr=nfmpi_redef(File%fh)
! Subprogram not used 
! Subprogram not used        case(pio_iotype_netcdf4p)
! Subprogram not used              ierr=nf90_redef(File%fh)
! Subprogram not used        case(pio_iotype_netcdf, pio_iotype_netcdf4c)
! Subprogram not used           if (ios%io_rank==0) then
! Subprogram not used              ierr=nf90_redef(File%fh)
! Subprogram not used           endif
...
! Subprogram not used 
! Subprogram not used        case default
! Subprogram not used           call bad_iotype(iotype,"nf_mod.F90",1510)
! Subprogram not used 
! Subprogram not used        end select
! Subprogram not used     endif
! Subprogram not used     call check_netcdf(File, ierr,"nf_mod.F90",1514)
! Subprogram not used   end function PIO_redef
...
!! @retval ierr @copydoc error_return
!<
! Subprogram not used   integer function pio_copy_att(infile, invarid, name, outfile, outvarid) result(ierr)
! Subprogram not used 
! Subprogram not used     type (File_desc_t), intent(in)  :: infile, outfile
! Subprogram not used     character(len=*), intent(in)    :: name
! Subprogram not used     integer, intent(in) :: invarid, outvarid
! Subprogram not used     integer :: iotype, mpierr, msg
! Subprogram not used     type(iosystem_desc_t), pointer :: ios
! Subprogram not used 
! Subprogram not used 
! Subprogram not used     ios => infile%iosystem
! Subprogram not used     ierr=PIO_noerr
! Subprogram not used     iotype = infile%iotype
! Subprogram not used     if(ios%IOproc) then
! Subprogram not used        select case(iotype)
! Subprogram not used 
! Subprogram not used        case(pio_iotype_pnetcdf)
! Subprogram not used 
! Subprogram not used           ierr = nfmpi_copy_att(infile%fh, invarid, name, &
! Subprogram not used                outfile%fh, outvarid)
! Subprogram not used        case(pio_iotype_netcdf,PIO_iotype_netcdf4c)
! Subprogram not used           if (ios%io_rank==0) then
! Subprogram not used              ierr = nf90_copy_att(infile%fh,invarid,name,&
! Subprogram not used                   outfile%fh,outvarid)     
! Subprogram not used           end if
! Subprogram not used        case(PIO_iotype_netcdf4p)
! Subprogram not used           ierr = nf90_copy_att(infile%fh,invarid,name,&
...
! Subprogram not used        end select
! Subprogram not used     end if
! Subprogram not used     call check_netcdf(outFile, ierr,"nf_mod.F90",1782)
! Subprogram not used   end function pio_copy_att
...
!! @retval ierr @copydoc error_return
!>
! Subprogram not used   integer function inquire_variable_vid(ncid, varid, name, xtype, ndims, dimids, natts) result(ierr)
! Subprogram not used     type(file_desc_t), intent(in) :: ncid
! Subprogram not used     integer,                         intent( in) :: varid
! Subprogram not used     character (len = *),   optional, intent(out) :: name
! Subprogram not used     integer,               optional, intent(out) :: xtype, ndims
! Subprogram not used     integer, dimension(:), optional, intent(out) :: dimids
! Subprogram not used     integer,               optional, intent(out) :: natts
! Subprogram not used 
! Subprogram not used     
! Subprogram not used     if(present(name)) ierr = pio_inq_varname(ncid, varid, name)
! Subprogram not used     if(present(ndims)) ierr = pio_inq_varndims(ncid, varid, ndims)
! Subprogram not used     if(present(dimids)) ierr = pio_inq_vardimid(ncid, varid, dimids)
! Subprogram not used     if(present(natts)) ierr = pio_inq_varnatts(ncid, varid, natts)
! Subprogram not used     if(present(xtype)) ierr = pio_inq_vartype(ncid, varid, xtype)
! Subprogram not used 
! Subprogram not used 
! Subprogram not used 
! Subprogram not used   end function inquire_variable_vid
...
!! @retval ierr @copydoc error_return
!>
! Subprogram not used   integer function PIO_inquire_dimension(ncid, dimid, name, len) result(ierr)
! Subprogram not used     type(file_desc_T),             intent(in)  :: ncid
! Subprogram not used     integer,                       intent( in) :: dimid
! Subprogram not used     character (len = *), optional, intent(out) :: name
! Subprogram not used     integer,             optional, intent(out) :: len
! Subprogram not used 
! Subprogram not used     if(present(len)) ierr = pio_inq_dimlen(ncid, dimid, len)
! Subprogram not used     if(present(name)) ierr = pio_inq_dimname(ncid, dimid,name)
! Subprogram not used 
! Subprogram not used   end function PIO_inquire_dimension
