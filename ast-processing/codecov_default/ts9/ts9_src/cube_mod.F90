...
! Subprogram not used   function covariant_rot(Da,Db) result(R)
! Subprogram not used 
! Subprogram not used     real (kind=real_kind) :: Da(2,2)
! Subprogram not used     real (kind=real_kind) :: Db(2,2)
! Subprogram not used     real (kind=real_kind) :: R(2,2)
! Subprogram not used 
! Subprogram not used     real (kind=real_kind) :: detDa
! Subprogram not used 
! Subprogram not used     detDa = Da(2,2)*Da(1,1) - Da(1,2)*Da(2,1)
! Subprogram not used 
! Subprogram not used     R(1,1)=(Da(2,2)*Db(1,1) - Da(1,2)*Db(2,1))/detDa
! Subprogram not used     R(1,2)=(Da(1,1)*Db(2,1) - Da(2,1)*Db(1,1))/detDa
! Subprogram not used     R(2,1)=(Da(2,2)*Db(1,2) - Da(1,2)*Db(2,2))/detDa
! Subprogram not used     R(2,2)=(Da(1,1)*Db(2,2) - Da(2,1)*Db(1,2))/detDa
! Subprogram not used 
! Subprogram not used   end function covariant_rot
  ...
  ! ========================================================
! Subprogram not used   subroutine dmap_elementlocal(D, elem, a,b)
! Subprogram not used     use element_mod, only : element_t
! Subprogram not used 
! Subprogram not used     type (element_t) :: elem
! Subprogram not used     real (kind=real_kind), intent(out)    :: D(2,2)
! Subprogram not used     real (kind=real_kind), intent(in)     :: a,b
! Subprogram not used 
! Subprogram not used     type (spherical_polar_t)      :: sphere
! Subprogram not used 
! Subprogram not used     type (cartesian3d_t)               ::  corners(4)   
! Subprogram not used     real(kind=real_kind)               ::  c(3,4), q(4), xx(3), r, lam, th, dd(4,2)
! Subprogram not used     real(kind=real_kind)               ::  sinlam, sinth, coslam, costh
! Subprogram not used     real(kind=real_kind)               ::  D1(2,3), D2(3,3), D3(3,2), D4(3,2)
! Subprogram not used     integer :: i,j
! Subprogram not used 
! Subprogram not used     sphere = ref2sphere(a,b,elem)
! Subprogram not used     corners = elem%corners3D
! Subprogram not used 
! Subprogram not used     c(1,1)=corners(1)%x;  c(2,1)=corners(1)%y;  c(3,1)=corners(1)%z; 
! Subprogram not used     c(1,2)=corners(2)%x;  c(2,2)=corners(2)%y;  c(3,2)=corners(2)%z; 
! Subprogram not used     c(1,3)=corners(3)%x;  c(2,3)=corners(3)%y;  c(3,3)=corners(3)%z; 
! Subprogram not used     c(1,4)=corners(4)%x;  c(2,4)=corners(4)%y;  c(3,4)=corners(4)%z; 
! Subprogram not used 
! Subprogram not used     q(1)=(1-a)*(1-b); q(2)=(1+a)*(1-b); q(3)=(1+a)*(1+b); q(4)=(1-a)*(1+b);
! Subprogram not used     q=q/4.0d0;
! Subprogram not used 
! Subprogram not used     do i=1,3
! Subprogram not used       xx(i)=sum(c(i,:)*q(:))
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used     r=sqrt(xx(1)**2+xx(2)**2+xx(3)**2)
! Subprogram not used 
! Subprogram not used     lam=sphere%lon; th=sphere%lat;
! Subprogram not used     sinlam=sin(lam); sinth=sin(th);
! Subprogram not used     coslam=cos(lam); costh=cos(th);
! Subprogram not used 
! Subprogram not used     D1(1,1)=-sinlam; D1(1,2)=coslam; D1(1,3)=0.0d0; 
! Subprogram not used     D1(2,1)=0.0d0;  D1(2,2)=0.0d0;    D1(2,3)=1.0d0;
! Subprogram not used 
! Subprogram not used     D2(1,1)=(sinlam**2)*(costh**2)+sinth**2; D2(1,2)=-sinlam*coslam*(costh**2); D2(1,3)=-coslam*sinth*costh;
! Subprogram not used     D2(2,1)=-sinlam*coslam*(costh**2); D2(2,2)=(coslam**2)*(costh**2)+sinth**2; D2(2,3)=-sinlam*sinth*costh;
! Subprogram not used     D2(3,1)=-coslam*sinth;           D2(3,2)=-sinlam*sinth;               D2(3,3)=costh;
! Subprogram not used 
! Subprogram not used     dd(1,1)=-1+b; dd(1,2)=-1+a;
! Subprogram not used     dd(2,1)=1-b; dd(2,2)=-1-a;
! Subprogram not used     dd(3,1)=1+b; dd(3,2)=1+a;
! Subprogram not used     dd(4,1)=-1-b; dd(4,2)=1-a;
! Subprogram not used 
! Subprogram not used     dd=dd/4.0d0
! Subprogram not used 
! Subprogram not used     do i=1,3
! Subprogram not used       do j=1,2
! Subprogram not used 	D3(i,j)=sum(c(i,:)*dd(:,j))
! Subprogram not used       enddo
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used     do i=1,3
! Subprogram not used       do j=1,2
! Subprogram not used 	D4(i,j)=sum(D2(i,:)*D3(:,j))
! Subprogram not used       enddo
! Subprogram not used     enddo   
! Subprogram not used 
! Subprogram not used     do i=1,2
! Subprogram not used       do j=1,2
! Subprogram not used 	D(i,j)=sum(D1(i,:)*D4(:,j))
! Subprogram not used       enddo
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used     D=D/r
! Subprogram not used   end subroutine dmap_elementlocal
...
! Subprogram not used   function cube_assemble(gbl,fld,elem,par,nelemd,nelem,ielem) result(ierr)
! Subprogram not used     use element_mod, only : element_t
! Subprogram not used 
! Subprogram not used     use parallel_mod, only : parallel_t, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_STATUS_SIZE, MPI_REAL8,MPI_TAG
! Subprogram not used     real (kind=real_kind) :: gbl(:,:,:,:)    ! global output field 
! Subprogram not used     real (kind=real_kind) :: fld(:,:,:)      ! local model field  
! Subprogram not used     type (element_t)      :: elem            ! element to assemble 
! Subprogram not used     type (parallel_t)     :: par             ! parallel structure 
! Subprogram not used     integer               :: nelemd          ! number of elements on the node
! Subprogram not used     integer               :: nelem           ! number of elements on the node
! Subprogram not used     integer               :: ielem           ! local element ctr 
! Subprogram not used     integer               :: ierr            ! returned error code
! Subprogram not used 
! Subprogram not used     ! Local variables
! Subprogram not used 
! Subprogram not used     integer :: ie,je,face_no
! Subprogram not used     integer :: ibase,jbase
! Subprogram not used     integer :: i,j,k
! Subprogram not used     integer :: elem_number
! Subprogram not used 
! Subprogram not used     integer :: ne1,ne2    ! element dimensions
! Subprogram not used     integer :: n1,n2      ! gbl face dimensions
! Subprogram not used     integer :: nface      ! number of faces (must be 6)
! Subprogram not used     integer :: nlyr       ! number of layers
! Subprogram not used 
! Subprogram not used     integer :: ectr       ! global element counter
! Subprogram not used     integer tag
! Subprogram not used     integer :: count      ! w/o "::", triggers PGI 3.1 F90 bug 
! Subprogram not used     integer pe
! Subprogram not used     integer status(MPI_STATUS_SIZE)
! Subprogram not used     integer mpi_err
! Subprogram not used 
! Subprogram not used     call abortmp('Because convert_gbl_index is not used cube_assemble is broken. ')
! Subprogram not used     ne1   = SIZE(fld,1)
! Subprogram not used     ne2   = SIZE(fld,2)
! Subprogram not used     nlyr  = SIZE(fld,3)
! Subprogram not used 
! Subprogram not used     n1    = SIZE(gbl,1)
! Subprogram not used     n2    = SIZE(gbl,2)
! Subprogram not used     nface = SIZE(gbl,3)
! Subprogram not used 
! Subprogram not used     ! =========================
! Subprogram not used     ! Enforce certain rules...
! Subprogram not used     ! =========================
! Subprogram not used 
! Subprogram not used     ierr=0
! Subprogram not used 
! Subprogram not used     if (MODULO(n1,ne1) /= 0) then
! Subprogram not used        ierr=-1
! Subprogram not used        return
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used     if (MODULO(n2,ne2) /= 0) then 
! Subprogram not used        ierr=-2
! Subprogram not used        return
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used     if (nface /= 6) then
! Subprogram not used        ierr=-3
! Subprogram not used        return
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used     ! =========================================================
! Subprogram not used     ! Perform global assembly procedure element by element ...
! Subprogram not used     ! =========================================================
! Subprogram not used 
! Subprogram not used     if (par%rank==par%root) then
! Subprogram not used 
! Subprogram not used        if (ielem<=nelemd) then
! Subprogram not used           elem_number = elem%vertex%number
! Subprogram not used 
! Subprogram not used           call convert_gbl_index(elem_number,ie,je,face_no)
! Subprogram not used           if (face_no /= elem%vertex%face_number) call abortmp('Error in getting face number')
! Subprogram not used 
! Subprogram not used           ibase=ie*ne1
! Subprogram not used           jbase=je*ne2
! Subprogram not used 
! Subprogram not used           do k=1,nlyr
! Subprogram not used              do j=1,ne2
! Subprogram not used                 do i=1,ne1
! Subprogram not used                    gbl(i+ibase,j+jbase,face_no,k)=fld(i,j,k)
...
! Subprogram not used 
! Subprogram not used        if (ielem==nelemd) then
! Subprogram not used           ectr=nelemd
! Subprogram not used           do while(ectr<nelem)
! Subprogram not used              pe    = MPI_ANY_SOURCE
! Subprogram not used              tag   = MPI_ANY_TAG
! Subprogram not used              count = ne1*ne2*nlyr
! Subprogram not used              call MPI_RECV(fld(1,1,1),   &
! Subprogram not used                   count,        &
! Subprogram not used                   MPI_REAL8,    &
! Subprogram not used                   pe,           &
! Subprogram not used                   tag,          &  
! Subprogram not used                   par%comm,     &
! Subprogram not used                   status,       &
! Subprogram not used                   mpi_err) 
! Subprogram not used 
! Subprogram not used              elem_number = status(MPI_TAG)
! Subprogram not used              ! call convert_gbl_index(elem_number,ie,je,face_no)
! Subprogram not used              call abortmp('Because convert_gbl_index is not used for neghbors, the _MPI version needs to be fixed')
! Subprogram not used 
! Subprogram not used              ibase=ie*ne1
! Subprogram not used              jbase=je*ne2
! Subprogram not used 
...
! Subprogram not used              end do
! Subprogram not used 
! Subprogram not used              ectr=ectr+1
! Subprogram not used           end do
! Subprogram not used        end if
! Subprogram not used 
! Subprogram not used     else
! Subprogram not used 
! Subprogram not used        pe    = par%root
! Subprogram not used        tag   = elem%vertex%number
! Subprogram not used        count = ne1*ne2*nlyr
! Subprogram not used        call MPI_SEND(fld(1,1,1),    &
! Subprogram not used             count,         &
! Subprogram not used             MPI_REAL8,     &
! Subprogram not used             pe,            &
! Subprogram not used             tag,           &
...
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used   end function cube_assemble
...
! Subprogram not used   function ref2sphere_longdouble(a,b, elem) result(sphere)         
! Subprogram not used     use element_mod, only : element_t
! Subprogram not used     type (element_t)      :: elem
! Subprogram not used     real(kind=longdouble_kind)    :: a,b
! Subprogram not used     type (spherical_polar_t)      :: sphere
! Subprogram not used 
! Subprogram not used     if (cubed_sphere_map==0) then
! Subprogram not used        sphere = ref2sphere_equiangular_longdouble(a,b,elem%corners,elem%facenum)
! Subprogram not used     elseif (cubed_sphere_map==1) then
! Subprogram not used !       sphere = ref2sphere_gnomonic_longdouble(a,b,corners,face_no)
! Subprogram not used     elseif (cubed_sphere_map==2) then
! Subprogram not used        sphere = ref2sphere_elementlocal_longdouble(a,b,elem)
! Subprogram not used     else
! Subprogram not used        call abortmp('ref2sphere_double(): bad value of cubed_sphere_map')
...
! map a point in the referece element to the sphere
!
! Subprogram not used   function ref2sphere_equiangular_longdouble(a,b, corners, face_no) result(sphere)         
! Subprogram not used     implicit none
! Subprogram not used     real(kind=longdouble_kind)    :: a,b
! Subprogram not used     integer,intent(in)            :: face_no
! Subprogram not used     type (spherical_polar_t)      :: sphere
! Subprogram not used     type (cartesian2d_t)          :: corners(4)
! Subprogram not used     ! local
! Subprogram not used     real(kind=real_kind)               :: pi,pj,qi,qj
! Subprogram not used     type (cartesian2d_t)                 :: cart   
! Subprogram not used 
! Subprogram not used     ! map (a,b) to the [-pi/2,pi/2] equi angular cube face:  x1,x2
! Subprogram not used     ! a = gp%points(i)
! Subprogram not used     ! b = gp%points(j)
! Subprogram not used     pi = (1-a)/2
! Subprogram not used     pj = (1-b)/2
! Subprogram not used     qi = (1+a)/2
! Subprogram not used     qj = (1+b)/2
! Subprogram not used     cart%x = pi*pj*corners(1)%x &
! Subprogram not used          + qi*pj*corners(2)%x &
! Subprogram not used          + qi*qj*corners(3)%x &
! Subprogram not used          + pi*qj*corners(4)%x 
! Subprogram not used     cart%y = pi*pj*corners(1)%y &
! Subprogram not used          + qi*pj*corners(2)%y &
! Subprogram not used          + qi*qj*corners(3)%y &
! Subprogram not used          + pi*qj*corners(4)%y 
! Subprogram not used     ! map from [pi/2,pi/2] equ angular cube face to sphere:   
! Subprogram not used     sphere=projectpoint(cart,face_no)
! Subprogram not used 
! Subprogram not used   end function ref2sphere_equiangular_longdouble
...
! -----------------------------------------------------------------------------------------
! Subprogram not used   function ref2sphere_elementlocal_double(a,b, elem) result(sphere)
! Subprogram not used     use element_mod, only : element_t
! Subprogram not used     implicit none
! Subprogram not used     real(kind=real_kind)    :: a,b
! Subprogram not used     type (element_t) :: elem
! Subprogram not used     type (spherical_polar_t)      :: sphere
! Subprogram not used     real(kind=real_kind)               ::  q(4) ! local
! Subprogram not used 
! Subprogram not used     q(1)=(1-a)*(1-b); q(2)=(1+a)*(1-b); q(3)=(1+a)*(1+b); q(4)=(1-a)*(1+b);
! Subprogram not used     q=q/4.0d0;
! Subprogram not used     sphere=ref2sphere_elementlocal_q(q,elem%corners3D)
! Subprogram not used   end function 
! Subprogram not used   function ref2sphere_elementlocal_longdouble(a,b, elem) result(sphere)
! Subprogram not used     use element_mod, only : element_t
! Subprogram not used     implicit none
! Subprogram not used     real(kind=longdouble_kind)    :: a,b
! Subprogram not used     type (element_t) :: elem
...
! Subprogram not used   function ref2sphere_elementlocal_q(q, corners) result(sphere)
! Subprogram not used     implicit none
! Subprogram not used     real(kind=real_kind)          :: q(4)
! Subprogram not used     type (spherical_polar_t)      :: sphere
! Subprogram not used     type (cartesian3d_t)          :: corners(4)
! Subprogram not used     ! local
! Subprogram not used     type (cartesian3d_t)                 :: cart   
! Subprogram not used     real(kind=real_kind)               ::  c(3,4),  xx(3), r
! Subprogram not used     integer :: i
! Subprogram not used 
! Subprogram not used !3D corners fo the quad
! Subprogram not used     c(1,1)=corners(1)%x;  c(2,1)=corners(1)%y;  c(3,1)=corners(1)%z; 
! Subprogram not used     c(1,2)=corners(2)%x;  c(2,2)=corners(2)%y;  c(3,2)=corners(2)%z; 
! Subprogram not used     c(1,3)=corners(3)%x;  c(2,3)=corners(3)%y;  c(3,3)=corners(3)%z; 
! Subprogram not used     c(1,4)=corners(4)%x;  c(2,4)=corners(4)%y;  c(3,4)=corners(4)%z; 
! Subprogram not used 
! Subprogram not used !physical point on a plane (sliced), not yet on the sphere
! Subprogram not used     do i=1,3
! Subprogram not used       xx(i)=sum(c(i,:)*q(:))
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used !distance from the plane point to the origin
! Subprogram not used     r=sqrt(xx(1)**2+xx(2)**2+xx(3)**2)
! Subprogram not used 
! Subprogram not used !projecting the plane point to the sphere
! Subprogram not used     cart%x=xx(1)/r; cart%y=xx(2)/r; cart%z=xx(3)/r;
! Subprogram not used 
! Subprogram not used !XYZ coords of the point to lon/lat
! Subprogram not used     sphere=change_coordinates(cart)
! Subprogram not used 
! Subprogram not used   end function 
