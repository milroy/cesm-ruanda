...
! Subprogram not used  subroutine vecinit_(SMatP)
! Subprogram not used !
! Subprogram not used ! !USES:
! Subprogram not used !
! Subprogram not used       use m_die
! Subprogram not used       use m_SparseMatrix, only : SparseMatrix_vecinit => vecinit
! Subprogram not used 
! Subprogram not used       implicit none
! Subprogram not used 
! Subprogram not used ! !INPUT/OUTPUT PARAMETERS:
! Subprogram not used 
! Subprogram not used       type(SparseMatrixPlus), intent(inout)  :: SMatP
! Subprogram not used 
! Subprogram not used ! !REVISION HISTORY:
! Subprogram not used ! 29Oct03 - R. Jacob <jacob@mcs.anl.gov> - initial prototype
...
! Subprogram not used 
! Subprogram not used   call SparseMatrix_vecinit(SMatP%Matrix)
! Subprogram not used 
...
! Subprogram not used  subroutine clean_(SMatP, status)
! Subprogram not used 
! Subprogram not used ! !USES:
! Subprogram not used 
! Subprogram not used       use m_die
...
! Subprogram not used       use m_SparseMatrix, only : SparseMatrix_clean => clean
! Subprogram not used 
! Subprogram not used       use m_Rearranger, only : Rearranger
! Subprogram not used       use m_Rearranger, only : Rearranger_clean => clean
! Subprogram not used 
! Subprogram not used       implicit none
! Subprogram not used 
! Subprogram not used ! !INPUT/OUTPUT PARAMETERS:
...
! Subprogram not used 
! Subprogram not used ! !REVISION HISTORY:
! Subprogram not used ! 30Aug02 - Jay Larson <larson@mcs.anl.gov> - API Specification
! Subprogram not used !EOP ___________________________________________________________________
! Subprogram not used !
! Subprogram not used   character(len=*),parameter :: myname_=myname//'::clean_'
! Subprogram not used 
! Subprogram not used   integer :: myStatus
! Subprogram not used   type(String) :: dummyStrategy ! SGI IR->WHIRL work-around
! Subprogram not used   character(len=5) :: myStrategy
! Subprogram not used 
! Subprogram not used        ! If status was supplied, set it to zero (success)
! Subprogram not used 
! Subprogram not used   if(present(status)) status = 0
! Subprogram not used 
! Subprogram not used        ! The following string copy is superfluous. It is placed here
! Subprogram not used        ! to outwit a compiler bug in the SGI and SunOS compilers. 
! Subprogram not used        ! It occurs when a component of a derived type is used as an
! Subprogram not used        ! argument to String_ToChar. This bug crashes the compiler 
! Subprogram not used        ! with the error message:
! Subprogram not used        ! Error: Signal Segmentation fault in phase IR->WHIRL Conversion
! Subprogram not used 
! Subprogram not used   call String_init(dummyStrategy, SMatP%Strategy)
! Subprogram not used   myStrategy = String_ToChar(dummyStrategy)
! Subprogram not used 
! Subprogram not used        ! Use SMatP%Strategy to determine which Rearranger(s) need
! Subprogram not used        ! to be destroyed.  The CHARACTER parameters Xonly, Yonly, 
! Subprogram not used        ! and XandY are inherited from the declaration section of 
! Subprogram not used        ! this module.
! Subprogram not used 
! Subprogram not used 
! Subprogram not used   select case(myStrategy)
! Subprogram not used   case(Xonly) ! destroy X-rearranger only
! Subprogram not used 
! Subprogram not used      call Rearranger_clean(SMatP%XToXprime, myStatus)
! Subprogram not used      if(myStatus /= 0) then ! something went wrong
! Subprogram not used 	if(present(status)) then
! Subprogram not used 	   status = myStatus
! Subprogram not used 	   return
! Subprogram not used 	else
! Subprogram not used 	   write(stderr,'(3a,i8)') myname_, &
! Subprogram not used 	     ':: ERROR - call to Rearranger_clean(SMatP%XToXprime) failed.', &
! Subprogram not used 	     ' stat = ',myStatus
! Subprogram not used 	endif
! Subprogram not used      endif
! Subprogram not used 
! Subprogram not used   case(Yonly) ! destroy Y-rearranger only
! Subprogram not used 
! Subprogram not used      call Rearranger_clean(SMatP%YprimeToY, myStatus)
! Subprogram not used      if(myStatus /= 0) then ! something went wrong
! Subprogram not used 	if(present(status)) then
! Subprogram not used 	   status = myStatus
! Subprogram not used 	   return
! Subprogram not used 	else
! Subprogram not used 	   write(stderr,'(3a,i8)') myname_, &
! Subprogram not used 	     ':: ERROR - call to Rearranger_clean(SMatP%YPrimeToY) failed.', &
! Subprogram not used 	     ' stat = ',myStatus
! Subprogram not used 	endif
! Subprogram not used      endif
! Subprogram not used 
! Subprogram not used   case(XandY) ! destroy both X- and Y-rearrangers
! Subprogram not used 
! Subprogram not used      call Rearranger_clean(SMatP%XToXprime, myStatus)
! Subprogram not used      if(myStatus /= 0) then ! something went wrong
! Subprogram not used 	if(present(status)) then
...
! Subprogram not used      endif
! Subprogram not used 
! Subprogram not used   case default ! do nothing--corresponds to purely data local case
! Subprogram not used   end select
! Subprogram not used 
! Subprogram not used        ! Zero out XPrimeLength and YPrimeLength
! Subprogram not used 
! Subprogram not used   SMatP%XPrimeLength = 0
! Subprogram not used   SMatP%YPrimeLength = 0
! Subprogram not used 
! Subprogram not used        ! Destroy the SparseMatrix component SMatP%Matrix
! Subprogram not used 
! Subprogram not used   call SparseMatrix_clean(SMatP%Matrix, myStatus)
! Subprogram not used   if(myStatus /= 0) then ! something went wrong
! Subprogram not used      if(present(status)) then
! Subprogram not used 	status = myStatus
! Subprogram not used 	return
...
! Subprogram not used 	  ':: ERROR - call to SparseMatrix_clean() failed with stat=',myStatus
! Subprogram not used      endif
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used        ! Destroy the String SMatP%Strategy and its copy
! Subprogram not used 
! Subprogram not used   call String_clean(SMatP%Strategy)
! Subprogram not used   call String_clean(dummyStrategy)
! Subprogram not used 
...
! Subprogram not used  logical function initialized_(sMatPlus)
! Subprogram not used !
! Subprogram not used ! !USES:
! Subprogram not used !
...
! Subprogram not used       use m_List,   only : List_identical => identical
...
! Subprogram not used !
! Subprogram not used       type(SparseMatrixPlus), intent(in)  :: sMatPlus
! Subprogram not used 
! Subprogram not used ! !REVISION HISTORY:
! Subprogram not used ! 26Sep02 - Jay Larson <larson@mcs.anl.gov> - Implementation
! Subprogram not used !EOP ___________________________________________________________________
! Subprogram not used !
! Subprogram not used  character(len=*),parameter :: myname_=myname//'::initialized_'
! Subprogram not used 
! Subprogram not used  integer :: XonlyLen, YonlyLen, XandYLen
! Subprogram not used  type(List) :: XonlyList, YonlyList, XandYList, stratList
! Subprogram not used 
! Subprogram not used   initialized_ = .FALSE.
! Subprogram not used   
! Subprogram not used   XonlyLen = len(trim(Xonly))
! Subprogram not used   YonlyLen = len(trim(Yonly))
! Subprogram not used   XandYLen = len(trim(XandY))
! Subprogram not used   
! Subprogram not used   if( (XonlyLen /= YonlyLen) .or. (XonlyLen /= XandYLen) ) then
! Subprogram not used      call die(myname_,"The length of the strategies are unequal. &
! Subprogram not used           &This routine needs to be rewritten.")
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used   if(associated(sMatPlus%strategy%c)) then
! Subprogram not used      if(String_len(sMatPlus%strategy) == XonlyLen) then
! Subprogram not used         call List_init(XonlyList,Xonly)
! Subprogram not used         call List_init(YonlyList,Yonly)
! Subprogram not used         call List_init(XandYList,XandY)
! Subprogram not used         call List_init(stratList,sMatPlus%strategy)
! Subprogram not used         if(List_identical(stratList,XonlyList)) initialized_ = .TRUE.
! Subprogram not used         if(List_identical(stratList,YonlyList)) initialized_ = .TRUE.
! Subprogram not used         if(List_identical(stratList,XandYList)) initialized_ = .TRUE.
! Subprogram not used         call List_clean(XonlyList)
! Subprogram not used         call List_clean(YonlyList)
! Subprogram not used         call List_clean(XandYList)
! Subprogram not used         call List_clean(stratList)
! Subprogram not used      endif
! Subprogram not used   endif
! Subprogram not used 
...
! Subprogram not used  function exportStrategyToChar_(sMatPlus)
! Subprogram not used !
! Subprogram not used ! !USES:
! Subprogram not used !
! Subprogram not used    use m_stdio
...
! Subprogram not used ! !OUTPUT PARAMETERS: 
! Subprogram not used !
! Subprogram not used       character(len=size(sMatPlus%Strategy%c)) :: exportStrategyToChar_
! Subprogram not used 
! Subprogram not used ! !REVISION HISTORY:
! Subprogram not used ! 01Aug07 - Jay Larson <larson@mcs.anl.gov> - Implementation
! Subprogram not used !EOP ___________________________________________________________________
! Subprogram not used !
! Subprogram not used  character(len=*),parameter :: myname_=myname//'::exportStrategyToChar_'
! Subprogram not used  type(String) :: dummyStrategy ! SGI IR->WHIRL work-around
! Subprogram not used 
! Subprogram not used    ! Check input argument to ensure it has been initialized.  If not,
! Subprogram not used    ! signal an error and terminate execution.
! Subprogram not used 
! Subprogram not used   if( .not. initialized_(sMatPlus) ) then
! Subprogram not used      write(stderr,'(3a)') myname_,':: Warning, input argument not initialized, ', &
! Subprogram not used           'returning empty character field for parallelization strategy.'
! Subprogram not used      exportStrategyToChar_ = ' '
! Subprogram not used      return
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used    ! Return in character form the parallelizaiton strategy
! Subprogram not used   call String_init(dummyStrategy, SMatPlus%Strategy)
! Subprogram not used 
! Subprogram not used   exportStrategyToChar_ = String_ToChar(dummyStrategy) 
! Subprogram not used 
! Subprogram not used   call String_clean(dummyStrategy)
! Subprogram not used 
! Subprogram not used  end function exportStrategyToChar_
