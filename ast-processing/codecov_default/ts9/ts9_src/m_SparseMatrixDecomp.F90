...
! Subprogram not used  subroutine ByColumnGSMap_(xGSMap, sMat, sMGSMap, root, comm)
! Subprogram not used !
...
! Subprogram not used    use m_AttrVect, only: AttrVect_copy => copy
! Subprogram not used    use m_AttrVect, only: AttrVect_clean => clean
! Subprogram not used    
...
! Subprogram not used    use m_GlobalMap, only : GlobalMap_init => init
! Subprogram not used    use m_GlobalMap, only : GlobalMap_clean => clean
! Subprogram not used 
...
! Subprogram not used    use m_GlobalSegMap, only: GlobalSegMap_peLocs => peLocs
...
! Subprogram not used    use m_SparseMatrix, only: SparseMatrix_lsize => lsize
...
! Subprogram not used !
! Subprogram not used    type(GlobalSegMap), intent(in)    :: xGSMap
...
! Subprogram not used !
! Subprogram not used    type(GlobalSegMap), intent(out) :: sMGSMap
! Subprogram not used 
! Subprogram not used ! !DESCRIPTION: This routine is invoked from all processes on the 
! Subprogram not used ! communicator {\tt comm} to create from an input {\tt SparseMatrix}
! Subprogram not used ! {\tt sMat} (valid only on the {\tt root} process) and an input 
! Subprogram not used ! {\bf x}-vector decomposition described by the {\tt GlobalSegMap} 
! Subprogram not used ! argument {\tt xGSMap} (valid at least on the {\tt root}) to create 
! Subprogram not used ! an output {\tt GlobalSegMap} decomposition of the matrix elements 
! Subprogram not used ! {\tt sMGSMap}, which is valid on all processes on the communicator.
! Subprogram not used ! This matrix {\tt GlobalSegMap} describes the corresponding column 
! Subprogram not used ! decomposition of {\tt sMat}.
! Subprogram not used !
! Subprogram not used ! {\bf N.B.}:  The argument {\tt sMat} is returned sorted in lexicographic 
! Subprogram not used ! order by column and row.
! Subprogram not used !
! Subprogram not used ! !REVISION HISTORY: 
! Subprogram not used !
! Subprogram not used ! 13Apr01 - J.W. Larson <larson@mcs.anl.gov> - initial API spec.
! Subprogram not used ! 26Apr01 - R.L. Jacob <jacob@mcs.anl.gov> - add use statements for
! Subprogram not used !           GlobalSegMap_init and GSMap_peLocs.
! Subprogram not used !           Add gsize argument required to GSMap_peLocs.
! Subprogram not used !           Add underscore to ComputeSegments call so it matches
! Subprogram not used !           the subroutine decleration.
! Subprogram not used !           change attribute on starts,lengths, and pe_locs to
! Subprogram not used !           pointer to match GSMap_init.
! Subprogram not used !           add use m_die statement
! Subprogram not used ! 26Apr01 - J.W. Larson <larson@mcs.anl.gov> - fixed major logic bug
! Subprogram not used !           that had all processes executing some operations that 
! Subprogram not used !           should only occur on the root.
! Subprogram not used ! 09Jul03 - E.T. Ong <eong@mcs.anl.gov> - call pe_locs in parallel. 
! Subprogram not used !           reduce the serial sort from gcol:grow to just gcol.
! Subprogram not used !EOP
! Subprogram not used !-------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used   character(len=*),parameter :: myname_=myname//'ByColumnGSMap_'
! Subprogram not used ! Process ID number
! Subprogram not used   integer :: myID, mySIZE
! Subprogram not used ! Attributes for the output GlobalSegMap
! Subprogram not used   integer :: gsize, comp_id, ngseg
! Subprogram not used ! Temporary array for identifying each matrix element column and 
! Subprogram not used ! process ID destination
! Subprogram not used   type(AttrVect) :: gcol
! Subprogram not used   type(AttrVect) :: dist_gcol
! Subprogram not used   type(AttrVect) :: element_pe_locs 
! Subprogram not used   type(AttrVect) :: dist_element_pe_locs
! Subprogram not used ! Index variables for the AttrVects
! Subprogram not used   integer :: dist_gsize
! Subprogram not used   integer :: gcol_index
! Subprogram not used   integer :: element_pe_locs_index
! Subprogram not used ! Temporary array for initializing GlobalMap Decomposition
! Subprogram not used   integer,dimension(:), allocatable :: counts
! Subprogram not used ! GlobalMap for setting up decomposition to call pe_locs
! Subprogram not used   type(GlobalMap) :: dist_GMap
! Subprogram not used ! Temporary arrays for matrix GlobalSegMap attributes
! Subprogram not used   integer, dimension(:), pointer :: starts, lengths, pe_locs
...
! Subprogram not used ! Loop index
! Subprogram not used   integer :: i
! Subprogram not used 
! Subprogram not used        ! Determine process id number myID
! Subprogram not used 
...
! Subprogram not used 
! Subprogram not used       ! Determine the number of processors in communicator
! Subprogram not used 
! Subprogram not used   call MPI_COMM_SIZE(comm, mySIZE, ierr)
! Subprogram not used   if(ierr /= 0) then
! Subprogram not used      call MP_perr_die(myname_,'call MPI_COMM_SIZE(...',ierr)
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used        ! Allocate space for GlobalMap length information
! Subprogram not used 
! Subprogram not used   allocate(counts(0:mySIZE-1),stat=ierr)
! Subprogram not used   if(ierr/=0) call die(myname_,"allocate(counts)",ierr)
! Subprogram not used 
! Subprogram not used        ! First step:  a lot of prep work on the root only:
! Subprogram not used 
! Subprogram not used   if(myID == root) then
! Subprogram not used 
! Subprogram not used        ! Sort the matrix entries in sMat by column.  
! Subprogram not used        ! First, create the key list...
! Subprogram not used 
! Subprogram not used      call List_init(sort_keys,'gcol')
! Subprogram not used 
! Subprogram not used        ! Now perform the sort/permute...
! Subprogram not used 
! Subprogram not used      call SparseMatrix_SortPermute(sMat, sort_keys)
! Subprogram not used 
! Subprogram not used      call List_clean(sort_keys)
! Subprogram not used 
! Subprogram not used        ! The global size of matrix GlobalSegMap is the number nonzero
! Subprogram not used        ! elements in sMat.  
! Subprogram not used 
! Subprogram not used      gsize = SparseMatrix_lsize(sMat)
! Subprogram not used 
! Subprogram not used        ! Allocate storage space for matrix element column indices and
! Subprogram not used        ! process ID destinations
! Subprogram not used 
! Subprogram not used      call AttrVect_init(aV=gcol, iList="gcol", lsize=gsize)
! Subprogram not used 
! Subprogram not used        ! Extract global column information and place in array gCol
! Subprogram not used 
! Subprogram not used      call AttrVect_copy(aVin=sMat%data, aVout=gcol, iList="gcol")
! Subprogram not used 
! Subprogram not used        ! Setup GlobalMap decomposition lengths:
! Subprogram not used 
! Subprogram not used      do i=0,mySIZE-1
! Subprogram not used         counts(i) = gsize/mySIZE
! Subprogram not used      enddo
! Subprogram not used      counts(mySIZE-1) = counts(mySIZE-1) + mod(gsize,mySIZE)
! Subprogram not used 
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used        ! Initialize GlobalMap so that we can scatter the global row
! Subprogram not used        ! information. The GlobalMap will inherit the component ID
! Subprogram not used        ! from xGSMap
! Subprogram not used 
! Subprogram not used   comp_id = GlobalSegMap_comp_id(xGSMap)
! Subprogram not used 
! Subprogram not used   call GlobalMap_init(GMap=dist_GMap, comp_id=comp_id, lns=counts, &
! Subprogram not used                       root=root, comm=comm)
! Subprogram not used 
! Subprogram not used   call AttrVect_scatter(iV=gcol, oV=dist_gcol, GMap=dist_GMap, &
! Subprogram not used                         root=root, comm=comm)
! Subprogram not used 
! Subprogram not used        ! Similarly, we want to scatter the element_pe_locs using the 
! Subprogram not used        ! same decomposition
! Subprogram not used   
! Subprogram not used   dist_gsize = AttrVect_lsize(dist_gcol)
! Subprogram not used 
! Subprogram not used   call AttrVect_init(aV=dist_element_pe_locs, iList="element_pe_locs", &
! Subprogram not used                      lsize=dist_gsize)
! Subprogram not used   call AttrVect_zero(dist_element_pe_locs)
! Subprogram not used      
! Subprogram not used        ! Compute process ID destination for each matrix element,
! Subprogram not used        ! and store in the AttrVect element_pe_locs
! Subprogram not used 
! Subprogram not used   gcol_index = AttrVect_indexIA(dist_gcol,"gcol", dieWith=myname_)
! Subprogram not used   element_pe_locs_index = AttrVect_indexIA(dist_element_pe_locs, &
! Subprogram not used                             "element_pe_locs", dieWith=myname_)
! Subprogram not used 
! Subprogram not used   call GlobalSegMap_peLocs(xGSMap, dist_gsize,      &
! Subprogram not used           dist_gcol%iAttr(gcol_index,1:dist_gsize), &
! Subprogram not used           dist_element_pe_locs%iAttr(element_pe_locs_index,1:dist_gsize))
! Subprogram not used 
! Subprogram not used   call AttrVect_gather(iV=dist_element_pe_locs, oV=element_pe_locs, &
! Subprogram not used                        GMap=dist_GMap, root=root, comm=comm)
! Subprogram not used 
! Subprogram not used        ! Back to the root operations
! Subprogram not used 
! Subprogram not used   if(myID == root) then
! Subprogram not used 
! Subprogram not used        ! Sanity check: Is the globalsize of sMat the same as the 
! Subprogram not used        ! gathered size of element_pe_locs?
! Subprogram not used                   
! Subprogram not used      if(gsize /= AttrVect_lsize(element_pe_locs)) then
! Subprogram not used         call die(myname_,"gsize /= AttrVect_lsize(element_pe_locs) &
! Subprogram not used                  & on root process")
! Subprogram not used      endif
! Subprogram not used        
! Subprogram not used        ! Using the entries of gCol and element_pe_locs, build the
! Subprogram not used        ! output GlobalSegMap attribute arrays starts(:), lengths(:),
! Subprogram not used        ! and pe_locs(:)
! Subprogram not used 
! Subprogram not used      gcol_index = AttrVect_indexIA(gcol,"gcol", dieWith=myname_)
! Subprogram not used      element_pe_locs_index = AttrVect_indexIA(element_pe_locs, &
! Subprogram not used                                 "element_pe_locs", dieWith=myname_)
! Subprogram not used 
! Subprogram not used      call ComputeSegments_(element_pe_locs%iAttr(element_pe_locs_index, &
! Subprogram not used                                                               1:gsize), &
! Subprogram not used                            gcol%iAttr(gcol_index,1:gsize), &
! Subprogram not used                            gsize, ngseg, starts, lengths, pe_locs)
! Subprogram not used        ! Clean up on the root
! Subprogram not used 
! Subprogram not used      call AttrVect_clean(gcol)
! Subprogram not used      call AttrVect_clean(element_pe_locs)
! Subprogram not used 
! Subprogram not used   endif ! if(myID == root)
! Subprogram not used 
! Subprogram not used        ! Non-root processes call GlobalSegMap_init with root_start, 
! Subprogram not used        ! root_length, and root_pe_loc, although these arguments are 
! Subprogram not used        ! not used in the subroutine. Since these correspond to dummy 
! Subprogram not used        ! shaped array arguments in initr_, the Fortran 90 standard 
! Subprogram not used        ! dictates that the actual arguments must contain complete shape 
! Subprogram not used        ! information. Therefore, these array arguments must be 
! Subprogram not used        ! allocated on all processes.
! Subprogram not used 
! Subprogram not used   if(myID /= root) then
! Subprogram not used      allocate(starts(0),lengths(0),pe_locs(0),stat=ierr)
! Subprogram not used      if(ierr /= 0) then
! Subprogram not used         call die(myname_,'non-root allocate(starts...',ierr)
! Subprogram not used      endif
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used        ! Using this local data on the root, create the SparseMatrix 
! Subprogram not used        ! GlobalSegMap sMGSMap (which will be valid on all processes
! Subprogram not used        ! on the communicator:
! Subprogram not used 
! Subprogram not used   call GlobalSegMap_init(sMGSMap, ngseg, starts, lengths, pe_locs, &
! Subprogram not used                          root, comm, comp_id, gsize)
! Subprogram not used 
! Subprogram not used        ! Clean up 
! Subprogram not used 
! Subprogram not used   call GlobalMap_clean(dist_GMap)
! Subprogram not used   call AttrVect_clean(dist_gcol)
! Subprogram not used   call AttrVect_clean(dist_element_pe_locs)
! Subprogram not used 
! Subprogram not used   deallocate(starts, lengths, pe_locs, counts, stat=ierr)
! Subprogram not used   if(ierr /= 0) then
! Subprogram not used      call die(myname_,'deallocate(starts...',ierr)
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used 
! Subprogram not used  end subroutine ByColumnGSMap_
