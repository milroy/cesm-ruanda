...
!=======================================================================================================! 
  ...
  subroutine prim_printstate(elem, tl,hybrid,hvcoord,nets,nete, fvm)
    ...
    !   BUT: 1 EUL defines mass as integral( ps ), so to be consistent, ignore ptop contribution; 
    Mass = Mass2*scale
...
! Subprogram not used subroutine prim_energy_halftimes(elem,hvcoord,tl,n,t_before_advance,nets,nete)
! Subprogram not used ! 
! Subprogram not used !  called at the end of a timestep, before timelevel update.  Solution known at
! Subprogram not used !  dynamics:     nm1,  n0,  np1.  
! Subprogram not used !
! Subprogram not used !                                                           LF code            RK code
! Subprogram not used !  This routine is called 4 times:  n=1:    t1=nm1, t2=n0   start              after forcing, before timestep
! Subprogram not used !                                   n=2:    t1=n0, t2=np1   after timestep     after timestep, including remap
! Subprogram not used !                                   n=3:    t1=n0, t2=np1   after forcing      after fixer 
! Subprogram not used !                                   n=4:    t1=n0, t2=np1   after Robert       before forcing
! Subprogram not used !
! Subprogram not used ! LF case we use staggered formulas:
! Subprogram not used !  compute the energies at time half way between timelevels t1 and t2,
! Subprogram not used !  using timelevels t1,t2.  store in location n of our elem()%accum%* variables
! Subprogram not used !    
! Subprogram not used !  in the Dry case, PE (and similar for IE) will be conserved exactly by Leapfrog, 
! Subprogram not used !  if it is defined as:
! Subprogram not used !
! Subprogram not used !   PE(n+.5)  = .5*(  T(n+1) dp(n) + T(n) dp(n+1) )
! Subprogram not used !
! Subprogram not used !  KE cannot be exactly conserved with Leapfrog.  but we still use:
! Subprogram not used !  
! Subprogram not used !   KE(n+.5) = .5*(  .5 u(n+1)^2 dp(n) +  .5 u(n)^2 dp(n+1) )
! Subprogram not used !
! Subprogram not used !
! Subprogram not used     use kinds, only : real_kind
! Subprogram not used     use dimensions_mod, only : np, np, nlev
! Subprogram not used     use control_mod, only : use_cpstar
! Subprogram not used     use hybvcoord_mod, only : hvcoord_t
! Subprogram not used     use element_mod, only : element_t
! Subprogram not used     use physical_constants, only : Cp, cpwater_vapor
! Subprogram not used     use physics_mod, only : Virtual_Specific_Heat, Virtual_Temperature
! Subprogram not used     use prim_si_mod, only : preq_hydrostatic
! Subprogram not used 
! Subprogram not used     integer :: t1,t2,n,nets,nete
...
! Subprogram not used     logical :: t_before_advance
! Subprogram not used 
! Subprogram not used     integer :: ie,k,i,j,nm_f
! Subprogram not used     real (kind=real_kind), dimension(np,np,nlev)  :: dpt1,dpt2   ! delta pressure
! Subprogram not used     real (kind=real_kind), dimension(np,np)  :: E
! Subprogram not used     real (kind=real_kind), dimension(np,np)  :: suml,suml2,v1,v2
! Subprogram not used     real (kind=real_kind), dimension(np,np,nlev)  :: sumlk, suml2k
! Subprogram not used     real (kind=real_kind), dimension(np,np,nlev)  :: p,T_v,phi
! Subprogram not used     real (kind=real_kind) :: cp_star1,cp_star2,qval_t1,qval_t2
! Subprogram not used     real (kind=real_kind) :: Qt
! Subprogram not used     logical :: wet
! Subprogram not used 
! Subprogram not used 
! Subprogram not used     logical tstagger
! Subprogram not used     integer:: t2_qdp, t1_qdp   ! the time pointers for Qdp are not the same
! Subprogram not used 
! Subprogram not used     nm_f = 1
! Subprogram not used     if (t_before_advance) then
! Subprogram not used        t1=tl%nm1
! Subprogram not used        t2=tl%n0
! Subprogram not used        call TimeLevel_Qdp( tl, qsplit, t2_qdp, t1_qdp) !get n0 level into t2_qdp 
! Subprogram not used     else
! Subprogram not used        t1=tl%n0
! Subprogram not used        t2=tl%np1
! Subprogram not used        call TimeLevel_Qdp(tl, qsplit, t1_qdp, t2_qdp) !get np1 into t2_qdp
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used 
! Subprogram not used 
! Subprogram not used ! energy_fixer
! Subprogram not used !     <0         disabled, but compute energy non-staggered in time
! Subprogram not used !     0          disabled, but compute energy staggered in time (for use with leapfrog code)
! Subprogram not used !    >0          Enabled.  energy fixer requires dry formulation (use_cpstar=0) and non-staggered in time
! Subprogram not used !
! Subprogram not used     tstagger = .false.
! Subprogram not used     if (energy_fixer==0) tstagger = .true.
! Subprogram not used 
! Subprogram not used 
! Subprogram not used     !   IE   Cp*dpdn*T  + (Cpv-Cp) Qdpdn*T
! Subprogram not used     !        Cp*dpdn(n)*T(n+1) + (Cpv-Cp) Qdpdn(n)*T(n+1)
! Subprogram not used     !        [Cp + (Cpv-Cp) Q(n)] *dpdn(n)*T(n+1) 
! Subprogram not used     do ie=nets,nete
! Subprogram not used 
! Subprogram not used        do k=1,nlev
! Subprogram not used           dpt1(:,:,k) = ( hvcoord%hyai(k+1) - hvcoord%hyai(k) )*hvcoord%ps0 + &
! Subprogram not used                ( hvcoord%hybi(k+1) - hvcoord%hybi(k) )*elem(ie)%state%ps_v(:,:,t1)
! Subprogram not used           dpt2(:,:,k) = ( hvcoord%hyai(k+1) - hvcoord%hyai(k) )*hvcoord%ps0 + &
! Subprogram not used                ( hvcoord%hybi(k+1) - hvcoord%hybi(k) )*elem(ie)%state%ps_v(:,:,t2)
! Subprogram not used        enddo
! Subprogram not used 
! Subprogram not used        do k=1,nlev
! Subprogram not used           sumlk(:,:,k)=0
! Subprogram not used           suml2k(:,:,k)=0
! Subprogram not used           do i=1,np
! Subprogram not used           do j=1,np
! Subprogram not used              if(use_cpstar == 1)  then
! Subprogram not used                 ! Cp_star = cp + (Cpwater_vapor - cp)*qval
! Subprogram not used                 qval_t1 = elem(ie)%state%Qdp(i,j,k,1,t1_qdp)/dpt1(i,j,k)
! Subprogram not used                 qval_t2 = elem(ie)%state%Qdp(i,j,k,1,t2_qdp)/dpt2(i,j,k)
! Subprogram not used                 cp_star1= Virtual_Specific_Heat(qval_t1)
! Subprogram not used                 cp_star2= Virtual_Specific_Heat(qval_t2)
! Subprogram not used              else
! Subprogram not used                 cp_star1=cp
! Subprogram not used                 cp_star2=cp
! Subprogram not used              endif
! Subprogram not used              if (tstagger) then
! Subprogram not used                 sumlk(i,j,k) = sumlk(i,j,k) + cp_star1*elem(ie)%state%T(i,j,k,t2) *dpt1(i,j,k)/2 
! Subprogram not used                 sumlk(i,j,k) = sumlk(i,j,k) + Cp_star2*elem(ie)%state%T(i,j,k,t1) *dpt2(i,j,k)/2
! Subprogram not used                 suml2k(i,j,k) = suml2k(i,j,k) + (cp_star1-cp)*elem(ie)%state%T(i,j,k,t2) *dpt1(i,j,k)/2 
! Subprogram not used                 suml2k(i,j,k) = suml2k(i,j,k) + (cp_star2-cp)*elem(ie)%state%T(i,j,k,t1) *dpt2(i,j,k)/2
! Subprogram not used              else
! Subprogram not used                 sumlk(i,j,k) = sumlk(i,j,k) + Cp_star2*elem(ie)%state%T(i,j,k,t2) *dpt2(i,j,k)
! Subprogram not used                 suml2k(i,j,k) = suml2k(i,j,k) + (cp_star2-cp)*elem(ie)%state%T(i,j,k,t2) *dpt2(i,j,k)
! Subprogram not used              endif
! Subprogram not used           enddo
! Subprogram not used           enddo
! Subprogram not used        enddo
! Subprogram not used        suml=0
! Subprogram not used        suml2=0
! Subprogram not used        do k=1,nlev
! Subprogram not used           suml(:,:) = suml(:,:) + sumlk(:,:,k)
! Subprogram not used           suml2(:,:) = suml2(:,:) + suml2k(:,:,k)
! Subprogram not used        enddo
! Subprogram not used        elem(ie)%accum%IEner(:,:,n)=suml(:,:)
! Subprogram not used        elem(ie)%accum%IEner_wet(:,:,n)=suml2(:,:)
! Subprogram not used 
! Subprogram not used     
! Subprogram not used     !   KE   .5 dp/dn U^2
! Subprogram not used        do k=1,nlev
! Subprogram not used           if (tstagger) then
! Subprogram not used              E = (elem(ie)%state%v(:,:,1,k,t2)**2 +  &
! Subprogram not used                   elem(ie)%state%v(:,:,2,k,t2)**2 ) / 2 
! Subprogram not used              sumlk(:,:,k) = E*dpt1(:,:,k)/2
! Subprogram not used              E = (elem(ie)%state%v(:,:,1,k,t1)**2 +  &
! Subprogram not used                   elem(ie)%state%v(:,:,2,k,t1)**2 ) / 2 
! Subprogram not used              sumlk(:,:,k) = sumlk(:,:,k) + E*dpt2(:,:,k)/2
! Subprogram not used           else
! Subprogram not used              E = (elem(ie)%state%v(:,:,1,k,t2)**2 +  &
! Subprogram not used                   elem(ie)%state%v(:,:,2,k,t2)**2 ) / 2 
! Subprogram not used              sumlk(:,:,k) = E*dpt2(:,:,k)
! Subprogram not used           endif
! Subprogram not used        enddo
! Subprogram not used        suml=0
! Subprogram not used        do k=1,nlev
! Subprogram not used           suml(:,:) = suml(:,:) + sumlk(:,:,k)
! Subprogram not used        enddo
! Subprogram not used        elem(ie)%accum%KEner(:,:,n)=suml(:,:)
! Subprogram not used 
! Subprogram not used 
! Subprogram not used     
! Subprogram not used     !   PE   dp/dn PHIs
! Subprogram not used        suml=0
! Subprogram not used        do k=1,nlev
! Subprogram not used           if (tstagger) then
! Subprogram not used              suml = suml + elem(ie)%state%phis(:,:)*dpt1(:,:,k)/2
! Subprogram not used              suml = suml + elem(ie)%state%phis(:,:)*dpt2(:,:,k)/2
! Subprogram not used           else
! Subprogram not used              suml = suml + elem(ie)%state%phis(:,:)*dpt2(:,:,k)
! Subprogram not used           endif
! Subprogram not used        enddo
! Subprogram not used        elem(ie)%accum%PEner(:,:,n)=suml(:,:)
! Subprogram not used 
! Subprogram not used 
! Subprogram not used 
! Subprogram not used !      compute alternate PE term which matches what is used in 1 physics
! Subprogram not used        wet =(moisture /= "dry")
! Subprogram not used        do k=1,nlev
! Subprogram not used           p(:,:,k)   = hvcoord%hyam(k)*hvcoord%ps0 + hvcoord%hybm(k)*elem(ie)%state%ps_v(:,:,t2)
! Subprogram not used           do j=1,np
! Subprogram not used              do i=1,np
! Subprogram not used                 if (wet) then
! Subprogram not used                    Qt = elem(ie)%state%Qdp(i,j,k,1,t2_qdp)/dpt2(i,j,k)
! Subprogram not used                    T_v(i,j,k) = Virtual_Temperature(elem(ie)%state%T(i,j,k,t2),Qt)
! Subprogram not used                 else
! Subprogram not used                    T_v(i,j,k) = elem(ie)%state%T(i,j,k,t2)
! Subprogram not used                 endif
...
! Subprogram not used     enddo
! Subprogram not used     
! Subprogram not used end subroutine prim_energy_halftimes
    
!=======================================================================================================! 
  

! Subprogram not used subroutine prim_diag_scalars(elem,hvcoord,tl,n,t_before_advance,nets,nete)
! Subprogram not used ! 
! Subprogram not used !  called at the end of a timestep, before timelevel update.  Solution known at
! Subprogram not used !  timelevel nm1,n0,np1.  
! Subprogram not used !
! Subprogram not used !  This routine is called twice:  n=1:    t1=nm1, t2=n0
! Subprogram not used !                                 n=2:    t1=n0, t2=np1
! Subprogram not used ! 
! Subprogram not used !  in all cases:
! Subprogram not used !     Q1mass(n) = Qdp(t2)    (or:  Q(t2)*dp(t2) )
! Subprogram not used ! 
! Subprogram not used !  We also compute 
! Subprogram not used !     Qmass(n)  = .5*(  Q(t2) dp(t1)  Q(t1) dp(t2) )       Leapfrog, concentration formulation
! Subprogram not used !     Qmass(n)  = .5*(  Qdp(t1) + Qdp(t2) )                Leapfrog, conservation formulation
! Subprogram not used !     Qmass(n)  =  Qdp(t2)                                 RK2, conservation formulation
! Subprogram not used !
! Subprogram not used !
! Subprogram not used     use kinds, only : real_kind
! Subprogram not used     use dimensions_mod, only : np, np, nlev
...
! Subprogram not used     type (hvcoord_t)                  :: hvcoord
! Subprogram not used     
! Subprogram not used     integer :: ie,k,q,nm_f
! Subprogram not used     real (kind=real_kind), dimension(np,np)  :: ps         ! pressure
! Subprogram not used     real (kind=real_kind), dimension(np,np)  :: dp         ! delta pressure
! Subprogram not used     real (kind=real_kind), dimension(np,np)  :: E
! Subprogram not used     real (kind=real_kind), dimension(np,np)  :: suml
! Subprogram not used     type (TimeLevel_t), intent(in)       :: tl
! Subprogram not used     logical :: t_before_advance
! Subprogram not used     integer:: t2_qdp, tmp   ! the time pointer for Qdp are not the same
! Subprogram not used 
! Subprogram not used     nm_f = 1
! Subprogram not used     if (t_before_advance) then
! Subprogram not used        t1=tl%nm1     
! Subprogram not used        t2=tl%n0
! Subprogram not used        call TimeLevel_Qdp( tl, qsplit, t2_qdp) !get n0 level into t2_qdp 
! Subprogram not used     else
! Subprogram not used        t1=tl%n0
! Subprogram not used        t2=tl%np1
! Subprogram not used        call TimeLevel_Qdp(tl, qsplit, tmp, t2_qdp) !get np1 into t2_qdp (don't need tmp)
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used 
! Subprogram not used     !
! Subprogram not used     !  RK2 forward scheme.  compute everything at t2
! Subprogram not used     !  (used by 1)
! Subprogram not used     !   Q has only one time dimension
! Subprogram not used     if (tstep_type>0) then
! Subprogram not used 
! Subprogram not used        do ie=nets,nete
! Subprogram not used           do q=1,qsize
! Subprogram not used              suml=0
! Subprogram not used              do k=1,nlev
! Subprogram not used                 suml = suml + elem(ie)%state%Qdp(:,:,k,q,t2_qdp)*elem(ie)%state%Q(:,:,k,q)
! Subprogram not used              enddo
! Subprogram not used              elem(ie)%accum%Qvar(:,:,q,n)=suml(:,:)
! Subprogram not used           enddo
! Subprogram not used        enddo
! Subprogram not used        
! Subprogram not used        do ie=nets,nete
...
! Subprogram not used              suml=0
! Subprogram not used              do k=1,nlev
! Subprogram not used                 suml = suml + elem(ie)%state%Qdp(:,:,k,q,t2_qdp)
! Subprogram not used              enddo
! Subprogram not used              elem(ie)%accum%Q1mass(:,:,q)=suml(:,:)
! Subprogram not used              elem(ie)%accum%Qmass(:,:,q,n)=suml(:,:)
! Subprogram not used           enddo
! Subprogram not used        enddo
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used 
! Subprogram not used 
! Subprogram not used end subroutine prim_diag_scalars
