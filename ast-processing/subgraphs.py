...
class metagraph(object):
    ...
    def __init__(self, name=None):
        ...
        self.outMaps = {
            "dtv": "dtk__vertical_diffusion_tend",
            "lhflx": "cam_in%lhf__diag_surf",
            "qflx": "cam_in%cflx__diag_surf",
            "qrefht": "cam_in%qref__diag_surf",
            "shflx": "cam_in%shf__diag_surf",
            "snowhlnd": "cam_in%snowhland__diag_surf",
            "taugwx": "tau0x__gw_tend",
            "taugwy": "tau0y__gw_tend",
            "taux": "cam_in%wsx__diag_surf",
            "tauy": "cam_in%wsy__diag_surf",
            "trefht": "cam_in%tref__diag_surf",
            "u10": "cam_in%u10__diag_surf",
            "wgustd": "wpert__vertical_diffusion_tend",
            "ts": "cam_in%ts__diag_surf",
            "flds": "cam_out%flwds__radiation_tend",
        }

        with open(
            "/root/cesm-ruanda/ast-processing/module-file-lists/atmfiles.csv", "r"
        ) as f:
            reader = csv.reader(f, delimiter=",")
            self.atmFiles = set(list(reader)[0])

        with open(
            "/root/cesm-ruanda/ast-processing/module-file-lists/csm_sharefiles.csv", "r"
        ) as f:
            reader = csv.reader(f, delimiter=",")
            self.csm_shareFiles = set(list(reader)[0])

        with open(
            "/root/cesm-ruanda/ast-processing/module-file-lists/dead_sharefiles.csv",
            "r",
        ) as f:
            reader = csv.reader(f, delimiter=",")
        ...
        with open(
            "/root/cesm-ruanda/ast-processing/module-file-lists/drvfiles.csv", "r"
        ) as f:
            reader = csv.reader(f, delimiter=",")
            self.drvFiles = set(list(reader)[0])

        with open(
            "/root/cesm-ruanda/ast-processing/module-file-lists/glcfiles.csv", "r"
        ) as f:
            reader = csv.reader(f, delimiter=",")
            self.glcFiles = set(list(reader)[0])

        with open(
            "/root/cesm-ruanda/ast-processing/module-file-lists/icefiles.csv", "r"
        ) as f:
            reader = csv.reader(f, delimiter=",")
            self.iceFiles = set(list(reader)[0])

        with open(
            "/root/cesm-ruanda/ast-processing/module-file-lists/lndfiles.csv", "r"
        ) as f:
            reader = csv.reader(f, delimiter=",")
            self.lndFiles = set(list(reader)[0])

        with open(
            "/root/cesm-ruanda/ast-processing/module-file-lists/ocnfiles.csv", "r"
        ) as f:
            reader = csv.reader(f, delimiter=",")
            self.ocnFiles = set(list(reader)[0])

        with open(
            "/root/cesm-ruanda/ast-processing/module-file-lists/roffiles.csv", "r"
        ) as f:
            reader = csv.reader(f, delimiter=",")
            self.rofFiles = set(list(reader)[0])

        with open(
            "/root/cesm-ruanda/ast-processing/module-file-lists/utilsfiles.csv", "r"
        ) as f:
            reader = csv.reader(f, delimiter=",")
            self.utilsFiles = set(list(reader)[0])

        with open(
            "/root/cesm-ruanda/ast-processing/module-file-lists/wavfiles.csv", "r"
        ) as f:
            reader = csv.reader(f, delimiter=",")
            self.wavFiles = set(list(reader)[0])

        with open(
            "/root/cesm-ruanda/ast-processing/module-file-lists/onlycam.csv", "r"
        ) as f:
            reader = csv.reader(f, delimiter=",")
        ...
        if not self._hasnode(n1):
            ...
            if "lines" in attributes1:
                if isinstance(attributes1["lines"]["line"], OOBTree):
                    self.node[n1]["attrs"]["lines"]["line"].update(
                        attributes1["lines"]["line"]
                    )
                elif isinstance(attributes1["lines"]["line"], int):
                    self.node[n1]["attrs"]["lines"]["line"].update(
                        {attributes1["lines"]["line"]: 0}
                    )
        ...
        if not self._hasnode(n2):
            ...
            if "lines" in attributes2:
                if isinstance(attributes2["lines"]["line"], OOBTree):
                    self.node[n2]["attrs"]["lines"]["line"].update(
                        attributes2["lines"]["line"]
                    )
                elif isinstance(attributes2["lines"]["line"], int):
                    self.node[n2]["attrs"]["lines"]["line"].update(
                        {attributes2["lines"]["line"]: 0}
                    )
        ...
        for use in self.uses:
            ...
            if use[1]["renamed"]:
                ...
                for item in use[1]["renamed"]:
                    ...
                    # check if subprogram
                    if (
                        sourceName not in self.functions
                        and sourceName not in self.subroutines
                    ):
                        ...
                        if originModule in self.modules:
                            ...
                            if (
                                sourceName
                                in self.modules[originModule]["vars"]
                                | self.modules[originModule]["public"]
                                | self.modules[originModule]["provides"]
                            ):
                                smlocation = self.modules[originModule]["location"]
                                matchedSources = [
                                    (
                                        sourceName + "__" + originModule,
                                        {
                                            "location": smlocation,
                                            "cname": sourceName,
                                            "lines": {"line": OOBTree({1: 0})},
                                        },
                                    )
                                ]
                                    ...
                                    if sourceName + suffix in self.nodes:
                                        ...
                                        if subprog in self.subroutines:
                                            if "location" in self.subroutines[subprog]:
                                                location = self.subroutines[subprog][
                                                    "location"
                                                ]

                                            elif self.subroutines[subprog]["isIface"]:
                                                subiface = (
                                                    self.subroutines[subprog][
                                                        "subroutines"
                                                    ]
                                                    .itervalues()
                                                    .next()
                                            ...
                                            if "location" in self.functions[subprog]:
                                                location = self.functions[subprog][
                                                    "location"
                                                ]

                                            elif self.functions[subprog]["isIface"]:
                                                subiface = (
                                                    self.functions[subprog]["functions"]
                                                    .itervalues()
                                                    .next()
                                                )
                                                location = subiface["location"]
                                        ...
                                        if location:
                                            lines = self.node[newSourceName]["attrs"][
                                                "lines"
                                            ]
                                            sourceattrs = {
                                                "location": location,
                                ...
                                if useLocation in self.modules:
                                    ...
                                    for subprog in self.modules[useLocation][
                                        ...
                                        if targetName + suffixT in self.nodes:
                                            ...
                                            if subprog in self.subroutines:
                                                if (
                                                    "location"
                                                    in self.subroutines[subprog]
                                                ):
                                                    location = self.subroutines[
                                                        subprog
                                                    ]["location"]

                                                elif self.subroutines[subprog][
                                                    "isIface"
                                                ]:
                                                    subiface = (
                                                        self.subroutines[subprog][
                                            ...
                                            if location:
                                                ...
                                                matchedTargets.append(
                                                    (newTargetName, targetattrs)
                                                )
                                    ...
                                    if targetName + suffixT in self.nodes:
                                        ...
                                        newTargetName = targetName + suffixT
                                        lines = self.node[newTargetName]["attrs"][
                                            "lines"
                                        ]
                                        targetattrs = {
                                            "location": self.subroutines[useLocation][
                                                "location"
                                            ],
                                            "cname": targetName,
                                            "lines": lines,
                                    ...
                                    if targetName + suffixT in self.nodes:
                                        ...
                                        targetattrs = {
                                            "location": self.functions[useLocation][
                                                "location"
                                            ],
                                            "cname": targetName,
                                            "lines": lines,
            ...
            if use[1]["same"]:
                ...
                if originModule in self.modules:
                    ...
                    for var in (
                        self.modules[originModule]["vars"]
                        | self.modules[originModule]["public"]
                        | self.modules[originModule]["provides"]
                    ):
                        sourceName = var
                        ...
                        if var not in self.functions and var not in self.subroutines:
                            ...
                            if originModule in self.modules:
                                ...
                                if (
                                    ...
                                    for subprog in self.modules[originModule][
                                        "subprogs"
                                    ]:
                                        suffix = "__" + subprog
                                            ...
                                            if targetName + suffixT in self.nodes:
                                                ...
                                                if subprog in self.subroutines:
                                                    ...
                                                    if (
                                                        ...
                                                        location = self.functions[
                                                            subprog
                                                        ]["location"]

                                                    elif self.functions[subprog][
                                                        "isIface"
                                                    ]:
                                                        subiface = (
                                                            self.functions[subprog][
                ...
                try:
                    ...
                    if subname in self.subroutines:
                        ...
                        for idx, _ in enumerate(location["subprogs"]):
                            ...
                            keyLoc = tuple([subname]) + tuple(
                                location["subprogs"][::-1][idx:][::-1]
                            )
                            ...
                            try:
                                ...
                                for k, subsubr in subroutine["subroutines"].iteritems():
                                    ...
                                    for carg, defarg in zip(callArgs, subsubr["args"]):
                                        processSubArgs(
                                            self, carg, defarg, location, subLoc
                                        )
        ...
        for statement in tqdm(self.funcLike, unit="fcalls"):
            ...
            try:
                ...
                if isinstance(stmt, str):
                    ...
                    if "%" not in LHS:
                        lhsSplit = (
                            stmt.split("=", 1)[0].strip().lower().split("(", 1)[0]
                        )
    ...
    def getSubgraph(self, name):
        nodes = [
            n for n in self.nodes() if name in self.node[n]["attrs"]["location"][0]
        ]
    ...
    def hashimoto(
        self, sgraph, inout=False, outCentrality=False, weight=None, max_iter=50, tol=0
    ):
        import scipy as sp

        if len(sgraph) == 0:
            raise g.NetworkXPointlessConcept(
        ...
        if not outCentrality:
            ...
            L = g.to_scipy_sparse_matrix(
                hashimoto_mat, nodelist=list(hashimoto_mat), dtype=float
            )
            eigenvalue, eigenvector = sp.sparse.linalg.eigs(
                L.T, k=1, which="LR", maxiter=50, tol=0
            )
        ...
        if inout or outCentrality:
            ...
            rL = g.to_scipy_sparse_matrix(
                rhashimoto_mat, nodelist=list(rhashimoto_mat), dtype=float
            )
            reigenvalue, reigenvector = sp.sparse.linalg.eigs(
                rL.T, k=1, which="LR", maxiter=50, tol=0
            )
        ...
        if not outCentrality and not inout:
            ...
            rnormsign = np.sign(rlargest.sum())
            tmpinNorm = (
                normsign * np.array(tmpin, dtype=np.float64) / np.linalg.norm(tmpin)
            )
            tmpoutNorm = (
                rnormsign * np.array(tmpout, dtype=np.float64) / np.linalg.norm(tmpout)
            )
            # since each centrality is normed to 1, norm sum to 1 by Cauchy-Schwartz:
            sumNorm = (tmpinNorm + tmpoutNorm) / sqrt(
                2.0 + np.inner(tmpinNorm, tmpoutNorm) + np.inner(tmpoutNorm, tmpinNorm)
            )
    ...
    def girvan_newman(
        self, inducedSgraph, nComms=1, hashi=False, outCentrality=False, inout=False
    ):
        ...
        for n in inducedSgraph:
            if (
                len(g.shortest_path_length(inducedSgraph, source=n)) < 4
                and len(g.shortest_path_length(inducedSgraph, target=n)) < 4
            ):
                ex.add(n)
            ...
            for c in communities:
                ...
                if len(c) > 3:
                    ...
                    try:
                        ...
                        if hashi:
                            eigens = self.hashimoto(
                                csg, inout=inout, outCentrality=outCentrality
                            )
                            ...
                            if outCentrality:
                                ...
                                oeigenNodes = g.eigenvector_centrality_numpy(
                                    csg.reverse()
                                )
                                tmpin = tmpout = []
                                for n in csg:
                                ...
                                tmpinNorm = np.array(
                                    tmpin, dtype=np.float64
                                ) / np.linalg.norm(tmpin)
                                tmpoutNorm = np.array(
                                    tmpout, dtype=np.float64
                                ) / np.linalg.norm(tmpout)
                                # since each centrality is normed to 1, norm to 1 by Cauchy-Schwartz:
                                sumNorm = (tmpinNorm + tmpoutNorm) / sqrt(
                                    2.0
                                    + np.inner(tmpinNorm, tmpoutNorm)
                                    + np.inner(tmpoutNorm, tmpinNorm)
                                )
                        ...
                        sorted_eigens = sorted(
                            eigens.items(), key=operator.itemgetter(1), reverse=True
                        )
    ...
    def eigenCentrality(
        ...
        if models:
            mdict = {
                "atm": self.atmFiles,
                "csm_share": self.csm_shareFiles,
                "dead_share": self.dead_shareFiles,
                "drv": self.drvFiles,
                "glc": self.glcFiles,
                "ice": self.iceFiles,
                "lnd": self.lndFiles,
        ...
        if not paths:
            if weights:
                inedgeLocs = [
                    self.__resolveNames(k, weight=v) for k, v in nameList.iteritems()
                ]
            ...
            if notVars:
                for k in self.camVars:
                    if k not in nameList and k not in set(
                        ["v", "ps", "t", "ts", "q", "u"]
                    ):
            ...
            if weights:
                inedgeLocs = [
                    self.isectPaths(k, outputs=outputs, cnames=cnames)[0]
                    for k, v in nameList.iteritems()
                ]
                ...
                if notVars:
                    ...
                    for k in self.camVars:
                        if k not in nameList and k not in set(
                            ...
                            ninedgeLocs[k] = self.isectPaths(
                                k, outputs=outputs, cnames=cnames
                            )[0]

            else:
                inedgeLocs = [
                    self.isectPaths(i, outputs=outputs, cnames=cnames)[0]
                    for i in nameList
                ]
            ...
            if not paths:
                ...
                if overlap:
                    for a, b in combinations(inedgeLocs, 2):
                        for ovrlp in a & b:
                            self.edges[ovrlp[0], ovrlp[1]]["weight"] *= 2.0
                ...
                for diffpath in inedgeLocs:
                    for var, notPath in ninedgeLocs.iteritems():
                        for isect in diffpath & notPath:
                            self.edges[isect[0], isect[1]]["weight"] /= 2.0
        ...
        if not quotient:
            ...
            eqRel = lambda u, v: (
                self.node[u]["attrs"]["location"][qindex]
                == self.node[v]["attrs"]["location"][qindex]
            )
            ...
            if not qweights:
                ...
                def edgeRel(U, V):
                    ...
                    for node in U:
                        for vert in V:
                            if self.graph.has_edge(node, vert) or self.graph.has_edge(
                                vert, node
                            ):
                                weight += 1
        ...
        if inedges:
            if weights:
                if not hashi:
                    eigenNodes = g.eigenvector_centrality_numpy(
                        inducedSgraph, weight="weight"
                    )

                else:
                    eigenNodes = self.hashimoto(
                        inducedSgraph,
                        inout=inout,
                ...
                if not hashi:
                    ...
                    eigenNodes = self.hashimoto(
                        inducedSgraph, inout=inout, outCentrality=outCentrality
                    )

        else:
            if weights:
                eigenNodes = g.eigenvector_centrality_numpy(
                    inducedSgraph.reverse(), weight="weight"
                )
        ...
        sorted_eigen = sorted(
            eigenNodes.items(), key=operator.itemgetter(1), reverse=True
        )
                        ...
                        if self.name not in nodeName:
                            if "origin" in self.node[nodeName]["attrs"]:
                                ...
                                if nodeName not in importantNodes:
                                    importantNodes.add(nodeName)
                                    key = tuple(
                                        self.node[nodeName]["attrs"]["location"]
                                    )
                                    if key not in importantLocs:
                                        importantLocs[key] = i[1]
        ...
        top = OrderedSet()
        for k in sorted(
            importantLocs.items(), key=operator.itemgetter(1), reverse=True
        ):
            if len(top) < topLocs:
                top.add(k[0][0])
            else:
        ...
        for path in self.shortestPath(target=node).items():
            ...
            for source in rpath[1:]:
                ...
                if ssubProg == currentSP:
                    assignLines = set.intersection(
                        *[
                            set(self.edges[source, target]["lines"]),
                    ...
                    if ssubProg not in spstack:
                        nextLine = self.node[source]["attrs"]["lines"]["line"].maxKey(
                            sys.maxsize
                        )
                        spstack[ssubProg] = nextLine
                        backslice.append((source, nextLine))
                        ...
                        try:
                            nextLine = self.node[source]["attrs"]["lines"][
                                "line"
                            ].maxKey(spstack[ssubProg] - 1)
                            spstack[ssubProg] = nextLine
                            backslice.append((source, nextLine))
                            target = source
                        except ValueError:
        ...
        for path in self.shortestPath(source=node).items():
            ...
            for target in rpath[1:]:
                ...
                if ssubProg == currentSP:
                    ...
                    currentSP = ssubProg
                    if ssubProg not in spstack:
                        spstack[ssubProg] = self.node[target]["attrs"]["lines"][
                            "line"
                        ].minKey(0)
                        forwardslice.append(target)
                        source = target
                        ...
                        try:
                            spstack[ssubProg] = self.node[source]["attrs"]["lines"][
                                "line"
                            ].minKey(spstack[ssubProg] + 1)
                            forwardslice.append(target)
                            source = target
                        except ValueError:
                            backslices.add(tuple(forwardslice))
