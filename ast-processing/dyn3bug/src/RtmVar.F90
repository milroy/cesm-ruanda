...
! Subprogram not used   subroutine RtmVarInit( )
! Subprogram not used     if (masterproc) then
! Subprogram not used        if (nsrest == iundef) then
! Subprogram not used           call shr_sys_abort( 'RtmVarInit ERROR:: must set nsrest' )
! Subprogram not used        end if
! Subprogram not used        if (nsrest == nsrBranch .and. nrevsn_rtm == ' ') then
! Subprogram not used           call shr_sys_abort( 'RtmVarInit ERROR: need to set restart data file name' )
! Subprogram not used        end if
! Subprogram not used        if (nsrest == nsrStartup ) then
! Subprogram not used           nrevsn_rtm = ' '
! Subprogram not used        end if
! Subprogram not used        if (nsrest == nsrContinue) then
! Subprogram not used           nrevsn_rtm = 'set by restart pointer file file'
! Subprogram not used        end if
! Subprogram not used        if (nsrest /= nsrStartup .and. nsrest /= nsrContinue .and. nsrest /= nsrBranch ) then
! Subprogram not used           call shr_sys_abort( 'RtmVarInit ERROR: nsrest NOT set to a valid value' )
! Subprogram not used        end if
! Subprogram not used     endif   
! Subprogram not used     RtmVar_isset = .true.
! Subprogram not used   end subroutine RtmVarInit
