...
! Subprogram not used    subroutine PrintTrouble_dbl(name,array)
! Subprogram not used 
! Subprogram not used      use ice_fileunits, only: nu_diag
! Subprogram not used 
! Subprogram not used      character(len=*), intent(in) :: name
! Subprogram not used      real(kind=dbl_kind), intent(in)         :: array(nx_block,ny_block) 
! Subprogram not used        
! Subprogram not used      write(nu_diag,*) TRIM(name),'(i,j) =',array(trouble_il,trouble_jl)
! Subprogram not used 
! Subprogram not used    end subroutine PrintTrouble_dbl

! Subprogram not used    subroutine PrintTrouble_int(name,array)
! Subprogram not used 
! Subprogram not used      use ice_fileunits, only: nu_diag
! Subprogram not used 
! Subprogram not used      character(len=*), intent(in) :: name
! Subprogram not used      integer(int_kind), intent(in)         :: array(nx_block,ny_block) 
! Subprogram not used         
! Subprogram not used      write(nu_diag,*) TRIM(name),'(i,j) =',array(trouble_il,trouble_jl)
! Subprogram not used 
! Subprogram not used    end subroutine PrintTrouble_int

! Subprogram not used    subroutine LocateTrouble
! Subprogram not used 
! Subprogram not used      type (block) :: this_block
! Subprogram not used      integer(int_kind) :: n,gbid,ib,ie,jb,je,ig,jg,i,j
! Subprogram not used      integer(int_kind) :: j_save,i_save,ib_save
! Subprogram not used      logical  :: foundi, foundj
! Subprogram not used     
! Subprogram not used 
! Subprogram not used       do n=1,nblocks_tot
! Subprogram not used 	  foundi=.false.;foundj=.false.
! Subprogram not used 	  gbid = all_blocks(n)%block_id
! Subprogram not used 	  ib   = all_blocks(n)%ilo
! Subprogram not used 	  ie   = all_blocks(n)%ihi
! Subprogram not used 	  jb   = all_blocks(n)%jlo
! Subprogram not used           je   = all_blocks(n)%jhi
! Subprogram not used 	  do i=ib,ie
! Subprogram not used 	     ig=all_blocks(n)%i_glob(i) 
! Subprogram not used 	     if(ig == trouble_ig) then 
! Subprogram not used 		i_save=i
! Subprogram not used 		foundi=.true.
! Subprogram not used 	     endif
! Subprogram not used           enddo
! Subprogram not used 	  do j=jb,je
! Subprogram not used 	     jg=all_blocks(n)%j_glob(j) 
! Subprogram not used 	     if(jg == trouble_jg) then
! Subprogram not used 		j_save=j;foundj=.true.
! Subprogram not used 	     endif
! Subprogram not used           enddo
! Subprogram not used           if(foundi .and. foundj) then 
! Subprogram not used 		ib_save = n
! Subprogram not used 	  endif
! Subprogram not used       enddo
! Subprogram not used       trouble_il  = i_save
! Subprogram not used       trouble_jl  = j_save
! Subprogram not used       trouble_ibl = ib_save
! Subprogram not used !      print *,'Trouble located: ib, i, j: ',ib_save,i_save,j_save
! Subprogram not used 
! Subprogram not used    end subroutine LocateTrouble
