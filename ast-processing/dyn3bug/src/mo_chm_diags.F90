...
! Subprogram not used   subroutine het_diags( het_rates, mmr, pdel, lchnk, ncol )
! Subprogram not used 
! Subprogram not used     use cam_history,  only : outfld
! Subprogram not used     use phys_grid,    only : get_wght_all_p
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     integer,  intent(in)  :: lchnk
! Subprogram not used     integer,  intent(in)  :: ncol
! Subprogram not used     real(r8), intent(in)  :: het_rates(ncol,pver,max(1,gas_pcnst))
! Subprogram not used     real(r8), intent(in)  :: mmr(ncol,pver,gas_pcnst)
! Subprogram not used     real(r8), intent(in)  :: pdel(ncol,pver)
! Subprogram not used 
! Subprogram not used     real(r8), dimension(ncol) :: noy_wk, sox_wk, nhx_wk, wrk_wd
! Subprogram not used     integer :: m, k, j
! Subprogram not used     integer :: plat
! Subprogram not used     real(r8) :: wght(ncol)
! Subprogram not used     !
! Subprogram not used     ! output integrated wet deposition field
! Subprogram not used     !
! Subprogram not used     noy_wk(:) = 0._r8
! Subprogram not used     sox_wk(:) = 0._r8
! Subprogram not used     nhx_wk(:) = 0._r8
! Subprogram not used 
! Subprogram not used     call get_wght_all_p(lchnk, ncol, wght)
! Subprogram not used 
! Subprogram not used     do m = 1,gas_pcnst
! Subprogram not used        !
! Subprogram not used        ! compute vertical integral
! Subprogram not used        !
! Subprogram not used        wrk_wd(:ncol) = 0._r8
! Subprogram not used        do k = 1,pver
! Subprogram not used           wrk_wd(:ncol) = wrk_wd(:ncol) + het_rates(:ncol,k,m) * mmr(:ncol,k,m) * pdel(:ncol,k) 
! Subprogram not used        end do
! Subprogram not used        !
! Subprogram not used        wrk_wd(:ncol) = wrk_wd(:ncol) * rgrav * wght(:ncol) * rearth**2
! Subprogram not used        !
! Subprogram not used 
! Subprogram not used        call outfld( wetdep_name(m), wrk_wd(:ncol),               ncol, lchnk )
! Subprogram not used        call outfld( wtrate_name(m), het_rates(:ncol,:,m), ncol, lchnk )
! Subprogram not used 
! Subprogram not used        if ( any(noy_species == m ) ) then
! Subprogram not used           noy_wk(:ncol) = noy_wk(:ncol) + wrk_wd(:ncol)*N_molwgt/adv_mass(m)
! Subprogram not used        endif
! Subprogram not used        if ( m == id_n2o5 ) then  ! 2 NOy molecules in N2O5
! Subprogram not used           noy_wk(:ncol) = noy_wk(:ncol) + wrk_wd(:ncol)*N_molwgt/adv_mass(m)
! Subprogram not used        endif
! Subprogram not used        if ( any(sox_species == m ) ) then
! Subprogram not used           sox_wk(:ncol) = sox_wk(:ncol) + wrk_wd(:ncol)*S_molwgt/adv_mass(m)
! Subprogram not used        endif
! Subprogram not used        if ( any(nhx_species == m ) ) then
! Subprogram not used           nhx_wk(:ncol) = nhx_wk(:ncol) + wrk_wd(:ncol)*N_molwgt/adv_mass(m)
! Subprogram not used        endif
! Subprogram not used 
! Subprogram not used     end do
! Subprogram not used     
! Subprogram not used     call outfld( 'WD_NOY', noy_wk(:ncol), ncol, lchnk )
! Subprogram not used     call outfld( 'WD_SOX', sox_wk(:ncol), ncol, lchnk )
! Subprogram not used     call outfld( 'WD_NHX', nhx_wk(:ncol), ncol, lchnk )
! Subprogram not used 
! Subprogram not used   end subroutine het_diags
