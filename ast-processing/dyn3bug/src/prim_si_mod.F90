...
! Subprogram not used 
! Subprogram not used #if (defined ELEMENT_OPENMP)
! Subprogram not used !$omp parallel do private(k,j,i,facp,facm)
...
! ORIGINAL             omega_p(i,j,1) = vgrad_p(i,j,1)/p(i,j,1)
! ORIGINAL             omega_p(i,j,1) = omega_p(i,j,1) - ckk*term
             ! omega_p(i,j,1) = (vgrad_p(i,j,1) - 0.5d0*divdp(i,j,1))/p(i,j,1)
             ! BUG HERE
             omega_p(i,j,1) = (vgrad_p(i,j,1) - 0.44d0*divdp(i,j,1))/p(i,j,1)
             ! omega_p(i,j,1) = (vgrad_p(i,j,1) - 0.499d0*divdp(i,j,1))/p(i,j,1)
             suml(i,j) = term
          end do
