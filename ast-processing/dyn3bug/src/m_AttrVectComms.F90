...
! Subprogram not used  subroutine send_(inAV, dest, TagBase, comm, status)
! Subprogram not used !
...
! Subprogram not used !
! Subprogram not used       type(AttrVect),     intent(in)  :: inAV
...
! Subprogram not used !  7Jun01 - J.W. Larson - initial version.
...
! Subprogram not used 
! Subprogram not used  logical :: ListAssoc(2)
! Subprogram not used  integer :: ierr
! Subprogram not used  integer :: AVlength
! Subprogram not used 
...
! Subprogram not used 
! Subprogram not used 
! Subprogram not used        ! Step 1. Are inAV%iList and inAV%rList filled?  Store
! Subprogram not used        ! the answers in the LOGICAL array ListAssoc and send.
! Subprogram not used 
! Subprogram not used   ListAssoc(1) = List_allocated(inAV%iList) 
! Subprogram not used   ListAssoc(2) = List_allocated(inAV%rList) 
! Subprogram not used 
! Subprogram not used   if(.NOT. (ListAssoc(1).or.ListAssoc(2)) ) then
! Subprogram not used      call die(myname_,"inAV has not been initialized")
! Subprogram not used   endif
! Subprogram not used      
! Subprogram not used   call MPI_SEND(ListAssoc, 2, MP_LOGICAL, dest, TagBase, comm, ierr)
! Subprogram not used   if(ierr /= 0) then
! Subprogram not used      call MP_perr_die(myname_,':: MPI_SEND(ListAssoc...',ierr)
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used 
! Subprogram not used        ! Step 2. Send non-blank inAV%iList and inAV%rList.
! Subprogram not used 
! Subprogram not used   if(ListAssoc(1)) then
! Subprogram not used     call List_send(inAV%iList, dest, TagBase+1, comm, ierr)
! Subprogram not used     if(ierr /= 0) then
! Subprogram not used        if(present(status)) then
! Subprogram not used 	  write(stderr,*) myname_,':: call List_send(inAV%iList...'
! Subprogram not used 	  status = ierr
! Subprogram not used 	  return
! Subprogram not used        else
! Subprogram not used 	  call die(myname_,':: call List_send(inAV%iList...',ierr)
! Subprogram not used        endif
! Subprogram not used     endif
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used   if(ListAssoc(2)) then
! Subprogram not used     call List_send(inAV%rList, dest, TagBase+3, comm, ierr)
! Subprogram not used     if(ierr /= 0) then
! Subprogram not used        if(present(status)) then
! Subprogram not used 	  write(stderr,*) myname_,':: call List_send(inAV%rList...'
! Subprogram not used 	  status = ierr
! Subprogram not used 	  return
! Subprogram not used        else
! Subprogram not used 	  call die(myname_,':: call List_send(inAV%rList...',ierr)
! Subprogram not used        endif
! Subprogram not used     endif
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used        ! Step 3. Determine and send the lengths of inAV%iAttr(:,:)
! Subprogram not used        ! and inAV%rAttr(:,:).
! Subprogram not used 
! Subprogram not used   AVlength = AttrVect_lsize(inAV)
! Subprogram not used 
! Subprogram not used   if(AVlength<=0) then
! Subprogram not used      call die(myname_,"Size of inAV <= 0",AVLength)
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used   call MPI_SEND(AVlength, 1, MP_type(AVlength), dest, TagBase+5, &
! Subprogram not used                 comm, ierr)
! Subprogram not used   if(ierr /= 0) then
! Subprogram not used      call MP_perr_die(myname_,':: call MPI_SEND(AVlength...',ierr)
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used        ! Step 4. If AVlength > 0, we may have INTEGER and REAL 
! Subprogram not used        ! data to send.  Send as needed.
! Subprogram not used 
! Subprogram not used   if(AVlength > 0) then
! Subprogram not used 
! Subprogram not used      if(ListAssoc(1)) then
! Subprogram not used 
! Subprogram not used        ! Send the INTEGER data stored in inAV%iAttr(:,:)
! Subprogram not used 
! Subprogram not used 	call MPI_SEND(inAV%iAttr(1,1), AVlength*List_nitem(inAV%iList), &
! Subprogram not used                       MP_type(inAV%iAttr(1,1)), dest, TagBase+6,   &
! Subprogram not used                       comm, ierr)
! Subprogram not used 	if(ierr /= 0) then
! Subprogram not used 	   call MP_perr_die(myname_,':: call MPI_SEND(inAV%iAttr...',ierr)
! Subprogram not used 	endif
! Subprogram not used 
! Subprogram not used      endif ! if(associated(inAV%rList))
! Subprogram not used 
! Subprogram not used      if(ListAssoc(2)) then
! Subprogram not used 
! Subprogram not used        ! Send the REAL data stored in inAV%rAttr(:,:)
! Subprogram not used 
! Subprogram not used 	call MPI_SEND(inAV%rAttr(1,1), AVlength*List_nitem(inAV%rList), &
! Subprogram not used                       MP_type(inAV%rAttr(1,1)), dest, TagBase+7,   &
! Subprogram not used                       comm, ierr)
! Subprogram not used 	if(ierr /= 0) then
! Subprogram not used 	   call MP_perr_die(myname_,':: call MPI_SEND(inAV%rAttr...',ierr)
! Subprogram not used 	endif
! Subprogram not used 
! Subprogram not used      endif ! if(associated(inAV%rList))
! Subprogram not used 
! Subprogram not used   endif ! if (AVlength > 0)
! Subprogram not used 
...
! Subprogram not used  subroutine recv_(outAV, dest, TagBase, comm, status)
! Subprogram not used !
! Subprogram not used ! !USES:
! Subprogram not used !
! Subprogram not used       use m_stdio
...
! Subprogram not used ! !OUTPUT PARAMETERS: 
! Subprogram not used !
! Subprogram not used       type(AttrVect),     intent(out) :: outAV
! Subprogram not used       integer, optional,  intent(out) :: status
! Subprogram not used 
! Subprogram not used ! !REVISION HISTORY:
! Subprogram not used !  7Jun01 - J.W. Larson - initial working version.
! Subprogram not used ! 13Jun01 - J.W. Larson <larson@mcs.anl.gov> - Initialize status
! Subprogram not used !           (if present).
! Subprogram not used !EOP ___________________________________________________________________
! Subprogram not used 
...
! Subprogram not used 
! Subprogram not used 
! Subprogram not used        ! Step 1. Are outAV%iList and outAV%rList filled?  TRUE
! Subprogram not used        ! entries in the LOGICAL array ListAssoc(:) correspond
! Subprogram not used        ! to Non-blank Lists...that is:
! Subprogram not used        !
! Subprogram not used        ! ListAssoc(1) = .TRUE. <==> associated(outAV%iList%bf)
! Subprogram not used        ! ListAssoc(2) = .TRUE. <==> associated(outAV%rList%bf)
! Subprogram not used 
! Subprogram not used   call MPI_RECV(ListAssoc, 2, MP_LOGICAL, dest, TagBase, comm, &
! Subprogram not used                 MPstatus, ierr)
! Subprogram not used   if(ierr /= 0) then
! Subprogram not used      call MP_perr_die(myname_,':: MPI_RECV(ListAssoc...',ierr)
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used 
! Subprogram not used        ! Step 2. Receive non-blank outAV%iList and outAV%rList.
! Subprogram not used 
! Subprogram not used   if(ListAssoc(1)) then
! Subprogram not used     call List_recv(outAV%iList, dest, TagBase+1, comm, ierr)
! Subprogram not used     if(ierr /= 0) then
! Subprogram not used        if(present(status)) then
! Subprogram not used 	  write(stderr,*) myname_,':: call List_recv(outAV%iList...'
! Subprogram not used 	  status = ierr
! Subprogram not used 	  return
! Subprogram not used        else
! Subprogram not used 	  call die(myname_,':: call List_recv(outAV%iList...',ierr)
! Subprogram not used        endif
! Subprogram not used     endif
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used   if(ListAssoc(2)) then
! Subprogram not used     call List_recv(outAV%rList, dest, TagBase+3, comm, ierr)
! Subprogram not used     if(ierr /= 0) then
! Subprogram not used        if(present(status)) then
! Subprogram not used 	  write(stderr,*) myname_,':: call List_recv(outAV%rList...'
! Subprogram not used 	  status = ierr
! Subprogram not used 	  return
! Subprogram not used        else
! Subprogram not used 	  call die(myname_,':: call List_recv(outAV%rList...',ierr)
! Subprogram not used        endif
! Subprogram not used     endif
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used        ! Step 3. Receive the lengths of outAV%iAttr(:,:) and outAV%rAttr(:,:).
! Subprogram not used 
! Subprogram not used   call MPI_RECV(AVlength, 1, MP_type(AVlength), dest, TagBase+5, &
! Subprogram not used                 comm, MPstatus, ierr)
! Subprogram not used   if(ierr /= 0) then
! Subprogram not used      call MP_perr_die(myname_,':: call MPI_RECV(AVlength...',ierr)
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used        ! Step 4. If AVlength > 0, we may have to receive INTEGER 
! Subprogram not used        ! and/or REAL data.  Receive as needed.
! Subprogram not used 
! Subprogram not used   if(AVlength > 0) then
! Subprogram not used 
! Subprogram not used      if(ListAssoc(1)) then
! Subprogram not used 
! Subprogram not used        ! Allocate outAV%iAttr(:,:)
! Subprogram not used 
! Subprogram not used         allocate(outAV%iAttr(List_nitem(outAV%iList),AVlength), stat=ierr)
! Subprogram not used         if(ierr/=0) call die(myname_,"allocate(outAV%iAttr)",ierr)
! Subprogram not used 
! Subprogram not used        ! Receive the INTEGER data to outAV%iAttr(:,:)
! Subprogram not used 
! Subprogram not used 	call MPI_RECV(outAV%iAttr(1,1), AVlength*List_nitem(outAV%iList), &
! Subprogram not used                       MP_type(outAV%iAttr(1,1)), dest, TagBase+6,   &
! Subprogram not used                       comm, MPstatus, ierr)
! Subprogram not used 	if(ierr /= 0) then
! Subprogram not used 	   call MP_perr_die(myname_,':: call MPI_RECV(outAV%iAttr...',ierr)
! Subprogram not used 	endif
! Subprogram not used 
! Subprogram not used      endif ! if(associated(outAV%rList))
! Subprogram not used 
! Subprogram not used      if(ListAssoc(2)) then
! Subprogram not used 
! Subprogram not used        ! Allocate outAV%rAttr(:,:)
! Subprogram not used 
! Subprogram not used         allocate(outAV%rAttr(List_nitem(outAV%rList),AVlength), stat=ierr)
! Subprogram not used         if(ierr/=0) call die(myname_,"allocate(outAV%rAttr)",ierr)
! Subprogram not used 
! Subprogram not used        ! Receive the REAL data to outAV%rAttr(:,:)
! Subprogram not used 
! Subprogram not used 	call MPI_RECV(outAV%rAttr(1,1), AVlength*List_nitem(outAV%rList), &
! Subprogram not used                       MP_type(outAV%rAttr(1,1)), dest, TagBase+7,   &
! Subprogram not used                       comm, MPstatus, ierr)
! Subprogram not used 	if(ierr /= 0) then
! Subprogram not used 	   call MP_perr_die(myname_,':: call MPI_RECV(outAV%rAttr...',ierr)
! Subprogram not used 	endif
! Subprogram not used 
! Subprogram not used      endif ! if(associated(outAV%rList))
! Subprogram not used 
...
! Subprogram not used  subroutine bcast_(aV, root, comm, stat)
! Subprogram not used !
! Subprogram not used ! !USES:
! Subprogram not used !
! Subprogram not used       use m_stdio
! Subprogram not used       use m_die
! Subprogram not used       use m_mpif90
! Subprogram not used       use m_String, only : String,bcast,char,String_clean
! Subprogram not used       use m_String, only : String_bcast => bcast
...
! Subprogram not used       use m_AttrVect, only : AttrVect_lsize => lsize
! Subprogram not used       use m_AttrVect, only : AttrVect_nIAttr => nIAttr
! Subprogram not used       use m_AttrVect, only : AttrVect_nRAttr => nRAttr
! Subprogram not used 
! Subprogram not used       implicit none
! Subprogram not used 
! Subprogram not used ! !INPUT PARAMETERS: 
...
! Subprogram not used !
! Subprogram not used       type(AttrVect),           intent(inout) :: aV ! (IN) on the root, 
! Subprogram not used                                                     ! (OUT) elsewhere
! Subprogram not used 
! Subprogram not used ! !OUTPUT PARAMETERS:
! Subprogram not used !
...
! Subprogram not used 
! Subprogram not used ! !REVISION HISTORY:
! Subprogram not used ! 27Apr98 - Jing Guo <guo@thunder> - initial prototype/prologue/code
! Subprogram not used ! 27Oct00 - J.W. Larson <larson@mcs.anl.gov> - relocated from
! Subprogram not used !           m_AttrVect
! Subprogram not used !  9May01 - J.W. Larson <larson@mcs.anl.gov> - tidied up prologue
! Subprogram not used ! 18May01 - R.L. Jacob <jacob@mcs.anl.gov> - use MP_Type function
! Subprogram not used !           to determine type for bcast
! Subprogram not used ! 19Dec01 - E.T. Ong <eong@mcs.anl.gov> - adjusted for case of AV with 
! Subprogram not used !           only integer or real attribute 
! Subprogram not used !EOP ___________________________________________________________________
! Subprogram not used 
! Subprogram not used   character(len=*),parameter :: myname_=myname//'::bcast_'
! Subprogram not used   type(String) :: iLStr,rLStr
! Subprogram not used   integer :: nIA, nRA, lsize
! Subprogram not used   integer :: myID
! Subprogram not used   integer :: ier
! Subprogram not used   integer :: mp_Type_aV
! Subprogram not used 
...
! Subprogram not used     call MP_perr_die(myname_,'MP_comm_rank()',ier)
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used        ! Broadcaast to all PEs
! Subprogram not used 
! Subprogram not used   if(myID == root) then
! Subprogram not used      nIA = AttrVect_nIAttr(aV)
! Subprogram not used      nRA = AttrVect_nRAttr(aV)
! Subprogram not used      lsize = AttrVect_lsize(aV)
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used   call MPI_bcast(nIA,1,MP_INTEGER,root,comm,ier)
! Subprogram not used   if(ier /= 0) then
! Subprogram not used     call MP_perr_die(myname_,'MPI_bcast(nIA)',ier)
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used   call MPI_bcast(nRA,1,MP_INTEGER,root,comm,ier)
! Subprogram not used   if(ier /= 0) then
! Subprogram not used     call MP_perr_die(myname_,'MPI_bcast(nRA)',ier)
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used   call MPI_bcast(lsize,1,MP_INTEGER,root,comm,ier)
! Subprogram not used   if(ier /= 0) then
! Subprogram not used     call MP_perr_die(myname_,'MPI_bcast(lsize)',ier)
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used 	! Convert the two Lists to two Strings 
! Subprogram not used 
! Subprogram not used   if(nIA>0) then
! Subprogram not used 
! Subprogram not used      if(myID == root) call List_get(iLStr,aV%iList)
! Subprogram not used 
! Subprogram not used      call String_bcast(iLStr,root,comm,stat=ier)	! bcast.String()
! Subprogram not used 
! Subprogram not used      if(ier /= 0) then
! Subprogram not used 	write(stderr,*) myname_,'bcast.String(iLstr), ier=',ier
! Subprogram not used 	if(present(stat)) then
! Subprogram not used 	   stat=ier
! Subprogram not used 	   return
! Subprogram not used 	else
! Subprogram not used 	   call die(myname_,'String_bcast(iLStr) failed',ier)
! Subprogram not used 	endif
! Subprogram not used      endif ! if(ier /= 0)...
! Subprogram not used 
! Subprogram not used   endif ! if(nIA > 0)...
! Subprogram not used 
! Subprogram not used 
! Subprogram not used   if(nRA>0) then
! Subprogram not used 
! Subprogram not used      if(myID == root) call List_get(rLStr,aV%rList)
! Subprogram not used 
! Subprogram not used      call String_bcast(rLStr,root,comm,stat=ier)	! bcast.String()
! Subprogram not used      if(ier /= 0) then
! Subprogram not used 	write(stderr,*) myname_,'bcast.String(iLstr), ier=',ier
! Subprogram not used 	if(present(stat)) then
! Subprogram not used 	   stat=ier
...
! Subprogram not used      endif ! if(ier /= 0)...
! Subprogram not used 
! Subprogram not used   endif ! if(nRA > 0)...
! Subprogram not used 
! Subprogram not used   if(myID /= root) then
! Subprogram not used      
! Subprogram not used      if( (nIA>0) .and. (nRA>0) ) then
! Subprogram not used 	call AttrVect_init(aV,iList=char(iLStr),rList=char(rLStr), &
! Subprogram not used 	                   lsize=lsize)
! Subprogram not used      endif
! Subprogram not used 
! Subprogram not used      if( (nIA>0) .and. (nRA<=0) ) then
! Subprogram not used 	call AttrVect_init(aV,iList=char(iLStr),lsize=lsize)
! Subprogram not used      endif
! Subprogram not used 
! Subprogram not used      if( (nIA<=0) .and. (nRA>0) ) then
! Subprogram not used 	call AttrVect_init(aV,rList=char(rLStr),lsize=lsize)
! Subprogram not used      endif
! Subprogram not used 
! Subprogram not used      if( (nIA<=0) .and. (nRA<=0) ) then
! Subprogram not used 	write(stderr,*) myname_,':: Nonpositive numbers of both ',&
! Subprogram not used 	     'real AND integer attributes.  nIA =',nIA,' nRA=',nRA
! Subprogram not used 	if(present(stat)) then
! Subprogram not used 	   stat = -1
! Subprogram not used 	   return
! Subprogram not used 	else
! Subprogram not used 	   call die(myname_,'AV has not been initialized',-1)
! Subprogram not used 	endif
! Subprogram not used      endif ! if((nIA<= 0) .and. (nRA<=0))...
! Subprogram not used 
! Subprogram not used      call AttrVect_zero(aV)
! Subprogram not used 
! Subprogram not used 
! Subprogram not used   endif ! if(myID /= root)...
! Subprogram not used 
! Subprogram not used   if(nIA > 0) then
! Subprogram not used 
! Subprogram not used      mp_Type_aV=MP_Type(av%iAttr)
! Subprogram not used      call MPI_bcast(aV%iAttr,nIA*lsize,mp_Type_aV,root,comm,ier)
! Subprogram not used      if(ier /= 0) then
! Subprogram not used 	call MP_perr_die(myname_,'MPI_bcast(iAttr) failed.',ier)
! Subprogram not used      endif
! Subprogram not used 
! Subprogram not used      call String_clean(iLStr)
! Subprogram not used 
! Subprogram not used   endif
! Subprogram not used 
! Subprogram not used   if(nRA > 0) then
! Subprogram not used 
! Subprogram not used      mp_Type_aV=MP_Type(av%rAttr)
! Subprogram not used      call MPI_bcast(aV%rAttr,nRA*lsize,mp_Type_aV,root,comm,ier)
! Subprogram not used      if(ier /= 0) then
! Subprogram not used 	call MP_perr_die(myname_,'MPI_bcast(rAttr) failed.',ier)
! Subprogram not used      endif
! Subprogram not used 
! Subprogram not used      call String_clean(rLStr)
! Subprogram not used 
! Subprogram not used   endif
! Subprogram not used 
