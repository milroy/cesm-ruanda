...
!---------------------------------------------------------------------------
   ...
   !-----------------------------------------------------------------
    ...
    ! Need to stop this at the end of every run phase in a 1 run.
...
! Subprogram not used   subroutine ice_setdef_mct( i2x_i )   
! Subprogram not used 
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     !-----------------------------------------------------
! Subprogram not used     type(mct_aVect)   , intent(inout) :: i2x_i
! Subprogram not used 
! Subprogram not used     !-----------------------------------------------------
! Subprogram not used 
! Subprogram not used     call mct_aVect_zero(i2x_i)
! Subprogram not used 
! Subprogram not used     ! tcraig : this is where observations could be read in
! Subprogram not used 
! Subprogram not used   end subroutine ice_setdef_mct

  !=======================================================================

! Subprogram not used   subroutine ice_coffset_mct(xoff,yoff,gsmap_a,dom_a,gsmap_b,dom_b,mpicom_i)
! Subprogram not used     implicit none
! Subprogram not used 
! Subprogram not used     integer        , intent(out)   :: xoff
! Subprogram not used     integer        , intent(out)   :: yoff
! Subprogram not used     type(mct_gsmap), intent(in)    :: gsmap_a
! Subprogram not used     type(mct_ggrid), intent(in)    :: dom_a
! Subprogram not used     type(mct_gsmap), intent(in)    :: gsmap_b
! Subprogram not used     type(mct_ggrid), intent(in)    :: dom_b
! Subprogram not used     integer        , intent(in)    :: mpicom_i
! Subprogram not used 
! Subprogram not used     type(mct_aVect) :: ava
! Subprogram not used     type(mct_aVect) :: avag
! Subprogram not used     integer :: k1,k2,k
! Subprogram not used     integer :: npt
! Subprogram not used     integer :: noff,noffg
! Subprogram not used     real(dbl_kind) :: x1,y1,x2,y2
! Subprogram not used     real(dbl_kind) :: dist,distmin,distming
! Subprogram not used     integer :: lsizea,lsizeb
! Subprogram not used     integer :: iam,ierr
! Subprogram not used     integer, pointer :: ipoints(:)
! Subprogram not used     character(len=*),parameter :: subname = "ice_coffset_mct"
! Subprogram not used 
! Subprogram not used     call mpi_comm_rank(mpicom_i,iam,ierr)
! Subprogram not used 
! Subprogram not used     lsizea = mct_aVect_lsize(dom_a%data)
! Subprogram not used     lsizeb = mct_aVect_lsize(dom_b%data)
! Subprogram not used 
! Subprogram not used     !--- compute lon/lat at dom_a (local) point (1,1)
! Subprogram not used 
! Subprogram not used     call mct_aVect_init(ava,rList='lon:lat',lsize=lsizea)
! Subprogram not used     call mct_aVect_copy(dom_a%data,ava,'lon:lat')
! Subprogram not used     call mct_aVect_gather(ava,avag,gsmap_a,0,mpicom_i)
! Subprogram not used 
! Subprogram not used     if (iam == 0) then
! Subprogram not used        k1 = mct_aVect_indexRA(avag,'lon',dieWith=subname//'_avag')
! Subprogram not used        k2 = mct_aVect_indexRA(avag,'lat',dieWith=subname//'_avag')
! Subprogram not used        npt = 1   ! actual corner points screwed up by U average/wraparound
! Subprogram not used        npt = nx_global + 2  ! use global point (2,2)
! Subprogram not used        x1 = mod(avag%rAttr(k1,npt)+360.0_r8,360.0_r8)
! Subprogram not used        y1 = avag%rAttr(k2,npt)
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used     call mct_aVect_clean(avag)
! Subprogram not used     call mct_aVect_clean(ava)
! Subprogram not used 
! Subprogram not used     call shr_mpi_bcast(x1,mpicom_i)
! Subprogram not used     call shr_mpi_bcast(y1,mpicom_i)
! Subprogram not used 
! Subprogram not used     !--- find x1,y1 point in dom_b (extended grid)
! Subprogram not used 
! Subprogram not used     noff = -1
! Subprogram not used     noffg = -1
! Subprogram not used 
! Subprogram not used     call mct_gsMap_orderedPoints(gsMap_b, iam, ipoints)
! Subprogram not used     if (size(ipoints) /= lsizeb) then
! Subprogram not used        write(nu_diag,*) subname,' size ipoints = ',size(ipoints),lsizeb
! Subprogram not used        call shr_sys_abort(subname//' :: error size of ipoints')
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used     k1 = mct_aVect_indexRA(dom_b%data,'lon',dieWith=subname//'_domb')
! Subprogram not used     k2 = mct_aVect_indexRA(dom_b%data,'lat',dieWith=subname//'_domb')
! Subprogram not used     distmin = 1.0e36
! Subprogram not used     do k = 1,lsizeb
! Subprogram not used        x2 = mod(dom_b%data%rAttr(k1,k)+360.0_r8,360.0_r8)
! Subprogram not used        y2 = dom_b%data%rAttr(k2,k)
! Subprogram not used        dist = abs((x1-x2)*(x1-x2))+abs((y1-y2)*(y1-y2))
! Subprogram not used        if (dist < distmin) then
! Subprogram not used           distmin = dist
! Subprogram not used           noff = ipoints(k)
! Subprogram not used        endif
! Subprogram not used        dist = abs((x1-x2-360.0_r8)*(x1-x2-360.0_r8))+abs((y1-y2)*(y1-y2))
! Subprogram not used        if (dist < distmin) then
! Subprogram not used           distmin = dist
! Subprogram not used           noff = ipoints(k)
! Subprogram not used        endif
...
! Subprogram not used 
! Subprogram not used     deallocate(ipoints)
! Subprogram not used 
! Subprogram not used     call shr_mpi_min(distmin,distming,mpicom_i,'distmin',all=.true.)
! Subprogram not used 
! Subprogram not used     if (distming /= distmin) then
! Subprogram not used        noff = -1
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used     call shr_mpi_max(noff,noffg,mpicom_i,'noffg',all=.true.)
! Subprogram not used 
! Subprogram not used     ! subtract extra -1 and -nxcpl for point (2,2)
! Subprogram not used     xoff = mod(noffg-1-1,nxcpl) + 1
! Subprogram not used     yoff = (noffg-1-nxcpl)/nxcpl + 1
! Subprogram not used 
! Subprogram not used     if (iam == 0) then
! Subprogram not used        write(nu_diag,*) subname,' :: x1,y1  = ',x1,y1
! Subprogram not used        write(nu_diag,*) subname,' :: offset = ',noffg,xoff,yoff
! Subprogram not used        call shr_sys_flush(nu_diag)
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used     if (noffg < 1) then
! Subprogram not used        call shr_sys_abort(subname//' :: noffg lt 1')
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used   end subroutine ice_coffset_mct

  !=======================================================================

! Subprogram not used   subroutine ice_setcoupling_mct(mpicom_i, ICEID, gsmap_i, dom_i)
! Subprogram not used 
! Subprogram not used     implicit none
! Subprogram not used     include 'netcdf.inc'
! Subprogram not used 
! Subprogram not used     integer        , intent(in)    :: mpicom_i
! Subprogram not used     integer        , intent(in)    :: ICEID
! Subprogram not used     type(mct_gsmap), intent(inout) :: gsmap_i
! Subprogram not used     type(mct_ggrid), intent(inout) :: dom_i
! Subprogram not used 
! Subprogram not used     integer :: n     ! counter
! Subprogram not used     integer :: iam   ! pe rank
! Subprogram not used     integer :: npes  ! number of pes
! Subprogram not used     integer :: ierr  ! error code
! Subprogram not used     integer :: rcode ! error code
! Subprogram not used     integer :: nx,ny ! grid size
! Subprogram not used     integer :: gsize ! global size
! Subprogram not used     integer :: lsize ! local size
! Subprogram not used     integer, pointer :: start(:),length(:),pe_loc(:)
! Subprogram not used     integer, pointer :: idata(:)
! Subprogram not used     real(dbl_kind),pointer :: data(:)
! Subprogram not used     type(mct_avect) :: avg, av1
! Subprogram not used     integer :: fid,did,vid
! Subprogram not used     character(len=8) :: avfld,dofld
! Subprogram not used     character(len=*), parameter  :: SubName = "ice_setcoupling_mct"
! Subprogram not used 
! Subprogram not used     call MPI_comm_rank(mpicom_i,iam,ierr)
! Subprogram not used     call MPI_comm_size(mpicom_i,npes,ierr)
! Subprogram not used 
! Subprogram not used     allocate(start(npes),length(npes),pe_loc(npes))
! Subprogram not used 
! Subprogram not used     if (iam == 0) then
! Subprogram not used        rcode = nf_open(gridcpl_file(1:len_trim(gridcpl_file)),NF_NOWRITE,fid)
! Subprogram not used        rcode = nf_inq_dimid (fid, 'ni', did)
! Subprogram not used        rcode = nf_inq_dimlen(fid, did, nx)
! Subprogram not used        rcode = nf_inq_dimid (fid, 'nj', did)
! Subprogram not used        rcode = nf_inq_dimlen(fid, did, ny)
! Subprogram not used        gsize = nx*ny
! Subprogram not used        nxcpl = nx
! Subprogram not used        nycpl = ny
! Subprogram not used 
! Subprogram not used        length = gsize / npes
! Subprogram not used        do n = 1,npes
! Subprogram not used           if (n <= mod(gsize,npes)) length(n) = length(n) + 1
! Subprogram not used        enddo
! Subprogram not used 
...
! Subprogram not used        enddo
! Subprogram not used        if ((start(npes) + length(npes) - 1) /= gsize) then
! Subprogram not used           write(nu_diag,*) &
! Subprogram not used             subname,' gsize, start, length = ',gsize,start(npes),length(npes)
! Subprogram not used           call shr_sys_flush(nu_diag)
! Subprogram not used           call shr_sys_abort( SubName//":: decomp inconsistent")
! Subprogram not used        endif
! Subprogram not used 
! Subprogram not used        write(nu_diag,*) subname,' read ',trim(gridcpl_file)
! Subprogram not used        write(nu_diag,*) subname,' size ',nx,ny,gsize
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used     call shr_mpi_bcast(nxcpl,mpicom_i)
! Subprogram not used     call shr_mpi_bcast(nycpl,mpicom_i)
! Subprogram not used     call shr_mpi_bcast(gsize,mpicom_i)
! Subprogram not used     call mct_gsmap_init(gsmap_i,npes,start,length,pe_loc,0,mpicom_i,ICEID,gsize)
! Subprogram not used     deallocate(start,length,pe_loc)
! Subprogram not used 
! Subprogram not used     lsize = mct_gsmap_lsize(gsmap_i,mpicom_i)
! Subprogram not used     call mct_gGrid_init( GGrid=dom_i, CoordChars=trim(seq_flds_dom_coord), &
! Subprogram not used        OtherChars=trim(seq_flds_dom_other), lsize=lsize )
! Subprogram not used     call mct_aVect_zero(dom_i%data)
! Subprogram not used 
! Subprogram not used     ! Determine global gridpoint number attribute, GlobGridNum, which is set automatically by MCT
! Subprogram not used 
! Subprogram not used     call mct_gsMap_orderedPoints(gsMap_i, my_task, idata)
! Subprogram not used     call mct_gGrid_importIAttr(dom_i,'GlobGridNum',idata,lsize)
! Subprogram not used     deallocate(idata)
! Subprogram not used 
! Subprogram not used     ! Initialize attribute vector with special value
! Subprogram not used 
! Subprogram not used     allocate(data(lsize))
! Subprogram not used     data(:) = -9999.0_R8 
! Subprogram not used     call mct_gGrid_importRAttr(dom_i,"lat"  ,data,lsize) 
! Subprogram not used     call mct_gGrid_importRAttr(dom_i,"lon"  ,data,lsize) 
! Subprogram not used     call mct_gGrid_importRAttr(dom_i,"area" ,data,lsize) 
! Subprogram not used     call mct_gGrid_importRAttr(dom_i,"aream",data,lsize) 
! Subprogram not used     data(:) = 0.0_R8     
! Subprogram not used     call mct_gGrid_importRAttr(dom_i,"mask",data,lsize) 
! Subprogram not used     call mct_gGrid_importRAttr(dom_i,"frac",data,lsize) 
! Subprogram not used     deallocate(data)
! Subprogram not used 
! Subprogram not used     ! Read domain arrays
! Subprogram not used 
! Subprogram not used     if (iam == 0) then
! Subprogram not used        call mct_avect_init(avg,rList='fld',lsize=gsize)
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used     do n = 1,5
! Subprogram not used 
! Subprogram not used        if (n == 1) avfld = 'lat'
! Subprogram not used        if (n == 1) dofld = 'yc'
! Subprogram not used        if (n == 2) avfld = 'lon'
! Subprogram not used        if (n == 2) dofld = 'xc'
! Subprogram not used        if (n == 3) avfld = 'area'
! Subprogram not used        if (n == 3) dofld = 'area'
! Subprogram not used        if (n == 4) avfld = 'frac'
! Subprogram not used        if (n == 4) dofld = 'frac'
! Subprogram not used        if (n == 5) avfld = 'mask'
! Subprogram not used        if (n == 5) dofld = 'mask'
! Subprogram not used        if (iam == 0) then
! Subprogram not used           rcode = nf_inq_varid(fid,trim(dofld),vid)
! Subprogram not used           if (n == 5) then
! Subprogram not used              allocate(idata(gsize))
! Subprogram not used              rcode = nf_get_var_int(fid,vid,idata)
! Subprogram not used              avg%rAttr(1,:) = idata
! Subprogram not used              deallocate(idata)
! Subprogram not used           else
! Subprogram not used              rcode = nf_get_var_double(fid,vid,avg%rAttr(1,:))
! Subprogram not used           endif
! Subprogram not used        endif
! Subprogram not used 
! Subprogram not used        call mct_aVect_scatter(avg,av1,gsmap_i,0,mpicom_i)
! Subprogram not used        call mct_aVect_copy(av1,dom_i%data,'fld',avfld)
! Subprogram not used 
! Subprogram not used        if (iam == 0) then
! Subprogram not used           call mct_avect_clean(av1)
! Subprogram not used        endif
! Subprogram not used 
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used     if (iam == 0) then
! Subprogram not used        call mct_avect_clean(avg)
! Subprogram not used     endif
! Subprogram not used 
! Subprogram not used   end subroutine ice_setcoupling_mct
!=======================================================================
! BOP
!
...
!
! !INTERFACE:
! Subprogram not used   character(len=char_len_long) function restart_filename( yr_spec, mon_spec, day_spec, sec_spec )
! Subprogram not used !
! Subprogram not used ! !DESCRIPTION: 
! Subprogram not used ! Create a restart filename 
! Subprogram not used !
! Subprogram not used ! !USES:
! Subprogram not used   use ice_restart, only : restart_file
! Subprogram not used !
! Subprogram not used ! !INPUT/OUTPUT PARAMETERS:
! Subprogram not used   integer         , intent(in)  :: yr_spec         ! Simulation year
! Subprogram not used   integer         , intent(in)  :: mon_spec        ! Simulation month
! Subprogram not used   integer         , intent(in)  :: day_spec        ! Simulation day
! Subprogram not used   integer         , intent(in)  :: sec_spec        ! Seconds into current simulation day
! Subprogram not used !
! Subprogram not used ! EOP
! Subprogram not used !
! Subprogram not used   integer             :: i, n      ! Loop variables
! Subprogram not used   integer             :: year      ! Simulation year
! Subprogram not used   integer             :: month     ! Simulation month
! Subprogram not used   integer             :: day       ! Simulation day
! Subprogram not used   integer             :: ncsec     ! Seconds into current simulation day
! Subprogram not used   character(len=char_len_long) :: rdate ! char date for restart filename
! Subprogram not used 
! Subprogram not used   !-----------------------------------------------------------------
! Subprogram not used   ! Determine year, month, day and sec to put in filename
! Subprogram not used   !-----------------------------------------------------------------
! Subprogram not used 
! Subprogram not used   year  = yr_spec
! Subprogram not used   month = mon_spec
! Subprogram not used   day   = day_spec
! Subprogram not used   ncsec = sec_spec
! Subprogram not used 
! Subprogram not used   if ( year > 99999   ) then
! Subprogram not used      write(rdate,'(i6.6,"-",i2.2,"-",i2.2,"-",i5.5)') year,month,mday,ncsec
! Subprogram not used   else if ( year > 9999    ) then
! Subprogram not used      write(rdate,'(i5.5,"-",i2.2,"-",i2.2,"-",i5.5)') year,month,mday,ncsec
! Subprogram not used   else
! Subprogram not used      write(rdate,'(i4.4,"-",i2.2,"-",i2.2,"-",i5.5)') year,month,mday,ncsec
! Subprogram not used   end if
! Subprogram not used   restart_filename = trim(restart_file) // "." // trim(rdate) 
! Subprogram not used 
! Subprogram not used end function restart_filename
