...
102 FORMAT('chlorine_loading_advance: not able to find data for : ',i4.4,'-',i2.2,'-',i2.2,'-',i5.5)
  ...
  ! private methods
  !---------------------------------------------------------------------------
! Subprogram not used   subroutine convert_dates( dates, secs, times )
! Subprogram not used 
! Subprogram not used     use time_manager, only: set_time_float_from_date
! Subprogram not used 
! Subprogram not used     integer,  intent(in)  :: dates(:)
! Subprogram not used     integer,  intent(in)  :: secs(:)
! Subprogram not used 
! Subprogram not used     real(r8), intent(out) :: times(:)
! Subprogram not used 
! Subprogram not used     integer :: year, month, day, sec,n ,i
! Subprogram not used 
! Subprogram not used     n = size( dates ) 
! Subprogram not used 
! Subprogram not used     do i=1,n
! Subprogram not used        year = dates(i)/10000
! Subprogram not used        month = (dates(i)-year*10000)/100
! Subprogram not used        day = dates(i)-year*10000-month*100
! Subprogram not used        sec = secs(i)
! Subprogram not used        call set_time_float_from_date( times(i), year, month, day, sec )
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used   end subroutine convert_dates

  !---------------------------------------------------------------------------
  !---------------------------------------------------------------------------
! Subprogram not used   subroutine convert_date( date, sec, time )
! Subprogram not used 
! Subprogram not used     integer,  intent(in)  :: date
! Subprogram not used     integer,  intent(in)  :: sec
! Subprogram not used     real(r8), intent(out) :: time
! Subprogram not used 
! Subprogram not used     integer :: dates(1), secs(1)
! Subprogram not used     real(r8) :: times(1)
! Subprogram not used     dates(1) = date
! Subprogram not used     secs(1) = sec
! Subprogram not used     call convert_dates( dates, secs, times )
! Subprogram not used     time = times(1)
! Subprogram not used   end subroutine convert_date

!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
! Subprogram not used   subroutine get_model_time( time, year, month, day, seconds )
! Subprogram not used 
! Subprogram not used     use time_manager, only: get_curr_date
! Subprogram not used 
! Subprogram not used     real(r8), intent(out) :: time
! Subprogram not used     integer, optional, intent(out) :: year, month, day, seconds
! Subprogram not used 
! Subprogram not used     integer  :: yr, mn, dy, sc, date
! Subprogram not used 
! Subprogram not used     call get_curr_date(yr, mn, dy, sc)
! Subprogram not used     date = yr*10000 + mn*100 + dy
! Subprogram not used     call convert_date( date, sc, time )
! Subprogram not used 
! Subprogram not used     if (present(year))    year = yr
! Subprogram not used     if (present(month))   month = mn
! Subprogram not used     if (present(day))     day = dy
! Subprogram not used     if (present(seconds)) seconds = sc
! Subprogram not used 
