...
! Purpose:
!
! Provides a control interface to 1 physics packages
!
! Revision history:
...
! Namelist variables:
character(len=16) :: cam_physpkg          = unset_str  ! 1 physics package [cam3 | cam4 | cam5 |
                                                       !   ideal | adiabatic].
character(len=32) :: cam_chempkg          = unset_str  ! 1 chemistry package [waccm_mozart | 
...
subroutine phys_ctl_readnl(nlfile)
   ...
   namelist /phys_ctl_nl/ cam_physpkg, cam_chempkg, waccmx_opt, deep_scheme, shallow_scheme, &
      ...
      do_clubb_sgs, do_tms, state_debug_checks, use_gw_oro, use_gw_front, &
      use_gw_convect
   !-----------------------------------------------------------------------------
