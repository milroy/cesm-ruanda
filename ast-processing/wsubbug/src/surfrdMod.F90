...
!
! !INTERFACE:
! Subprogram not used   subroutine surfrd_get_topo(domain,filename)
! Subprogram not used !
! Subprogram not used ! !DESCRIPTION:
! Subprogram not used ! Read the topo dataset grid related information:
! Subprogram not used ! Assume domain has already been initialized and read
! Subprogram not used !
! Subprogram not used ! !USES:
! Subprogram not used     use domainMod , only : domain_type
! Subprogram not used     use fileutils , only : getfil
! Subprogram not used !
! Subprogram not used ! !ARGUMENTS:
! Subprogram not used     implicit none
! Subprogram not used     type(domain_type),intent(inout) :: domain   ! domain to init
! Subprogram not used     character(len=*) ,intent(in)    :: filename ! grid filename
! Subprogram not used !
! Subprogram not used ! !CALLED FROM:
! Subprogram not used ! subroutine initialize
! Subprogram not used !
! Subprogram not used ! !REVISION HISTORY:
! Subprogram not used ! Created by T Craig
! Subprogram not used !
! Subprogram not used !
! Subprogram not used ! !LOCAL VARIABLES:
! Subprogram not used !EOP
! Subprogram not used     type(file_desc_t) :: ncid      ! netcdf file id
! Subprogram not used     integer :: n                   ! indices
! Subprogram not used     integer :: ni,nj,ns            ! size of grid on file
! Subprogram not used     integer :: dimid,varid         ! netCDF id's
! Subprogram not used     integer :: ier                 ! error status
! Subprogram not used     real(r8):: eps = 1.0e-12_r8             ! lat/lon error tolerance
! Subprogram not used     integer :: beg,end                      ! local beg,end indices
! Subprogram not used     logical             :: isgrid2d         ! true => file is 2d lat/lon
! Subprogram not used     real(r8),pointer    :: lonc(:),latc(:)  ! local lat/lon
! Subprogram not used     character(len=256)  :: locfn            ! local file name
! Subprogram not used     logical :: readvar                      ! is variable on file
! Subprogram not used     character(len=32) :: subname = 'surfrd_get_topo'     ! subroutine name
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used     if (masterproc) then
! Subprogram not used        if (filename == ' ') then
! Subprogram not used           write(iulog,*) trim(subname),' ERROR: filename must be specified '
! Subprogram not used           call endrun()
! Subprogram not used        else
! Subprogram not used           write(iulog,*) 'Attempting to read lnd topo from flndtopo ',trim(filename)
! Subprogram not used        endif
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used     call getfil( filename, locfn, 0 )
! Subprogram not used     call ncd_pio_openfile (ncid, trim(locfn), 0)
! Subprogram not used     call ncd_inqfdims(ncid, isgrid2d, ni, nj, ns)
! Subprogram not used 
! Subprogram not used     if (domain%ns /= ns) then
! Subprogram not used        write(iulog,*) trim(subname),' ERROR: topo file mismatch ns',&
! Subprogram not used             domain%ns,ns
! Subprogram not used        call endrun()
! Subprogram not used     endif
! Subprogram not used     
! Subprogram not used     beg = domain%nbeg
! Subprogram not used     end = domain%nend
! Subprogram not used 
! Subprogram not used     allocate(latc(beg:end),lonc(beg:end))
! Subprogram not used 
! Subprogram not used     call ncd_io(ncid=ncid, varname='LONGXY', flag='read', data=lonc, &
! Subprogram not used          dim1name=grlnd, readvar=readvar)
! Subprogram not used     if (.not. readvar) call endrun( trim(subname)//' ERROR: LONGXY  NOT on topodata file' )
! Subprogram not used 
! Subprogram not used     call ncd_io(ncid=ncid, varname='LATIXY', flag='read', data=latc, &
! Subprogram not used          dim1name=grlnd, readvar=readvar)
! Subprogram not used     if (.not. readvar) call endrun( trim(subname)//' ERROR: LONGXY  NOT on topodata file' )
! Subprogram not used 
! Subprogram not used     do n = beg,end
! Subprogram not used        if (abs(latc(n)-domain%latc(n)) > eps .or. &
! Subprogram not used            abs(lonc(n)-domain%lonc(n)) > eps) then
! Subprogram not used           write(iulog,*) trim(subname),' ERROR: topo file mismatch lat,lon',latc(n),&
! Subprogram not used                domain%latc(n),lonc(n),domain%lonc(n),eps
! Subprogram not used           call endrun()
! Subprogram not used        endif
! Subprogram not used     enddo
! Subprogram not used 
! Subprogram not used     call ncd_io(ncid=ncid, varname='TOPO', flag='read', data=domain%topo, &
! Subprogram not used          dim1name=grlnd, readvar=readvar)
! Subprogram not used     if (.not. readvar) call endrun( trim(subname)//' ERROR: LONGXY  NOT on topodata file' )
! Subprogram not used 
! Subprogram not used     deallocate(latc,lonc)
! Subprogram not used 
! Subprogram not used     call ncd_pio_closefile(ncid)
! Subprogram not used 
! Subprogram not used   end subroutine surfrd_get_topo
...
!
! !INTERFACE:
! Subprogram not used   subroutine surfrd_wtxy_veg_dgvm()
! Subprogram not used !
! Subprogram not used ! !DESCRIPTION:
! Subprogram not used ! Determine wtxy and vegxy for CNDV mode.
! Subprogram not used !
! Subprogram not used ! !USES:
! Subprogram not used     use pftvarcon , only : noveg, crop
! Subprogram not used     use clm_varctl, only : create_crop_landunit
! Subprogram not used !
! Subprogram not used ! !ARGUMENTS:
! Subprogram not used     implicit none
! Subprogram not used !
! Subprogram not used ! !CALLED FROM:
! Subprogram not used ! subroutine surfrd in this module
! Subprogram not used !
! Subprogram not used !
! Subprogram not used ! !REVISION HISTORY:
! Subprogram not used ! Created by Mariana Vertenstein 12/04
! Subprogram not used !
! Subprogram not used !
! Subprogram not used ! !LOCAL VARIABLES:
! Subprogram not used !EOP
! Subprogram not used     integer :: m,nl         ! indices
! Subprogram not used     integer  :: begg,endg   ! beg/end gcell index
! Subprogram not used !-----------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used     call get_proc_bounds(begg,endg)
! Subprogram not used     do nl = begg,endg
! Subprogram not used        do m = 1, maxpatch_pft ! CNDV means allocate_all_vegpfts = .true.
! Subprogram not used           if (create_crop_landunit) then ! been through surfrd_wtxy_veg_all
! Subprogram not used              if (crop(m-1) == 0) then    ! so update natural vegetation only
! Subprogram not used                 wtxy(nl,m) = 0._r8       ! crops should have values >= 0.
! Subprogram not used              end if
! Subprogram not used           else                   ! not been through surfrd_wtxy_veg_all
! Subprogram not used              wtxy(nl,m) = 0._r8  ! so update all vegetation
! Subprogram not used              vegxy(nl,m) = m - 1 ! 0 (bare ground) to maxpatch_pft-1 (= 16)
! Subprogram not used           end if
! Subprogram not used        end do
! Subprogram not used        ! bare ground weights
! Subprogram not used        wtxy(nl,noveg+1) = max(0._r8, 1._r8 - sum(wtxy(nl,:)))
! Subprogram not used        if (abs(sum(wtxy(nl,:)) - 1._r8) > 1.e-5_r8) then
! Subprogram not used           write(iulog,*) 'all wtxy =', wtxy(nl,:)
! Subprogram not used           call endrun()
! Subprogram not used        end if ! error check
! Subprogram not used     end do
! Subprogram not used 
! Subprogram not used   end subroutine surfrd_wtxy_veg_dgvm
