...
!--------------------------------------------------------------------------
! 1 and WACCM gravity wave parameterizations were merged by Sean Patrick
! Santos in Summer 2013, and at the same time, gw_drag was split into
! various modules. This is the 1 interface and driver module. The below
! notes are for the old 1 and WACCM versions of gw_drag.
!--------------------------------------------------------------------------
...
!
  ...
  ! the readnl phase and the init phase of the 1 physics; only gw_common
...
! Subprogram not used subroutine gw_init_beres(mfcc)
! Subprogram not used 
...
! Subprogram not used 
! Subprogram not used   real(r8), intent(out) :: mfcc(maxh,-maxuh:maxuh,-pgwv:pgwv)
! Subprogram not used 
! Subprogram not used   ! VARIABLES NEEDED TO READ IN TABLE OF SPECTRA FROM FILE
! Subprogram not used 
! Subprogram not used   ! netCDF file id, variable id, and error code.
! Subprogram not used   integer :: ncid, varid, ncstat
! Subprogram not used   ! The number of gravity waves in the file (i.e. the file has a gravity
! Subprogram not used   ! wave spectrum from -ngwv_file to ngwv_file). This should be determined
! Subprogram not used   ! from the file itself, rather than hard-coded.
! Subprogram not used   integer, parameter :: ngwv_file = 40
! Subprogram not used   character(len=256) :: gw_drag_file_loc ! local filepath of gw_drag_file
! Subprogram not used 
! Subprogram not used   !----------------------------------------------------------------------
! Subprogram not used   ! read in look-up table for source spectra
! Subprogram not used   !-----------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used   if (masterproc) then
! Subprogram not used 
! Subprogram not used      call getfil(gw_drag_file, gw_drag_file_loc)
! Subprogram not used      ncstat = NF90_OPEN (gw_drag_file_loc,0,ncid)
! Subprogram not used 
! Subprogram not used      if (ncstat .ne. 0) then
! Subprogram not used         write(iulog,*) 'Error reading in netcdf file ',gw_drag_file,'.  ',&
! Subprogram not used              NF90_STRERROR(ncstat)
! Subprogram not used         write(iulog,*) 'Check that the namelist variable gw_drag_file is &
! Subprogram not used              &correct.'
! Subprogram not used         call endrun
! Subprogram not used      endif
! Subprogram not used 
! Subprogram not used      ncstat = NF90_INQ_VARID (ncid,'mfcc',varid)
! Subprogram not used 
! Subprogram not used      if (ncstat .ne. 0) then
! Subprogram not used         write(iulog,*) 'Error reading data from ',gw_drag_file,'.  ',&
! Subprogram not used              NF90_STRERROR(ncstat)
! Subprogram not used         call endrun
! Subprogram not used      endif
! Subprogram not used 
! Subprogram not used      ncstat = NF90_GET_VAR(ncid,varid,mfcc,start=[1,1,ngwv_file-pgwv+1])
! Subprogram not used      ncstat = NF90_CLOSE(ncid)
! Subprogram not used 
! Subprogram not used      write(iulog,*) 'Read-in source spectra from file'
! Subprogram not used      write(iulog,*) 'MFCC=',maxval(mfcc),minval(mfcc)
! Subprogram not used 
! Subprogram not used   endif
! Subprogram not used   !     Broadcast results
! Subprogram not used 
! Subprogram not used   call mpibcast (mfcc, maxh*(2*pgwv+1)*(2*maxuh+1),mpir8,0,mpicom)
! Subprogram not used 
! Subprogram not used 
! Subprogram not used end subroutine gw_init_beres
...
! Subprogram not used subroutine gw_spec_addflds(prefix, scheme, history_waccm)
! Subprogram not used   use cam_history, only: addfld, add_default, phys_decomp
! Subprogram not used   use gw_common,   only: cref
! Subprogram not used 
! Subprogram not used   !------------------------------Arguments--------------------------------
! Subprogram not used 
! Subprogram not used   ! One character prefix prepended to output fields.
! Subprogram not used   character(len=1), intent(in) :: prefix
! Subprogram not used   ! Gravity wave scheme name prepended to output field descriptions.
! Subprogram not used   character(len=*), intent(in) :: scheme
! Subprogram not used   ! Whether or not to call add_default for fields output by WACCM.
! Subprogram not used   logical, intent(in) :: history_waccm
! Subprogram not used 
! Subprogram not used   !---------------------------Local storage-------------------------------
! Subprogram not used 
! Subprogram not used   integer :: l
! Subprogram not used   ! 7 chars is enough for "-100.00"
! Subprogram not used   character(len=7)  :: fnum
! Subprogram not used   ! 10 chars is enough for "BTAUXSn32"
! Subprogram not used   character(len=10) :: dumc1x, dumc1y
! Subprogram not used   ! Allow 80 chars for description
! Subprogram not used   character(len=80) dumc2
! Subprogram not used 
! Subprogram not used   !-----------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used   ! Overall wind tendencies.
! Subprogram not used   call addfld (trim(prefix)//'UTGWSPEC','m/s2',pver, 'A', &
! Subprogram not used        trim(scheme)//' U tendency - gravity wave spectrum',  phys_decomp)
! Subprogram not used   call addfld (trim(prefix)//'VTGWSPEC','m/s2',pver, 'A', &
! Subprogram not used        trim(scheme)//' V tendency - gravity wave spectrum',  phys_decomp)
! Subprogram not used 
! Subprogram not used   ! Wind tendencies broken across five spectral bins.
! Subprogram not used   call addfld (trim(prefix)//'UTEND1','m/s2',  pver, 'A', &
! Subprogram not used        trim(scheme)//' U tendency   c < -40',                phys_decomp)
! Subprogram not used   call addfld (trim(prefix)//'UTEND2','m/s2',  pver, 'A', &
! Subprogram not used        trim(scheme)//' U tendency  -40 < c < -15',           phys_decomp)
! Subprogram not used   call addfld (trim(prefix)//'UTEND3','m/s2',  pver, 'A', &
! Subprogram not used        trim(scheme)//' U tendency  -15 < c <  15',           phys_decomp)
! Subprogram not used   call addfld (trim(prefix)//'UTEND4','m/s2',  pver, 'A', &
! Subprogram not used        trim(scheme)//' U tendency   15 < c <  40',           phys_decomp)
! Subprogram not used   call addfld (trim(prefix)//'UTEND5','m/s2',  pver, 'A', &
! Subprogram not used        trim(scheme)//' U tendency   40 < c ',                phys_decomp)
! Subprogram not used 
! Subprogram not used   ! Reynold's stress toward each cardinal direction, and net zonal stress.
! Subprogram not used   call addfld (trim(prefix)//'TAUE' ,'Pa',   pver+1, 'A', &
! Subprogram not used        trim(scheme)//' Eastward Reynolds stress',            phys_decomp)
! Subprogram not used   call addfld (trim(prefix)//'TAUW' ,'Pa',   pver+1, 'A', &
! Subprogram not used        trim(scheme)//' Westward Reynolds stress',            phys_decomp)
! Subprogram not used   call addfld (trim(prefix)//'TAUNET' ,'Pa', pver+1, 'A', &
! Subprogram not used        trim(scheme)//' E+W Reynolds stress',                 phys_decomp)
! Subprogram not used   call addfld (trim(prefix)//'TAUN' ,'Pa',   pver+1, 'A', &
! Subprogram not used        trim(scheme)//' Northward Reynolds stress',           phys_decomp)
! Subprogram not used   call addfld (trim(prefix)//'TAUS' ,'Pa',   pver+1, 'A', &
! Subprogram not used        trim(scheme)//' Southward Reynolds stress',           phys_decomp)
! Subprogram not used 
! Subprogram not used   ! Momentum flux in each direction.
! Subprogram not used   call addfld (trim(prefix)//'EMF','Pa',       pver, 'A', &
! Subprogram not used        trim(scheme)//' Eastward MF',                         phys_decomp)
! Subprogram not used   call addfld (trim(prefix)//'WMF','Pa',       pver, 'A', &
! Subprogram not used        trim(scheme)//' Westward MF',                         phys_decomp)
! Subprogram not used   call addfld (trim(prefix)//'NMF','Pa',       pver, 'A', &
! Subprogram not used        trim(scheme)//' Northward MF',                        phys_decomp)
! Subprogram not used   call addfld (trim(prefix)//'SMF','Pa',       pver, 'A', &
! Subprogram not used        trim(scheme)//' Southward MF',                        phys_decomp)
! Subprogram not used 
! Subprogram not used   ! Temperature tendency terms.
! Subprogram not used   call addfld (trim(prefix)//'TTGWSDF' ,'K/s', pver, 'A', &
! Subprogram not used        trim(scheme)//' t tendency - diffusion term',         phys_decomp)
! Subprogram not used   call addfld (trim(prefix)//'TTGWSKE' ,'K/s', pver, 'A', &
! Subprogram not used        trim(scheme)//' t tendency - kinetic energy conversion term', &
! Subprogram not used        phys_decomp)
! Subprogram not used 
! Subprogram not used   ! Gravity wave source spectra by wave number.
! Subprogram not used   do l=-pgwv,pgwv
! Subprogram not used      ! String containing reference speed.
! Subprogram not used      write (fnum,fmt='(f7.2)') cref(l)
! Subprogram not used 
! Subprogram not used      dumc1x = tau_fld_name(l, prefix, x_not_y=.true.)
! Subprogram not used      dumc1y = tau_fld_name(l, prefix, x_not_y=.false.)
! Subprogram not used      dumc2 = trim(scheme)//" tau at c= "//trim(fnum)//" m/s"
! Subprogram not used      call addfld (trim(dumc1x),'Pa   ',pver, 'A',dumc2,phys_decomp)
! Subprogram not used      call addfld (trim(dumc1y),'Pa   ',pver, 'A',dumc2,phys_decomp)
! Subprogram not used 
! Subprogram not used   end do
! Subprogram not used 
! Subprogram not used   if (history_waccm) then
! Subprogram not used      call add_default(trim(prefix)//'UTGWSPEC', 1, ' ')
! Subprogram not used      call add_default(trim(prefix)//'VTGWSPEC', 1, ' ')
! Subprogram not used 
! Subprogram not used      call add_default(trim(prefix)//'UTEND1', 1, ' ')
! Subprogram not used      call add_default(trim(prefix)//'UTEND2', 1, ' ')
! Subprogram not used      call add_default(trim(prefix)//'UTEND3', 1, ' ')
! Subprogram not used      call add_default(trim(prefix)//'UTEND4', 1, ' ')
! Subprogram not used      call add_default(trim(prefix)//'UTEND5', 1, ' ')
! Subprogram not used 
! Subprogram not used      call add_default(trim(prefix)//'TAUE', 1, ' ')
! Subprogram not used      call add_default(trim(prefix)//'TAUW', 1, ' ')
! Subprogram not used      call add_default(trim(prefix)//'TAUNET', 1, ' ')
! Subprogram not used      call add_default(trim(prefix)//'TAUN', 1, ' ')
! Subprogram not used      call add_default(trim(prefix)//'TAUS', 1, ' ')
! Subprogram not used   end if
! Subprogram not used 
! Subprogram not used end subroutine gw_spec_addflds

!==========================================================================

! Outputs for spectral waves.
! Subprogram not used subroutine gw_spec_outflds(prefix, lchnk, ncol, ngwv, c, u, v, xv, yv, &
! Subprogram not used      gwut, dttdf, dttke, tau, utgw, vtgw, taucd)
! Subprogram not used 
! Subprogram not used   use gw_common, only: west, east, south, north
! Subprogram not used 
! Subprogram not used   ! One-character prefix prepended to output fields.
! Subprogram not used   character(len=1), intent(in) :: prefix
! Subprogram not used   ! Chunk and number of columns in the chunk.
! Subprogram not used   integer, intent(in) :: lchnk
! Subprogram not used   integer, intent(in) :: ncol
! Subprogram not used   ! Max (+/-) wavenumber in gravity wave spectrum.
! Subprogram not used   integer, intent(in) :: ngwv
! Subprogram not used   ! Wave phase speeds for each column.
! Subprogram not used   real(r8), intent(in) :: c(ncol,-pgwv:pgwv)
! Subprogram not used   ! Winds at cell midpoints.
! Subprogram not used   real(r8), intent(in) :: u(ncol,pver)
! Subprogram not used   real(r8), intent(in) :: v(ncol,pver)
! Subprogram not used   ! Unit vector in the direction of wind at source level.
! Subprogram not used   real(r8), intent(in) :: xv(ncol)
! Subprogram not used   real(r8), intent(in) :: yv(ncol)
! Subprogram not used   ! Wind tendency for each wave.
! Subprogram not used   real(r8), intent(in) :: gwut(ncol,pver,-ngwv:ngwv)
! Subprogram not used   ! Temperature tendencies from diffusion and kinetic energy.
! Subprogram not used   real(r8) :: dttdf(ncol,pver)
! Subprogram not used   real(r8) :: dttke(ncol,pver)
! Subprogram not used   ! Wave Reynolds stress.
! Subprogram not used   real(r8), intent(in) :: tau(ncol,-pgwv:pgwv,pver)
! Subprogram not used   ! Zonal and meridional total wind tendencies.
! Subprogram not used   real(r8), intent(in) :: utgw(ncol,pver)
! Subprogram not used   real(r8), intent(in) :: vtgw(ncol,pver)
! Subprogram not used   ! Reynolds stress for waves propagating in each cardinal direction.
! Subprogram not used   real(r8), intent(in) :: taucd(ncol,0:pver,4)
! Subprogram not used 
! Subprogram not used   ! Indices
! Subprogram not used   integer :: i, k, l
! Subprogram not used   integer :: ix(ncol, -ngwv:ngwv), iy(ncol, -ngwv:ngwv)
! Subprogram not used   integer :: iu(ncol), iv(ncol)
! Subprogram not used 
! Subprogram not used   ! Zonal wind tendency, broken up into five bins.
! Subprogram not used   real(r8) :: utb(ncol, pver, 5)
! Subprogram not used   ! Definition of the bin boundaries.
! Subprogram not used   real(r8), parameter :: bounds(4) = (/ -40._r8, -15._r8, &
! Subprogram not used        15._r8, 40._r8 /)
! Subprogram not used 
! Subprogram not used   ! Momentum flux in the four cardinal directions.
! Subprogram not used   real(r8) :: mf(ncol, pver, 4)
! Subprogram not used 
! Subprogram not used   ! Wave stress in zonal/meridional direction
! Subprogram not used   real(r8) :: taux(ncol,-ngwv:ngwv,pver)
! Subprogram not used   real(r8) :: tauy(ncol,-ngwv:ngwv,pver)
! Subprogram not used 
! Subprogram not used   ! Temporaries for output
! Subprogram not used   real(r8) :: dummyx(ncol,pver)
! Subprogram not used   real(r8) :: dummyy(ncol,pver)
! Subprogram not used   ! Variable names
! Subprogram not used   character(len=10) :: dumc1x, dumc1y
! Subprogram not used 
! Subprogram not used 
! Subprogram not used   ! Accumulate wind tendencies binned according to phase speed.
! Subprogram not used 
! Subprogram not used   utb = 0._r8
! Subprogram not used 
! Subprogram not used   ! Find which output bin the phase speed corresponds to.
! Subprogram not used   ix = find_bin(c(:, -ngwv:ngwv))
! Subprogram not used 
! Subprogram not used   ! Put the wind tendency in that bin.
! Subprogram not used   do l = -ngwv, ngwv
! Subprogram not used      do k = 1, pver
! Subprogram not used         do i = 1, ncol
! Subprogram not used            utb(i,k,ix(i,l)) = utb(i,k,ix(i,l)) + gwut(i,k,l)
! Subprogram not used         end do
! Subprogram not used      end do
! Subprogram not used   end do
! Subprogram not used 
! Subprogram not used   ! Find just the zonal part.
! Subprogram not used   do l = 1, 5
! Subprogram not used      do k = 1, pver
! Subprogram not used         utb(:, k, l) = utb(:, k, l) * xv
! Subprogram not used      end do
! Subprogram not used   end do
! Subprogram not used 
! Subprogram not used   call outfld(trim(prefix)//'UTEND1', utb(:,:,1), ncol, lchnk)
! Subprogram not used   call outfld(trim(prefix)//'UTEND2', utb(:,:,2), ncol, lchnk)
! Subprogram not used   call outfld(trim(prefix)//'UTEND3', utb(:,:,3), ncol, lchnk)
! Subprogram not used   call outfld(trim(prefix)//'UTEND4', utb(:,:,4), ncol, lchnk)
! Subprogram not used   call outfld(trim(prefix)//'UTEND5', utb(:,:,5), ncol, lchnk)
! Subprogram not used 
! Subprogram not used   ! Output temperature tendencies due to diffusion and from kinetic energy.
! Subprogram not used   call outfld(trim(prefix)//'TTGWSDF', dttdf / cpair, ncol, lchnk)
! Subprogram not used   call outfld(trim(prefix)//'TTGWSKE', dttke / cpair, ncol, lchnk)
! Subprogram not used 
! Subprogram not used 
! Subprogram not used   ! Output tau broken down into zonal and meridional components.
! Subprogram not used 
! Subprogram not used   taux = 0._r8
! Subprogram not used   tauy = 0._r8
! Subprogram not used 
! Subprogram not used   ! Project c, and convert each component to a wavenumber index.
! Subprogram not used   ! These are mappings from the wavenumber index of tau to those of taux
! Subprogram not used   ! and tauy, respectively.
! Subprogram not used   do l=-ngwv,ngwv
! Subprogram not used      ix(:,l) = c_to_l(c(:,l)*xv)
! Subprogram not used      iy(:,l) = c_to_l(c(:,l)*yv)
! Subprogram not used   end do
! Subprogram not used 
! Subprogram not used   ! Find projection of tau.
! Subprogram not used   do k = 1, pver
! Subprogram not used      do l = -ngwv,ngwv
! Subprogram not used         do i = 1, ncol
! Subprogram not used            taux(i,ix(i,l),k) = taux(i,ix(i,l),k) &
! Subprogram not used                 + abs(tau(i,l,k)*xv(i))
! Subprogram not used            tauy(i,iy(i,l),k) = tauy(i,iy(i,l),k) &
! Subprogram not used                 + abs(tau(i,l,k)*yv(i))
! Subprogram not used         end do
! Subprogram not used      end do
! Subprogram not used   end do
! Subprogram not used 
! Subprogram not used   do l=-ngwv,ngwv
! Subprogram not used 
! Subprogram not used      dummyx = taux(:,l,:)
! Subprogram not used      dummyy = tauy(:,l,:)
! Subprogram not used 
! Subprogram not used      dumc1x = tau_fld_name(l, prefix, x_not_y=.true.)
! Subprogram not used      dumc1y = tau_fld_name(l, prefix, x_not_y=.false.)
! Subprogram not used 
! Subprogram not used      call outfld(dumc1x,dummyx,ncol,lchnk)
! Subprogram not used      call outfld(dumc1y,dummyy,ncol,lchnk)
! Subprogram not used 
! Subprogram not used   enddo
! Subprogram not used 
! Subprogram not used 
! Subprogram not used   ! Output momentum flux in each cardinal direction.
! Subprogram not used   mf = 0._r8
! Subprogram not used 
! Subprogram not used   do k = 1, pver
! Subprogram not used 
! Subprogram not used      ! Convert wind speed components to wavenumber indices.
! Subprogram not used      iu = c_to_l(u(:,k))
! Subprogram not used      iv = c_to_l(v(:,k))
! Subprogram not used 
! Subprogram not used      ! Sum tau components in each cardinal direction.
! Subprogram not used      ! Split west/east and north/south based on whether wave speed exceeds
! Subprogram not used      ! wind speed.
! Subprogram not used      do l = -ngwv, ngwv
! Subprogram not used 
! Subprogram not used         where (iu > l)
! Subprogram not used            mf(:,k,west) = mf(:,k,west) + taux(:,l,k)
! Subprogram not used         elsewhere
! Subprogram not used            mf(:,k,east) = mf(:,k,east) + taux(:,l,k)
! Subprogram not used         end where
! Subprogram not used 
! Subprogram not used         where (iv > l)
! Subprogram not used            mf(:,k,south) = mf(:,k,south) + tauy(:,l,k)
! Subprogram not used         elsewhere
! Subprogram not used            mf(:,k,north) = mf(:,k,north) + tauy(:,l,k)
! Subprogram not used         end where
! Subprogram not used 
! Subprogram not used      end do
! Subprogram not used 
! Subprogram not used   end do
! Subprogram not used 
! Subprogram not used   call outfld(trim(prefix)//'WMF',mf(:,:,west),ncol,lchnk)
! Subprogram not used   call outfld(trim(prefix)//'EMF',mf(:,:,east),ncol,lchnk)
! Subprogram not used   call outfld(trim(prefix)//'SMF',mf(:,:,south),ncol,lchnk)
! Subprogram not used   call outfld(trim(prefix)//'NMF',mf(:,:,north),ncol,lchnk)
! Subprogram not used 
! Subprogram not used   ! Simple output fields written to history file.
! Subprogram not used   ! Total wind tendencies.
! Subprogram not used   call outfld (trim(prefix)//'UTGWSPEC', utgw , ncol, lchnk)
! Subprogram not used   call outfld (trim(prefix)//'VTGWSPEC', vtgw , ncol, lchnk)
! Subprogram not used 
! Subprogram not used   ! Tau in each direction.
! Subprogram not used   call outfld (trim(prefix)//'TAUE', taucd(:,:,east), ncol, lchnk)
! Subprogram not used   call outfld (trim(prefix)//'TAUW', taucd(:,:,west), ncol, lchnk)
! Subprogram not used   call outfld (trim(prefix)//'TAUN', taucd(:,:,north), ncol, lchnk)
! Subprogram not used   call outfld (trim(prefix)//'TAUS', taucd(:,:,south), ncol, lchnk)
! Subprogram not used 
! Subprogram not used   call outfld (trim(prefix)//'TAUNET', taucd(:,:,east)+taucd(:,:,west), &
! Subprogram not used        ncol, lchnk)
! Subprogram not used 
! Subprogram not used contains
! Subprogram not used 
! Subprogram not used   ! Given a value, finds which bin marked by "bounds" the value falls
! Subprogram not used   ! into.
! Subprogram not used   elemental function find_bin(val) result(idx)
! Subprogram not used     real(r8), intent(in) :: val
! Subprogram not used 
! Subprogram not used     integer :: idx
! Subprogram not used 
! Subprogram not used     ! We just have to count how many bounds are exceeded.
! Subprogram not used     if (val >= 0._r8) then
! Subprogram not used        idx = count(val > bounds) + 1
! Subprogram not used     else
! Subprogram not used        idx = count(val >= bounds) + 1
! Subprogram not used     end if
! Subprogram not used 
! Subprogram not used   end function find_bin
! Subprogram not used 
! Subprogram not used   ! Convert a speed to a wavenumber between -ngwv and ngwv.
! Subprogram not used   elemental function c_to_l(c) result(l)
! Subprogram not used     real(r8), intent(in) :: c
! Subprogram not used 
! Subprogram not used     integer :: l
! Subprogram not used 
! Subprogram not used     l = min( max(int(c/dc),-ngwv), ngwv )
! Subprogram not used 
! Subprogram not used   end function c_to_l
! Subprogram not used 
! Subprogram not used end subroutine gw_spec_outflds
