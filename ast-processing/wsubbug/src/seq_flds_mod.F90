...
! Subprogram not used    subroutine seq_flds_getField(outfield, nfld, cstring)
! Subprogram not used 
! Subprogram not used      ! !USES:
! Subprogram not used      use mct_mod
! Subprogram not used 
! Subprogram not used      ! !INPUT/OUTPUT PARAMETERS:
! Subprogram not used 
! Subprogram not used      character(len=*),intent(out) :: outfield   ! output field name
! Subprogram not used      integer         ,intent(in ) :: nfld       ! field number
! Subprogram not used      character(len=*),intent(in ) :: cstring    ! colon delimited field string
! Subprogram not used 
! Subprogram not used      !EOP
! Subprogram not used 
! Subprogram not used      type(mct_list)   :: mctIstr  ! mct list from input cstring
! Subprogram not used      type(mct_string) :: mctOStr  ! mct string for output outfield
! Subprogram not used      character(len=*),parameter :: subname = '(seq_flds_getField) '
! Subprogram not used 
! Subprogram not used      !-------------------------------------------------------------------------------
! Subprogram not used      !
! Subprogram not used      !-------------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used      outfield = ''
! Subprogram not used 
! Subprogram not used      call mct_list_init(mctIstr,cstring)
! Subprogram not used      call mct_list_get(mctOStr,nfld,mctIstr)
! Subprogram not used      outfield = mct_string_toChar(mctOStr)
! Subprogram not used      call mct_list_clean(mctIstr)
! Subprogram not used      call mct_string_clean(mctOStr)
! Subprogram not used 
! Subprogram not used    end subroutine seq_flds_getField
...
! Subprogram not used    subroutine seq_flds_esmf_metadata_get(shortname, longname, stdname, units)
! Subprogram not used 
! Subprogram not used      ! !USES:
! Subprogram not used      use shr_string_mod, only : shr_string_lastindex
! Subprogram not used      implicit none
! Subprogram not used 
! Subprogram not used      ! !INPUT/OUTPUT PARAMETERS:
! Subprogram not used      character(len=*), intent(in)  :: shortname 
! Subprogram not used      character(len=*),optional, intent(out) :: longname
! Subprogram not used      character(len=*),optional, intent(out) :: stdname  
! Subprogram not used      character(len=*),optional, intent(out) :: units    
! Subprogram not used 
! Subprogram not used      !EOP
! Subprogram not used 
! Subprogram not used      !--- local ---
! Subprogram not used      integer :: i,n
! Subprogram not used      character(len=80) :: llongname, lstdname, lunits, lshortname  ! local copies
! Subprogram not used      character(len=*),parameter :: undef = 'undefined'
! Subprogram not used      character(len=*),parameter :: unknown = 'unknown'
! Subprogram not used      logical :: found
! Subprogram not used      character(len=*),parameter :: subname = '(seq_flds_esmf_metadata_get) '
! Subprogram not used 
! Subprogram not used      !--- define field metadata (name, long_name, standard_name, units) ---
! Subprogram not used 
! Subprogram not used      llongname = trim(unknown)
! Subprogram not used      lstdname  = trim(unknown)
! Subprogram not used      lunits    = trim(unknown)
! Subprogram not used 
! Subprogram not used      found = .false.
! Subprogram not used 
! Subprogram not used      if (.not.found) then
! Subprogram not used         i = 1
! Subprogram not used         do while (i <= n_entries .and. .not.found)
! Subprogram not used            lshortname = trim(shortname)
! Subprogram not used            if (trim(lshortname) == trim(lookup_entry(i,1))) then
! Subprogram not used               llongname = trim(lookup_entry(i,2)) 
! Subprogram not used               lstdname  = trim(lookup_entry(i,3))
! Subprogram not used               lunits    = trim(lookup_entry(i,4))    
...
! Subprogram not used         i = 1
! Subprogram not used         do while (i <= n_entries .and. .not.found)
! Subprogram not used            n = shr_string_lastIndex(shortname,"_")
! Subprogram not used            lshortname = ""
! Subprogram not used            if (n < len_trim(shortname)) lshortname = shortname(n+1:len_trim(shortname))
! Subprogram not used            if (trim(lshortname) == trim(lookup_entry(i,1))) then
! Subprogram not used               llongname = trim(lookup_entry(i,2)) 
! Subprogram not used               lstdname  = trim(lookup_entry(i,3))
! Subprogram not used               lunits    = trim(lookup_entry(i,4))    
...
! Subprogram not used      endif
! Subprogram not used 
! Subprogram not used      if (present(longname)) then
! Subprogram not used         longname = trim(llongname)
! Subprogram not used      endif
! Subprogram not used      if (present(stdname))  then
! Subprogram not used         stdname = trim(lstdname)
! Subprogram not used      endif
! Subprogram not used      if (present(units)) then
! Subprogram not used         units = trim(lunits)
! Subprogram not used      endif
! Subprogram not used 
! Subprogram not used    end subroutine seq_flds_esmf_metadata_get
