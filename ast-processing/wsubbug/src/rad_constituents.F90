...
integer, parameter :: n_rad_cnst = 30
...
integer function get_cam_idx(source, name, routine)
   ...
   ! get index of name in internal 1 array; either the constituent array
...
subroutine list_init1(namelist, gaslist, aerlist, ma_list)
   ...
   do ii = 1, namelist%ncnst
      ...
      if (namelist%type(ii) == 'A') then 
         ...
         ! which is hardwired into the code.  The 1 interface to the RT code uses
...
! Subprogram not used    subroutine parse_error(msg, str)
! Subprogram not used 
! Subprogram not used       character(len=*), intent(in) :: msg
! Subprogram not used       character(len=*), intent(in) :: str
! Subprogram not used 
! Subprogram not used       write(iulog,*) routine//': ERROR: '//msg
! Subprogram not used       write(iulog,*) ' input string: '//trim(str)
! Subprogram not used       call endrun(routine//': ERROR: '//msg)
! Subprogram not used 
! Subprogram not used    end subroutine parse_error
...
! Subprogram not used subroutine rad_cnst_get_aer_mmr_by_idx(list_idx, aer_idx, state, pbuf, mmr)
! Subprogram not used 
! Subprogram not used    ! Return pointer to mass mixing ratio for the aerosol from the specified
! Subprogram not used    ! climate or diagnostic list.
! Subprogram not used 
! Subprogram not used    ! Arguments
! Subprogram not used    integer,                     intent(in) :: list_idx    ! index of the climate or a diagnostic list
! Subprogram not used    integer,                     intent(in) :: aer_idx
! Subprogram not used    type(physics_state), target, intent(in) :: state
! Subprogram not used    type(physics_buffer_desc), pointer      :: pbuf(:)
! Subprogram not used    real(r8),                    pointer    :: mmr(:,:)
! Subprogram not used 
! Subprogram not used    ! Local variables
! Subprogram not used    integer :: lchnk
! Subprogram not used    integer :: idx
! Subprogram not used    character(len=1) :: source
! Subprogram not used    type(aerlist_t), pointer :: aerlist
! Subprogram not used    character(len=*), parameter :: subname = 'rad_cnst_get_aer_mmr_by_idx'
! Subprogram not used    !-----------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used    if (list_idx >= 0 .and. list_idx <= N_DIAG) then
! Subprogram not used       aerlist => aerosollist(list_idx)
! Subprogram not used    else
! Subprogram not used       write(iulog,*) subname//': list_idx =', list_idx
! Subprogram not used       call endrun(subname//': list_idx out of bounds')
! Subprogram not used    endif
! Subprogram not used 
! Subprogram not used    lchnk = state%lchnk
! Subprogram not used 
! Subprogram not used    ! Check for valid input aerosol index
! Subprogram not used    if (aer_idx < 1  .or.  aer_idx > aerlist%numaerosols) then
! Subprogram not used       write(iulog,*) subname//': aer_idx= ', aer_idx, '  numaerosols= ', aerlist%numaerosols
! Subprogram not used       call endrun(subname//': aerosol list index out of range')
! Subprogram not used    end if
! Subprogram not used 
! Subprogram not used    ! Get data source
! Subprogram not used    source = aerlist%aer(aer_idx)%source
! Subprogram not used    idx    = aerlist%aer(aer_idx)%idx
! Subprogram not used    select case( source )
! Subprogram not used    case ('A')
! Subprogram not used       mmr => state%q(:,:,idx)
! Subprogram not used    case ('N')
! Subprogram not used       call pbuf_get_field(pbuf, idx, mmr)
! Subprogram not used    case ('Z')
! Subprogram not used       mmr => zero_cols
! Subprogram not used    end select
! Subprogram not used 
! Subprogram not used end subroutine rad_cnst_get_aer_mmr_by_idx
...
! Subprogram not used integer function rad_cnst_get_aer_idx(list_idx, aer_name)
! Subprogram not used 
! Subprogram not used    ! Return the index of aerosol aer_name in the list specified by list_idx.
! Subprogram not used 
! Subprogram not used     ! Arguments
! Subprogram not used    integer,             intent(in) :: list_idx    ! 0 for climate list, 1-N_DIAG for diagnostic lists
! Subprogram not used    character(len=*),    intent(in) :: aer_name    ! aerosol name (in state or pbuf)
! Subprogram not used 
! Subprogram not used    ! Local variables
! Subprogram not used    integer :: i, aer_idx
! Subprogram not used    type(aerlist_t), pointer :: aerlist
! Subprogram not used    character(len=*), parameter :: subname = "rad_cnst_get_aer_idx"
! Subprogram not used    !-------------------------------------------------------------------------
! Subprogram not used    
! Subprogram not used    if (list_idx >= 0 .and. list_idx <= N_DIAG) then
! Subprogram not used       aerlist => aerosollist(list_idx)
...
! Subprogram not used    endif
! Subprogram not used 
! Subprogram not used    ! Get index in aerosol list for requested name
! Subprogram not used    aer_idx = -1
! Subprogram not used    do i = 1, aerlist%numaerosols
! Subprogram not used       if (trim(aer_name) == trim(aerlist%aer(i)%camname)) then
! Subprogram not used          aer_idx = i
! Subprogram not used          exit
! Subprogram not used       end if
! Subprogram not used    end do
! Subprogram not used 
! Subprogram not used    if (aer_idx == -1) call endrun(subname//": ERROR - name not found")
! Subprogram not used   
! Subprogram not used    rad_cnst_get_aer_idx = aer_idx
! Subprogram not used 
! Subprogram not used end function rad_cnst_get_aer_idx

!================================================================================================

! Subprogram not used subroutine rad_cnst_get_aer_props_by_idx(list_idx, &
! Subprogram not used    aer_idx,  opticstype, &
! Subprogram not used    sw_hygro_ext, sw_hygro_ssa, sw_hygro_asm, lw_hygro_ext, &
! Subprogram not used    sw_nonhygro_ext, sw_nonhygro_ssa, sw_nonhygro_asm, &
! Subprogram not used    sw_nonhygro_scat, sw_nonhygro_ascat, lw_ext, &
! Subprogram not used    refindex_aer_sw, refindex_aer_lw, &
! Subprogram not used    r_sw_ext, r_sw_scat, r_sw_ascat, r_lw_abs, mu, &
! Subprogram not used    aername, density_aer, hygro_aer, dryrad_aer, dispersion_aer, num_to_mass_aer)
! Subprogram not used 
! Subprogram not used    ! Return requested properties for the aerosol from the specified
! Subprogram not used    ! climate or diagnostic list.
! Subprogram not used 
! Subprogram not used    use phys_prop, only: physprop_get
! Subprogram not used 
! Subprogram not used 
! Subprogram not used    ! Arguments
! Subprogram not used    integer,                     intent(in)  :: list_idx ! index of the climate or a diagnostic list
! Subprogram not used    integer,                     intent(in)  :: aer_idx  ! index of the aerosol
! Subprogram not used    character(len=ot_length), optional, intent(out) :: opticstype
! Subprogram not used    real(r8),          optional, pointer     :: sw_hygro_ext(:,:) 
! Subprogram not used    real(r8),          optional, pointer     :: sw_hygro_ssa(:,:) 
! Subprogram not used    real(r8),          optional, pointer     :: sw_hygro_asm(:,:) 
! Subprogram not used    real(r8),          optional, pointer     :: lw_hygro_ext(:,:)         
! Subprogram not used    real(r8),          optional, pointer     :: sw_nonhygro_ext(:)
! Subprogram not used    real(r8),          optional, pointer     :: sw_nonhygro_ssa(:)
! Subprogram not used    real(r8),          optional, pointer     :: sw_nonhygro_asm(:)
! Subprogram not used    real(r8),          optional, pointer     :: sw_nonhygro_scat(:)
! Subprogram not used    real(r8),          optional, pointer     :: sw_nonhygro_ascat(:)
! Subprogram not used    real(r8),          optional, pointer     :: lw_ext(:)         
! Subprogram not used    complex(r8),       optional, pointer     :: refindex_aer_sw(:)
! Subprogram not used    complex(r8),       optional, pointer     :: refindex_aer_lw(:)
! Subprogram not used    character(len=20), optional, intent(out) :: aername           
! Subprogram not used    real(r8),          optional, intent(out) :: density_aer
! Subprogram not used    real(r8),          optional, intent(out) :: hygro_aer
! Subprogram not used    real(r8),          optional, intent(out) :: dryrad_aer        
! Subprogram not used    real(r8),          optional, intent(out) :: dispersion_aer    
! Subprogram not used    real(r8),          optional, intent(out) :: num_to_mass_aer   
! Subprogram not used 
! Subprogram not used    real(r8),          optional, pointer     :: r_sw_ext(:,:)         
! Subprogram not used    real(r8),          optional, pointer     :: r_sw_scat(:,:)         
! Subprogram not used    real(r8),          optional, pointer     :: r_sw_ascat(:,:)         
! Subprogram not used    real(r8),          optional, pointer     :: r_lw_abs(:,:)         
! Subprogram not used    real(r8),          optional, pointer     :: mu(:)         
! Subprogram not used 
! Subprogram not used    ! Local variables
! Subprogram not used    integer :: id
! Subprogram not used    character(len=*), parameter :: subname = 'rad_cnst_get_aer_props_by_idx'
! Subprogram not used    type(aerlist_t), pointer :: aerlist
! Subprogram not used    !------------------------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used    if (list_idx >= 0 .and. list_idx <= N_DIAG) then
...
! Subprogram not used    else
! Subprogram not used       write(iulog,*) subname//': list_idx = ', list_idx
! Subprogram not used       call endrun(subname//': list_idx out of range')
! Subprogram not used    endif
! Subprogram not used 
! Subprogram not used    if (aer_idx < 1 .or. aer_idx > aerlist%numaerosols) then
! Subprogram not used       write(iulog,*) subname//': aerosol list index out of range: ', aer_idx ,' list index: ',list_idx
! Subprogram not used       call endrun(subname//': aer_idx out of range')
! Subprogram not used    end if
! Subprogram not used 
! Subprogram not used    id = aerlist%aer(aer_idx)%physprop_id
! Subprogram not used 
! Subprogram not used    if (present(opticstype))        call physprop_get(id, opticstype=opticstype)
! Subprogram not used 
! Subprogram not used    if (present(sw_hygro_ext))      call physprop_get(id, sw_hygro_ext=sw_hygro_ext)
! Subprogram not used    if (present(sw_hygro_ssa))      call physprop_get(id, sw_hygro_ssa=sw_hygro_ssa)
! Subprogram not used    if (present(sw_hygro_asm))      call physprop_get(id, sw_hygro_asm=sw_hygro_asm)
! Subprogram not used    if (present(lw_hygro_ext))      call physprop_get(id, lw_hygro_abs=lw_hygro_ext)
! Subprogram not used 
! Subprogram not used    if (present(sw_nonhygro_ext))   call physprop_get(id, sw_nonhygro_ext=sw_nonhygro_ext)
! Subprogram not used    if (present(sw_nonhygro_ssa))   call physprop_get(id, sw_nonhygro_ssa=sw_nonhygro_ssa)
! Subprogram not used    if (present(sw_nonhygro_asm))   call physprop_get(id, sw_nonhygro_asm=sw_nonhygro_asm)
! Subprogram not used    if (present(sw_nonhygro_scat))  call physprop_get(id, sw_nonhygro_scat=sw_nonhygro_scat)
! Subprogram not used    if (present(sw_nonhygro_ascat)) call physprop_get(id, sw_nonhygro_ascat=sw_nonhygro_ascat)
! Subprogram not used    if (present(lw_ext))            call physprop_get(id, lw_abs=lw_ext)
! Subprogram not used 
! Subprogram not used    if (present(refindex_aer_sw))   call physprop_get(id, refindex_aer_sw=refindex_aer_sw)
! Subprogram not used    if (present(refindex_aer_lw))   call physprop_get(id, refindex_aer_lw=refindex_aer_lw)
! Subprogram not used 
! Subprogram not used    if (present(aername))           call physprop_get(id, aername=aername)
! Subprogram not used    if (present(density_aer))       call physprop_get(id, density_aer=density_aer)
! Subprogram not used    if (present(hygro_aer))         call physprop_get(id, hygro_aer=hygro_aer)
! Subprogram not used    if (present(dryrad_aer))        call physprop_get(id, dryrad_aer=dryrad_aer)
! Subprogram not used    if (present(dispersion_aer))    call physprop_get(id, dispersion_aer=dispersion_aer)
! Subprogram not used    if (present(num_to_mass_aer))   call physprop_get(id, num_to_mass_aer=num_to_mass_aer)
! Subprogram not used 
! Subprogram not used    if (present(r_lw_abs))          call physprop_get(id, r_lw_abs=r_lw_abs)
! Subprogram not used    if (present(r_sw_ext))          call physprop_get(id, r_sw_ext=r_sw_ext)
! Subprogram not used    if (present(r_sw_scat))         call physprop_get(id, r_sw_scat=r_sw_scat)
! Subprogram not used    if (present(r_sw_ascat))        call physprop_get(id, r_sw_ascat=r_sw_ascat)
! Subprogram not used    if (present(mu))                call physprop_get(id, mu=mu)
! Subprogram not used 
! Subprogram not used end subroutine rad_cnst_get_aer_props_by_idx
