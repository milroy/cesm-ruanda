...
! Subprogram not used subroutine modal_aero_calcsize_diag(state, pbuf, list_idx_in, dgnum_m)
! Subprogram not used 
! Subprogram not used    !-----------------------------------------------------------------------
! Subprogram not used    !
! Subprogram not used    ! Calculate aerosol size distribution parameters 
! Subprogram not used    !
! Subprogram not used    ! ***N.B.*** DGNUM for the modes in the climate list are put directly into
! Subprogram not used    !            the physics buffer.  For diagnostic list calculations use the
! Subprogram not used    !            optional list_idx and dgnum args.
! Subprogram not used    !-----------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used    ! arguments
! Subprogram not used    type(physics_state), intent(in), target :: state   ! Physics state variables
! Subprogram not used    type(physics_buffer_desc), pointer :: pbuf(:)      ! physics buffer
! Subprogram not used 
! Subprogram not used    integer,  optional, intent(in)   :: list_idx_in    ! diagnostic list index
! Subprogram not used    real(r8), optional, pointer      :: dgnum_m(:,:,:) ! interstital aerosol dry number mode radius (m)
! Subprogram not used 
! Subprogram not used    ! local
! Subprogram not used    integer  :: i, k, l1, n
! Subprogram not used    integer  :: lchnk, ncol
! Subprogram not used    integer  :: list_idx, stat
! Subprogram not used    integer  :: nmodes
! Subprogram not used    integer  :: nspec
! Subprogram not used 
! Subprogram not used    real(r8), pointer :: dgncur_a(:,:) ! (pcols,pver)
! Subprogram not used 
! Subprogram not used 
! Subprogram not used    real(r8), parameter :: third = 1.0_r8/3.0_r8
! Subprogram not used 
! Subprogram not used    real(r8), pointer :: mode_num(:,:) ! mode number mixing ratio
! Subprogram not used    real(r8), pointer :: specmmr(:,:)  ! specie mmr
! Subprogram not used    real(r8)          :: specdens      ! specie density
! Subprogram not used 
! Subprogram not used    real(r8) :: dryvol_a(pcols,pver)   ! interstital aerosol dry volume (cm^3/mol_air)
! Subprogram not used 
! Subprogram not used    real(r8) :: dgnum, dgnumhi, dgnumlo
! Subprogram not used    real(r8) :: dgnyy, dgnxx           ! dgnumlo/hi of current mode
! Subprogram not used    real(r8) :: drv_a                  ! dry volume (cm3/mol_air)
! Subprogram not used    real(r8) :: dumfac, dummwdens      ! work variables
! Subprogram not used    real(r8) :: num_a0                 ! initial number (#/mol_air)
! Subprogram not used    real(r8) :: num_a                  ! final number (#/mol_air)
! Subprogram not used    real(r8) :: voltonumbhi, voltonumblo
! Subprogram not used    real(r8) :: v2nyy, v2nxx           ! voltonumblo/hi of current mode
! Subprogram not used    real(r8) :: sigmag, alnsg
! Subprogram not used    !-----------------------------------------------------------------------
! Subprogram not used 
! Subprogram not used    lchnk = state%lchnk
! Subprogram not used    ncol  = state%ncol
! Subprogram not used 
! Subprogram not used    list_idx = 0  ! climate list by default
! Subprogram not used    if (present(list_idx_in)) list_idx = list_idx_in
! Subprogram not used 
! Subprogram not used    call rad_cnst_get_info(list_idx, nmodes=nmodes)
! Subprogram not used 
! Subprogram not used    if (list_idx /= 0) then
! Subprogram not used       if (.not. present(dgnum_m)) then
! Subprogram not used          call endrun('modal_aero_calcsize_diag called for'// &
! Subprogram not used                      'diagnostic list but dgnum_m pointer not present')
! Subprogram not used       end if
! Subprogram not used       allocate(dgnum_m(pcols,pver,nmodes), stat=stat)
! Subprogram not used       if (stat > 0) then
! Subprogram not used          call endrun('modal_aero_calcsize_diag: allocation FAILURE: dgnum_m')
! Subprogram not used       end if
! Subprogram not used    end if
! Subprogram not used 
...
! Subprogram not used 
! Subprogram not used       if (list_idx == 0) then
! Subprogram not used          call pbuf_get_field(pbuf, dgnum_idx, dgncur_a, start=(/1,1,n/), kount=(/pcols,pver,1/))
! Subprogram not used       else
! Subprogram not used          dgncur_a => dgnum_m(:,:,n)
! Subprogram not used       end if
! Subprogram not used 
! Subprogram not used       ! get mode properties
! Subprogram not used       call rad_cnst_get_mode_props(list_idx, n, dgnum=dgnum, dgnumhi=dgnumhi, dgnumlo=dgnumlo, &
! Subprogram not used                                    sigmag=sigmag)
! Subprogram not used 
! Subprogram not used       ! get mode number mixing ratio
! Subprogram not used       call rad_cnst_get_mode_num(list_idx, n, 'a', state, pbuf, mode_num)
! Subprogram not used 
! Subprogram not used       dgncur_a(:,:) = dgnum
! Subprogram not used       dryvol_a(:,:) = 0.0_r8
! Subprogram not used 
! Subprogram not used       ! compute dry volume mixrats = 
! Subprogram not used       !      sum_over_components{ component_mass mixrat / density }
! Subprogram not used       call rad_cnst_get_info(list_idx, n, nspec=nspec)
! Subprogram not used       do l1 = 1, nspec
! Subprogram not used 
! Subprogram not used          call rad_cnst_get_aer_mmr(list_idx, n, l1, 'a', state, pbuf, specmmr)
! Subprogram not used          call rad_cnst_get_aer_props(list_idx, n, l1, density_aer=specdens)
! Subprogram not used 
! Subprogram not used          ! need qmass*dummwdens = (kg/kg-air) * [1/(kg/m3)] = m3/kg-air
! Subprogram not used          dummwdens = 1.0_r8 / specdens
! Subprogram not used 
! Subprogram not used          do k=top_lev,pver
! Subprogram not used             do i=1,ncol
! Subprogram not used                dryvol_a(i,k) = dryvol_a(i,k)    &
! Subprogram not used                   + max(0.0_r8, specmmr(i,k))*dummwdens
! Subprogram not used             end do
! Subprogram not used          end do
! Subprogram not used       end do
! Subprogram not used 
! Subprogram not used       alnsg  = log( sigmag )
! Subprogram not used       dumfac = exp(4.5_r8*alnsg**2)*pi/6.0_r8
! Subprogram not used       voltonumblo = 1._r8 / ( (pi/6._r8)*(dgnumlo**3)*exp(4.5_r8*alnsg**2) )
! Subprogram not used       voltonumbhi = 1._r8 / ( (pi/6._r8)*(dgnumhi**3)*exp(4.5_r8*alnsg**2) )
! Subprogram not used       v2nxx = voltonumbhi
! Subprogram not used       v2nyy = voltonumblo
! Subprogram not used       dgnxx = dgnumhi
! Subprogram not used       dgnyy = dgnumlo
! Subprogram not used 
! Subprogram not used       do k = top_lev, pver
! Subprogram not used          do i = 1, ncol
! Subprogram not used 
! Subprogram not used             drv_a = dryvol_a(i,k)
! Subprogram not used             num_a0 = mode_num(i,k)
! Subprogram not used             num_a = max( 0.0_r8, num_a0 )
! Subprogram not used 
! Subprogram not used             if (drv_a > 0.0_r8) then
! Subprogram not used                if (num_a <= drv_a*v2nxx) then
! Subprogram not used                   dgncur_a(i,k) = dgnxx
! Subprogram not used                else if (num_a >= drv_a*v2nyy) then
! Subprogram not used                   dgncur_a(i,k) = dgnyy
! Subprogram not used                else
! Subprogram not used                   dgncur_a(i,k) = (drv_a/(dumfac*num_a))**third
! Subprogram not used                end if
! Subprogram not used             end if
! Subprogram not used 
! Subprogram not used          end do
! Subprogram not used       end do
! Subprogram not used 
! Subprogram not used    end do ! nmodes
! Subprogram not used 
! Subprogram not used end subroutine modal_aero_calcsize_diag
