...
def process_stmt(stmt, graphstack, nstack):
    ...
    if isinstance(stmt, fparser.block_statements.Module):
        ...
        modVars = {
            "".join(x.translate(None, "".join(["'", "/", '"'])).strip().lower().split())
        ...
        graphstack[0].modules[modname] = {
            "provides": modProvides,
            "vars": modVars,
            "public": modPublic,
            "private": modPrivate,
            "location": namestack,
            "subprogs": modSubprogs,
        }
        ...
        if not callArgs == []:
            ...
            if callName == "addfld":
                graphstack[0].calls.append(
                    (callName, caList, location, {"processed": False})
                )
                ...
                for n1 in name1:
                    ...
                    for n2 in namen2:
                        ...
                        graphstack[0].addEdge(
                            namen2loc,
                            name1loc,
                            attrs1=attrsn2,
                            attrs2=attrs1,
                            edgeLine=attrs1["lines"]["line"],
                        )
        ...
        graphstack[0].subroutines[subname] = {
            "args": saList,
            "vars": subVars,
            "location": namestack,
            "isIface": False,
        ...
        if "%" not in stmt.variable:
            ...
            assigned = re.sub(
                r"\([^)]*\)", "", "".join(stmt.variable.strip().lower().split())
            )
            if ")" in assigned:
        ...
        graphstack[0].addEdge(
            lhs, rhs, attrs1=aattrs, attrs2=aaeettrs, edgeLine=aattrs["lines"]["line"]
        )
        graphstack[0].addEdge(
            rhs, lhs, attrs1=aaeettrs, attrs2=aattrs, edgeLine=aattrs["lines"]["line"]
        )
