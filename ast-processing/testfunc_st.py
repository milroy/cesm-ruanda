...
class stdict(dict):
    ...
    def __getitem__(self, approxkey):
        ...
        for key in self.keys():
            if key.find(approxkey) > -1:
                ret.append(dict.__getitem__(self, key))
        if ret == []:
            raise KeyError
        else:
            return ret
    def __contains__(self, approxkey):
        for key in self.keys():
            if key.find(approxkey) > -1:
                return True
        return False
...
def gen_argtree(fstring):
    ...
    for rets in parenthetic_contents(fstring):
        if not rets[0] in argTree:
            argTree[rets[0]] = [{'expression': rets[1], 'stInput': STree.STree(rets[1]), \
                                 'output': rets[1], 'inFromBelow' : rets[1]}]
        else:
            argTree[rets[0]].append({'expression': rets[1], 'stInput': STree.STree(rets[1]), \
                                     'output': rets[1], 'inFromBelow' : rets[1]})
    return argTree
...
def stmt_to_fnnames(string):
    ...
    if not string == '':
        ...
        names = set(names[1:])
        functions.update(zerofunc)
        functions.update(arrays)
    else:
...
# Now the real work
s = 'rc = alpha(iota) * get_calday() * alpha(fubar) + delta ** delta + 1._r8 / ((1._r8 / rsmx(ispec)) + (1._r8 / rlux(ispec)) + (1._r8 / (rdc + rclx(ispec))) + (1._r8 / (rac(index_season, wesveg) + rgsx(ispec))) + alpha(beta(gamma, delta, call(urbanradiation(epsilon + (zeta(foxtrot + echo)), kappa(:, 2:charlie + golf), hit(assoc), nclex(omega : yotta), lambda(:, 2:1 + magma*eta(gfunc), :))))))'
