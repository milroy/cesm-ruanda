...
summary = nio.open_file(summaryFile, 'r')
vs = []
for i in xrange(summary.variables['vars'].shape[0]):
    vs.append(summary.variables['vars'][i,:].tostring().strip())

if isinstance(testmeans, list):
  testmeans = dict(testmeans)
  testtmp = OrderedDict()
  for idx, var in enumerate(vs):
    testtmp[var] = testmeans['000'][idx]

  testmeans = {}
  testmeans['000'] = testtmp

global_means = defaultdict(OrderedDict)

for var in vs:
  global_means[0][var] = ens['000'][var]
  global_means[1][var] = testmeans['000'][var]
